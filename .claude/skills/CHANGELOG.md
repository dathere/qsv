...
- **MCP tool counts corrected in `README-MCP.md`** — `qsvmcp` exposes **69** commands and the full `qsv` binary **78** (previously documented as 66 and 75). Verified by enumeration against the `Command` enum in `src/main.rs`: 79 variants total, less `Log` (`#[cfg(feature = "mcp")]`, absent from `all_features`) for qsv's 78, and less the 9 documented exclusions plus `py` for qsvmcp's 69. The skill-based command count is a separate, smaller number and remains 55.
