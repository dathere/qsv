...
### Changed
- **`qsv-viz` skill regenerated for `--tour-steps`, which replaces `--no-tour`** ([#4389](https://github.com/dathere/qsv/issues/4389)). The guided tour's panel budget is now a flag rather than a hardcoded constant: `viz smart --tour-steps <n>` sets how many chart panels the tour spotlights (default 8), and `--tour-steps 0` suppresses the tour - and its new first-visit hint beacons - entirely. **The old `--no-tour` flag is gone**, so an agent still passing it now gets a hard error instead of a silently different page; any saved invocation needs updating to `--tour-steps 0`. Worth knowing when an agent picks a value: `<n>` budgets the per-panel spotlights only, not the whole walk (the introduction, Data Dictionary chapter, data viewer chapter and conclusion are always included), a dictionary's `x-qsv.tour.panel_order` is capped by the same number, and an explicitly-passed budget is a HARD cap while the default is soft - a page that would otherwise spotlight no frequency distribution gains one extra step rather than losing an overview panel. Skill count unchanged at 56.
...
- **`qsv-viz` skill regenerated for the new `--no-tour` flag** ([#4389](https://github.com/dathere/qsv/issues/4389)). `viz smart` HTML dashboards now ship a guided tour (vendored driver.js) that auto-starts on a reader's first visit and walks the Data Dictionary drawer, the data viewer and the summary panels with context-sensitive explanations of why each chart was chosen; `--no-tour` suppresses it. The narration can be refined via a dictionary's dataset-level `x-qsv.tour` object (`overrides` by step id, `panels` by raw field name or `@kind` token, `panel_order` for spotlight selection) — the contract an agent (or a future `--tour-audience` LLM pass) writes into `<stem>.schema.json`. Skill count unchanged at 55.
