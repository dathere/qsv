...
- **Beware ID columns**: all-unique columns (`uniqueness_ratio` = 1 in the stats cache) keep their correlation rows, with MI/NMI left empty. That is right for continuous measures, but an identifier or sequence number can still correlate strongly with a date or another monotone column simply because IDs are issued in order (e.g. a 311 `Unique Key` vs `Created Date` at ρ ≈ 0.99). Before reporting a correlation, check whether either field is ID-like (all-unique integer or string, `xsd_type` of an unsigned/positive integer); never present such a pair as a substantive finding.
- An empty or header-only bivariate file means no pair qualified for the requested stats (e.g. a frequency-only `bivariate_stats` with only all-unique numeric fields). It is not a failure, and it is never left over from an earlier run.
