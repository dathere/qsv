...
|-----------|-----------|-----------------|
| `.mcpb` (MCP Server) | The qsv tools Claude can call in Chat + Cowork | 51 data-wrangling skills (backed by qsvmcp's 62 commands), SQL queries, file conversion. **Auto-installs the qsv binary** — no separate download needed. |
| Cowork Plugin (via marketplace) | Workflow layer for Cowork sessions | 5 domain skills, 8 slash commands, 3 subagents for guided data workflows |
