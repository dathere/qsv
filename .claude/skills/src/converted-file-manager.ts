...
class ConversionLock {
  ...
  async acquireLock(sourcePath: string): Promise<boolean> {
    ...
    try {
      ...
      this.lockFd = await open(
        this.currentLockPath,
  ...
  async releaseLock(): Promise<void> {
    ...
    if (this.currentLockPath !== null) {
      try {
        await unlink(this.currentLockPath);
        console.error(
          `[ConversionLock] Released lock: ${basename(this.currentLockPath)}`,
        );
      } catch (error: unknown) {
    ...
    try {
      ...
      if (age > ConversionLock.STALE_LOCK_AGE_MS) {
        try {
          await unlink(this.currentLockPath);
          console.error(
            `[ConversionLock] Cleaned up stale lock (${Math.round(age / 1000)}s old)`,
          );
        } catch (error: unknown) {
          if (!isNodeError(error) || error.code !== "ENOENT") {
            // Ignore race where another process already removed the stale lock
            console.error(
              "[ConversionLock] Error deleting stale lock file:",
              error,
            );
          }
        }
  ...
  async withLock<T>(
    ...
    if (!acquired) {
      console.warn(
        `[ConversionLock] Cannot acquire lock for ${sourcePath} - another process may be converting this file`,
      );
...
export class ConvertedFileManager {
  ...
  constructor(workingDir: string, maxSizeGB?: number) {
    ...
    this.conversionsFilePath = join(
      workingDir,
      ConvertedFileManager.CONVERSIONS_FILE,
    );
  ...
  private validateSizeLimit(sizeGB: number): number {
    ...
    if (!Number.isFinite(sizeGB) || sizeGB <= 0) {
      console.error(
        `[Converted File Manager] Invalid size limit: ${sizeGB}, using default ${ConvertedFileManager.DEFAULT_MAX_SIZE_GB} GB`,
      );
    ...
    if (sizeGB < 0.1 || sizeGB > 100) {
      console.error(
        `[Converted File Manager] Size limit out of valid range: ${sizeGB} GB (must be 0.1-100 GB), using default ${ConvertedFileManager.DEFAULT_MAX_SIZE_GB} GB`,
      );
      return ConvertedFileManager.DEFAULT_MAX_SIZE_GB;
    }
    ...
    if (sizeGB < 0.5) {
      console.warn(
        `[Converted File Manager] Unusually small size limit: ${sizeGB} GB (recommended minimum: 0.5 GB)`,
      );
    }
  ...
  private hasSourceChanged(
    ...
    if (
      entry.sourceInode !== undefined &&
      sourceStats.ino !== undefined &&
      sourceStats.ino !== 0
    ) {
      if (entry.sourceInode !== sourceStats.ino) {
        console.error(
          "[Converted File Manager] Source file inode changed (file replaced)",
        );
    ...
    if (sourceStats.mtime.getTime() > entry.sourceTimestamp) {
      console.error(
        "[Converted File Manager] Source file mtime changed (fallback check)",
      );
      return true;
    }
  ...
  private async loadCache(): Promise<ConvertedFileCacheV1> {
    try {
      ...
      console.error(
        "[Converted File Manager] Cache load failed, attempting recovery:",
        error,
      );
   ...
   */
  private async migrateV0ToV1(
    cacheV0: ConvertedFileCacheV0,
  ): Promise<ConvertedFileCacheV1> {
    // Backup v0 cache
    ...
    const sortedEntries = [...cacheV0.entries].sort(
  ...
  private async recoverFromCorruption(): Promise<ConvertedFileCacheV1> {
    ...
    try {
      const corruptData = await readFile(this.cacheFilePath, "utf-8").catch(
        () => null,
      );
      if (corruptData) {
    ...
    console.error(
      "[Converted File Manager] Rebuilding cache from filesystem...",
    );
        ...
        if (file.includes(".converted.") && file.endsWith(".csv")) {
          ...
          try {
            ...
            if (markerIndex === -1) {
              ...
              console.error(
                `[Converted File Manager] Malformed converted filename: ${fileBasename}`,
              );
              continue;
            }
  ...
  private async saveCache(cache: ConvertedFileCacheV1): Promise<void> {
    ...
    this.metrics.errors.cacheSaveErrors++;
    console.error(
      "[Converted File Manager] Failed to save cache after retries:",
      lastError,
    );
  ...
  async getValidConvertedFile(
    ...
    try {
      ...
      const entry = cache.entries.find(
        (e) => e.sourcePath === sourcePath && e.convertedPath === convertedPath,
      );
      ...
      if (entry) {
        ...
        if (this.hasSourceChanged(entry, sourceStats)) {
          console.error(
            `[Converted File Manager] Source modified (enhanced check), reconverting: ${sourcePath}`,
          );
        ...
        if (sourceStats.mtime.getTime() > convertedStats.mtime.getTime()) {
          console.error(
            `[Converted File Manager] Source modified (mtime check), reconverting: ${sourcePath}`,
          );
          this.metrics.cache.misses++;
          return null; // Source has changed, need to reconvert
        }
      ...
      console.error(
        `[Converted File Manager] Reusing existing converted file: ${convertedPath}`,
      );
  ...
  async registerConversionComplete(sourcePath: string): Promise<void> {
    try {
      ...
      tracker.conversions = tracker.conversions.filter(
   ...
   */
  private async saveConversionTracker(
  ...
  async cleanupPartialConversions(): Promise<void> {
    try {
      ...
      const staleThreshold =
        Date.now() - ConvertedFileManager.STALE_CONVERSION_AGE_MS;
      ...
      for (const conversion of tracker.conversions) {
        ...
        if (conversion.startedAt < staleThreshold) {
          ...
          try {
            ...
            if (!isNodeError(error) || error.code !== "ENOENT") {
              console.error(
                "[Converted File Manager] Failed to delete partial file:",
                error,
              );
            } else {
              // File already gone
  ...
  async registerConvertedFile(
    ...
    if (result === null) {
      console.warn(
        `[Converted File Manager] Failed to register ${convertedPath} - could not acquire lock (another process may be registering the same file)`,
      );
  ...
  private async enforceSizeLimit(cache: ConvertedFileCacheV1): Promise<void> {
    ...
    const timeSinceLastCleanup = cache.lastCleanup
      ? Date.now() - cache.lastCleanup
      : Infinity;
    ...
    if (
      cache.totalSize <= hysteresisThreshold &&
      timeSinceLastCleanup < oneHourMs
    ) {
    ...
    for (const entry of toDelete) {
      ...
      try {
        ...
        deleted = true;
        console.error(
          `[Converted File Manager] Deleted: ${entry.convertedPath} (${formatBytes(entry.size)})`,
        );
      } catch (error: unknown) {
        if (isNodeError(error) && error.code === "ENOENT") {
          ...
          console.error(
            `[Converted File Manager] Failed to delete ${entry.convertedPath}:`,
            error,
          );
        ...
        if (deleted) {
          cache.entries = cache.entries.filter(
            (e) => e.convertedPath !== entry.convertedPath,
          );
          ...
          console.error(
            `[Converted File Manager] Keeping entry in cache due to deletion failure: ${entry.convertedPath}`,
          );
        }
      }
    }
    ...
    if (Math.abs(recalculated - cache.totalSize) > TOLERANCE_BYTES) {
      console.warn(
        `[Converted File Manager] Size discrepancy after cleanup, fixing (${cache.totalSize} -> ${recalculated})`,
      );
    ...
    try {
      ...
      for (const entry of cache.entries) {
        try {
          ...
          console.error(
            `[Converted File Manager] Removing orphaned entry: ${entry.convertedPath}`,
          );
          ...
          if (file.startsWith(`${cacheFilename}.tmp.`)) {
            ...
            try {
              ...
              if (stats.mtime.getTime() < staleThreshold) {
                ...
                console.error(
                  `[Converted File Manager] Deleted stale temp file: ${file}`,
                );
              }
            } catch {
