...
WORKFLOW ORDER: For new files: (1) qsv_list_files to discover files, (2) qsv_index for files >10MB, (3) qsv_stats --cardinality --stats-jsonl to create the initial stats cache, (4) qsv_moarstats to roughly double the stats in the cache — set output_file to the stats cache path (<FILESTEM>.stats.csv, e.g. for data.csv use output_file=data.stats.csv) to update the cache in place, (5) then run analysis/transformation commands. The stats cache accelerates: frequency, schema, tojsonl, sqlp, joinp, pivotp, describegpt, moarstats, sample. SQL queries on CSV inputs auto-convert to Parquet before execution.
