...
WORKFLOW ORDER: For new files: (1) qsv_list_files to discover files, (2) qsv_index for files >10MB, (3) qsv_stats --cardinality --stats-jsonl to create the initial stats cache, (4) qsv_moarstats to roughly double the stats in the cache — set output_file to <input>.stats.csv to update the cache in place, (5) then run analysis/transformation commands. The stats cache accelerates: frequency, schema, tojsonl, sqlp, joinp, pivotp, describegpt, moarstats, sample. SQL queries on CSV inputs auto-convert to Parquet before execution.
...
CACHE AWARENESS: Before running commands, check if a stats cache (<input>.stats.csv) or frequency cache (<input>.freq.csv) exists for the input file. When caches exist, read the compact CSV cache files first to inform your choices and minimize token usage, e.g.: use cardinality data to pick optimal join order (smaller cardinality on right in joinp), skip sorting if stats show data is already sorted, and skip dedup if a key column is already all-unique. The corresponding .data.jsonl variants are for qsv's internal smart command reuse and are less token-efficient. Pass cached column type info from the stats cache to sqlp/joinp for accurate type inference instead of re-sniffing.
