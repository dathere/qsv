...
REPRODUCIBILITY LOG: Use qsv_log to create a verifiable audit trail. User prompts are logged automatically — do NOT log them manually. Log key decisions (entry_type: "agent_reasoning"), actions taken (entry_type: "agent_action"), and outcomes (entry_type: "result_summary"). The log file (qsvmcp.log) records both automatic tool invocations (s-/e- prefixed) and your explicit entries (u- prefixed). Keep entries concise but sufficient for a third party to reproduce your workflow. Avoid excessive logging — for simple interactions, a result_summary alone is enough. Reserve agent_reasoning for non-obvious decisions.${config.isPluginMode ? `
