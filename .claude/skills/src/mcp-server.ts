...
CACHE AWARENESS: Before running commands, check if stats cache (.stats.csv.data.jsonl) or frequency cache (.freq.csv.data.jsonl) exist for the input file. When caches exist, read them to inform your choices to optimize operations and reduce token usage, e.g.: use cardinality data to pick optimal join order (smaller cardinality on right in joinp), skip sorting if stats show data is already sorted, and skip dedup if a key column is already all-unique. Pass cached column type info to sqlp/joinp for accurate type inference instead of re-sniffing.
