...
const COMMAND_GUIDANCE: Record<string, CommandGuidance> = {
  ...
  stats: {
    ...
    commonPattern: "Run 2nd (after index). Creates cache used by frequency, schema, tojsonl, sqlp, joinp, pivotp, describegpt, moarstats, sample. After stats, run moarstats with output_file set to <input>.stats.csv to roughly double the statistics in the cache at minimal cost.",
  ...
  moarstats: {
    ...
    commonPattern: "Index → Stats → Moarstats (set output_file to <input>.stats.csv to update cache in place) for richest analysis at minimal cost. With --bivariate: main stats to --output, bivariate stats to <FILESTEM>.stats.bivariate.csv (separate file next to input).",
