...
| `x-qsv.aggregation` | per property, one of `sum`/`mean`/`min`/`max` | declares how a numeric measure combines across a group, overriding qsv's column-NAME heuristic in both directions. Use `mean` for anything per-unit or per-record (a unit price, a rating, a temperature, a duration); `sum` only for a quantity each row contributes. This is the language-neutral signal — the name heuristic is English-first and cannot read `precio_unitario` (issue #4401). qsv drops it unless the column is a numeric measure |
