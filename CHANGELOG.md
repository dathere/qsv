...
- `viz` **geo maps are now theme-aware**. The `geo`-subplot charts (`choropleth`, `geo`, and the `viz smart` map/region panels) previously always rendered light-gray land on a (possibly dark) page; under a dark `--theme` (e.g. `plotly_dark`) they now use dark land/ocean fills so the map matches the page. The `viz smart` light/dark toggle also recolors the geo land/ocean (not just the background) when switching modes live ([#302](https://github.com/dathere/qsv/issues/302)).
