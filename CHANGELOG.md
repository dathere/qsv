...
### Fixed
- `viz`: **a degenerate `x-qsv.denominator` no longer renders a rate panel that is just the count panel rescaled.** When a dictionary denominator hint resolves to a single distinct value across the matched regions, dividing by it rescales every region's count by the same constant - identical ranks, identical map shape, only the axis label and magnitude change, so the panel carries no information a reader can act on. Several unrelated causes produce this (a placeholder column, a join that collapsed, a genuinely uniform denominator) and they are deliberately not told apart, because the panel is uninformative in all of them. `viz smart` now compares the distinct denominator values (`D`) against the matched regions (`R`) at the same consumption site that already checks region-constancy and positivity, and skips the rate panel when `D == 1 && R >= 2`, degrading exactly like every other denominator problem: a stderr note and a "Panels not drawn" record, the count panel keeping its raw-count caveat, never an error - a dictionary is a hand-editable sidecar, and a bad hint must not take the Data Schematic down. The check is deliberately threshold-free and stays at `D == 1`: it is a statement about the *panel* being degenerate, not about which geography the denominator describes, and the distinct-values/regions RATIO generalization was investigated on #4526 and disproved with measured data there (a legitimate per-city denominator scores 0.227 while the coarse-geography bug scores 0.500 - the classes are inverted, and no threshold separates them) ([#4547](https://github.com/dathere/qsv/issues/4547)).
