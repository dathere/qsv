...
### Fixed
- **A derived cache whose source CSV carries an ACL is now owner-only.** `stats` and `frequency` write caches that hold the DATA - every non-high-cardinality value and its count, a column's min/max/mode/antimode - so `util::DerivedFile` already refuses to grant anything the source's own mode and gid do not (23.0.1). An **ACL is invisible to all of that**: a `0644` CSV carrying `chmod +a "user:nobody deny read"` still reports `0644` to `stat`, and on macOS it is invisible to the xattr API too (`getxattr(…, "com.apple.system.Security")` returns `EPERM` with *and* without an ACL, and the name never appears in `listxattr`) - so a source that *denies* a user could still yield a cache that admits them. The rule is now: **any ACL on the source, owner bits only** (`.stats.csv`, `.stats.csv.data.jsonl` and `.freq.csv.data.jsonl` alike). The ACL is never parsed and effective access is never computed - there is no userland "could user X read this" oracle, so this is a presence test that fails closed, via `acl_get_file` on macOS (hand-declared: `libc` binds no `acl_*` symbol for any target) and `getxattr("system.posix_acl_access")` on Linux. An unexpected errno clamps rather than failing the command. **Two consequences worth knowing.** While the source carries an ACL, a deliberate `chmod` on the *cache* no longer survives a refresh - the clamp is re-applied every time, because honoring a once-widened cache forever would keep exactly the hole this closes. And although an ACL is rare on a file (0 of 1098 files in a real `~/Downloads` carry one - macOS puts its default `group:everyone deny delete` ACE on the *folders*, not on the files in them), a directory with **inheritable** ACEs - some MDM-managed home directories, some sync and backup tools - hands one to *every* file it holds, and every cache derived from those files is then owner-only. That is the accepted fail-closed cost, not a bug. NFSv4 ACLs are deliberately **not** probed: on an NFSv4 mount every file exposes one (the mode bits are synthesized from it), so detecting them would clamp every cache for anyone whose data lives on NFS. Other unix (FreeBSD, illumos) has no detection and behaves as before. xattrs and MAC labels (SELinux, AppArmor, the macOS sandbox) remain out of scope, as in #4619.
