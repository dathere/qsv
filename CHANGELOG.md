...
- **`viz smart` no longer hides full-precision numeric measurements from its correlation view, and `--bivariate` no longer drops them altogether** ([#4653](https://github.com/dathere/qsv/issues/4653)). Two rules treated "every value distinct" as "this is an ID":
  ...
  In the gallery, `smart_timeseries` now correlates all four of `open`/`high`/`low`/`close` instead of two, and its drill-down follows the stronger pair (open vs low). `smart_smarter` and `smart_dict_treemap` gain a `monthly_spend` x `account_age_days` heatmap; no panel was displaced. The standalone `viz heatmap` has no stats to type-check, so its auto mode still drops every near-unique column.
