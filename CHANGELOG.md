...
### Added
- **`--tool-definition <command>` and `--export-tool-definitions <dir>`: JSON tool definitions for EVERY command, plus the help Markdown, now ship inside the `qsv` and `qsvmcp` binaries** ([#4638](https://github.com/dathere/qsv/issues/4638)). Agents and tools that integrate with qsv can now read definitions matching the *installed* version instead of fetching them from GitHub master, which may be newer than the local binary. `qsv --tool-definition stats` prints one command's JSON to stdout. `qsv --export-tool-definitions <dir>` writes `<dir>/tool_definitions/qsv-<cmd>.json`, `<dir>/help/<cmd>.md` (plus `TableOfContents.md`) and a `manifest.json` recording the qsv version, the binary, its installed commands and which of them are in the curated MCP skill set. Only the commands compiled into that binary are exported, and re-exporting into a used directory removes files left over from another version or feature set. Both flags work from any directory and in the released binaries. The definitions previously existed only for the 56 MCP commands; `--update-mcp-skills` now also writes one for every command in the README command table (79) to the new `docs/tool-definitions/`, of which `.claude/skills/qsv/` remains the curated, byte-identical MCP subset (the MCP server loads every JSON in that directory as a tool, so the rest live apart). Those generators still parse the qsv source and so only run inside a checkout, and `--update-mcp-skills` needs the `mcp` feature, which the prebuilt `qsv` lacks; their out-of-repo errors now point to the export. The files are the committed ones, embedded by `build.rs` at compile time (the wiki-lint CI job keeps them in sync with the generators), and add about 1.2 MB per binary.
