...
### Added
- **`--tool-definition <command>` and `--export-tool-definitions <dir>`: the MCP tool definitions and help Markdown now ship inside the `qsv` and `qsvmcp` binaries** ([#4638](https://github.com/dathere/qsv/issues/4638)). Agents and tools that integrate with qsv can now read definitions matching the *installed* version instead of fetching `.claude/skills/qsv/*.json` from GitHub master, which may be newer than the local binary. `qsv --tool-definition stats` prints one command's JSON to stdout. `qsv --export-tool-definitions <dir>` writes `<dir>/mcp_skills/qsv-<cmd>.json`, `<dir>/help/<cmd>.md` (plus `TableOfContents.md`) and a `manifest.json` recording the qsv version, the binary and its installed commands, covering only the commands compiled into that binary. Both work from any directory and in the released binaries. By contrast, `--update-mcp-skills` and `--generate-help-md` still regenerate from source and so only run inside a qsv checkout, and `--update-mcp-skills` needs the `mcp` feature, which the prebuilt `qsv` lacks. Those two flags' out-of-repo errors now point to the export. The files are the committed ones, embedded by `build.rs` at compile time (the wiki-lint CI job keeps them in sync with the generators), and add about 1 MB per binary. Tool-definition JSON still covers only the MCP command set; other commands export their help Markdown only.
