...
- `util`: `safe_header_names` now keeps the duplicate-suffix variant within the documented 60-character limit. Previously the 60-char truncation was applied to the base name *before* the disambiguation suffix was appended, so a 60-char base plus `_2`/`_3` overflowed to 62+ characters — past the documented bound and uncomfortably close to PostgreSQL's 63-byte identifier limit. The base is now trimmed to leave room for the suffix so the final name is always ≤60 chars. Affects every caller (`safenames`, `applydp`, `apply`, `fetch`, `python`); see BREAKING note in *Changed*.
...
- **BREAKING** `util::safe_header_names`: the 60-character cap is now enforced on the *final* name, including any duplicate-disambiguation suffix. Previously the base name was truncated to 60 chars and a `_<n>` suffix could push the result to 62+ chars; now the base is trimmed to leave room for the suffix so the final name is always ≤60 chars. Long header names that previously generated suffixed variants of 61–63 chars (in `safenames`, `applydp`, `apply`, `fetch`, `python`) will now be one or two characters shorter at the boundary. Stored mappings keyed on the over-long form will not match.
