...
- **`moarstats`: 17 more statistical measures (56 -> 73), each checked against scipy or a closed-form oracle.**
  - **Derived from the stats cache, no extra pass:**
    ...
    - `robust_min_zscore` / `robust_max_zscore`, the Iglewicz-Hoaglin modified z-scores of the extremes.
    - `kelly_skewness` and `percentile_ratio_90_10`. Both need P10 and P90, which the default percentile list includes.
    - `zero_share`, which flags zero-inflation.
    - `berger_parker_dominance`, the mode's share of rows. It works for all field types.
  - **`--advanced`, from each column's full value vector:**
    - `moment_skewness` (G1).
    - The L-moment ratios `l_cv`, `l_skewness` (tau3) and `l_kurtosis` (tau4). These are robust, bounded analogues of CV, skewness and kurtosis, and match `scipy.stats.lmoment`.
    - `lag1_autocorrelation` in file order, which exposes trends, drift and clustered blocks that `sortiness` misses. It is byte-identical under `--jobs 1` and parallel indexed processing.
    - `hoover_index`.
    - `benford_mad`, Nigrini's first-digit conformity test as a data-quality and fabrication signal. It is only emitted for numeric data with at least 100 non-zero values spanning at least two orders of magnitude.
  - **`--bivariate`:**
    - `cramersv` gives Cramér's V for any pair of field types. It is computed in closed form from the joint frequency table mutual information already builds, and shares its `--cardinality-threshold` gate.
    - `regression` gives the OLS `regression_slope`, `regression_intercept` and `r_squared` of field2 on field1, from the streaming Pearson state.
    - Both are included in `--bivariate-stats all`.
- **`sqlp`: new SQL surface from the Polars bump to `9d5804d`** (upstream `py-1.44.0` -> the pinned rev). None of this needed qsv changes beyond the bump, but none of it was covered by tests either, so `tests/test_sqlp.rs` now exercises each one:
...
### Changed
- **`moarstats --bivariate` no longer drops every pair that involves an all-unique field.** A field whose cardinality equals the row count used to be skipped from the bivariate sidecar entirely. That was meant to keep mutual information from saturating on ID-like columns, but it also discarded Pearson, Spearman, Kendall, covariance and regression, which are perfectly well-defined for such columns. Continuous measurements (prices, coordinates, durations at full precision) are usually all-unique, so on typical data most numeric pairs silently vanished. Those pairs are now kept, with correlation, covariance and regression reported and the frequency statistics (`mutual_information`, `normalized_mutual_information`, Theil's U, `cramers_v`) left empty. The all-unique test now shares one gate with `--cardinality-threshold` (and uses `>=`, since an approximate cardinality can overshoot the row count). A pair is still dropped when nothing could be computed for it: an all-unique field paired with a non-numeric field, or a frequency-only `--bivariate-stats` run. **The sidecar gains rows**, including numeric ID columns correlated against other numeric columns. `viz smart --bivariate` output is unchanged: it ignores rows without NMI, and every all-unique pair has none. Its moarstats subprocess does compute those rows, though, since it requests `pearson,spearman`, and then discards them. Spearman and Kendall buffer each pair's values, so on a file with many all-unique numeric columns both that subprocess and a direct `--bivariate-stats all` run use more memory and time. That is the same cost near-unique columns above `--cardinality-threshold` already paid, and `--bivariate-batch` still bounds it. On `nyc_311.csv` (10k rows) `-S all` went from 780 to 785 rows and from 346.8 MB to 352.3 MB peak RSS.
- **BREAKING (x86_64 prebuilts): now built for `x86-64-v3`, requiring a CPU with AVX2/FMA/BMI2** ([#4649](https://github.com/dathere/qsv/issues/4649)). Previously the x86_64 prebuilts targeted baseline SSE2, so auto-vectorized hot loops (the algebraic-float batch reductions in qsv-stats, `moarstats` and `viz`'s correlation matrix, plus the dependency tree) could only use 128-bit vectors. `-C target-cpu=x86-64-v3` lets the compiler use 256-bit AVX2 and FMA everywhere without the per-runner SIGILL risk of `target-cpu=native`. This covers `qsv`, `qsvlite` and `qsvmcp` on Linux (gnu, musl) and Windows (msvc, gnu), plus the `qsvpy` and Debian-package builds, and therefore the MCPB bundle. The x86_64 nightly builds also move from `target-cpu=native` (which tied them to the build runner's exact CPU) to `x86-64-v3`. **`qsvdp` is exempt** and stays on the portable baseline, binary and `.deb` alike, since DataPusher+ servers often run on VMs without AVX2. aarch64, ppc64le and s390x prebuilts are unchanged. **Not every post-2013 machine qualifies:** Intel Pentium/Celeron/Atom parts up to ~2020 and VMs with a conservative CPU model (QEMU/Proxmox `kvm64`/`qemu64`) lack AVX2 - check with `grep -o avx2 /proc/cpuinfo | head -1`. On such CPUs the prebuilts die with `Illegal instruction`, and **`qsv --update` will install a binary that cannot run - or update itself back.** Do not self-update on a non-AVX2 CPU; if you already have, re-download a 23.0.x release from the [releases page](https://github.com/dathere/qsv/releases) or compile from source, which still defaults to the portable baseline.
- **`viz`: vendored DataTables bundle bumped to `dt-3.1.1/b-4.1.1/cc-2.1.1/date-2.1.1/sb-2.1.0`** (DataTables 3.0.4 -> 3.1.1, Buttons 4.0.3 -> 4.1.1, ColumnControl 2.0.2 -> 2.1.1, DateTime 2.0.0 -> 2.1.1, SearchBuilder 2.0.1 -> 2.1.0). Upstream's 3.1 line is a packaging release: npm `exports`/ESM loading and evaluation in windowless environments (SSR / React Server Components), with the extensions bumped in lockstep and no functional changes of their own. The stylesheet is byte-identical to 3.0.4's apart from its version banners, and the JS grows ~1.2 KB: windowless-evaluation guards, plus the new Plus evaluation-notice strings described below. The two DataTables core fixes are both inert for the data viewer: an element with `id="jQuery"` could previously be mistaken for the jQuery library (qsv runs DataTables in vanilla mode and emits no such element), and a column-width miscalculation that needs nested tables (the drawer has none). ColumnControl 2.1.0's fix only applies to a table with no header markup at all, and the drawer always builds its own `thead`. 3.1 also adds a "DataTables Plus - Evaluation Mode" corner notice, but only the commercial Plus extensions can trigger it, via `DataTable.plus()`. The vendored bundle contains no calls to it, and none of the Plus suite is vendored. Re-verified in Chrome across all three delivery paths: the default lazy gzip inflate on first drawer open, `QSV_VIZ_NO_COMPRESS=1` inline, and `QSV_VIZ_CDN=1` with SRI. In each, all four extensions register, the console stays empty and no notice appears. Also re-checked on 3.1.1, with results matching what [#4592](https://github.com/dathere/qsv/pull/4592) recorded for 3.0.4:
  - title-click sorting (`[] -> [[1,"asc"]] -> [[1,"desc"]]`), with clicks on the ColumnControl search row still not sorting;
  - the `search.fixed("dtcc")` Clear Filters counter ("Clear Filters (1)", then all 10,000 rows restored on clear);
  - SearchBuilder date conditions opening the DateTime picker;
  - `DataTable.render.text()` escaping (zero elements injected);
  - the `dt.settings()[0].columns[].width/.visible` internals behind the [#4325](https://github.com/dathere/qsv/pull/4325) Safari workaround;
  - header geometry, measured against a 3.0.4 baseline of the same nyc311 page and identical to it: 87px/45px `thead` rows, a 31.5px search input, 5742px total width, and search widgets on the same 29 of 41 columns.

  All seven vendored DataTables locale files were sha256-compared against the 3.1.1 plug-in locales and were byte-identical, so only their documented source URLs moved. The upstream `zh` SearchBuilder date-condition inversion is still unfixed, so the splice-time override stays. Subresource Integrity hashes for the `QSV_VIZ_CDN` path were regenerated alongside the bundle.
- **Nightly builds: the `nightly` feature now enables `encoding_rs/simd-accel` and `encoding_rs/std`.** `simd-accel` gives encoding_rs explicit SIMD fast paths: SSE2/AVX2 on x86_64 and NEON on aarch64. `std` adds runtime CPU-feature multiversioning of additional code paths on x86 (the ASCII AVX2 dispatch works without it). Both require nightly Rust, so stable builds are unchanged. qsv's main CSV path never goes through encoding_rs; the change only affects commands that decode text through it: legacy `.xls` in `excel` (via calamine), string columns in `readstat`, response charsets in `fetch`/`fetchpost`/`profile`, and encoding detection in `sniff`.
- CI: bump the pinned nightly toolchain from `nightly-2026-04-01` to `nightly-2026-09-01` (same as the pinned polars revision). The old pin was rustc 1.96, below qsv's 1.98 MSRV, so none of the nightly workflows could build qsv at all. The test harness now locates the binary under test via `CARGO_BIN_EXE_*` instead of next to the test executable, which newer nightly cargo (the build-dir layout) no longer guarantees.
- **`profile`: the DCAT-US v3 projection is now emitted in its canonical form, which is a breaking change to the `projection` block.** DCAT-US v3 stopped being JSON-LD. GSA deleted the JSON-LD context and the SHACL shapes upstream on 2026-04-02 ([`99ef81c9`](https://github.com/GSA/dcat-us/commit/99ef81c9), *"Move aside outdated files and emphasize the JSON Schema"*), the canonical examples under [`jsonschema/examples/`](https://github.com/GSA/dcat-us/tree/main/jsonschema/examples) use **unprefixed** keys and carry no `@context`, and migration Step 14 says to delete the key outright. qsv was still emitting the CURIE-keyed JSON-LD form, so the projection had **none of the four required Dataset keys** under the names the schema declares - `title`, `description`, `contactPoint` and `identifier` were all hidden behind `dct:`/`dcat:` prefixes. Pasted into data.gov's own validator at <https://harvest.data.gov/validate/> (which runs the same GSA bundle qsv vendors) the output was **rejected outright**; the `@context` it advertised, `doi-do.github.io/dcat-us/context.jsonld`, has been a dead 404 since the DOI-DO repo was archived. The projection now emits unprefixed keys and no `@context`, with `@type` values to match (`Dataset`, `Distribution`, `Catalog`, `Kind`, `Organization`, `Location`, `Standard`, `Checksum`, `PeriodOfTime`) - `publisher` is an `Organization` carrying `name`, and `contactPoint` a vcard `Kind`. **Anything reading `projection.dct:*` must read the bare names instead**; the `ckan` block, which datapusher-plus actually consumes, is unchanged. Value shapes follow the migration guide: `rights` is an array (Step 7), `describedBy` a Distribution object with `describedByType` folded into its `mediaType` (Step 8), `conformsTo` a `Standard` keyed `identifier` rather than `@id` and pointing at the version IRI rather than the docs page (Steps 10 & 13), `landingPage` a `Document` (Step 11), and `accrualPeriodicity` an ISO 19115 / Dublin Core **code string** - it had been emitting `{"@id": "<publications.europa.eu IRI>"}`, a DCAT-AP (EU) convention matching none of the four branches v3 allows, and those IRIs remain correct in `dcat-ap-v3.yaml`. `accessRestriction`/`useRestriction` are arrays, `spatial` Locations carry a bare WKT string, and `inSeries` is gone because GSA removed it from `Dataset.json`. `dcat-us:bureauCode`, `dcat-us:programCode` and `dcat-us:accessLevel` keep their prefixes and keep being emitted: none is a v3 property - they appear in **none** of the 26 definitions - but agencies still need them for OMB M-13-13 / Project Open Data, and migration Step 5 explicitly says to retain `accessLevel` during the transition ("the v3.0 schema will not reject it"). Discovery merge gains `normalize_curies`, because publisher markup is still overwhelmingly CURIE-keyed and a discovered `dct:title` would otherwise land *beside* the inferred `title` instead of filling it. The emitted Catalog now returns **"No validation errors found"** from harvest.data.gov/validate.
- **`profile`: `--strict` now aborts on Required-severity findings only, and severity comes from the schema.** `classify_severity` had been substring-matching the Debug-formatted `jsonschema` error kind for `"Required"`, which collapsed every `type`, `enum`, `pattern` and `anyOf` failure to Recommended - while `--strict` aborted on *any* non-empty finding vector, making severity decorative in both directions. Severity is now read from the GSA bundle's own `requirementLevel` annotations, resolved against the **class that owns the path**: `modified` is Recommended on `Dataset` but Mandatory on `CatalogRecord`, so a flat property-name index would escalate an ordinary Dataset date problem to Required and abort the run. Nested value objects that are not modeled fall back to an any-class lookup taking the strongest level, so `--strict` over-reports rather than under-reports.

- **`sqlp`: three Polars SQL behavior changes that can break an existing query.** All three arrive with the bump to `9d5804d` and are now pinned by tests:
  - **`CAST(<string> AS DATE/TIME/TIMESTAMP)` parses rather than casts.** The string->temporal cast kernel was removed upstream ([#28062](https://github.com/pola-rs/polars/pull/28062)) and SQL casts were re-routed through format inference ([#28986](https://github.com/pola-rs/polars/pull/28986)), so the spelling still works - but `CAST` is now **strict** and fails the whole query on a value it cannot parse, where `TRY_CAST` nulls just that row. The failure has two shapes: if *some* rows parse you get ``conversion from `str` to `date` failed``, naming the offending value; if *none* parse you get `could not find an appropriate format to parse dates`.
  - **SQL scope rules follow Postgres** ([#28937](https://github.com/pola-rs/polars/pull/28937)). Once a relation is aliased, its original name is out of scope: `SELECT t1.a FROM t1 AS f` now errors with `no table or struct column named 't1' found`. An unqualified `ORDER BY` key still resolves, and an outer alias is still visible to a correlated subquery.
  - **a scalar subquery's aggregate binds to its own relation** ([#28939](https://github.com/pola-rs/polars/pull/28939)), so aggregating an outer-relation column inside the subquery is an error instead of silently resolving; and **unaliased constants in a `SELECT` with `GROUP BY`** are projected once per group as `literal`, `literal:1`, `literal:2`, ... rather than colliding on one column ([#29367](https://github.com/pola-rs/polars/pull/29367)).
- **`reqwest`'s `charset` and `system-proxy` features are now enabled explicitly.** Both were already active for the full `qsv` binary, but only by accident of Cargo feature unification: `cpc` (via `apply`), `plotly_static` (via `viz_static`) and `polars-io` all depend on `reqwest` *without* `default-features = false`, and reqwest's defaults include them. Listing them on qsv's own dependency pins that behavior, and turns it on for the leaner binaries that nothing else was pulling it into - **`qsvlite`, `qsvdp` and `qsvmcp` now**:
  - decode HTTP response bodies according to the `charset` parameter of the response's `Content-Type` (with BOM sniffing and stripping) instead of a blind lossy-UTF-8 conversion, and
  - honor **OS-level proxy configuration** (macOS SystemConfiguration, Windows registry) in addition to the `HTTP_PROXY`/`HTTPS_PROXY` environment variables.

  No new crates: `encoding_rs` and `mime` were already in the dependency tree via `calamine`, `csv-nose`, `chardetng`, `quick-xml` and `ureq`.
- **`stats`: ~2% faster by default, via qsv-stats 0.56.1** ([#4647](https://github.com/dathere/qsv/issues/4647)). The string-length accumulator is fed on every non-null cell of every column but only ever reports length stddev/variance, yet each sample still paid a `ln()` and a reciprocal to maintain geometric/harmonic sums nobody read. It now uses the new `OnlineStats::without_hg_sums()` constructor, which skips that work. The main numeric accumulator still computes geometric/harmonic means. On the 1M-row NYC 311 benchmark, three 20-40-run hyperfine passes measured -0.9%, -3.7% and -1.9%. `stats` output is byte-identical to before the change, both with and without `--everything` and both unindexed and indexed (the parallel path that merges per-chunk accumulators).

### Fixed
- **`moarstats --bivariate` no longer reports correlations for String columns.** The bivariate pass parsed every column that was not a Date numerically, String columns included. A column of names was therefore correlated on the few values that happened to look numeric, while `n_pairs` still counted every row. On `allegheny_dog_licenses.csv`, `DogName` x `ValidDate` reported Spearman rho = **0.949** from 4 of 50,013 rows (the dogs named `2`, `1001` and `4128`). A synthetic table of 300 names plus 3 numeric "names" reported rho = 1 and r = 0.84 with `n_pairs` = 303. Pearson, Spearman, Kendall, covariance and regression now take values only from Integer, Float, Date, DateTime and Boolean columns. A String column still takes part in its pairs' frequency statistics (mutual information, Theil's U, Cramér's V). This also reached `viz smart --bivariate`: a String pair with an NMI score could show a false "nonlinear" hover warning, because its bogus Spearman and Pearson values diverged by more than the 0.15 threshold. **The sidecar changes** for any String column holding some numeric-looking values, such as a ZIP-code column typed String because of one `N/A`. Those pairs now have empty correlation cells.
- **`viz`: near-unique Float measures are no longer treated as IDs anywhere, and a unit is never stated twice.** Follow-ups to the entry below ([#4653](https://github.com/dathere/qsv/issues/4653)):
  - **`viz heatmap` (auto mode, no `--cols`)** now drops a near-unique column only when every value is written as an integer, the same syntactic test `stats` uses to type a column Integer. A near-unique Float measure such as a unit price used to be dropped as a presumed identifier. **The matrix can gain columns.**
  - **`viz smart`'s measure-by-dimension panel** ("revenue by region") excluded every untagged near-unique column, so it was stricter than the correlation heatmap. It now excludes only near-unique Integers, unless the dictionary tags the column `measure`. **Pages can gain this panel** when a full-precision Float measure is strongly explained by a low-cardinality dimension.
  - **A title or header that already ends with its unit or currency no longer gets it again.** An LLM-inferred dictionary titled `depth_km` "Depth (km)" while also setting `x-qsv.unit: km`, so the panel subtitle read "Depth (km) (km)" (roborev 4884). The same guard covers the scatter, bubble and 3D axis titles, where a header like `Temp (°C)` read "Temp (°C) (°C)". The match is exact and case-sensitive, so "Mass (kg)" still gets "(g)" appended if its unit is grams.
  - **`--bivariate`'s "moarstats produced no bivariate pairs" note** was false whenever pairs existed but none had a mutual-information score, which is common now that all-unique numeric pairs are kept. It now says "no pairs with an NMI score" in all 8 locales.
  - Known limit: `stats` types an integer too large for i64 (20+ digits) as Float, so `viz smart` treats such an ID as a measure. The standalone `viz heatmap`'s syntactic check still excludes it ([#4657](https://github.com/dathere/qsv/issues/4657)).
- **`viz smart` no longer hides full-precision numeric measurements from its correlation view, and `--bivariate` no longer drops them altogether** ([#4653](https://github.com/dathere/qsv/issues/4653)). Two rules treated "every value distinct" as "this is an ID":
  - `describegpt --infer-content-type` stamps any field whose cardinality equals the row count as `unique_id` (role `identifier`, concept `id.surrogate_key`). A continuous measurement at full precision (a price, a duration, a coordinate) is all-unique as a matter of course, so it was classified as a key. `viz smart --bivariate` infers a dictionary, which routed every such column to Skip and removed it from **every** panel. On a synthetic 500-row table, `price`, `qty` and `weight` (price vs qty rho = -0.986) disappeared from the Data Schematic entirely. **Float fields are now exempt**; Integer, String and Date fields keep the rule. The LLM still cannot assign `unique_id` itself, so an exempt Float gets its content type, role and concept from the LLM like any other field.
  - `viz smart`'s correlation matrix dropped every column with `uniqueness_ratio > 0.95`. It now drops only near-unique **Integer** columns, matching the rule `classify` already used to decide what to chart. Mutual information cannot score these pairs either, so until now no panel showed them.
  In the gallery, `smart_timeseries` now correlates all four of `open`/`high`/`low`/`close` instead of two, and its drill-down follows the stronger pair (open vs low). `smart_smarter` and `smart_dict_treemap` gain a `monthly_spend` x `account_age_days` heatmap; no panel was displaced. The standalone `viz heatmap` and `viz smart`'s measure-by-dimension panel now follow the same rule; see the next entry.

  **An existing inferred dictionary keeps the old classification.** `viz smart --dictionary infer` (and so `--bivariate`) reuses `<stem>.schema.json` when it exists. To pick up the fix, delete that sidecar or set `QSV_VIZ_DICT_FRESH=1` for one run.

  **describegpt output change:** the Float exemption also applies to primary-key inference in the Semantic Markdown dictionary, and to a column's `1:1` join class. A table with an Integer `id` and an all-unique Float used to report two unique columns and no primary key. It now infers `id` as the primary key.
- **`moarstats` and `pragmastat` no longer keep an older qsv's values after an upgrade.** Both append their columns to `<FILESTEM>.stats.csv` and skip any column that is already there, while judging that file current by mtime alone. An extended cache built by an earlier release was therefore reused indefinitely, so a formula fixed in a later release never reached it. After this release's `jarque_bera`/`bimodality_coefficient`/`kurtosis` fix, re-running `moarstats --advanced` on such a cache kept the old values and added the new `moment_skewness` beside them, which made the row inconsistent. Both commands now also treat the baseline as stale when its `.stats.csv.json` sidecar was written by a different qsv version (the same rule `stats` already applies to its own cache), so the baseline is regenerated and every appended column is recomputed. Regenerating the baseline drops all appended columns, so a `pragmastat` run rebuilds only the `ps_*` columns, and `moarstats` columns come back on the next `moarstats` run (the same thing that already happened when the input changed). A cache with no sidecar is still reused as before.
- **`moarstats --bivariate` no longer leaves a stale sidecar behind when no pair qualifies.** When every candidate pair was filtered out, the `<FILESTEM>.stats.bivariate.csv` write was skipped, so the file from an earlier run survived and read as this run's result. For example, an `-S all` sidecar still sat there after a later `-S nmi` run that produced nothing. The sidecar is now always rewritten, header-only when no pair survives. `viz smart --bivariate` already guarded against this with its own freshness check; other readers, including agents following the qsv skills, did not.
- **`moarstats --advanced`: Jarque-Bera and the Bimodality Coefficient were computed from the wrong skewness, and kurtosis from rounded, mismatched inputs. All three now match scipy exactly, so their values change.** Both formulas are defined on *moment* skewness, but they were fed the base `stats` `skewness` column. That column is the quartile-based Bowley skewness `(q3 - 2q2 + q1) / iqr`, which is capped at +/-1, so heavily skewed data looked far more normal than it is. On a lognormal sample (n=200), BC came out **0.026** where the correct value is **0.64**, on the wrong side of the 0.555 threshold. Separately, `kurtosis` was fed the cached `stddev` squared, which is a *sample* variance, into a formula that expects a *population* variance. That cached mean and stddev are also rounded (default 4 dp), which badly distorts small-scale data: kurtosis read **0.112** where scipy gives **-0.096** on values around 3e-4. `--advanced` now computes the exact mean and central moments from each column's values:
  - `kurtosis` is the sample excess kurtosis G2 (scipy `kurtosis(bias=False)`).
  - The new `moment_skewness` column is G1 (scipy `skew(bias=False)`).
  - `jarque_bera` uses the classical biased moments, matching `scipy.stats.jarque_bera`.
  - `bimodality_coefficient` uses the finite-sample form `(G1² + 1) / (G2 + 3(n-1)² / ((n-2)(n-3)))` (Pfister et al., 2013).

  `mean_ad`, `gini_coefficient` and `atkinson_index` now also use the exact mean and sum instead of the rounded cached ones. `jarque_bera` and `mean_ad` are now populated for Date/DateTime columns (in days); before, they were silently left empty. Existing `.stats.csv` files that already hold these columns are recomputed automatically on the first `moarstats` run after upgrading (see the stale-cache fix above), or immediately with `--force`.

  `pivotp --agg smart` now requires negative excess kurtosis before it treats a column as bimodal. Once BC is computed correctly, heavily skewed *unimodal* columns (amounts, counts, durations) also clear the 0.555 threshold. Those columns are leptokurtic, while a genuinely two-peaked distribution is flat-topped. This is the same guard `viz smart` already applies. Without it, a plain lognormal `amount` column was reported as "Bimodal" and pivoted to `Len`.
- **`viz smart` (without `--smarter`) now computes the bimodality coefficient the same way `moarstats --advanced` does, so a column charts identically with or without `--smarter`.** Plain `viz smart` builds its own BC in `enrich_bimodality`, and it still used the formula `moarstats` just replaced: the cache's quantile (Bowley) `skewness`, plus kurtosis from the cache's *rounded* mean and variance. It now accumulates exact central moments in its single streaming pass (Terriberry's online update, still O(1) memory per column and no cached mean needed) and feeds them into the same shared `CentralMoments` definitions of G1, G2 and the finite-sample BC. The visible case is an *asymmetric* bimodal column. For example, with 77% of rows in one cluster and 23% in a far one, every quartile falls inside the big cluster. Bowley skew is then ~0, so plain `viz smart` drew a box plot whose whiskers hid the second peak, while `--smarter` drew a histogram. Both now draw the histogram. The platykurtic guard is unchanged, so heavily skewed unimodal columns still stay box plots.
- **`viz smart`: choropleth pages with click-to-filter are byte-stable again** ([#4645](https://github.com/dathere/qsv/issues/4645)). The `RAWS`/`QUAL_RAWS` region-filter lookup tables were serialized in `HashMap` order, which is randomized per process, so every gallery regen reordered their keys. That added a spurious diff to `smart_pa_crashes.html` and `smart_wpa_211_requests.html`. They are now emitted in sorted key order. Rendering was never affected.
- **`count` (and every `util::count_rows()` consumer): the un-indexed row count is multithreaded again** ([#4603](https://github.com/dathere/qsv/issues/4603)). `polars_count_input()` counted by running `SELECT COUNT(*)` through a `SQLContext` - as the `read_csv('...')` table function for a plain comma-delimited CSV, and over a registered `LazyFrame` for everything else. Polars 1.44.x stopped lowering SQL `COUNT(*)` to its parallel fast-count path, so **both** spellings counted on a single thread. On the 538 MB / 1M-row NYC 311 benchmark file `qsv count` went from 82 ms to 664 ms while **total CPU time was unchanged** (655 ms -> 643 ms): the work never grew, it just stopped being spread across cores. Forcing the *old* binary to one thread (`POLARS_MAX_THREADS=1`) reproduces the new timing exactly (577 ms vs 678 ms), which accounts for the entire regression - and no polars env toggle (`POLARS_MAX_THREADS`, `POLARS_ENGINE_AFFINITY=in-memory|streaming`) brought the threading back. The one formulation that never regressed is `scan_csv(...).select(len())`, which is precisely the one qsv did not use, so the count now drops `SQLContext` altogether and uses a single `LazyCsvReader` + `select(len())` under the same `OptFlags` the non-SQL branch already applied. That also collapses the two query-construction branches into one and removes the SQL path-escaping. `qsv count` is back to **103 ms** (6.3x parallel, and `--low-memory` from 600 ms to 101 ms), and the commands that funnel through `count_rows()` follow: `frequency` 1.085 s -> 543 ms, `searchset` 1.483 s -> 915 ms, `split --chunks` 1.348 s -> 834 ms. Note this was **not** a re-regression of [pola-rs/polars#27504](https://github.com/pola-rs/polars/pull/27504) - that fix targeted `select(pl.len())`, which still works - but a distinct regression on the SQL side, reported upstream as [pola-rs/polars#29393](https://github.com/pola-rs/polars/issues/29393) and bisected to py-1.43.2 -> py-1.44.1 (still present on 2.0.0rc1). **The `SQLContext` removal is a workaround, not a preference**, and can be revisited if upstream restores the lowering.
- **`viz`: static image export no longer depends on a path from the machine that built the binary** ([#4620](https://github.com/dathere/qsv/issues/4620)). `plotly_static` resolves its webdriver as `WEBDRIVER_PATH` -> `option_env!("WEBDRIVER_DOWNLOAD_PATH")` -> error, and that second value is an absolute path its build script bakes in - `$HOME/.local/bin/chromedriver` **on the build host**. There is no `PATH` lookup anywhere in that chain, so a prebuilt `qsv viz ... -o chart.svg` failed with `Invalid WebDriver path '/Users/<whoever-built-it>/.local/bin/chromedriver'`, and an installed chromedriver sitting on the user's `PATH` was never even considered. It affected both `viz_static` prebuilt families - the `x86_64-unknown-linux-gnu` binaries bake `/home/runner/...`, and `aarch64-apple-darwin` builds on a self-hosted runner, which is how a maintainer's home directory ended up shipping to users. `viz` now probes for a real chromedriver before any image export - `WEBDRIVER_PATH` (only if it actually exists), `$WEBDRIVER_INSTALL_PATH`, `~/.local/bin`, `PATH`, then `/opt/homebrew/bin`, `/usr/local/bin`, `/usr/bin` - and sets `WEBDRIVER_PATH` to the first hit. `~/.local/bin` is probed **before** `PATH` on purpose: on a source build the build script has already downloaded a driver matched to the Chrome it detected, and resolving to a different chromedriver on `PATH` would risk a major-version mismatch. When nothing is found the environment is left untouched (a correct baked-in path still wins on a source build) and the failure now names `WEBDRIVER_PATH`/`BROWSER_PATH`, the Chrome-major-version requirement, and [Chrome for Testing](https://googlechromelabs.github.io/chrome-for-testing/) - Homebrew's `chromedriver` cask was disabled 2026-09-01, so it is no longer a way to satisfy the default.
- **`viz`: the docs no longer offer Firefox for static export, because it cannot work.** `viz_static` enables `plotly/static_export_default`, which selects `plotly_static/chromedriver` - and `chromedriver`/`geckodriver` are **mutually exclusive** upstream features (`compile_error!` if both). The browser name, the options key and the driver's own CLI arguments are all `#[cfg]`-selected at compile time, so pointing `WEBDRIVER_PATH` at a geckodriver fails twice over: the spawn passes chromedriver's `--verbose`, which geckodriver rejects, and the session then requests `browserName: "chrome"`, which it cannot serve. This is a qsv build-configuration consequence, **not** a `plotly_static` bug - upstream supports Firefox fine in a build that selects it. `viz --help`, `docs/FEATURES.md`, `Cargo.toml` and the wiki now say Chrome/Chromium only.
- **`profile`: an ISO-8601 interval in `metadata_modified` no longer aborts `--strict`.** `R/P1Y` is exactly the shape migration Step 1 says agencies are moving away from, so it is common real input from legacy CKAN instances. `sanitize_iso_8601_interval` correctly returns minijinja UNDEFINED for it, but `dcat_us_date` took a `&str`, so minijinja could not coerce the undefined and raised `invalid operation: value is not a string` — which `emit_field` reports at **Required** severity, and which `--strict` (now gating on Required) treated as fatal. A legacy dataset hard-failed the whole command over one optional date. The filter takes a `Value` and passes UNDEFINED straight through, so the field is simply omitted. Date normalization also preserves sub-second precision now: the fixed `%H:%M:%S` format silently truncated `2024-12-15T08:30:00.123456` to `...T08:30:00Z`.
- **`profile`: `THIRD_PARTY_NOTICES.md` cites the DCAT-US pin actually vendored.** Both spellings (the 7-char form in the summary table and the full SHA in the detail section) still named the previous commit after the bundle refresh, so shipped provenance — an attribution claim, not a cosmetic one — pointed at the wrong snapshot. A test now asserts both against `MANIFEST.json`, since the existing pin test re-hashes the vendored files but said nothing about the notices.
- **`profile`: `--validate` now asserts string formats, and dates are normalized to UTC Zulu.** Found by submitting the emitted catalog to data.gov's validator: qsv reported a clean bill of health and data.gov rejected it with ``$.dataset[0].distribution[0].modified, '2024-12-15T08:30:00' does not match any of the acceptable formats``. Two defects, and the systemic one is the first. JSON Schema 2020-12 treats `format` as an **annotation**, not an assertion, so `"format": "date-time"` was collected and never enforced - every format-shaped defect was invisible locally while failing the only check users run `--validate` for. Formats are now asserted. Separately, CKAN's `last_modified` is an offset-less datetime, which matches none of the accepted forms; migration Steps 12 and 15 both say to normalize to UTC Zulu, so the new `dcat_us_date` filter passes `YYYY` / `YYYY-MM` / `date` / RFC 3339 through untouched, stamps a `Z` on an offset-less datetime, and suppresses the field entirely for anything else rather than emitting it invalid. The filter and the format-assertion switch are a pair - removing either re-opens the hole.
- **`profile`: unknown keys in the DCAT-US v3 projection are now reported.** No schema in the GSA bundle sets `additionalProperties`, so an unknown key - a typo, a stale property upstream has removed, or a key under the wrong namespace - validated silently; `dcat-us:` was also missing from the CURIE strip list, so every `dcat-us:*` key bypassed inspection entirely. That blind spot is exactly where the `dcat:describedBy` (should be `dcat-us:describedBy`), `dcat:versionNotes` (should be `adms:versionNotes`) and dead-`@context` bugs lived. A lint now reports emitted keys the target definition does not declare, scoped to the node types qsv constructs (root, each Dataset in a Catalog, each Distribution) with the five deliberate qsv extensions allowlisted. It caught three real stragglers on its first run. A companion test asserts every `path:` the profile declares still exists in the vendored schema, so an upstream rename fails on the next pin refresh rather than whenever a fixture happens to populate the slot.
- `profile`: the vendored GSA DCAT-US v3 schema bundle is refreshed from `cf878900` (2026-05-26) to [`1ca074b6`](https://github.com/GSA/dcat-us/tree/1ca074b67dd1ad3cea8f0dd4ae951864491fd104/jsonschema/definitions). Upstream demoted `publisher` from Mandatory to Recommended and dropped it from `Dataset.json`'s `required` array, **removed** `inSeries`, and added `accessRestriction`, `cuiRestriction`, `useRestriction`, `inventoried` and `license` to Dataset (plus `inventoried` on Distribution and `datasetSeries` on Catalog). Conformance had been measured against a four-month-old target.
- `profile`: DCAT discovery no longer aborts silently on a body it cannot read as strict UTF-8. The three HTTP probes (`Link: rel=describedBy`, sibling-URL, and landing-page JSON-LD) read their capped body with `read_to_string`, which hard-errors on **any** non-UTF-8 byte - windows-1252 landing pages are common on open-data portals - and *also* on a perfectly valid UTF-8 body whose 4 MiB cap happened to slice a multi-byte codepoint. Because every caller swallows the error with `.ok()?`, either case ended discovery with no diagnostic. The capped read now decodes **according to the response's own `Content-Type` `charset`** (via `encoding_rs`, mirroring what `reqwest`'s `text_with_charset` does internally, BOM sniffing and stripping included) - so a windows-1252 landing page yields `Café` rather than either nothing at all or a corrupted `Caf` + U+FFFD. Bodies with no usable charset declaration fall back to lossy UTF-8, and a body actually truncated by the 4 MiB cap is still truncated - lossiness only stops truncation from being a second, earlier failure mode.
...
The headliner is the **Data Schematic's deep US Census integration**. A choropleth colored by raw counts is largely a population map - the big region wins because it is big. A choropleth colored by a *rate* is a finding ([example](https://dathere.github.io/qsv/smart_pa_crashes.html)).

Closing that gap used to mean sourcing boundary files, sourcing population tables, joining both to your data and computing the rate by hand; `viz smart` now does all of it from nothing but a CSV. `--geojson auto` fetches US boundaries from the [US Census TIGERweb REST services](https://tigerweb.geo.census.gov/tigerwebmain/TIGERweb_restmapservice.html), `--denominator census` fetches the matching population denominators, and a `--locations` column of county *names* - not just [FIPS codes](https://www.census.gov/library/reference/code-lists/ansi.html) - resolves against the Census's own name table with nothing supplied but the CSV, while `--geocode` extends the same path to *city*-name columns. [Census Designated Places](https://en.wikipedia.org/wiki/Census-designated_place) make `census:place` the full place universe rather than incorporated places alone ([#4394](https://github.com/dathere/qsv/issues/4394), [#4395](https://github.com/dathere/qsv/issues/4395), [#4397](https://github.com/dathere/qsv/issues/4397), [#4417](https://github.com/dathere/qsv/issues/4417), [#4540](https://github.com/dathere/qsv/issues/4540)).

`describegpt` closes the loop, which is what makes this more than a flag. It *derives* an `x-qsv.denominator` from a population column and declares the geography that denominator describes, so the [neuro-symbolic](https://en.wikipedia.org/wiki/Neuro-symbolic_AI) dictionary carries the denominator and `viz` charts the rate without the reader ever naming either. It also knows when **not** to: a denominator that is constant across regions, or one measured over a different geography than the region key, is refused rather than charted as a rescaled count ([example](https://dathere.github.io/qsv/gallery.html#fig-smart-data-schematic-denominator-aware-region-rate-real-counties)).

And the human is not merely *in* the loop - the human stays **in control** (thanks Liz Tupper!).

Everything above is an LLM **proposal**, and the artifact that actually drives a Data Schematic is the [editable JSON Schema data dictionary](https://github.com/dathere/qsv/blob/master/examples/viz/nyc311_dict.schema.json) sitting beside your CSV: an inferred dictionary is a *draft*, and the reviewed copy is the artifact of record. Domain experts fine-tune those inferences in a terminal UI - the [`visual-data-dictionary`](https://github.com/dathere/qsv/tree/master/.claude/skills/visual-data-dictionary) skill's `edit_dictionary.py` walks every column, edits the five fields that steer panel selection (`role`, `concept`, label, description, `aggregation`), and **previews how each edit reroutes the column before you render** (`--summary` prints the current routing non-interactively). The control is real but bounded, which is the point: qsv re-verifies every hint on read, so a hand-edit can correct the model where it guessed wrong, but **cannot assert a hint the data does not support** - a declared money measure on a zero-padded code column is refused, not drawn. Human judgment overrides the model; neither overrides the data.

Also new, and notable in its own right: **[`readstat`](https://github.com/dathere/qsv/blob/master/docs/help/readstat.md)** converts SAS (`.sas7bdat`, `.xpt`), Stata (`.dta`) and SPSS (`.sav`, `.zsav`, `.por`) files to CSV, so the decades of public-sector and academic data locked in proprietary statistical formats join the qsv pipeline without a licence ([#4542](https://github.com/dathere/qsv/issues/4542)).

Context also means **units**. `describegpt` now proposes a per-field [UCUM](https://ucum.org/) `x-qsv.unit` and a first-class `measure.money` concept, and `viz` carries both into KPI tiles, panel subtitles, hover text and axis titles - so a Data Schematic says "18.4 °C" and "$192B" rather than a bare float and an SI "G".
...
In the Age of AI - we compile all this context for Humans, Agents and Machines alike - as we march towards **_Accelerated Civic Intelligence (ACI)_** - using AI as a Tool towards Actionable Insights.
...
The [interactive benchmark Data Schematic](https://dathere.github.io/qsv/benchmarks/) and the [Benchmarks wiki page](https://github.com/dathere/qsv/wiki/Benchmarks) have been regenerated against the 23.0.1 run — 277 benchmarks on a 1,000,000-row × 41-column, 520 MB slice of NYC 311 data. Same machine and same prebuilt-PGO build as the 22.0.1 run, so the release-over-release deltas are real rather than a change of hardware.

⚡ **With an index, the flagship commands are the FASTEST they have ever been!** 🏎️ 

Of the 22 measurable indexed variants across `stats`, `frequency`, `validate` and `moarstats`, **17 set all-time highs** in 23.0.1 — among them indexed `frequency` (1,992,032 recs/sec), indexed `stats --everything` (1,805,054) and every indexed `moarstats` bivariate variant. The standout is full JSON Schema `validate` with an index, which **more than doubled**: 657,030 → 1,338,688 recs/sec, mean run time down from 1.52s to 0.75s (cc @Stranger6667 ).

**However, there is a regression, and I would rather flag it than have you find it.** Un-indexed `count` is **~8.6x slower** than in 22.0.1 (9.0M → 1.05M recs/sec), and the commands that need a row count without an index went with it — `sample_1000_systematic` (-58%), `frequency_selregex` (-53%), `extsort` (-47%), `split_chunks` (-41%), `searchset` (-39%) and `frequency` (-34%).

It is **not** the blank-line correctness fix, which is the obvious suspect since it landed in the same release. That fix only takes the slower csv-reader path when a file *may* contain a blank line, and on the benchmark file it never fires — forcing it is actually **faster** (0.35s vs 0.58s). The regression instead tracks this release's move of polars from crates.io `0.55.2` to the `py-1.44.2` git pin: `util::count_rows()` reaches polars **only when there is no index**, which is exactly the split the benchmarks show. Being chased down in [#4603](https://github.com/dathere/qsv/issues/4603).

*With* an index, none of it applies — `count` reads a stored row count at ~100M recs/sec, ~96x the un-indexed scan, and every affected command takes the same shortcut. **SO ALWAYS CREATE INDICES!**
...
---
<a id="v23-changes" name="v23-changes"></a>

### Breaking Changes
- **`pivotp`: the 100,000-column cap is now enforced unconditionally** ([#4589](https://github.com/dathere/qsv/issues/4589), [#4590](https://github.com/dathere/qsv/pull/4590)). `pivotp` already knew how to refuse an unreasonable pivot - `validate_pivot_operation()` hard-errored above 100,000 estimated output columns - but in 22.0.1 that guard ran only behind opt-in `--validate`, so the DEFAULT behaviour for a high-cardinality pivot column was not the clean error the command could already emit; it was unbounded memory growth until the OS killed something. Measured on a 200k-row slice of the NYC 311 sample (pivot column cardinality 173,103), debug build: with `--validate`, exit 1 in seconds and "Pivot would create too many columns"; without it, **12 GiB RSS in 40s and still climbing** when a watchdog killed it. The guard is split into its two unrelated jobs - `report_pivot_cardinality()` keeps the cardinality dump and the >1,000-column warning, still `--validate`-only and byte-identical, while `enforce_max_columns()` is the hard cap and **always runs**. The cap counts EXACTLY rather than estimating, because `metadata.estimated_columns` is the product of the per-column cardinalities - an upper bound - and capping on it is wrong twice over once the cap is always-on. It **false-positives**: on correlated `<on-cols>` the product is quadratic while the real width is linear, verified on a fixture where product and region correlate perfectly (real output 2 columns, estimate 4, so `--max-columns 3` aborted a pivot that works). And it **fails open**: `calculate_pivot_metadata` returns `None` when stats are unavailable, so the guard would silently not run - the second gap #4589 documents, left wide open by the fix meant to close it. `enforce_max_columns` takes `unique_df.height()` instead, the exact count of distinct `<on-cols>` combinations, already computed unconditionally immediately before the pivot; it touches stats not at all, so it can neither false-positive nor fail open, and an explicit non-smart `--agg` is not newly forced into a stats run. New **`--max-columns <n>`** `[default: 100000]`, `0` = unlimited - without an escape hatch this would REMOVE a capability, since omitting `--validate` was how you asked for a deliberately wide pivot, and the error names the flag because an escape hatch nobody can discover is the same as none. `--validate`'s own help text said it "validates" a pivot, which is now false - it reports and nothing more - so it was reworded rather than left pointing at the wrong flag. **A pivot wider than 100,000 columns that succeeded in 22.0.1 now aborts**; `--max-columns 0`, or a higher value, restores it.
...
- **`viz`: a Data Schematic's pills now pulse once, on a reader's first visit** ([#4389](https://github.com/dathere/qsv/issues/4389)). A page carries up to three affordances a first-time reader has no reason to look for - the **Data Schematic** dictionary drawer, the **Tour** relaunch pill, and the **Explore**/**Preview** data viewer tucked into the metadata table's Rows row - and until now the only thing that ever pointed at them was the guided tour itself, which a reader may dismiss in the first two seconds. They now carry driver.js **hint beacons**: a pulsing dot that opens a one-line popover and retires permanently on "Got it", on a click of the pill it points at, or on either of those on any later visit. Timing is the whole design. The beacons are armed from the tour's single teardown path and **only** there, so a first-time reader sees the tour first and the beacons after it ends - never a full-screen overlay and three pulsing dots competing for the same moment - and a reader who relaunches the tour from the pill months later does not get them again, because the first-run answer is latched at page boot rather than recomputed at teardown (by teardown it is always stale: the page is marked seen when a tour STARTS, not when it finishes). They keep their own storage key for the same reason - sharing the tour's would let one completed tour suppress the beacons forever. Each entry is emitted server-side only when its pill is, so `--preview-threshold 0` ships no data-viewer beacon and a page without `--dict-info` ships no dictionary one, and the beacon config rides inside the existing `#qsv-tour-config` payload rather than a second `<script>` tag - which is also why `--tour-steps 0` drops the beacons along with everything else. The widget is a SEPARATE dist file from the tour engine (the vendored `driver.js.iife.js` carries no hints API at all), vendored at the same pinned 1.8.0 under the same MIT license, and inflated lazily the first time beacons are armed - so it costs nothing at page load, and nothing at all on a page whose tour never runs. Its stylesheet duplicates the core popover block byte-for-byte rather than depending on it, so hint popovers carry `.driver-popover` and the page's existing dark-theme rules re-theme them for free; it also stops the pulse under `prefers-reduced-motion` on its own. Beacons deliberately do **not** appear inside the gallery's embedded iframes: they are `position: fixed` within the frame, and the first-run latch already sits inside the tour's top-window check.
- **`readstat`: a new command that converts SAS, Stata & SPSS files to CSV** ([#4542](https://github.com/dathere/qsv/issues/4542)). Reads `.sas7bdat` and SAS transport (`.xpt`/`.xpt5`/`.xpt8`), Stata `.dta`, and SPSS `.sav`/`.zsav`/`.por`, removing the need for the ReadStat CLI or `pyreadstat` to get data out of the formats data stewards and research communities actually publish in. Powered by [`polars-readstat-rs`](https://crates.io/crates/polars-readstat-rs), which is **pure Rust** - no ReadStat C library, no bindgen, nothing new in the build toolchain. Three decisions are load-bearing. **Coded values are written as their underlying codes by default**, not their labels, so the conversion is lossless and `stats`/`schema` type inference sees the numbers the file actually stores; `--value-labels` decodes them instead (Stata & SPSS only - SAS keeps value labels in a separate `.sas7bcat` catalog, which this command does not read yet and says so rather than silently ignoring the flag). **Row order is preserved at any `--jobs`**: the readers only stay ordered by construction on a single thread, switching above that to a chunk-interleaving path that is ordered only when `preserve_order` buffers chunks back into sequence, so both guards are set - `--jobs` therefore trades memory, not order, and defaults to 1 so memory stays flat. **The output is streamed batch-by-batch** through a polars `BatchedWriter` whose schema comes from the file's own variable dictionary rather than from the first batch, so the header is written exactly once even for a file with no rows. `--metadata csv|json|pretty-json` dumps the rich variable metadata these formats carry - variable labels, value labels, missing-value codes, measure and display settings, file encoding - instead of the data; the per-variable key is `columns` for SAS and `variables` for Stata/SPSS, so the CSV form derives its columns from the data rather than hardcoding either shape. Gated behind a new `readstat` feature (implies `polars`), which joins `distrib_features` and `qsvmcp` and the polars-bearing publish feature lists, so the prebuilt binaries carry it; qsvlite and qsvdp do not, and it inherits the existing big-endian exclusion for free since s390x/ppc64le are built without polars. Marked EXPERIMENTAL: the readers are young. The test fixtures are cross-checked **against pyreadstat, value-for-value**, rather than round-tripped through the reader under test, and the row-order test is mutation-verified against a 2000-row uncompressed fixture - the only shape that actually reaches the parallel path, so a smaller fixture would have made the assertion vacuous.
- **`describegpt`: a derived denominator now declares the geography it describes.** `viz smart` charts a per-capita rate map from an `x-qsv.denominator` hint, and `verify_denominators` derives that hint from a `measure.population` column using cardinality alone - which cannot tell a denominator from a COARSER geography than the region it will be charted against. A county extract that joined in state population and carries no `state` column leaves `county_fips` as the only region candidate, so it receives the hint, and the resulting map divides county counts by state populations: confident, plausible and wrong. No amount of counting separates the two cases - cardinality was tried in both directions and reviewed as a defect both times, and a distinct-values/regions RATIO was disproved with measured data (a legitimate per-city denominator scores 0.227 while the bug scores 0.500, so the classes are inverted). The information is simply absent from the data precisely when the coarser column is. So it is now DECLARED rather than inferred: for a numeric measure the LLM proposes an **`x-qsv.geo_level`** - the AREAL unit its values are measured over (`geo.state` on a state population), which is a different question from the field's own `concept` (`measure.population` says what the values ARE, not the area each covers). `verify_denominators` copies the level of the column it derived a hint FROM onto that hint, as `{"column": "state_pop", "level": "geo.state"}`. The prompt asks the question in the form that has a right answer - *what geography do this column's values describe* - rather than asking the column for its own level, which would return the level of the column the hint lands on and confidently confirm the very error it exists to catch. Unlike `x-qsv.unit`'s UCUM table the accepted tokens are **not injected as a vocabulary of their own**: they are the areal `geo.` members of the concept list the model is already shown, so nothing new joins the prompt-cache fingerprint and there is no second list to drift out of step. The level is verified but **never emitted on the field itself** - one copy, on the hint, so a hand-edited sidecar cannot hold two that disagree. This is the producer half only: `viz` does not read the key yet, and the coarse-geography bug stays open until it does ([#4571](https://github.com/dathere/qsv/issues/4571); consumption half [#4526](https://github.com/dathere/qsv/issues/4526)).
- **`viz`: a rendering-contract test that asserts what a Data Schematic RENDERS, not what it emits.** Every hover test qsv had asserted the plotly template string it *emits* - and that is exactly how the `%%` bug shipped: `escape_template_pct` doubled every `%` for the life of the function, and all three of its unit tests pinned the doubled form as correct. A test that asserts what we emit cannot discover that what we emit renders wrong; it is self-confirming, and only a browser could tell the difference. `viz_static_hover_rendering_contract` renders one page in headless Chrome, hovers a real point, and asserts the text a reader actually sees. The contract is a single sentence: **every column header reaches the reader byte-for-byte as it appears in the CSV** - hover text renders as pseudo-HTML, so `&amp;` becomes `&`, `&lt;` becomes `<` and `&#123;` becomes `{`, and the escaping exists only to survive the trip. One fixture carries all five load-bearing cases in a single tooltip: a lone `%`, a `%`-bearing unit reading, a `%{...}`-shaped header, markup that must be escaped exactly once, and an `<extra></extra>` sentinel that must not terminate the template. It needs **no new dependency** - it speaks the W3C WebDriver protocol directly over the already-present `reqwest`, because the obvious client crate (`fantoccini`) defaults to `native-tls` and would have pulled `openssl` into the musl CI job, where none exists. Scope is honest and narrow: one panel, one browser, one page, run by `rust-viz-static.yml` (and compile-checked everywhere else). Mutation-tested against both historical bugs - re-introducing the `%%` doubling renders `gdp %% index: 60 %%`, and disabling brace neutralization makes plotly eat the header outright, rendering `55 wellbeing_index` where `%{y} wellbeing_index` belongs.
- **`viz`: the extensive-vs-intensive guess now reads the data's OWN language.** `viz smart` decides whether a measure headlines a Total or a Mean by reading its column NAME (`unit_price`, `failure_rate`, `avg_temp` are intensive; `revenue` is not), and that vocabulary was English-only. A Spanish dataset therefore headlined "Total de Precio Unitario" - summing a per-unit price, the exact meaningless KPI the heuristic exists to prevent - and a reader whose dictionary already declared the language got no benefit from it. The guess now unions a **curated per-language lexicon** onto the English tables for the language `describegpt` detected (`x-qsv.detected_language_code`), covering `es`, `fr`, `de`, `it`, `pt-BR`, `ja` and `zh-CN`. Four design points are load-bearing. The lexicon follows the **DATA** language, tracked separately from the presentation locale: `--language es` on an English-headed dataset must render Spanish chrome while still matching English column names, so keying the lexicon off the active UI locale would match on the wrong language by construction. English **never stops applying** - a non-English dataset routinely keeps English headers, and a dictionary `label` (translated) and `field` (raw header) can be in different languages outright - so every lexicon is a union, never a replacement. Entries pass a **cross-language collision audit**, a hard requirement rather than a review nicety: a token that is intensive in one language but a common additive word in another silently turns a real total into a meaningless average, which is strictly worse than the missed match it was added to fix (es/it `media`, fr `note`/`tarif`, pt `taxa`/`nota`, de `quote` are all excluded on those grounds, and the exclusions are pinned by a test). And CJK is matched by **substring, not token**: `field_name_tokens` splits on non-alphanumerics, but Han/Kana are alphanumeric and caseless, so a CJK column name is one undifferentiated token that could never equal a dictionary entry. The tokenizer also gained Unicode case folding - it lower-cased ASCII only, so "Indice"/"Duracion" with their real accents could never match a lower-cased table entry in any language. Adjacency is preserved exactly as [#4404](https://github.com/dathere/qsv/pull/4404) established it for English: the lexicon extends each side of the money/per-unit pair without relaxing the requirement that the two be adjacent, so `precio unitario` is intensive while `unidad_vendida_costo_total` stays additive. A lexicon also carries its language's **count vocabulary**, not only its intensive one: teaching a language `puntuaciones`/`punktzahl` without `numero`/`anzahl` regresses the very language it was added for, because `numero_de_puntuaciones` then matches the new entry with nothing left to veto it and a real total is averaged - English has been guarded against that shape since the tables were written, and gained `number`/`numbers` here so the guard is no weaker in English than in the languages a lexicon covers. A count marker is a veto, so one that sits INSIDE an intensive entry of its own language would silently disable that entry; that exclusion is mechanically pinned rather than reviewed by eye, and is why German lists `anzahl` but never bare `zahl` (inside the intensive `punktzahl`), and why CJK lists whole count words but never a bare single-character one (which sits inside the entries for index and median). Finally, every multi-word phrase now ships its snake_case twin (`pro einheit` **and** `pro_einheit`): the phrases are substring-matched, real headers are snake_case, and a space-only entry could never match one - English already shipped `per capita` and `per_capita` for exactly this reason ([#4559](https://github.com/dathere/qsv/issues/4559); follow-up to [#4401](https://github.com/dathere/qsv/issues/4401)).
- **`viz`: the unit follows the number into the hover and onto the axes.** Follow-up to [#4552](https://github.com/dathere/qsv/pull/4552), which put `x-qsv.unit` on the KPI tile and the panel subtitle. That left the surfaces a reader actually inspects: a `ScatterPair` cell has **no axis titles at all** (the pair is named in the panel title), so its hover was the only place its three encoded columns could state a unit - and they are three *different* columns, so `air_temp: 18.4 °C`, `distance_km: 116 km` and `energy: 46 kWh` now each take their own. `Scatter3D` and the animated bubble/scatter-pair panels name their columns twice, and the two surfaces take **different forms**: an axis *names* a column, so it parenthesizes (`air_temp (°C)`); a hover line *states a reading*, so it suffixes - the same split #4552 drew between the panel subtitle and the KPI tile. A declared pipeline's hover is marked too, but only in the one shape whose numbers are readings of a single measure column: `value_unit` is bound to `FunnelShape::RowsMeasure` rather than to the panel, which makes leaking it onto a `RowsCount` row tally or a `Columns` total (spanning several differently-named stage columns) a **type error** instead of a review catch. The bridge's *delta* line stays bare on the same reasoning - it is the gap between two readings, not a reading. Two boundaries were established rather than assumed. `fmt_measure` is `fn(f64) -> String` with ~25 call sites, several formatting numbers that are not column readings at all (a snap radius in km, a computed rate), so the suffix is applied per-site with that column's semantics in scope and never inside the formatter. And `%` **is** in the curated UCUM table while these are plotly *hovertemplates*, so a bare `%` would be misparsed as the start of a `%{...}` token - the unit is `escape_template_pct`-doubled in the templates and deliberately not in the axis titles, each pinned by its own assertion. Two of the issue's hover bullets turned out to be unreachable and were dropped rather than implemented: `viz scatter`'s encoded hover and `viz geo`/`viz map`'s point hover live in standalone chart subcommands, and `--dictionary` is a `viz smart` option, so no `ColSemantics` exists there to thread. On-mark bar-top value labels (`fmt_magnitude`) were considered and **declined**: a unit repeated on every mark is noise, which is the argument #4552 already used for naming it once in the subtitle ([#4551](https://github.com/dathere/qsv/issues/4551)).
- **`viz`: a Data Schematic now says "°C".** Phase 2 of [#4525](https://github.com/dathere/qsv/issues/4525) - `describegpt` proposes a per-field `x-qsv.unit` (a curated UCUM code), and `viz smart` now consumes it: a non-monetary numeric measure headlines its KPI tile with the unit's display symbol (`18.4 °C`, `1.2B kWh`) and names that symbol once in the panel subtitle. The symbol is resolved from the authoritative table at the consumption site, never read out of the sidecar, so a hand-edited dictionary cannot invent its own glyph for a real code - the discipline `x-qsv.currency`/`currency_prefix` already follow. Three things are load-bearing and each is pinned by a mutation-tested assertion. The lookup is **case-sensitive**, because UCUM distinguishes `m` (metre) from the `M` (mega) prefix, so `xq_unit` trims but never folds - unlike its `currency` neighbour. There is **no bare-code fallback**: an unrecognized ISO currency still renders as `XOF 1.2B` because a 3-letter code is self-describing, but `qty (furlong)` would assert a unit qsv cannot vouch for, so off-table codes vanish. And the KPI tile's magnitude suffix (`k`/`M`/`B`) shares plotly's single `suffix` slot, so the two **compose** (`"B kWh"`) - while the unit is folded in *after* the number-format branch reads the magnitude, since that branch asks "was this value rescaled?" and a unit-only tile was not. viz re-verifies the whole annotation rather than trusting describegpt, because a hand-edited sidecar never passed through `verify_unit`: it re-derives the money precedence too, so a dictionary declaring both a currency and a unit on one column keeps only the currency. Deliberately NOT in this pass, and filed separately: hover text (~8 label-value constructions with no shared chokepoint, several of which format non-column values) and the scatter/bubble/3D axis titles. `viz smart`'s distribution panels have no axis titles at all - they are title-less on both axes by design to keep the cells compact - so the panel subtitle is where a column's unit belongs ([#4525](https://github.com/dathere/qsv/issues/4525), [#4552](https://github.com/dathere/qsv/pull/4552); follow-up [#4551](https://github.com/dathere/qsv/issues/4551)).
- **`describegpt`: numeric measures carry their UNIT, as a UCUM code.** qsv already carried units piecemeal - `x-qsv.currency` is the unit for money, `x-qsv.denominator.unit` is the unit of an area denominator - and every other measure had none. A temperature column could say "average" (via `x-qsv.aggregation`) but not "°C", so a chart axis read "temperature" rather than "temperature (°C)". `describegpt --dictionary --format jsonschema --infer-content-type` now also proposes a per-field **`x-qsv.unit`**: a [UCUM](https://ucum.org/ucum) code (`Cel`, `[degF]`, `km`, `kg`, `kW.h`, `m/s`, `%`, ...), the standard behind FHIR/HL7 and what schema.org's `unitCode` is most often mapped from - machine-parseable and unambiguous, unlike a free-text "degrees". Validation is a **curated subset of 44 common codes with display symbols**, deliberately not a UCUM parser; an off-table code is dropped rather than half-honored. Unlike `currency`, the vocabulary is **injected into the prompt** (with each code glossed by its symbol), because ISO-4217 alpha-3 is something a model emits unprompted while UCUM is not - left to itself it writes `degC` or `°C`, and a non-injected design would silently drop most correct-in-spirit proposals. It follows `currency`/`gauge_range`'s propose-then-verify discipline: shape and vocabulary membership on parse, semantics after the role is final. The **precedence rule** matters as much as the annotation - for money the currency IS the unit, so `verify_unit` drops a proposed unit on any column identified as money. Identity, not the mere presence of a code: the prompt permits omitting a currency that cannot be determined from the data and an off-register proposal is dropped at parse, so `concept: measure.money` and a `money` content type disqualify a unit on their own, which is what stops a `kg` landing on an amount of dollars. That positive test stops deliberately short of `verify_currency`'s `money_ish`, which also admits the generic `measure.amount` - the very concept a temperature, a distance and an energy carry, so treating it as money would swallow the feature. For that ambiguous case the only money signal is a *verified* currency, read after `verify_currency` runs so a currency that verification rejected cannot suppress a legitimate unit. Two smaller invariants are load-bearing and now pinned by tests: the membership check trims but **never case-folds**, because UCUM is case-sensitive (`m` is metre, and the `k`/`M` prefixes differ by case alone) unlike the `currency` and `aggregation` arms beside it; and the table joins the prompt-cache vocabulary fingerprint, since a Rust-const vocabulary is invisible to a template fingerprint and a warm cache would otherwise replay an answer produced under the old vocabulary. This is the describegpt half; `viz smart` consumption (axis, hover and KPI-tile suffixes) follows separately ([#4525](https://github.com/dathere/qsv/issues/4525), [#4550](https://github.com/dathere/qsv/pull/4550)).
- **`viz`: Data Schematics open with a guided Tour.** A Data Schematic is pitched as a storytelling Data Dictionary, but a first-time reader landed on a wall of panels with no narrator: nothing explained that the dictionary drawer exists, that the data viewer has three complementary search surfaces, or *why* each summary panel was chosen. `viz smart` HTML now ships a guided tour (vendored [driver.js](https://driverjs.com) 1.8.0 — MIT, ~8KB gz, offline-capable like everything else in the page; Shepherd went AGPL in 2024 and Hopscotch is dead) that auto-starts on a reader's **first** visit and walks the dictionary drawer (downloadable sidecars, the color-coded role chips, field definitions, what was left out), the data viewer (Advanced Filter, global search, per-column search, CSV export), and the summary panels (eight by default, set by `--tour-steps`; see Changed below for the one case that yields a ninth) — each with a **context-sensitive explanation recorded at panel-selection time**, not boilerplate: the tour says why *this* column got *this* chart. `--tour-steps 0` opts out; the tour is replayable from a header pill, page-scoped via a `localStorage` seen-key, suppressed inside iframes, and localized across all 8 UI catalogs. Refinable narration is a **schema contract, not a template**: a dictionary's dataset-level `x-qsv.tour` object (`version` must be the integer `1` or the whole block is ignored — a contract revision can never half-apply; `overrides` by step id, `panels` by raw field name or `@kind` token, `panel_order` capped at the resolved `--tour-steps` budget, `language` honored only when it prefix-matches the page locale) replaces the built-in prose. Only prose and panel selection are schema-controlled — step structure, targets and drawer choreography stay in Rust, and all prose is HTML-escaped once at the emit sink, so schema content can never inject selectors or markup. Two driver.js behaviors cost real debugging and are worth recording: `onDestroyed` is skipped when a tour is dismissed before the highlight animation commits, so lifecycle cleanup hooks `onDestroyStarted` (which fires unconditionally), and concurrent `moveTo` calls racing an in-flight transition corrupt overlay state, so navigation is serialized ([#4389](https://github.com/dathere/qsv/issues/4389), [#4497](https://github.com/dathere/qsv/pull/4497)).
- **`describegpt`/`viz`: the Data Schematic's guided Tour speaks in your audience's voice.** The Tour mode that shipped with [#4497](https://github.com/dathere/qsv/pull/4497) renders `x-qsv.tour` narration when a dictionary carries one — but nothing *produced* one. `describegpt --tour-audience <text>` now has the LLM also write that narration during dictionary inference (`--dictionary --format jsonschema`), addressed to the given audience, and `viz smart --tour-audience` forwards it on a fresh `--dictionary infer` — defaulting to **"Explain like I'm 10"**, so every freshly inferred Data Schematic now opens with a guided tour a ten-year-old can follow. The LLM contributes **prose only**: overrides are filtered against the 13 public step ids and panel narrations against the real column names, while the envelope is built by qsv — `version` is the hardcoded integer `1` (viz discards the whole block on anything else, so a model can never poison the contract), `audience` is recorded verbatim, `language` is mapped to BCP-47 (an inline table cross-checked against `viz_i18n::LOCALES` by test, since describegpt also compiles into builds without viz), and `generated_by` carries qsv/model provenance. Under `--two-pass` the tour rides the first-pass response like `grain` and `relationships` — the refine prompt is untouched. Plain describegpt runs without the flag stay byte-identical, and the flag joins the prompt-cache key so changing the audience actually re-asks the model. Because `--dictionary infer` reuses an existing `<stem>.schema.json`, a changed audience against a reused sidecar is called out with the escape hatch (delete it, or `QSV_VIZ_DICT_FRESH=1`) instead of silently doing nothing ([#4389](https://github.com/dathere/qsv/issues/4389)).
- **`viz`: the Data Schematic now explains what it left out.** `viz smart` explained its omissions only on stderr, so the artifact people keep, share and open later carried no record of what was skipped or why - a recipient saw five of twelve columns charted with no way to learn about the other seven. The `--dict-info` drawer now carries a per-column "not charted" note and a dataset-level "Panels not drawn" section. Reasons are recorded **at the decision**, never re-derived at render time (a re-derived predicate drifts from the real one silently, which is the worst failure mode for a provenance surface): the classifiers return the skip reason, twin detectors report which sibling survived, and 30 refusal sites now feed a collector so the drawer shows the *same* string the pipeline printed. Two look-alike cases are distinguished rather than papered over - an unused date column no longer renders the identical sentence as the dataset's chosen time axis, and a `--max-charts` casualty reads as "lost a ranking contest", not "not chartable". Scope is the drawer only: a plain `viz smart` run is unchanged and the stderr roll-up stays byte-identical. Column reasons are localized across all 8 catalogs ([#4399](https://github.com/dathere/qsv/pull/4399)).
- **`describegpt`/`viz`: money is a first-class concept, and money KPIs read "$192B" rather than SI "G".** `describegpt` had no machine-readable notion of money - `currency_code` describes the ISO-code *column*, not an amount, and the "price + currency code = a MONEY value" hint in the refine prompt only ever produced prose, so a dollar column landed on the generic `measure.amount` and viz had no slot for a currency at all (even a hand-authored `x-qsv.currency` was silently dropped). This adds a `money` content type and `measure.money` concept, seeded deterministically from the content type, plus `x-qsv.currency` - an ISO-4217 alpha-3 code following `gauge_range`'s propose-then-verify discipline, validated against the ISO register on parse and kept only when the column really is a numeric money measure. It deliberately also accepts `measure.amount`, so dictionaries authored before `measure.money` existed work by adding the code alone. `money` joins synthesize's `NON_FAKER_TOKENS`, since it is numeric and a faker would destroy the column's real min/max/mean. Separately, d3-format has no locale hook for SI prefixes (`~s` always emits "G"), so the suffix convention moved into a single owner feeding bar and waterfall labels, plotly's native axis mode and the KPI tile alike - English pages read 1e9 as "B", every other locale keeps SI "G", and all sites flip together, making "no chart mixes suffixes" structural rather than a review obligation. Gauge and delta tiles keep their unscaled value on purpose: a gauge draws against an unscaled `[lo,hi]` axis, so scaling the number alone would render a needle at 2.4 on a 0..5e9 dial ([#4393](https://github.com/dathere/qsv/issues/4393), [#4400](https://github.com/dathere/qsv/pull/4400)).
- **`viz`: region choropleths can chart a RATE, not just a raw count.** A choropleth colored by row counts is largely a population map - the region with the most people (or the most activity) tallies the most rows, so the map ranks regions by size rather than by intensity. Boston 311 picks out Dorchester; Allegheny dog licenses pick out zip 15237. Two ways to say what to divide by: `--denominator-key <k>` reads each region's denominator from a `--geojson` feature property (addressed exactly like `--feature-id-key`, and accepting the quoted numbers census exports routinely emit), and `--denominator <col>` reads it from a dataset column, hard-erroring when the value is not constant within a region - a denominator that changes row to row is a row-level amount passed by mistake, and taking the first value would yield a confident wrong rate. `viz choropleth` with either flag becomes a rate map: the colorbar says so, the hover keeps the raw numerator and the named denominator visible, and there is no share-of-total line, because a rate is intensive and a percentage of one would be a fabricated statistic. The display scale (per 1,000 / 10,000 / 100,000) is chosen from the **median** rate, so one freak region cannot rescale the map. In `viz smart`, an `x-qsv.denominator` key on a region-code column charts a rate panel beside the raw-count panel - and when it cannot, the count panel says so. The denominator source is always **declared, never guessed**: a wrong denominator produces a plausible wrong map, which is worse than the raw counts it replaces ([#4394](https://github.com/dathere/qsv/issues/4394), [#4413](https://github.com/dathere/qsv/pull/4413)).
- **`viz`: `--geojson auto` fetches US boundaries for you.** `viz choropleth --geojson auto` (or `census`) resolves US boundaries from the Census TIGERweb REST service, scoped to the states the data names, and sets `--feature-id-key` to `properties.GEOID`. The user supplies nothing but the CSV. Four service facts were established by probing the live endpoint, each invalidating a plausible shortcut and each recorded in the module docs so they are not rediscovered: layer ids are **not** stable across vintages (Counties is 82 in ACS2023/Current, 84 in ACS2019, 78 in ACS2021 - and a hardcoded id does not fail loudly, it silently fetches a different geography), so layers are resolved by *name* from the catalog; `properties.GEOID` is canonical for every targeted layer, ZCTAs included, so there is no per-layer id special-casing; the ZCTA layer carries no `STATE` field, so ZCTAs need exact code-set scoping (and deriving a state set from ZIP prefixes was never viable, since prefixes cross state lines); and vintages are read from the catalog rather than computed from the current year, which would rot annually. Whole states are fetched rather than only the regions named in the data - scoring a candidate column against a feature set derived from that same column is circular and would report a perfect match for any column at all. The resolved set is materialized to `~/.qsv-cache/viz-boundaries` under a name deterministic in provider, vintage and scope, which doubles as the artifact you can pin as an explicit `--geojson` for a network-free run (verified byte-identical to the auto path). `viz smart` is covered too: it picks its region column from the data dictionary, which does not exist when `--geojson` is resolved up front, so detection and resolution are split - the fetch happens once `col_sems` is derived, candidates are chosen by geometry-free probes (`returnGeometry=false`) rather than fetch-and-see, fetched-vs-supplied provenance rides in the region panel's subtitle, and outliers are still drawn but no longer decide the frame (6 stray owner ZIPs in Florida and Indiana had been rendering 127 Allegheny ZCTAs as a speck) ([#4397](https://github.com/dathere/qsv/issues/4397), [#4418](https://github.com/dathere/qsv/pull/4418); [#4416](https://github.com/dathere/qsv/issues/4416), [#4419](https://github.com/dathere/qsv/pull/4419)).
- **`viz`: `--geojson auto` resolves place and county NAMES, not just FIPS codes** ([#4417](https://github.com/dathere/qsv/issues/4417), [#4475](https://github.com/dathere/qsv/pull/4475) Part A, [#4480](https://github.com/dathere/qsv/pull/4480) Part B, [#4481](https://github.com/dathere/qsv/issues/4481), [#4482](https://github.com/dathere/qsv/pull/4482)). A `--locations` column of county names ("Allegheny County") now drives `--geojson auto` with nothing supplied but the CSV: the distinct names are resolved against the **Census's own name table for the vintage in play**, the boundaries those GEOIDs name are fetched, and per-row cells canonicalize through a name-alias tier in `RegionMatcher`. #4417 proposed building this on Geonames `admin2Codes.txt`; measured against the live service that was the wrong mechanism, and the issue's own dismissed option turned out to be strictly better. Part A adds the city-name path, which does need `--geocode` because it goes through geosuggest. The sharp edge is ambiguity: **422 county names are carried by more than one county, covering 1,684 of 3,235 counties (52%), skewed populous** - a bare-name alias map therefore DROPPED any name resolving to different codes under different rows and omitted every row carrying it, even though lookup had resolved each row correctly (`Washington County` + PA/MD resolved to 42125 and 24043 fine; the map just could not hold both). Aliases are now keyed by `(name, qualifier)` taken from a per-row column.
- **`viz`: denominators are unit-aware - real `AREALAND` renders as "per km²".** `--denominator-key` took its label from the property path's last segment and divided by the raw value. Fine for `properties.POP2020`, and unusable for the one denominator real published boundary files reliably carry: land area, in square **metres**. The rate landed around 1e-9, the scale ladder clamped at 100,000, and the map read "per 100,000 AREALAND" at approximately zero everywhere - and `--geojson auto` had just made those boundaries the ones most users will have in hand. A closed two-entry vocabulary (`m2`, `km2`, with spelling variants) now converts the **values**, not the display, before the rate is computed, so the colorbar, the panel title and the per-region hover cannot disagree about what unit they show. The unit is declared from three sources, most explicit first: the new `--denominator-unit` flag, a dictionary `x-qsv.denominator.unit` key, and the boundary document's own declaration - `viz smart --geojson auto --denominator-key properties.AREALAND` reads "per km²" with no unit flag at all, because the census fetch stamps `x-qsv.property_units` onto the FeatureCollection it builds (a GeoJSON foreign member per RFC 7946 §6.1, so the declaration survives the cache). qsv never infers a unit from a field name, and an unrecognized token is rejected up front rather than silently falling back to raw metres. The scale ladder also gains rungs at 1e6 and 1e9, so an *undeclared* small-unit denominator stops rendering as 0.0001 everywhere ([#4414](https://github.com/dathere/qsv/issues/4414), [#4420](https://github.com/dathere/qsv/pull/4420)).
- **`viz`: `--denominator census` fetches US population denominators.** Phase 2 of the denominator work, and the piece that makes the whole story run on nothing but a CSV: a raw-count region map becomes a per-capita rate with no denominator supplied. Scoped to **state and county** for this pass - the two geographies whose ACS coverage is complete and uncaveated; tracts and ZCTAs are deferred rather than approximated, since a ZCTA is not a ZIP code and that has to be stamped into the provenance rather than quietly assumed. This is a new integration rather than an extension of the boundary fetch (TIGERweb serves geometry from an ArcGIS catalog, `api.census.gov` serves ACS estimates as arrays-of-arrays); only the HTTP client, the transient-vs-semantic error rule and the disk cache are shared, and column order is read from the response's own header row rather than assumed positionally. **⚠️ `QSV_CENSUS_API_KEY` is required**, contradicting both the Bureau's older docs and the issue text: verified against the live service, every unkeyed request - including the Bureau's own documented one-row example - 302s to `data/missing_key.html`, which answers 200 with HTML, so an unkeyed run used to fail with "response is not valid JSON" and read like a qsv bug. qsv now demands the key up front and names the signup URL, and the requirement is skipped when `QSV_CENSUS_API_URL` points elsewhere - a mirror or a mock owes the Bureau nothing. The boundary document also states `x-qsv.layer`, because a 5-digit ZCTA is indistinguishable from a 5-digit county FIPS: a ZIP-keyed dataset would otherwise have fetched county population and matched almost nothing, and now fails immediately, naming the geography and the way out ([#4395](https://github.com/dathere/qsv/issues/4395), [#4421](https://github.com/dathere/qsv/pull/4421)).

- `geocode`: **`index-load 1000` accepts the denser `cities1000` prebuilt index**, so a dataset of small towns no longer needs a full local rebuild. The default prebuilt is `cities15000` (~26k cities worldwide, 22.6 MiB), which omits places under 15,000 population - the binding constraint on resolving small-town place names. Getting past it previously meant `index-update --cities-url 1000`, which downloads ~200 MB from Geonames and rebuilds from scratch; `index-load 1000` now fetches a prebuilt (82.7 MiB, 32 MiB compressed) in one command. The download URL was already parameterized by population floor (`…rkyv.cities{N}.sz`), so the change is a published asset plus widening the two guards that hardcoded 15000 - now a single `PREBUILT_CITIES_INDEXES` list, so the accepted shortcuts and the error message that names them cannot drift apart. **The global default is deliberately unchanged**: a denser index costs every `geocode` user ~4x the index size and different `suggest` results, and it is not a fix for cross-country mis-resolution (see [#4427](https://github.com/dathere/qsv/issues/4427)) - it only widens coverage. A floor this binary knows about can still be missing from the release it was built from, so that download failure now names the release and points at the local-rebuild alternative instead of surfacing a bare 404 - `util::download_file` streams a 404 body to disk and reports success, so the payload's Snappy framing is checked at the download site and a bogus one is removed rather than left to be mistaken for an index.
- `get`: **`cache-list` now shows each entry's age and staleness** ([#4519](https://github.com/dathere/qsv/pull/4519)), so a cached response you are about to reuse can be judged before you reuse it rather than after.

### Changed
- **`viz`: the guided tour's panel budget is now `--tour-steps <n>`, replacing `--no-tour`** ([#4389](https://github.com/dathere/qsv/issues/4389)). **Not a breaking change**: the tour and `--no-tour` are both part of this same unreleased cycle (see the Tour entry under Added), so no tagged release ever carried `--no-tour` and there is nothing to migrate - it is called out here only so the flag's brief life in `master` does not look like an accidental removal. `--tour-steps 0` is how a Data Schematic ships without a guided tour. The panel budget had been a hardcoded constant (raised 6 -> 8 in this same release) doing a job a flag should do, and `--no-tour` was a second flag for one end of that same knob; `<n>` budgets the per-panel **spotlights** only - the introduction, the Data Dictionary chapter, the data viewer chapter and the conclusion are always included, so a default tour is ~22 steps of which 8 are panels. A dictionary's `x-qsv.tour.panel_order` is capped by the same budget. The flag is deliberately typed as an *optional* value with **no docopt default**, which is what lets an explicit `--tour-steps 8` be told apart from an absent flag - the distinction the frequency-distribution guarantee below turns on, and one a docopt `[default: 8]` would have erased.
- **`viz`: a tour now always spotlights a frequency distribution** ([#4389](https://github.com/dathere/qsv/issues/4389)). Panel selection pushes one panel per distinct overview `@kind` FIRST and appends the top-3 per-column panels only afterwards, then truncates - so on a rich dashboard the per-column picks were cut off entirely. Measured across all 22 gallery pages at the old cap: **132 panel steps, 114 of them overview panels, 18 per-column, and only 3 frequency bars** in the whole gallery; 13 of the 22 pages spotlighted no per-column panel at all. The budget was only half the cause - `panel_interest` tops a frequency bar out near 2.0 while a box/violin reaches 5.0 and a histogram starts at 2.5, so bars lose the ranking race on numeric-heavy data even when slots survive. A page that would otherwise spotlight no bar now gets its highest-interest one, and **how** it is fitted depends on who set the budget: left at the default, the bar is APPENDED past it (a 9th step, costing no overview panel - the default is a soft cap); passed explicitly, the budget is hard and the bar substitutes for the weakest pick, because a reader who asks for `--tour-steps 4` should get four. The guarantee is a strict no-op on a page with no frequency bar, and it deliberately does **not** touch a dictionary-supplied `x-qsv.tour.panel_order`: that is a complete, author-chosen walk and is never amended.
- **`viz`: vendored DataTables bundle bumped to `dt-3.0.3/b-4.0.2/cc-2.0.1/date-2.0.0/sb-2.0.0`** (DataTables 3.0.1 -> 3.0.3, Buttons 4.0.1 -> 4.0.2, ColumnControl 2.0.0 -> 2.0.1; DateTime and SearchBuilder were already current). Upstream patch releases only - notably an empty-string `^$` search-regex fix and a `parseerror` status-text typo in DataTables core. The data-viewer drawer's CSS contract is untouched: the only stylesheet changes are cosmetic Buttons tweaks, so the sticky two-row `thead`, the `scrollX` scroll region and the dark-theme hooks all behave as before. All seven vendored DataTables locale files were sha256-compared against the 3.0.3 plug-in locales and were byte-identical, so only their documented source URLs moved. Subresource Integrity hashes for the `QSV_VIZ_CDN` path were regenerated alongside the bundle.
- **`stats`: `--quantile-method approx` and `--cardinality-method approx` now return different - and markedly more accurate - numbers.** The `datasketches` bump from 0.4.0 to 0.5.0 carries upstream fixes to T-Digest interpolation/tail calculations and to HLL union estimate stability, so approximate output moves. **This is a visible output change** for anyone re-running `stats` with either flag, and cached stats written by an earlier release are invalidated normally by the `qsv_version` comparison. Measured against 0.4.0 on `resources/test/311011.csv`, using the exact (non-approx) values as ground truth: `--quantile-method approx` is closer on all seven of q1, median, q3, iqr, both inner fences and skewness, in each of `--jobs 1`, parallel-unindexed and parallel-indexed - absolute errors fall from 100-1456 down to <= 1.0, and on a uniform ascending column the median is now exact (19814, was 19316.5) with skewness 0 (was 0.0638). `--cardinality-method approx` is byte-identical under `--jobs 1` and parallel-unindexed; only the parallel **indexed** path changes, because that is the one exercising the HLL union in `Commute::merge` - the `Referrals` column goes 909 to 923 against an exact 932, i.e. 2.5% error down to 0.97%. Worth naming for future bumps: the approx tests assert tolerance *envelopes* rather than exact values, so none of this is visible to them - it was measured by building both versions and diffing, and a `--jobs 1` capture alone reports cardinality as unchanged because it never reaches the union path. 0.5.0 also makes the fallible sketch constructors return `Result`, which the `frequency` and `stats` call sites now handle - `FrequentItemsSketch::new` propagates (its accepted map size is a power of two in [8, 2^30], and only the lower bound was validated up front), while the t-digest and HLL constructors take compile-time constants inside their accepted ranges ([#4555](https://github.com/dathere/qsv/pull/4555)).
- **docs: an inferred data dictionary is a *draft*; the reviewed copy is the artifact of record.** Re-inferring a dictionary over the same data with the same model can flip a numeric column from `role: measure` to `role: dimension`. That is not a bug - LLM inference is not reproducible, and qsv already assumes a human corrects it (viz's USAGE calls the semantic half "all correctable by a human Data Steward", `DATA_SCHEMATIC.md` makes editability a conformance MUST, and viz re-applies describegpt's verification gates **on read** precisely because sidecars get hand-edited). What was missing is that editing the sidecar was documented as a convenience and never as a *reproducibility practice*: reusing the reviewed `<stem>.schema.json` - not re-rolling the model - is what makes a later render reproducible. The gallery has worked this way since [#4404](https://github.com/dathere/qsv/pull/4404); users running `--dictionary infer` on their own data were never told to. The issue's other two suggestions were deliberately **not** implemented - a verification-time "numeric column tagged dimension is suspect" warning, and defaulting dictionary inference to a low temperature - since neither is needed once the sidecar is curated, and both add a heuristic that can be wrong ([#4407](https://github.com/dathere/qsv/issues/4407), [#4408](https://github.com/dathere/qsv/pull/4408)).
- **dead code is gated out of the binary flavors that cannot reach it** ([#4578](https://github.com/dathere/qsv/pull/4578)), so `qsvlite`, `qsvdp` and `qsvmcp` stop compiling items only a fuller build can call.
- **`viz`: vendored DataTables bundle bumped to `dt-3.0.4/b-4.0.3/cc-2.0.2/date-2.0.0/sb-2.0.1`** (DataTables 3.0.4, Buttons 4.0.3, ColumnControl 2.0.2, SearchBuilder 2.0.1; DateTime stays 2.0.0). Unlike the 3.0.1 -> 3.0.3 bump earlier in this release, **this one is not purely cosmetic**: ColumnControl 2.0.2 rewires the per-column search inputs to inherit DataTables core's `--dt-input_*` custom properties, so they are **4px taller** (`padding` 5px -> `7px 5px`; measured in-browser, the input goes 27px -> 31px) and, in dark mode, **actually opaque**. The old `--dtcc-search-input_background: var(--dt-html-background)` referenced a custom property that is *never defined anywhere* - not in the stylesheet, not in the JS bundle, not in qsv - so the declaration was invalid at computed-value time and fell back to `transparent`. Measured against the same page with the old declarations restored, the dark-mode input and its operation-type `<select>` go from `rgba(0, 0, 0, 0)` to `rgb(44, 48, 51)`; light mode is unchanged, because there `transparent` was already the resolved value. Together with a new `color: inherit` on the operation-type `<select>`, that is upstream's fix for those controls rendering bright white in dark-mode Chrome - and it reaches qsv's drawer because `syncDark()` deliberately mirrors `body.qsv-dark` onto `html.dark`, which is what DataTables keys on. The taller input is accepted as-is rather than pinned back, so the sticky two-row `thead` was re-verified against it. Elsewhere: Buttons 4.0.3 fixes keyboard activation (the drawer's three buttons are now Enter-activatable, and qsv has no hand-rolled keydown shim to double-fire); SearchBuilder 2.0.1's Moment/Luxon date fix and throttle -> debounce change are both inert here (qsv loads neither library and never sets `searchDelay`); DataTables 3.0.4 fixes `titleRow` ordering with `rowspan` headers and `:visible` column selection with multi-row headers, which the two-row `thead` was re-tested against: clicking a title cell still sorts (`[] -> [[0,"asc"]] -> [[0,"desc"]]`) and clicking ColumnControl's search row still does not. Re-verified alongside them: the `search.fixed("dtcc")` contract behind the Clear Filters counter (typing a per-column filter yields "Clear Filters (1)", enabled, and clearing restores every row), `DataTable.render.text()` still rendering tag-bearing cells as literal text with zero elements injected, and the `dt.settings()[0].columns[].width/.visible` internals that the [#4325](https://github.com/dathere/qsv/pull/4325) Safari min-width workaround reads. All seven vendored DataTables locale files were sha256-compared against the 3.0.4 plug-in locales and were again byte-identical, so only their documented source URLs moved - and the upstream `zh` SearchBuilder date-condition inversion is *still* unfixed at 3.0.4, so the splice-time override and its canary stay. Subresource Integrity hashes for the `QSV_VIZ_CDN` path were regenerated alongside the bundle.
- **`viz`: `--help` is 30% shorter, and the rationale now lives in the wiki** ([#4595](https://github.com/dathere/qsv/pull/4595)). `qsv viz --help` had grown to 1,052 lines and buried what a user at a terminal actually needs - flag names, accepted values, defaults - under design rationale, threshold tables and worked JSON. Cut to 738 lines by moving the *why* to the [Visualization wiki](https://github.com/dathere/qsv/wiki/Visualization), which is reorganized in the same pass to be the navigable reference the USAGE now points into. The keep/cut rule, applied per line: it stays in USAGE if a user with no network cannot act without it. So every option definition, every `[default:]` and every enumerated value list stays - the `--style` basemaps, the `--projection` and `--color-scale` names, `--location-mode`, the `--agg` functions, the auto/on/off mode sets, the `census:<layer>` selectors - along with every mutual exclusion and requirement ("Cannot be combined with `--series`", "Requires `--geojson`", "Only affects `smart`", the `QSV_CENSUS_API_KEY` requirement) and the reserved `census` value. What moved is the reasoning, the thresholds and the JSON examples. Biggest reductions: `--dictionary` 124 -> 55 lines, `--geojson` 42 -> 19, `--dict-info` 39 -> 11, `--violin` 32 -> 18, `--box-points` 30 -> 14, `--denominator` 27 -> 17. The `x-qsv` hint descriptions keep one clause each naming their shape, because the MCP skill JSON embeds these descriptions verbatim and an agent cannot follow a wiki URL.
- **`stats`: the "reproducible across `--jobs` values" claim for approximate cardinality is corrected.** The claim lived in `stats.rs`'s docopt USAGE - which is what `qsv stats --help` prints *and* what generates the MCP skill JSON - as well as in `docs/STATS_DEFINITIONS.md`, so the wrong claim was user-facing in the place users are most likely to read it. HLL sketches are reproducible across *parallel* `--jobs` values, and `--jobs 1` never reaches the union at all; both are now spelled out. "They diverge once the sketch starts estimating" is softened to "can diverge", since two estimates can coincide by chance. `sort_order` casing was fixed in the same pass, and `docs/STATS_DEFINITIONS.md` gained an "Applies to qsv X.Y.Z" marker that CI now asserts against `Cargo.toml` (see CI/Actions).

### Performance

- **qsv-stats 0.55.0 adopts unconditional algebraic float operations**, and `viz`'s correlation matrix and `moarstats`' Spearman/Pearson core were rewritten to use them ([#4391](https://github.com/dathere/qsv/issues/4391)). Rust's algebraic float intrinsics let the compiler reassociate and vectorize floating-point reductions that strict IEEE ordering forbids. The important scoping insight, recorded here so it is not "finished" later by mistake: algebraic ops help **batch reductions only** and do nothing for Welford-style online accumulators, so `online.rs` was deliberately left untouched. Outputs are byte-identical to the previous release. This is the change that moved the MSRV to Rust 1.98.
- `stats`: **the row-count pre-pass is skipped on unindexed files** ([#4473](https://github.com/dathere/qsv/pull/4473)). On the unindexed path `run()` called `util::count_rows()` before the compute pass, reading the whole file **twice** - and used the result only as a capacity hint for the per-column accumulators. With polars the pre-pass is a cheap mem-mapped scan, but without it the wasted scan is a genuine CSV parse: roughly **30% of a plain qsvlite run**.
- `validate`: **a pre-allocated record is reused for each entry in a batch, and the `serde_json::Value` conversion is avoided entirely** ([#4508](https://github.com/dathere/qsv/pull/4508), [#4509](https://github.com/dathere/qsv/pull/4509)). Both contributed by Dmitry Dygalo.
- `diskcache`: uses `bytecount` for its line counting, per a clippy suggestion.

### Removed

- **`qsvmcp` no longer ships the `get` command, `dc:` cache-prefix inputs, or cloud (`s3://`, `gs://`, `az://`) sources** ([#4374](https://github.com/dathere/qsv/pull/4374)). **This is breaking for `qsvmcp` users only** - `qsv`, `qsvdp` and the `distrib_features` prebuilt binaries keep full cloud support, and within `qsvmcp` the `http(s)://`, `dathere://` and `ckan://` schemes are unaffected. Auto-decompression of compressed sources also still works, because `flate2`/`zstd` come from `feature_capable` rather than from `get`. Both features had to go together: dropping only `get` would have been inert, since `get_cloud = ["get", "dep:object_store"]` re-enables it - the same trap [#4350](https://github.com/dathere/qsv/issues/4350) hit with `viz` vs `viz_static`, where the published diff turned out to change nothing. The motivation is Windows build time. `get_cloud` turns on `object_store`'s aws/azure/gcp backends, the largest resolved-feature delta between the `qsvmcp` and `qsv` builds - invisible in a crate-count comparison because `object_store` sits in *both* graphs (polars pulls it), and visible only when diffing resolved `(package, features)` rows. Stated honestly, the measured delta is small: 689 -> 687 crates and 924 -> 921 `(package, features)` rows, against a `qsvmcp` Windows build that had failed to finish in 268 minutes; the aws/azure/gcp modules genuinely stop compiling, but this change alone was never expected to be the whole fix. `get` was never exposed as an MCP skill, so the skill count is unchanged by this.

### Fixed
- **`viz`: the tour no longer describes a logarithmic bar panel as linear** ([#4389](https://github.com/dathere/qsv/issues/4389)). The tour's "its axis is logarithmic" sentence was gated on `Panel::value_log`, a field only box and violin panels ever carry - they resolve their verdict at classification time from the cached stats. **Frequency bars and measure-by-dimension bars decide at RENDER time** from the values actually plotted, so `value_log` is always `false` for them and the narration silently claimed a linear axis beside a chart whose own axis title read `count (log)`. The gate now asks `panel_is_log`, which is already the single source of truth the bar renderer and the grid's left-margin sizing both use, so all four logging panel kinds are covered by one question instead of three answers. The wording splits by what the axis actually carries: a frequency bar's logged axis holds COUNTS - matching the chart's `count (log)` title - while box, violin and measure-by-dimension panels log a VALUE axis and keep the original sentence. Both are spelled-out literals rather than a computed message key, which the file's catalog-coverage test requires.
- **`--output` naming the input no longer silently zeroes it.** Most commands construct the `--output` writer BEFORE opening the input reader, so `File::create` truncated the input to zero length, the command then read an empty file, and it exited **0 with no warning** - `qsv fmt big.csv -o big.csv` returned success and left 0 bytes behind. A sweep of 39 single-input commands found at least 18 that zeroed the file outright (`behead`, `cat`, `dedup`, `explode`, `fill`, `fixedwidth`, `fixlengths`, `fmt`, `input`, `jsonl`, `pseudo`, `rename`, `reverse`, `sample`, `select`, `slice`, `table`, `transpose`) and several more that wrote truncated output instead (`enum`, `replace`, `safenames`). Errors like `Selector name 'a' does not exist as a named header` were a *symptom*: the file was already empty by the time it was read. Unlike shell redirection (`cmd f > f`), where the shell opens the file before qsv starts, **`-o` is opened by qsv itself**, so qsv can refuse - and now does, before any work begins. The guard lives in `util::get_args`, which every command routes through, so it covers all four binary flavors uniformly rather than per-command; it runs *after* docopt, so `--help` and `--version` still short-circuit. It is deliberately an **argv-level** scan: the output `Config` is a separate instance that does not know the input path, and roughly 40 `File::create` sites bypass `Config::io_writer` entirely, so there is no single writer chokepoint holding both paths - `argv` is the only place both appear. Identity is tested with `same_file::is_same_file` (inode/dev, file-id on Windows), which catches `./data.csv`, symlinks **and hard links**; canonicalizing cannot, since two hard links to one inode have two distinct canonical paths. The guard reads docopt's **parsed values**, not raw argv, and that distinction is load-bearing: docopt accepts `--flag value`, `--flag=value` and `-fvalue` interchangeably, but only the first spelling puts the path in an argv token of its own, so a token scan reads `fetchpost --payload-tpl=payload.tpl ... -o payload.tpl` as one opaque non-path string, misses that the template is an input, and lets the template get zeroed - the very bug being guarded against. Splitting `Docopt::deserialize` into its own `parse()` and `ArgvMap::deserialize()` halves costs nothing (that is literally its implementation) and makes every spelling equivalent - including CLUSTERED short options, where docopt reads `-ntpayload.tpl` as `-n -t payload.tpl`, so an attached value does not begin after the first character. Ordinary invocations stay quiet because the guard refuses to look further when the output does not already exist as a file, so a fresh `--output` - and an `--output` that overwrites an unrelated existing file - are both untouched. The scan counts only what the user actually TYPED - the parsed map also carries docopt-populated defaults, and `frequency`'s `--sketch-method` defaults to `exact`, so an unfiltered scan refuses `qsv frequency in.csv -o exact` over a value nobody passed and nobody can see. A presence-in-argv filter is not enough there either, because when the candidate and the output are the same string the `-o` spelling is itself an occurrence; the guard therefore requires a SECOND, independent occurrence when the strings coincide - which is exactly the flagship `qsv fmt data.csv -o data.csv`, where the path is typed twice. Docopt still reports values but not their ROLE, so the one accepted imprecision is that any user-supplied argument which happens to name an existing file counts as an input: `qsv select letter in.csv -o letter`, with a file named `letter` present, is refused even though the selector is not an input. Separating the two needs per-command schema knowledge that does not exist at this chokepoint, and that trade is the right way round - the failure mode is a clear message telling you to pick another `--output`, against a destroyed file on the other side. `same-file` is now a non-optional dependency (it was gated on `feature_capable`), because a data-loss guard has to be present in `qsvlite` and `qsvdp` too. This generalizes the narrower check `denull --apply` already carried, which is kept as defense in depth for a RESOLVED input path an argv scan cannot see. **The guard is uniform, and that is a deliberate behavior change**: a handful of commands previously survived `-o <input>` by accident - `sort`, `stats`, `transpose` and `dedup`'s in-memory path buffer the whole input before the writer is ever created, so the read had already finished by the time the file was truncated - and they now refuse along with everything else. Whether in-place output happened to work was a function of a command's internal buffering strategy, which a user can neither see nor rely on, so a uniform refusal is safer than a capability that silently differs per command; the workaround there is a temp file plus a rename, the same shape `edit --in-place` already implements. Known bound: a path named inside a query string - `qsv sqlp "select * from read_csv('data.csv')" -o data.csv` - is not seen by the scan ([#4580](https://github.com/dathere/qsv/issues/4580)).
- **`viz`: a rate map no longer divides county counts by state populations.** `viz smart` charts a per-capita rate panel from a dictionary `x-qsv.denominator` hint, and nothing checked that the denominator describes the same GEOGRAPHY as the region key. A county extract that joined in state population for normalization and carries no `state` column leaves `county_fips` as the only region candidate, so it receives the hint - and the map divides county counts by state populations. It passed both existing consumption-site checks *honestly*: a state population **is** constant within each of its counties, and **is** positive. The verified repro exits 0 with an empty stderr beside a confident `per 10,000 residents` panel. No count-derived rule separates the two classes, and three have now been tried - finest-plausible and nearest-enclosing ownership both shipped and were reviewed as defects in *opposite* directions, and a distinct-values/regions ratio was disproved with measured data (a legitimate per-city denominator scores 0.227 while the bug scores 0.500, so the classes are inverted and no threshold separates them). Granularity is simply not a property of counts. So the level is **declared**: viz now reads the `x-qsv.denominator.level` describegpt began emitting in [#4571](https://github.com/dathere/qsv/issues/4571) - inert until now - and compares it against the region column's own concept through a new `geo_level_class`, an equivalence over the AREAL UNIT rather than the token, so `geo.county` and `geo.county_fips` name one geography and a correct per-county denominator does not lose its panel merely because the region column happens to be FIPS-coded. `geo.zip_code` and `geo.zcta` are deliberately **not** such a pair - [#4524](https://github.com/dathere/qsv/issues/4524) split them because a ZIP is a mail-delivery ROUTE set and a ZCTA is a TABULATION area - and keeping them apart is pinned by its own test, since it is the one entry where "obviously equivalent" is wrong. The refusal fires only on a **positive mismatch**: both sides must resolve to a known areal unit before a disagreement means anything, so an unresolvable region concept (the lenient `geo.fips` alias, which may be a state, county, place or tract code) has *no answer* and degrades to a caveat rather than a refusal - asserting one would be the same inference the three failed rules made. And a hint whose level is undeclared or unverifiable still charts, with its subtitle saying so, because the harm here was never the wrong number alone: it was the empty stderr beside a confident map ([#4526](https://github.com/dathere/qsv/issues/4526)).
- **`describegpt`: the dictionary prompt now SHOWS a `unit` and a `gauge_range`, not just asks for them.** Both annotations shipped an instruction block with no entry in the prompt's worked JSON example, while `currency`, `aggregation` and `geo_level` were all demonstrated there. Nothing was broken - a proposal that arrives is verified and emitted correctly - but the example is the one place in the prompt that shows a *shape*, and it is the part a model imitates most literally: a model reading the monetary field learned that a money measure carries a `currency` and an `aggregation` and nothing else. The gap costs most on `unit`, precisely because UCUM is not a register a model emits unprompted - that is why [#4525](https://github.com/dathere/qsv/issues/4525) injects the vocabulary rather than relying on recall, and an instruction the model has read but never seen exercised is the weakest form of that ask. Two fields join the example: a temperature carrying `"unit": "Cel"` - the case-sensitive, copy-it-literally rule the instruction spends a paragraph on - and a percentage carrying `"gauge_range": [0, 100]` beside `"unit": "%"`, the one dimensionless carve-out the Unit instruction permits. Both declare `"aggregation": "mean"`, retiring the example's incidental implication that every measure sums. The block is hand-fenced - `{% raw %}` interleaved with `{% if infer_content_type %}` guards, and the comma separating the last field from `"grain"` lives *inside* that guard, so it moves whenever a field is appended - so the test that pins the prompt now PARSES the rendered example in both flag arms instead of only substring-matching it. A misplaced comma there would ship a worked example that is not valid JSON, and no `contains` assertion would notice ([#4573](https://github.com/dathere/qsv/issues/4573)).
- `viz`: **a `%{...}`-shaped column header no longer vanishes from the association panel's hover.** The NMI dot plot builds a plotly *hoverTEMPLATE* - its own comment says so - but escaped the pair label with `escape_hover` alone, which handles `&`, `<` and `>` and nothing else. A raw CSV header shaped like `%{x}` therefore survived into the template as a live plotly token, so plotly **interpolated it away** and the reader saw a number, or nothing, where the column's name belonged. Every other template sink in `viz` already composes `escape_template_token(&escape_hover(..))`; this one was simply missed. Found while building the rendering-contract test for the same class of bug (issue #4557) - the class where a template we emit renders as something other than what it says.
- `viz`: **`per capita` is now recognized in camelCase, and no longer fires inside `upper_capital_limit`.** `is_intensive_measure` decides whether a measure headlines a Total or a Mean, and it matched the phrase `per capita` as a **substring** of a column's label+field text. That one mistake failed in both directions at once. A camelCase name has no literal space, so `perCapitaIncome` was never intensive while `per_capita_income` was - a missed match, harmless in itself, since the column simply kept the Total it already had. But `per_capita` is *also* a literal substring of `upper_capital_limit`, `hyper_capital_reserve` and `super_capital_gain` (`up` + `per_capita` + `l`), so those additive columns were silently **averaged** - a wrong number, not a missed one. The phrase now matches as an adjacent-**token** window (`per` followed by `capita`) in `has_per_unit_phrase`, exactly as `per unit`/`per item` have since the same hazard was fixed for them: the tokenizer splits camelCase, so `perCapitaIncome` reaches it, and adjacency means `upper capital limit` cannot. Note that the obvious one-line fix - testing the phrase against the token join - would have reintroduced the very false positive it was fixing, because `upper capital limit` still *contains* `per capita`. The per-language phrases from the locale lexicons (`per cápita`, `par habitant`) are still substring-matched and keep the camelCase gap; that half is tracked separately.
- `viz`: **a constant `x-qsv.denominator` no longer draws a rate panel that is only the count panel rescaled.** When a dictionary denominator hint resolves to ONE distinct value across the matched regions, dividing by it rescales every region by the same constant - identical ranks, identical relative comparisons, identical map shape, with only the axis label and the magnitude changing - so the panel carries no information a reader can act on. Unrelated causes produce it (a placeholder column, a join that collapsed, a genuinely uniform denominator) and they are deliberately not told apart, because the panel is uninformative in all of them. `viz smart` now skips it with a note naming both the constant and the region counts as "N of M" - the constant tells the reader whether this is a placeholder or a real uniform denominator, and the counts say how much of the map the check ranged over, since a region whose denominator is unusable is excluded from the panel and therefore from the check. It degrades the way every other denominator problem does: a stderr note, a "Panels not drawn" record, the count panel keeping its raw-count caveat, and never an error - a hand-editable sidecar must not take the Data Schematic down. Three invariants are load-bearing: the positive-value filter moved UP into the hint arm so the check and the panel see the same map; equality mirrors the row pass's within-region constancy test and is never more PERMISSIVE than it, so a pair this check calls equal is one the row pass would call constant too - the converse does not hold, since the two scale their tolerance by different operands, but the two never judge the same pair and the gap errs toward drawing the panel; and the anchor is the MINIMUM rather than whichever entry the map yields first, since `HashMap` iteration order is nondeterministic and under a tolerance predicate the anchor is observable. **Deliberately PARTIAL**: the check is threshold-free and stays at one distinct value, which is a statement about the *panel* being degenerate rather than about which geography the denominator describes. It does NOT address [#4526](https://github.com/dathere/qsv/issues/4526), whose verified repro is two distinct values across four regions; the distinct-values/regions RATIO that would fire on it was investigated there and disproved with measured data - a legitimate per-city denominator scores 0.227 while the coarse-geography bug scores 0.500, so the classes are inverted and no threshold separates them ([#4547](https://github.com/dathere/qsv/issues/4547), [#4564](https://github.com/dathere/qsv/pull/4564)).
- **`viz`: a `%` in a hover no longer prints as `%%`, and a `%{...}`-shaped column header is no longer eaten.** `escape_template_pct` doubled every `%` in text interpolated into a plotly `hovertemplate`, on the stated theory that a bare `%` starts a `%{...}` token. Both halves of that were wrong, and were only caught because [#4551](https://github.com/dathere/qsv/issues/4551) made a **`%` unit** reachable there ("18.4 %%"). Verified against plotly 3.7.0 in a browser: the token regex requires the **brace**, so a lone `%` was never dangerous and doubling it reached the reader as two percent signs - while `%{evil}` doubled to `%%{evil}` *still* contained `%{evil}`, so plotly still interpolated it away, now leaving a stray `%` in front of the hole. Doubling therefore corrupted the safe case and failed to fix the dangerous one. The helper (now `escape_template_token`) neutralizes only the brace, as the numeric character reference `%&#123;` - hover text renders as pseudo-HTML, so it displays as a literal `%{` while no longer matching the regex, and it is inserted after `escape_hover` so the entity stays live. Seven pre-existing sinks are fixed along with the new ones: the map density weight label, the contour hover's x/y/label, the Gapminder bubble template's entity and axis labels, the histogram hover's binned-value label, and standalone `viz scatter3d`'s three axis labels. The last two had never called the helper at all - the histogram was explicitly exempted by a comment reasoning that applying it "would be a behavior change", which was true only while it doubled percent signs; neutralizing the brace alone is a no-op for every header except the `%{...}`-shaped one that was already broken. The Lorenz and contour tests asserted the old doubled output and now assert the rendered truth ([#4551](https://github.com/dathere/qsv/issues/4551)).
- **a failed download is no longer written to disk as if it were the file.** `util::download_file` streamed whatever the server returned and never checked the status, so a 404 page became the payload: an HTML "Not Found" body was saved as a Geonames index, a JSON Schema, or a CSV, and the failure surfaced later as a parse error about the *contents* rather than the download. It now fails on any non-2xx response, naming the status. This affects every download path - `geocode` index fetches, `validate`'s remote JSON Schema, `sample` and `snappy` remote inputs - none of which have any use for an error body ([#4433](https://github.com/dathere/qsv/issues/4433)).
- `geocode`: **`index-load <floor>` no longer litters the working directory.** The numeric shortcut downloaded to its bare argument as a relative path, so `qsv geocode index-load 1000` dropped `1000` and `1000.rkyv` into whatever directory the command happened to run in - the repo checkout, more often than not. The payload is now staged in the geocode cache dir under a descriptive name (`cities1000.rkyv.sz`), where it also serves as a warm copy for a later re-install ([#4433](https://github.com/dathere/qsv/issues/4433)).
- `geocode`: **`index-load` installs a working index**, rather than a corrupt one it reports as a success. Immediate follow-up to the validity fix below, and only reachable once that landed: having accepted a valid index, `index-load` then *re-serialized the loaded engine* to install it, via a `Storage::dump_to` whose result was also discarded (`let _ = …`). The rewritten index came out **exactly 696 bytes shorter** - for every index, whatever its size - and could not be loaded at all, failing with ``Error initializing Engine: subtree pointer overran range``. So the command printed "successfully copied" and left an unusable index in place. It now copies the file, which is exact and cannot corrupt it: the source has already been loaded and validated, so there is nothing to re-encode. Copying onto the active index is skipped when both paths resolve to the same file, and a failed copy is now an error instead of being silently ignored. The success-path test gained the assertions that would have caught this - the installed index must be byte-identical to its source, and must actually resolve a city afterwards ([#4433](https://github.com/dathere/qsv/issues/4433)).
- `geocode`: **`index-load` no longer rejects every index as invalid**, including qsv's own published prebuilt. Both forms failed, and the command contradicted itself in a single run - `Valid: /tmp/15000.rkyv` immediately followed by ``usage error: Alternate Geonames index file /tmp/15000.rkyv is invalid``. The numeric shortcut failed after doing all the work correctly: it downloaded the published `.sz` (9,496,541 bytes) and decompressed it (23,696,368 bytes), then refused the result. The cause is two code paths asking different questions of the same file - `index-check` reads metadata with `Storage::read_metadata` and finds it, while `index-load` gated on `EngineData::metadata` from `Storage::load_from`, which never populates that field, so the "invalid" arm fired for every input. `index-load` now validates through the same `read_metadata` API, against the **resolved** path (a numeric shortcut is downloaded under the bare number and decompressed to a sibling `.rkyv`, so the argument passed in is not what was loaded). The premature `Valid:` line is gone too: that message came from a helper that only checks the file's extension and existence, so it now says `Found index file:` and validity is reported only once the index has actually loaded. **This was not a recent regression** - the gate arrived with the rkyv index format in May 2025 and shipped in 37 releases starting with 10.0.0, unnoticed because the subcommand had no success-path test; it has one now, verified to fail without this fix ([#4431](https://github.com/dathere/qsv/issues/4431)).
- `viz`: **`--geocode` no longer searches every populated place on Earth**, so a US dataset stops resolving abroad. The two region resolvers passed no country filter to the Geonames engine, with different symptoms per `--location-mode`: under `iso3` a place matched abroad was **drawn as real data** (six US city names produced `PAK`, `GBR`, `USA` — Montgomery to Pakistan, Birmingham to the UK, 14% of the measure attributed to countries absent from the data), and under `usa-states` it was **silently dropped**, because the US-state code is only set for US matches (30 of 210 units vanished with no warning). The coordinate source was affected too, since reverse geocoding returns the nearest *populated place*: San Ysidro CA resolved to Tijuana MX and Point Roberts WA to Delta BC. Neither symptom was an index-resolution problem - both reproduced identically against a `cities1000` index. Three changes: a `usa-states` choropleth now **defaults its country to `us`** on both sources, which needs no user input and is what fixes the silent drop; new **`--geocode-country <col>` and `--geocode-admin1 <col>`** hint columns constrain the lookup per row, which is the only shape that works for a world map where each row may need a different country; and unresolved rows are now **counted and reported** in rows *and* measure units, since an unreported drop understates the total, which is what made this invisible. Hinted lookups are **strict**: where `geocode suggest --admin1` falls back to its top hit (`Seattle` hinted to Alabama returns Washington), the viz path reports the miss instead - `geocode suggest`'s own advisory behavior is deliberately unchanged. A bare two-letter hint like `AL` is promoted to the qualified `US.AL` and compared as a *code*, because admin1 *names* are matched by prefix, under which `AL` silently accepts **Alaska**; spelled-out names are rejected outright for the same reason. The hinted path also scores a much deeper candidate list than `geocode suggest`'s top 10 (a small town in the hinted state sits below every same-named larger city: `Springfield` hinted to `US.CO` returned Springfield *Missouri*), which is free because the engine sorts and dedupes the full candidate set before applying the limit. Finally, an unknown country code is now rejected up front: the engine silently discards one it doesn't know, so `UK` (for `GB`) would have resolved zero rows and read as "none of your places exist". **This is a visible output change** - rows that previously vanished now count, and rows previously attributed abroad move ([#4427](https://github.com/dathere/qsv/issues/4427), [#4430](https://github.com/dathere/qsv/pull/4430)).
- `viz`: **per-unit money measures are averaged instead of summed**, and `x-qsv.aggregation` can override the choice. `viz smart` rendered a "Total Unit Price" KPI tile - summing a per-unit price, which is meaningless. `unit_price` and `shipping_cost` are both `role: measure, concept: measure.amount`, so nothing distinguished them and both took `Sum`. The distinction is not the money noun but whether the measure is quoted **per single item**, so the new rule is a conjunction: a money noun (price/cost) *and* a per-single-item qualifier (unit/unitary/each/apiece). Adding a bare "price"/"cost" to the intensive tokens instead would have wrongly averaged `total_price`, `sale_price`, `order_price` and `shipping_cost` - the committed gallery's "Total Shipping Cost" tile is the live canary for that. A bare `per` is deliberately not a qualifier ("price per order" on one-row-per-order data sums fine), so the conclusive `per unit`/`per item` phrases join `per capita` as substrings, where a phrase survives tokenization. Worth naming: the intensive-measure test has 5 call sites, so making `unit_price` intensive also drops it from inequality/Lorenz candidacy and funnel-stage eligibility - defensible, since a per-unit price is not a distributable quantity, but wider than the KPI tile alone ([#4401](https://github.com/dathere/qsv/issues/4401), [#4404](https://github.com/dathere/qsv/pull/4404)).
- `describegpt`: **dataset language detection is now gated on chunk agreement, not confidence.** `describegpt --dictionary` read a table of city, country and continent names as *Italian* at 0.9308 confidence, which cleared the 0.8 threshold - so `viz smart` localized an entire Data Schematic to Italian (conteggio, Correlazione, Mappa, Paesi) and the LLM was asked for Italian labels. Root cause: whatlang's `confidence` is the **margin between its top two candidates**, not evidence that the text *is* that language. A proper-noun bag can win a wide margin by accident, while genuinely Portuguese data scores 0.389 because Spanish is a close - and correct - runner-up. No threshold separates those: raising it loses Portuguese, lowering it keeps Italian, and `Info::is_reliable()` is literally `confidence > 0.9`, so it fails identically. The detection sample is now split into 3 disjoint chunks and every chunk must agree with the full-sample verdict (a chunk whose `detect()` returns `None` - all dates and numbers - counts as disagreement); text genuinely written in one language is stable across its own subsamples, incidental n-gram noise is not. Confidence is still computed and recorded, it just no longer decides. Agreement is a *necessary* condition, not a sufficient one - a long, homogeneous proper-noun table could still return the same wrong language in every chunk. Separately, an explicit `--language` now suppresses the recorded detection entirely: `detected_language_code` is what viz localizes from, so recording a detection under a pinned language let viz render in the *detected* language instead, which made `--language English` only a partial workaround. Chat mode (`--prompt`) is unchanged and keeps its 0.8 gate - it detects the language of the user's own prose, which is whatlang's actual design domain. Verified across 19 example datasets plus two external ones: zero non-English misdetections remain ([#4406](https://github.com/dathere/qsv/issues/4406), [#4411](https://github.com/dathere/qsv/pull/4411)).
...
- `stats`: **a four-batch review of `stats.rs` fixed panics, undefined behavior, a tempfile leak, and a cluster of outright wrong results.** The largest single body of correctness work in this release, split into reviewable batches rather than one unreadable diff ([#4438](https://github.com/dathere/qsv/pull/4438) Batch A, [#4439](https://github.com/dathere/qsv/pull/4439) Batch B, [#4443](https://github.com/dathere/qsv/pull/4443) Batch C, [#4450](https://github.com/dathere/qsv/pull/4450) Batch D). Batch A: `QSV_STATS_STRING_MAX_LENGTH` truncated min/max strings at a raw **byte** index, panicking on multibyte UTF-8 - it now truncates on a char boundary, mirroring the antimodes path that already did; and the integer range computed `max - min` in `i64`, which panics under overflow-checks and silently wraps to `-1` in release for a column spanning more than `i64::MAX` (one holding both `i64::MIN` and `i64::MAX`) - the subtraction is now done in `i128`, which is free because `range` is consumed as an `f64` downstream. Batch D removed the dead `Stats.sum_stotlen` field, which was declared, zero-initialized and merged (`0 + 0`) but never incremented and never read - worse than dead weight, because it carried a "frequently updated counter" comment and sat in the cache-line-1 group the layout comments reason about, so anyone chasing a `stotlen` bug would plausibly have edited the wrong field.
- `stats`: **an args-changed recompute left a stale sidecar behind, and every stats-cache consumer inherited the wrong statistics** ([#4439](https://github.com/dathere/qsv/pull/4439)). Deleting `<FILESTEM>.stats.csv` on an args change did not delete its `<FILESTEM>.stats.csv.json` sidecar, so the next run validated against the stale sidecar and **trusted** it - `qsv stats f.csv -E -c 1` then `qsv stats f.csv --typesonly` then `qsv stats f.csv -E` served the `--typesonly` output as though it were `--everything`, and `schema`, `frequency`, `tojsonl` and `viz smart` all inherited it. The sidecar is now deleted with its CSV.
- `stats`: **parallel stats no longer caches a PREFIX of the file's statistics when a worker dies** ([#4442](https://github.com/dathere/qsv/pull/4442)). `parallel_stats` merged chunk results by draining the channel until the last sender dropped, never checking that every chunk actually arrived. A panicking worker never sends its chunk, so the loop ended early: later chunks stayed stranded in `pending`, `next_chunk` stopped at the gap, and `merged.unwrap_or_default()` returned a prefix of the statistics - or an **empty vector** when chunk 0 was the one that died. `run()` then wrote, printed *and cached* that partial answer with **exit code 0**, so every later run reused statistics computed from a fraction of the file. Reproduced end-to-end rather than by reading, using the TOCTOU the worker's own `expect()` anticipates: deleting the index mid-run on a 400k-row file chunked into 28 pieces.
- `stats`: **the epoch date is no longer excluded from date min/max** ([#4444](https://github.com/dathere/qsv/pull/4444)). Timestamp 0 is `1970-01-01T00:00:00Z` - a real date, and a common placeholder in real data - but the date min/max accumulator skipped it behind an `int_val != 0` guard, so a column holding the epoch reported the wrong minimum, a truncated range, and computed `sort_order`/`sortiness` over the wrong sample set. On a 5-row file starting at `1970-01-01`, min was reported as `2020-01-15` with range 19205 while the *same run's* `percentiles` cell showed `5: 1970-01-01` - the row was in the data and in the quantile sample, and only min/max dropped it. Now min=`1970-01-01`, range=19357. The guard looked necessary because `from_sample` also returns 0 as its "no date" value, but 0 cannot mean "no value" at that call site.
- `stats`: **date renderings are no longer coerced to `0.0` in the stats cache** ([#4441](https://github.com/dathere/qsv/pull/4441)), and **the `field` key survives an empty column name** ([#4410](https://github.com/dathere/qsv/issues/4410), [#4412](https://github.com/dathere/qsv/pull/4412)).
- `stats`: **a negative `--cache-threshold` cleanup targeted the wrong file on non-CSV inputs** ([#4443](https://github.com/dathere/qsv/pull/4443)). A negative `--cache-threshold` sets the autoindex size, and one ending in `5` also asks for the autoindex and stats cache to be deleted afterwards. The cleanup built its path with `with_extension("csv.idx")`, which **replaces** the input's extension, while `util::idx_path` - which wrote the file - **appends** `.idx`. So a `data.tsv` autoindex was written to `data.tsv.idx` and then looked for at `data.csv.idx`: the index the user explicitly asked to delete survived, and silently, because the removal only `log::warn!`s on failure. Extensionless inputs were affected the same way. It worked for `.csv` inputs purely by coincidence.
- **`count` no longer disagrees with itself on files containing blank lines** ([#4456](https://github.com/dathere/qsv/issues/4456), [#4472](https://github.com/dathere/qsv/pull/4472)). Polars materializes every literal blank line as an all-null row, while qsv's csv reader - and the index built with it - skip blank lines entirely, and polars 0.55 offers no skip-blank-lines option. So on a file with a blank line the **unindexed** count (polars) exceeded the **indexed** count (csv reader) by one per blank line. Because `util::count_rows` funnels through `polars_count_input`, the inflated count caused real misbehavior downstream: `slice --index -1` returned nothing (**silent data loss**), `sample` with a fractional size oversampled, and `frequency` missed `<ALL_UNIQUE>` detection - each of them flipping to correct behavior the moment an index happened to exist. A `may_contain_blank_lines` memchr2 guard now falls back to the csv reader, and all 13 `count_rows` consumers were audited and repaired. Still open, and not fixed here: polars-backed commands (`sqlp`, `joinp`, …) genuinely *process* the phantom null row.
- `stats`: **`record_count` now comes from the compute pass, not a separate pre-pass** ([#4458](https://github.com/dathere/qsv/pull/4458)). Same root cause as the `count` fix above, and the reason it mattered twice: `util::count_rows()` counts blank lines while the csv reader feeding `compute()` skips them, so seeding `RECORD_COUNT` from the pre-pass inflated every per-record denominator in `to_record()` - `uniqueness_ratio`, and the `cardinality == record_count` test behind the `<ALL_UNIQUE>` antimode sentinel. The trigger is a file ending in `"...\n\n"`, i.e. ordinary real-world data, and the same file produced **different statistics depending only on whether an index happened to exist**.
- **the auto-indexing machinery is now race-free, atomic and permission-preserving** ([#4463](https://github.com/dathere/qsv/issues/4463), [#4466](https://github.com/dathere/qsv/pull/4466), [#4459](https://github.com/dathere/qsv/pull/4459), [#4468](https://github.com/dathere/qsv/pull/4468)). `Config::index_files` carried a fast path keyed on a process-global `AUTO_INDEXED: AtomicBool` set whenever **any** file was autoindexed. The flag carried no path, so it also fired for a *different* input, and its `fs::File::open(idx_path(self.path))` was unconditional - turning a missing sibling index into a hard `ENOENT` instead of a graceful `Ok(None)` and a sequential read (`QSV_AUTOINDEX_SIZE=1 qsv stats --infer-dates f.zip` → `io error: No such file or directory`, exit 1). The global fast path is gone, resolved `Config`s are now **shared** with parallel workers rather than rebuilt per worker, and stale-index rebuilds are serialized, atomic and preserve permissions. Worth recording honestly: the first version of the concurrency fix passed 12/12 repro runs and the full suite locally, then **broke master on macOS and qsvlite** - a green local suite is not evidence about CI for a concurrency change.
- **a special-format input's converted temp file is never autoindexed** ([#4445](https://github.com/dathere/qsv/pull/4445)), and `stats` **regained parallel processing for those inputs** ([#4462](https://github.com/dathere/qsv/issues/4462), [#4469](https://github.com/dathere/qsv/pull/4469)). A `.gz`/`.zip`/`.parquet`/`.jsonl` input is read through a converted temp file, and every `Config` instance resolved its **own** temp path - so autoindexing that temp created an index only the parent's `Config` could find. `qsv stats -E --cache-threshold -105 data.csv.gz` produced a stats file with headers and **zero data rows, at exit code 0**, after printing 17 worker panics to stderr, because a panicking pool worker only unwinds its own thread. #4445 was the narrow fix - refuse to autoindex, and therefore always run sequentially - and #4469 restores parallelism properly by adopting the shared-`Config` pattern from #4459 across `split`/`search`/`searchset`/`pragmastat`.
- **`TEMP_FILE_DIR` is now cleaned up in non-polars builds too** ([#4461](https://github.com/dathere/qsv/issues/4461), [#4467](https://github.com/dathere/qsv/pull/4467)). `util::log_end` removes the directory when a command finishes, but that cleanup was gated on the `polars` feature while the directory is populated in **every** build - `extract_zip_to_temp` is always compiled (it needs only the non-optional `zip` crate), and the stdin spill temps in `split`, `stats` and `partition` land there too. A non-polars build therefore left a full decompressed copy of every `.zip` input on disk, once per invocation, until the OS reclaimed the temp dir - on many systems only at reboot.
- `moarstats`: **special-format inputs are read through `Config` rather than raw paths, the resolved input's delimiter is honored, and the primary header stays primary** ([#4460](https://github.com/dathere/qsv/issues/4460), [#4464](https://github.com/dathere/qsv/pull/4464), [#4465](https://github.com/dathere/qsv/pull/4465)). Also: **`--pct-thresholds` is forwarded into the stats percentile list** ([#4471](https://github.com/dathere/qsv/pull/4471)), so a requested threshold is actually computed instead of silently ignored.
- **a failed write is an I/O error, not a qsv crash** ([#4516](https://github.com/dathere/qsv/issues/4516), [#4517](https://github.com/dathere/qsv/pull/4517)). A write that fails - a full device, a closed fd - reached the user as "qsv had a problem and crashed", complete with a crash report to file. [#2661](https://github.com/dathere/qsv/issues/2661) reported this for `qsv sniff --json | jaq <bad filter>` and [#2664](https://github.com/dathere/qsv/issues/2664) fixed the broken-pipe case by resetting `SIGPIPE`, but every other write failure still presented as a panic. A full disk is a condition to report, not a bug to file.
- `get`/`dc:`: **a failed stale-cache refresh no longer falls back to the stale copy silently** ([#4511](https://github.com/dathere/qsv/issues/4511), [#4512](https://github.com/dathere/qsv/pull/4512), [#4514](https://github.com/dathere/qsv/pull/4514)). The refresh path now warns, and the warning was deliberately hardened rather than written with `wwarn!`: every other `wwarn!` call site sits in a command or in `main`, whereas this one is in a shared path reached by **every** command that reads a `dc:` handle, and the macro's `writeln!(..).unwrap()` would itself panic on a closed stdout.
- `apply`: **negative `currencytonum` amounts with three decimals, and sub-dollar sign handling** ([#4478](https://github.com/dathere/qsv/issues/4478), [#4483](https://github.com/dathere/qsv/issues/4483), [#4484](https://github.com/dathere/qsv/pull/4484)). `qsv_currency` 0.7.0 returned `0.00` for negative amounts with three decimal places; the 0.8 bump fixed it but nothing in the suite covered the case, so the regression could have returned unnoticed. `-1.234`/`-12.345`/`-0.999` and their `$`-prefixed equivalents are now pinned in both the strict and non-strict tests.
- `viz`: **a region-constant column is no longer summed across rows** ([#4528](https://github.com/dathere/qsv/issues/4528), [#4529](https://github.com/dathere/qsv/pull/4529)), and **the KPI tile now headlines the region-deduped total instead of being omitted** ([#4534](https://github.com/dathere/qsv/issues/4534), [#4536](https://github.com/dathere/qsv/pull/4536)). A column that describes the **region** rather than the row - a population, an area, a fleet size - repeats identically on every row of that region, and `viz smart` routed it like any other numeric measure and summed it, so on event-level data every panel built from it was inflated by the rows-per-region factor (measured on 8 counties × 12 rows: "Total Population 138.8M" for 11.5M residents). #4528 suppressed the tile rather than print a wrong number; #4534 goes further and prints the **deduped** figure, which the stats cache alone cannot supply because it carries no `(region, value)` pair count - the limit `owning_region_idx` documents.
- `viz`: **a Census Designated Place now resolves, making `census:place` the full place universe** ([#4540](https://github.com/dathere/qsv/issues/4540), [#4545](https://github.com/dathere/qsv/pull/4545)). A Census place is an incorporated place **or** a CDP - the unincorporated communities Census delineates for tabulation - but `Layer::Place` matched exactly one catalog entry (`name == "Incorporated Places"`), so a CDP-keyed column resolved against nothing. The user-visible failure was worse than a partial map, because `--geojson auto` scores each layer by the fraction of rows it resolves.
- `viz`: **map-hover gap-filling suppression is resolved per row, not per column** ([#4541](https://github.com/dathere/qsv/issues/4541), [#4546](https://github.com/dathere/qsv/pull/4546)).
- `viz`: **the guardrail now clears `x-qsv.currency`/`x-qsv.unit` when it rejects a measure reading** ([#4553](https://github.com/dathere/qsv/pull/4553)). `derive_semantics` carries both through unconditionally - deliberately, so a `Defer` column still gets its panel subtitle - but `guardrail` downgrades a `Measure` to a `Dimension` when the observed stats contradict the dictionary, and it cleared nothing on the way out. A hand-edited sidecar declaring `role: measure` on a zero-padded code column therefore rendered a frequency bar captioned `Padded Code (USD)` or `(kg)`: a denomination on something that is not a quantity.
- `viz`: **duplicate KPI aggregation labels, underscores inside KPI identifiers, short-option arity, and assorted rendering and data-handling edge cases** ([#4500](https://github.com/dathere/qsv/pull/4500), [#4510](https://github.com/dathere/qsv/pull/4510)). `ArgvExtras::option_takes_value` looked one token past a matched flag for a `<placeholder>`, which never holds for a short alias declared beside a long one, so arity is now derived from the whole usage spec. Also: the tour's first popover takes focus on start, embedded galleries scroll the parent page to the popover, the Tour pill merged into the Data Schematic row, and the drawer step calls itself a "Data Schematic" rather than a "Data Dictionary" ([#4506](https://github.com/dathere/qsv/pull/4506), [#4507](https://github.com/dathere/qsv/pull/4507), [#4530](https://github.com/dathere/qsv/pull/4530)).
- **`viz smart`: a country/US-state choropleth panel can no longer disappear in silence.** The reverse-geocoded overview panel drawn beside a point map reached the geocode engine through `reverse_geocode_regions(..).ok()?`, so any hard setup failure - the tokio runtime, a failed index download, a corrupt or truncated Geonames index on disk, engine init - was discarded and the panel simply ceased to exist: no stderr line, no "Panels not drawn" entry, exit 0, and every panel after it renumbered. That is how two committed gallery dashboards - `smart_world_events.html` and `smart_world_choropleth.html` - were published minus their choropleth and stayed that way until a version-bump diff showed an unexpected net-negative line count months later. Every way out of the builder now either draws a panel or says why it did not, over two new omission notices: an engine that could not be loaded at all (carrying the engine's own error text), and an engine that loaded yet resolved no usable region code - an index whose city records survive but whose country table does not resolves every point and still yields no country, because a `GeoRegion` can carry a valid ISO-2 code with an empty ISO-3 one. A *single* resolved region stays silent: that is the documented point-only case for metro data. Measured on `world_events_dated.csv` against a deliberately poisoned index: **7 panels -> 6 with no output whatsoever before, the same 6 panels plus a named reason after** (the issue's table says 8 -> 7; 7 is what the committed page's `Plotly.newPlot` panel calls and the issue's own trace list both come to). A network failure is **reported like any other engine failure rather than aborting**, unlike `--geojson auto` and `viz choropleth` where a `Network` error propagates - that rule exists so network weather cannot decide which candidate column wins the region contest, and there is no contest here, just one unrequested companion to the map; measured, aborting would have made `qsv viz smart` on any globe-spanning CSV exit 3 offline with no cached index, naming a GitHub URL the user never mentioned. An image export does not pay for either companion choropleth at all: the image branch discards both the reverse-geocoded panel and the `--geojson` point-in-polygon one, so building them there loaded the ~23 MB Geonames index - and on a cold cache downloaded it - and binned every core point into the boundaries, all for panels nothing renders; and once those builders started reporting their failures, an image export began announcing skipped panels it would never have drawn even when healthy (a healthy one says nothing about them). Skipping the point-in-polygon builder costs no validation: a missing file, unparseable GeoJSON and a `--feature-id-key` that resolves on no feature are all rejected upstream in `validate_geojson_source`, verified still erroring on an image export, and the boundary OVERLAY - which is attached to the map panel and so is wanted by the image path - is built independently of the panel. `examples/viz/gen_gallery.py` gained an independent artifact-level net beneath all of this: it records each page's panel count immediately before overwriting it and warns on a decrease, alongside its existing `warn_stale_data_viewers` / `warn_stale_parcats_pages` checks, so a panel that vanishes for a reason qsv cannot name still surfaces in the regen log instead of in a diff. The underlying non-determinism is NOT fixed - the original failure has resisted 7 of 7 reproduction attempts - and [#4591](https://github.com/dathere/qsv/issues/4591) stays open for it; what changed is that the next occurrence will print the error text that has been missing.
- **`viz`: the data viewer's column headers no longer float in the middle of a tall header row.** The title row stretches to its *tallest* title, and `th` defaults to `vertical-align: middle` - so on a wide table one column whose name wraps to four lines set the height for every column, and short titles were centred in the leftover space. Measured on the 41-column nyc311 Data Schematic: an 85px title row, a 22px one-line title, and the 63px left over split evenly, putting **~32px of dead space between each label and the search input it belongs to** - which reads as two disconnected bands rather than 41 title+filter pairs. Title cells are now bottom-aligned, so each label sits with its own control: **median gap 32px -> 13px**, with the row height and every column width unchanged (5103px before and after), so DataTables' sizing pass is untouched and nothing reflows. On a table whose titles already fit one line there is no slack to redistribute and the change is a no-op, which is why it is invisible on narrow dashboards. A two-line clamp on the title was also tried, and **deliberately dropped**: it shortened the header by a further 38px, but only by clipping 10 of nyc311's 41 titles behind a `title` tooltip - and `title` never fires on touch and does not appear on keyboard focus, so the reveal was missing for exactly the readers who could not simply read the name. Header cells are not focusable either (0 of 41), so there is nothing to attach a focus-triggered popover to without adding 41 tab stops to a drawer that already has 41 search inputs. A legible column name is worth more than 38 vertical pixels. Verified in Safari 27 and Chrome 153.
- **`viz`: the data drawer's CSV export and guided tour now have regression tests.** Both depended on ColumnControl-owned DOM that nothing asserted: the export strips the search row by filtering `headerStructure` on `data-dt-order="disable"`, and the tour spotlights `#qsv-data-drawer thead tr:nth-child(2)`. An upstream rename would have silently produced a blank line between the CSV header and its first record, and a tour step pointing at nothing - neither raises an error. The stale comment claiming the drawer builds "ONE header row ... there is no second filter row" was corrected at the same time; ColumnControl has created that second row since [#4302](https://github.com/dathere/qsv/pull/4302).

### CI/Actions
- **the gallery build fails when a caption contradicts its own chart.** The gallery's lead caption quoted numbers out of a generated artifact ("287 of 417", "default 10 km snap cap") and went stale when `--snap-max-dist` became auto-derived, so the published caption pointed readers at a panel title that disagreed with it. The generator now parses the committed page's `Regions (...)` panel title and fails closed on two self-keying claims: an "X of Y" pair where Y is that panel's total point count must state the dropped count, and a distance written near the word "snap" must include the cap the panel actually applied (presence, not exclusivity, since the same caption legitimately cites two other distances). A missing or unparseable title also fails, so a retitled panel cannot quietly disable the check; a bare snapped count is deliberately *not* checked, since a lone integer cannot validate itself. This fails rather than warns because the caption lives in `gen_gallery.py` and is always fixable ([#4392](https://github.com/dathere/qsv/issues/4392), [#4398](https://github.com/dathere/qsv/pull/4398)).
- **no test asserts a command's exit status and its stderr against two different executions**, and CI now gates on it. `Workdir::output_stderr` *runs the command a second time*, so the common shape - `wrk.output(&mut cmd)` for status and stdout, then `wrk.output_stderr(&mut cmd)` for stderr - checked two separate runs. Either direction can be masked: a run that succeeds without emitting the expected warning, or a second run that fails but happens to print the expected text. `wrk.assert_err`/`assert_success` execute the command too, so pairing either with `output_stderr` is the same defect wearing a different helper - and the original scan missed those. One sweep across the suite removed **234** `output_stderr` call sites in **43** test files, reading stderr from the already-captured `Output` instead (and using the single-run `stderr_on_success` helper on success paths), eliminating that many redundant `qsv` executions per test run. `scripts/double-run-check.py --check` now prints each offending site with the helper to use and exits 1, running on PRs that touch `tests/` plus a nightly cron for pushes that bypass a PR; sites whose two runs are separated by an `.arg()`/`.env()` call are genuinely different commands and are noted rather than failed, and a test that runs one command twice on purpose (caching, idempotence, indexed vs unindexed) opts out with a `// double-run-check: intentional` comment. Python-stdlib only, ~0.3s. Test-only; no production behavior changed ([#4415](https://github.com/dathere/qsv/issues/4415); [#4422](https://github.com/dathere/qsv/pull/4422), [#4423](https://github.com/dathere/qsv/pull/4423), [#4424](https://github.com/dathere/qsv/pull/4424), [#4425](https://github.com/dathere/qsv/pull/4425), [#4426](https://github.com/dathere/qsv/pull/4426)).
- **the `geosuggest-core` dev-dependency is gated by ARCH, not endianness**, unbreaking the s390x and ppc64le native runners. Two traps are worth recording: an unconditional `[dev-dependencies]` entry is built by `cargo test` **regardless of `--features`**, so feature-gating the consumer does not help; and gating on endianness is the wrong axis, because ppc64le is little-endian yet still outside the x86/ARM allowlist. `cargo tree --target <triple> -e normal,build,dev -i <crate>` is the tool that actually proves which graph a crate is in - without `--target` it proves nothing. `rust-ppc64le.yml` is the easily-forgotten twin of `rust-s390x.yml`; both needed the change.
- **the benchmark script no longer leaks environment variable VALUES into published results, and `to_sqlite` stopped measuring its own failure path** ([#4494](https://github.com/dathere/qsv/pull/4494)). Both matter because `scripts/benchmarks.sh` output is committed and published to [qsv.dathere.com/benchmarks](https://qsv.dathere.com/benchmarks) and the GitHub wiki, so a result-integrity bug is a *publishing* bug. `qsv --envlist` prints variable **values** - a live `QSV_CENSUS_API_KEY` among them - and that output was embedded verbatim into the committed `results/run_info_history.tsv`; only the variable **names** are recorded now, `;`-joined onto one line, which also stops the multi-line envlist output from corrupting the one-row-per-run TSV. (No values had leaked: every historical row predates the variable being set.) Separately, `to_sqlite` had been timing its own error path for its entire history - the `;rm benchmark_work.db` cleanup ran once at **queue** time rather than between hyperfine runs, and `to sqlite` errors out when the target table already exists, so `hyperfine -i` dutifully recorded the failure as a plausible-looking timing. All 60 affected rows were pruned. The general lesson: `hyperfine -i` masks failures as credible numbers, so a suspiciously fast benchmark deserves a sanity check.
- **the "Applies to qsv X.Y.Z" marker in `docs/STATS_DEFINITIONS.md` is asserted against `Cargo.toml`.** On its own the marker is a comment, and a comment is exactly what six weeks of drift had already slipped past. `scripts/docs-drift-check.py` now re-derives the version from `Cargo.toml` for any doc that opts in via `VERSION_MARKER_DOCS`. A MISSING marker is a finding too, unlike the neighbouring `QSV_USER_AGENT` check which silently passes when its pattern is absent - the asymmetry is deliberate and is written into the code, because that check guards an example line which may legitimately be restructured away, whereas this marker exists precisely because these files drift without leaving a trace, so deleting it re-opens the hole it was added to close. The check is DELIBERATELY noisy at release time: bumping `Cargo.toml` reds it until someone re-reads the doc against current behavior and *then* bumps the marker, and the failure message says so explicitly, because bumping the marker alone silences it without doing the work it exists to prompt. Mutation-tested in both directions rather than only confirming a green run.
- **PGO training no longer OOM-kills the publish runner.** `scripts/pgo-train.sh` pivoted on `Created Date` (841,014 distinct values) inside its `--try-parsedates` training case, building an enormous 841k-column frame that exhausted a GitHub hosted runner's memory and took the VM down with it - surfacing as exit 143 / "the hosted runner lost communication with the server", with nothing memory-related in the log to point at the cause. `--try-parsedates` is a *reader* option and has nothing to do with the pivot key, so the script now pivots on low-cardinality `Agency` instead. The underlying command bug that let an unbounded pivot get that far is fixed separately - see Breaking Changes.
- **a Linux-only publish retry workflow, and the matrix-sync check now validates both retry matrices.** `scripts/check-publish-matrix-sync.py` validates the Windows *and* Linux retry matrices against `publish.yml`. Windows must match fully; Linux may be a subset, because only the historically failing target needs retries. This stops a retry upload shipping binaries built with a different feature set than the main release.

**Full Changelog**: https://github.com/dathere/qsv/compare/22.0.1...23.0.1
...
- **`viz`: a whole new visualization command.** Interactive [plotly](https://plotly.com/javascript/) charts and maps from CSV, with 20+ standalone chart subcommands and a `viz smart` mode that auto-designs an entire **Data Schematic** from qsv's existing stats & frequency caches. Output is self-contained, offline-capable HTML, with static PNG/SVG/PDF export via `viz_static`. See the [gallery](https://dathere.github.io/qsv/gallery.html) ([#302](https://github.com/dathere/qsv/issues/302); [#4019](https://github.com/dathere/qsv/pull/4019)).
- **The Data Schematic is explorable, not just viewable.** An embedded DataTables **data viewer drawer** puts the underlying rows beside the charts, cross-linked with map points in both directions, alongside a browsable Data Dictionary drawer - all in one shareable file ([#4283](https://github.com/dathere/qsv/issues/4283); [#4284](https://github.com/dathere/qsv/pull/4284), [#4306](https://github.com/dathere/qsv/pull/4306)).
- **`denull`: detect the null sentinels that silently corrupt typing.** Literal `NULL`/`N/A` text makes `stats` type a numeric column as String, quietly degrading `viz smart`, `schema` and `describegpt` downstream ([#4175](https://github.com/dathere/qsv/pull/4175)).
- **`fixedwidth`: convert fixed-width text to CSV**, with positions auto-detected from a header comment so `qsv table --align leftfwf` output round-trips ([#4168](https://github.com/dathere/qsv/pull/4168)).
- **`clean`: remove qsv-generated cache files**, with verify-before-delete safety and `--dry-run` as the default ([#3373](https://github.com/dathere/qsv/issues/3373); [#4015](https://github.com/dathere/qsv/pull/4015)).
- **Your Schematic and dictionary speak your data's language.** `describegpt` detects the dataset's content language locally with whatlang (zero tokens) and `viz smart` renders its entire UI, chart strings and coverage notes in it ([#4301](https://github.com/dathere/qsv/pull/4301), [#4310](https://github.com/dathere/qsv/pull/4310), [#4313](https://github.com/dathere/qsv/pull/4313)).
...
- `describegpt`: new `--format okf` output emits the Data Dictionary as an [Open Knowledge Format](https://github.com/GoogleCloudPlatform/knowledge-catalog/tree/main/okf) document - a leaner, vendor-neutral plain-markdown-plus-YAML-frontmatter sibling of SemanticMd (`type`/`title`/`description`/`resource`/`timestamp`/`tags` frontmatter + a `Column | Type | Description` Schema table). New `--okf-type` flag sets the required `type` key (default `"CSV Table"`); `--ds-source`/`--ds-updated` also map to OKF's `resource`/`timestamp` ([#4018](https://github.com/dathere/qsv/issues/4018)).
...
- `viz smart`: the footer qsv logo now stays legible under any `--theme` and after the in-browser light/dark toggle. The logo gains a theme-aware outline halo - a light halo under the dark theme so its navy shield reads against near-black paper (e.g. `plotly_dark`), and a faint dark halo on light themes - keyed on the same `body.qsv-dark` class the toggle drives ([#302](https://github.com/dathere/qsv/issues/302)).
- `viz`: `viz smart --dict-info` drawer and panel polish - Examples are annotated with their counts (`Dog Lifetime Spayed Female (16,350), …`), an `Other (1)` bucket is charted under the value's real name instead of an opaque label (the `(N)` counts *distinct* values rolled up, so 1 means nothing was aggregated), bar hovers carry the full category name rather than plotly's truncated `ticktext` echo, and a polar panel's title is no longer clipped by the panel edge. `describegpt`'s jsonschema output gains untruncated `examples_full` feeding `examples`/`x-qsv.example_counts`, while `--truncate-str` keeps bounding the LLM prompt ([#4262](https://github.com/dathere/qsv/pull/4262)).
- `viz`: the [visualizations gallery](https://dathere.github.io/qsv/gallery.html) gains a **Boston 311 (2025)** visual data dictionary (267,187 rows, 28 auto-chosen panels, a 25-neighborhood choropleth), and **NYC 311** moves from an embedded iframe to a clickable screenshot link-out beside it - dropping the page's heaviest iframe (3.6 MB) while keeping the full Data Schematic one click away. All three link-out previews are now WebP (2.0 MB PNG → 0.37 MB). `examples/viz/README.md` corrected alongside: three datasets and five committed dictionaries that the gallery charts were undocumented, the NYC example still called for an LLM it no longer needs, and `icicle`/`parcats`/`splom` had no example though the intro claims every chart type is covered. The Pittsburgh link-out's copy-pasteable command now reproduces its artifact too - it named an input that does not exist (`pitt311data.csv`; the page is built from `pittsburgh_311.tsv`) and an output that is not the linked file ([#4263](https://github.com/dathere/qsv/pull/4263), [#4264](https://github.com/dathere/qsv/pull/4264), [#4269](https://github.com/dathere/qsv/pull/4269)).

- **prebuilt binaries now ship `viz`** - and **`viz`/`viz_static` no longer build on big-endian targets, by design.** No release had ever shipped `viz`: it landed four days after 21.1.0 was tagged, and only `publish.yml`'s three x86_64 entries were ever updated to enable it. The portable, qsvpy and both macOS publish workflows still built without it - so the "p for portable" binaries the README recommends as the SIGILL workaround silently dropped `viz` (and, on Windows, `magika` and `color` too). `viz` is now enabled across the little-endian publish targets. Separately, `viz` and `viz_static` do not work on **big-endian** targets (s390x, ppc64 BE); that limitation was previously documented only on `viz_static` and phrased as a browser/headless concern rather than an endianness one, so `cargo build -F distrib_features` on s390x compiled clean and produced a **silently broken binary**. A `compile_error!` on the `viz` feature (which `viz_static` enables, so both tiers are covered) now fails the build loudly instead - if you build for a big-endian target, drop `viz`/`viz_static` from your feature set. Note powerpc64**LE** is little-endian, so the guard deliberately does not fire there; `viz` is simply not enabled for the ppc64le prebuilt, which has no viz CI coverage. CI coverage was widened to match: macOS now compiles the `viz_static` tier (as Linux already did), musl gains a base-`viz` step, and `aarch64-unknown-linux-gnu` - the only published target with no test workflow at all - gets one. Windows was widened to `viz_static` too, but reverted to base `viz`: the extra ~54 crates (`plotly_static`, `fantoccini`, `native-tls`, …) pushed the job from ~27min past its 90min timeout without ever reaching the tests. That now matches what Windows actually ships (see the `viz_static` note above), so the `viz_static` tier still has no Windows CI coverage ([#4350](https://github.com/dathere/qsv/pull/4350), [#4351](https://github.com/dathere/qsv/pull/4351)).
- **licensing & attribution**: qsv vendors and redistributes several open source components but shipped no NOTICE/THIRD_PARTY/CREDITS file and had no license-checking automation. MIT, BSD-3-Clause and CC-BY all require the copyright notice and license text to travel with copies - in several places they weren't. Those notices are now attributed and shipped ([#4297](https://github.com/dathere/qsv/pull/4297), [#4161](https://github.com/dathere/qsv/pull/4161)).
- `json`: the flatten-to-CSV step is now **inlined**, dropping the unmaintained `json-objects-to-csv` (last published 2022) and its transitive `flatten-json-object`. qsv needed a `preserve_key_order` method upstream lacks, so `Cargo.toml` pinned a git-branch patch to a personal fork - and because that's a git branch rather than a crates.io release, **`cargo install qsv` was fragile**, which is exactly what [#3523](https://github.com/dathere/qsv/issues/3523) reported. Both dependencies are gone ([#3523](https://github.com/dathere/qsv/issues/3523), [#4014](https://github.com/dathere/qsv/pull/4014)).
- `self-update`: background update checks are now **throttled to roughly twice a month** (~15-day interval) via a persisted last-check time, replacing a stateless 50% coin-flip that remembered nothing. Both background call sites route through the guard - the no-command screen (previously the coin-flip) and the `help` command (previously an **unconditional** check on every render) ([#4246](https://github.com/dathere/qsv/pull/4246)).
- `moarstats`: **`-C/--cardinality-threshold` now defaults to half the row count (floored at 1000) instead of a fixed 1,000,000.** The old default could never fire on any input smaller than a million rows, and on the 1M-row benchmark it sat *exactly at* the row count - so the guard was inert on the very file it mattered for. The gate also moved out of finalization and into the accumulation decision, so an excluded pair no longer builds a full joint-frequency table and then throws it away; on the 1M-row/41-column benchmark that is 345 of 780 pairs, worth a further 27% of peak RSS (11.1 → 8.05 GiB) and 27% of wall clock. **This is a user-visible output change:** `mutual_information`, `normalized_mutual_information` and both uncertainty coefficients are now reported empty for pairs where either field's cardinality exceeds the threshold. Mutual information between near-unique columns saturates at log(n) and is noise regardless of how efficiently it is computed. `n_pairs` and every correlation column are unaffected - the scan still counts rows for an excluded pair, it just skips building the map. Pass `-C` explicitly to restore any previous behavior ([#4356](https://github.com/dathere/qsv/issues/4356)).
...
### Performance
- `moarstats`: **`--bivariate`'s merge accumulator reserved row-count-sized value vectors for every field pair, including pairs that can never fill them.** `x_values`/`y_values` feed Spearman/Kendall and are only pushed when *both* sides of a pair parse numeric. The per-chunk allocator has always gated its reservation on that; the run-wide one did not, so with `--bivariate-stats all` it reserved `2 × rowcount × 8` bytes per pair unconditionally - on the 41-column benchmark, all 780 pairs where only ~36 can ever fill. Now gated to match. **Worth ~1 GiB (≈5%) of peak RSS on a 160-column, 100k-row input (21.88 → 20.87 GiB, measured A/B on the same build).** Note this is *not* where the `O(columns²)` blowup lives, despite the arithmetic (`12,720 pairs × 100k rows × 16 B ≈ 20.3 GB`) landing suspiciously near the reported peak: reserved-but-untouched pages are not resident, and the 41-column case settles it - the same formula predicts 11.6 GiB of reservation against a measured 8.05 GiB peak, which is impossible if reservations counted. The real cost is the joint-frequency maps, which is what `--bivariate-batch` bounds ([#4360](https://github.com/dathere/qsv/issues/4360)).
- `moarstats`: **`--bivariate-stats all` used 23 GiB on the 1M-row/41-column benchmark; it now uses ~8 GiB.** Enabling `mi`/`nmi`/`u` makes every field pair accumulate a joint-frequency map, and those were keyed by owned `String` pairs - ~200 bytes per joint cell across ~123M cells, and, because `entry()` takes its key by value, two `String`s allocated, hashed, compared and immediately dropped for *every pair of every row* (~1560 allocations per record). Each column now interns its distinct values into an `IndexSet<Box<[u8]>>` and the joint key is a single `u64` packing two `u32` symbols: ~19 bytes per cell and no allocation on the hot path. Columns are also decoded once per record rather than once per pair - each column took part in 40 of the 780 pairs, so every value was fetched, UTF-8-validated and parsed 40 times per row. Peak RSS 3.86 → 1.31 GiB at 50k rows, 11.1 → 3.01 GiB at 200k, ~27.7 → 8.05 GiB at 1M; wall clock 455s → 64s. **This unblocks PGO training**, which had to bound bivariate training to a 50k-row slice after the full run OOM-killed both Linux targets of this release's aborted first publish - 8 GiB now fits inside a GitHub hosted runner's 15.57 GiB ([#4356](https://github.com/dathere/qsv/issues/4356)).

### Fixed
- `frequency`: **duplicate-named columns are now classified by position, not name.** Per-column cardinality and all-unique detection were keyed by header *name*, so every column sharing a name inherited the FIRST duplicate's statistics - all marked identically `<ALL_UNIQUE>`, `<HIGH_CARDINALITY>` or normal, regardless of their real data. A user-facing correctness bug in normal output, not merely a cache artifact ([#4041](https://github.com/dathere/qsv/pull/4041)).
- `viz`: **the dictionary's per-column date format is now honored for DMY/MDY.** A `--dictionary` declares each column's own strftime format via `x-qsv.content_type` (e.g. `date:%m/%d/%Y`), but `route_from_content_type` split the base token off to pick a panel route and **threw the suffix away**; every date parse then fell back to the one process-wide `QSV_PREFER_DMY`. When the two disagreed qsv silently misread every date in the column - chart axis values *and* data-viewer sort keys - with no warning, and both readings look plausible ([#4303](https://github.com/dathere/qsv/issues/4303), [#4318](https://github.com/dathere/qsv/pull/4318)).
- `viz`: selecting a record in the data viewer could **freeze or kill the browser tab** on a large map Data Schematic. `applySelection` painted every selection with `Plotly.restyle(gd, {selectedpoints: sel}, …)`, and plotly declares `selectedpoints` as `editType: "calc"` - so each click recalculated the *entire* trace (points and hover strings) and rebuilt MapLibre's GeoJSON, leaking ~44 MB per click on a 265k-point map. Above a 50k-point threshold the row pin now carries the selection and the dimming restyle is skipped ([#4341](https://github.com/dathere/qsv/pull/4341)).
- `viz`: the data viewer's table **collapsed instead of scrolling in Safari**. Under `scrollX`, DataTables leaves the table at `width: 100%` and holds columns open by writing `min-width` onto each `<col>` - but CSS 2.1 §17.6.1 limits the properties that apply to column elements to border, background, width and visibility; `min-width` is not among them. Blink honors it as an extension, WebKit does not (a minimal probe renders 1206px in Chrome and 200px in Safari) ([#4325](https://github.com/dathere/qsv/pull/4325)).
- `get`: `--timeout` is now an **inactivity timeout** rather than a total-request one. reqwest's `.timeout()` aborted a slow-but-steady download mid-stream (e.g. a large CSV trickling in over a slow CKAN endpoint or proxy), surfacing as a cryptic "error decoding response body". The shared async client now uses `read_timeout()`, which resets on every received chunk, so only a genuinely stalled connection trips it; `connect_timeout` is retained ([#4123](https://github.com/dathere/qsv/pull/4123)).
- `get`/`dc:`: a `dc:` handle is now resolved **exactly once per run** instead of repeatedly ([#4257](https://github.com/dathere/qsv/issues/4257), [#4273](https://github.com/dathere/qsv/pull/4273)).
- `self-update`: added timeouts, retries and structured error messages ([#4182](https://github.com/dathere/qsv/pull/4182)).
- `describegpt`: a **single stray backslash in an LLM response no longer silently discards the whole dictionary**. Models occasionally emit a backslash in structural position (`"role":\ "measure",` - observed from `google/gemma-4-26b-a4b`), which is invalid JSON. `try_fix_json` had no `in_string` guard, so it passed the backslash through, the whole object failed to parse, and `extract_json_from_output` fell through to its brace scan - which returned the first **nested** object that happened to parse (one field's `{label, description, …}` body) *as if it were the entire dictionary*. Every real column lookup then missed, so a single-pass run produced a dictionary with **no LLM labels at all**, a `--two-pass` run silently discarded its baseline pass, and `grain`/`relationships` vanished - all with no error anywhere, after paying for the inference. A bare backslash outside a string is never valid JSON, so it is now dropped (escapes *inside* strings are preserved); and `parse_llm_dictionary_response` now warns when a response parses but matches not one of the real column names, so this class of failure can never be silent again ([#4321](https://github.com/dathere/qsv/issues/4321)).
- `viz`/`describegpt`: the dataset **`grain` phrase no longer leaks English into a localized Data Schematic**. `describegpt --language xx` emitted `grain` in English regardless (the `--language` directive in `dictionary_prompt` enumerated its scope as Labels/Descriptions/Content Types and never reached grain), and `viz smart` then parsed an entity noun out of that English sentence - splitting on the English literal `" one "` - and interpolated it into an otherwise fully-translated title template. A `--language pt` Data Schematic rendered `product collection record per location ao longo de Data de Coleta`, plus the same English noun in the axis title and the raw English sentence in the dictionary drawer's `Granularidade:` line. describegpt now writes `grain` in the target language AND emits a separate structured `x-qsv.grain_unit` - the bare entity noun - which viz uses verbatim, so no natural-language parsing is involved and languages with neither articles nor spaces (`ja`, `zh-CN`) work too. Dictionaries without `grain_unit` keep today's behavior exactly: viz falls back to the legacy grain parse, then to the localized `viz.chart.records` ([#4321](https://github.com/dathere/qsv/issues/4321)).
- `viz`: the Data Dictionary drawer's **Attribution block is now localized**. describegpt's attribution footer (`Generated by … describegpt` / `Command line:` / `Prompt file:` / `Model:` / `LLM API URL:` / `Language:` / `Timestamp:` / the LLM `WARNING:`) is authored when the dictionary is generated and stored verbatim in the schema as `x-qsv.generated_by`, which `viz` rendered byte-for-byte - so it stayed English at the foot of an otherwise fully translated drawer. The dictionary is written *once* while `viz --language` is chosen later and may differ, so it is translated at **render time** instead, on the same contract as the "Notable Characteristics" heading: the match is exact and line-oriented, so a hand-written dictionary, an older describegpt layout, or an uncurated `--language` passes through unchanged. The stored schema is untouched - it stays a stable machine-readable record, and the drawer's "Export JSONSchema" button still serves the original bytes. **Labels only**: the command line, prompt-file kind, model, API URL, language code and RFC3339 timestamp all ride through verbatim, so the footer remains a faithful, copy-pasteable record of how the dictionary was built (which is why a dictionary built with `describegpt --language Portuguese` renders `Idioma: Portuguese` - that string is a value, not a label). All five of describegpt's `WARNING:` variants are covered, not just the two a self-inferred dictionary can produce, since `--dictionary <file>` accepts any describegpt output ([#4345](https://github.com/dathere/qsv/pull/4345)).
- `test(viz)`: the browser-gated `viz_static` tests now actually verify what they claim. `viz_static_more_than_eight_panels` asserted that panels `cat09`–`cat12` appear in the exported image, but its fixture built all 12 columns from one shared modulus (`v{(i + c) % 4}`), making every pair a 1:1 bijection - so the twin-collapse from [#4221](https://github.com/dathere/qsv/issues/4221) folded them into a **single** panel and the assertion could never hold. Its sibling `viz_static_max_charts_caps_panels` passed *vacuously* for the same reason: with one panel eligible, `--max-charts 4` was never exercised and the test would have kept passing had the flag been deleted outright. The test was valid when written and was silently invalidated a month later - #4221's own fixture-migration pass updated six sibling tests but missed these two, because the whole family is **doubly dead**: `#[ignore]`d with no workflow passing `--ignored`, *and* `viz_static` absent from all 17 `cargo test` invocations, so not even `cargo check` type-checked them. Both fixtures now give each column its own modulus (12 distinct cardinalities), the vacuous `!stderr.contains("limited to")` assertion is replaced by one that pins the fixture's premise, and the `--max-charts` assertions are inverted to match actual behavior - the trim keeps the **highest-interest** panels, so `cat12` survives and `cat01` is dropped, the opposite of what was asserted. The Linux workflow now builds with `viz_static` so these tests are at least compiled, a new `rust-viz-static.yml` runs them against a matched Chrome/chromedriver pair, and a new non-ignored test pins the fixture premise on every PR without needing a browser ([#4343](https://github.com/dathere/qsv/issues/4343)).
- `viz`: `viz smart` no longer overflows Windows' 1 MB main-thread stack in **debug builds** (`STATUS_STACK_OVERFLOW`, which failed all 266 `test_viz` tests when `viz` was enabled in Windows CI; release builds - including the shipped Windows binary - were never affected). At `opt-level=0` rustc gives every local its own stack slot with no reuse across branches, and `smart_inline_panel_plot` built 14 `Plot`s and ~42 multi-KB plotly `Layout` temporaries in one 1,100-line frame, with the 18-arm `panel_trace` nested inside it - needing 3–4 MB of stack. Both are now split into one `#[inline(never)]` function per panel kind behind a thin dispatcher (so only one small frame is ever live), and `assign_typed_axis` takes `&mut Layout` instead of moving the `Layout` through a per-arm builder chain. A debug `viz smart` now runs in **under 512 KB** (heaviest gallery invocation: under 768 KB). The Linux CI workflow gains a 1 MB-rlimit smoke test so a stack regression fails fast on every PR, and the `viz` feature is re-enabled in the Windows and macOS test workflows ([#4328](https://github.com/dathere/qsv/issues/4328)).
- `viz`: 42 findings from a **whole-file review** of `src/cmd/viz.rs`. The headline correctness fixes: the `viz smart` **KPI row silently disappeared** on most real-world datasets (tiles were rejoined to their stats row by display title, which classification had already decorated - e.g. `amount (right-skewed)` - so the lookup missed and the whole row was dropped); the **AnimatedGeo panel blew past its point cap** by ~nb/2x, because cumulative frames re-serialize each point once per subsequent frame, turning a nominal 150k-point budget into tens of MB of HTML; `build_heatmap_pivot` allocated an **unbounded dense `y_cats x x_cats` matrix** (two 50k-cardinality key columns is ~20 GB) and is now capped like every other multi-category builder; and issue [#4219](https://github.com/dathere/qsv/issues/4219)'s log-axis clamp did not cover the raw-value panels, so a single `0` still broke the whole box/violin trace on a log axis. Also: CSV headers and cell values reaching plotly's pseudo-HTML renderer are now **escaped exactly once per sink** (identities - animation frame keys and the dictionary anchor - deliberately left raw, since escaping them would break slider/frame matching and click-to-scroll); a `--geojson` spec is loaded **once** per run instead of 2-3 times (a URL spec was multiple HTTP GETs, and a non-deterministic endpoint produced a genuine TOCTOU where points were binned against one fetch and drawn from another), and remote fetches now have a timeout and a bounded read; `is_intensive_measure` matches whole **tokens** rather than substrings (`ratio` is inside `duration`/`generation`/`operation`, which silently flipped additive amounts from Sum to Mean); `time.duration` columns are charted as measures instead of being dropped as temporal; the map's fitted zoom is centered in Mercator space so high-latitude extents actually fit; and a 100%-outside point/boundary mismatch is now reported instead of silently producing an empty map. Plus `pip_assign` spatial pruning, a `--slider` frame cap, and several per-row allocation hot paths. The plotly.js CDN tag now carries an **SRI hash** verified against the embedded bundle, and gallery regeneration is byte-stable ([#4247](https://github.com/dathere/qsv/pull/4247)).
- **stats cache**: a cache built with different parsing options is no longer reused. The cache is located by input path and validated by mtime, but was not keyed by `--no-headers`/`--delimiter` - so a cache describing a *different logical CSV* passed every freshness check. Reachable through `viz smart`, the one consumer that force-regenerates under non-default parsing: that force rewrote the shared sidecar, so the next default-parsing run read a headerless cache - field names became positional (`0`, `1`, …), the header row was counted as data, and panels keyed off column names (the KPI row) vanished. With a wrong `--delimiter` it was worse than wrong output: the cache held one fused column and the next plain run **failed outright** on a valid file until the sidecar was deleted by hand. `get_stats_records_readonly` already validated this; its sibling `get_stats_records` - used by `viz`, `schema`, `frequency`, `joinp`, `pivotp`, `sample`, `pragmastat`, `sortcheck`, `extsort` and `profile` - wrote both fields and checked neither, so the check is now shared between them. Also keeps the canonical metadata sidecar in sync with the JSONL it describes, which had desynced for symlinked inputs ([#4251](https://github.com/dathere/qsv/pull/4251)).
- `pragmastat`: fixed two intermittently flaky tests. `pragmastat_onesample_basic`/`pragmastat_twosample_basic` asserted that the randomized spread/disparity **bounds contain the sample point estimate** - not an invariant the algorithm provides. The bounds are a distribution-free interval for the TRUE parameter (order statistics of a random disjoint pairing), while `spread` is the Shamos estimator over all C(n,2) differences: two different estimators, so the interval need not straddle the point. Compounded by a time-seeded RNG that `--seed` does not reach. Replaced with invariants guaranteed by construction (`lower <= upper`, non-negative spread, finite), verified over 5,600 sampled runs. Test-only; no production behavior changed ([#4250](https://github.com/dathere/qsv/pull/4250)).
- `viz`: **plotly trace names are now escaped** - 19 sites the #4247/#4254 sweeps left behind. plotly renders its markup subset in the legend and hover box, so a trace name resolved from a raw CSV header (`panel.name`, the histogram/box/violin `y_label`) reached that renderer as live markup. Two carve-outs are deliberate and were verified rather than assumed: the dictionary anchor (`Annotation::name`, matched by a `qsvdict-` prefix in the embedded JS) and the animation frame keys stay raw as identities, and the `--series` bubble-animation legend was already escaped at read time - re-escaping it would have shipped `&amp;amp;` to the reader. Same spoofing class as #4247/#4254; script execution stays blocked by plotly’s tag whitelist. Note this also fixes a case with no untrusted input at all: a correlation panel’s own generated suffix `(<1% of rows omitted)` was escaped in the panel title but raw in the trace name, so the two disagreed ([#4331](https://github.com/dathere/qsv/issues/4331)).
- `viz`: **slider step labels and the current-value readout are now escaped** - the animation surface #4331 deferred. Verified in-browser rather than assumed: plotly's `drawLabel` and `drawCurrentValue` both pipe their text through `svgTextUtils.convertToTspans`, and the `data-notex` attr on those nodes only suppresses MathJax, not the HTML subset. An `<a href>` in a slider frame cell drew **two live anchors** - one in the step label, one in the current-value readout - so this was clickable link injection, not just the bold-text spoofing the issue estimated. Only the two RENDERED strings are escaped: the step `label`, and the `currentvalue.prefix` built from a raw CSV header (the site the issue's own inventory missed). The step `value`, the `animate` target and `Frame::name` stay byte-identical raw, preserving the #4247 identity decision - and `.value()` must keep being set EXPLICITLY, since `sliders/defaults.js` does `coerce('value', label)` and would otherwise inherit the escaped label and desynchronize the slider. `javascript:` URLs stay blocked by plotly's `sanitizeHref` protocol whitelist, so this is spoofing/phishing, not XSS ([#4333](https://github.com/dathere/qsv/issues/4333)).
- `viz`: **axis and colorbar titles are now escaped on the standalone chart paths** - 11 sites the #4247 escaping sweep left behind, each resolving its label from a raw CSV header. A header like `<a href="https://evil.example">Amount</a>` drew a **live clickable link** as the axis or colorbar title. Same spoofing class as the panel-title fix in #4247; script execution stays blocked by plotly's tag whitelist, so this is spoofing/phishing, not XSS ([#4254](https://github.com/dathere/qsv/pull/4254)).
- `viz`: **map cluster bubbles were unreadable and inert.** plotly builds the cluster count layer with an empty paint (so MapLibre's default black `text-color` applies, with no trace attribute to override it) and defaults `cluster.color` to the trace's *marker* color - an identity color doing a magnitude job. Measured contrast of the count on those defaults was 2.55:1 white-on-orange and 4.08:1 on plotly blue, both under the 4.5:1 floor for 12px text. Bubbles are now a single-hue **sequential ramp keyed to `point_count`**, per-basemap (the dark-basemap ramp is paler - a step dark enough to vanish into a dark map fails the mark-vs-surface floor), with the label ink picked per step by measured contrast (worst case 6.59:1 light / 7.86:1 dark) and a surface ring so a bubble reads over a busy patch of basemap. Counts also stopped disappearing: MapLibre drops symbols that collide with basemap labels and plotly set no overlap policy, so the number - the entire point of the bubble - silently vanished at some zooms. Clusters additionally gained the **hover** plotly never gave them (it filters clustered features out of the hover-bearing layer): a tooltip with the point count, and click-to-expand to the zoom where the cluster breaks apart ([#4266](https://github.com/dathere/qsv/pull/4266), [#4268](https://github.com/dathere/qsv/pull/4268)).
- `moarstats`: **`--bivariate` produced different output on identical input.** Chunk results were merged in *completion* order, and `merge_correlation_states` is Welford - not associative in floating point - while `x_values`/`y_values` are order-sensitive inputs to the Spearman/Kendall rankings. Three consecutive runs of the same binary over the same 1M-row file produced three different covariance values, and peak RSS swung 24.2–28.5 GiB run-to-run for the same reason. Results are now merged in ascending chunk order via a reorder buffer, matching what the outlier/KGA path already did. Note this makes a given invocation reproducible; it does not make different `--jobs` values agree, since folding N chunks pairwise is genuinely different arithmetic from one streaming pass ([#4356](https://github.com/dathere/qsv/issues/4356)).
...
### CI/Actions
- **PGO training silently truncated because it OOM-killed the runner.** `moarstats --bivariate-stats all` over the full 1M-row training file peaked at 23 GiB against a GitHub hosted runner's 15.57 GiB, killing the runner VM outright on both `aarch64-unknown-linux-gnu` and `x86_64-unknown-linux-gnu`. Because the runner agent itself died, the training harness never observed an exit code and simply stopped mid-list - Windows escaped it only by training with `--minimal`, and musl does not PGO. **This is what aborted the first attempt to publish this release.** Bivariate training is now bounded to a 50k-row slice, measured at 1.31 GiB; the slice is indexed on purpose, so the parallel chunked path that is actually worth profiling still gets trained. The `moarstats` work above independently brought the full file down to 8.05 GiB, so the slice is now headroom rather than survival ([#4357](https://github.com/dathere/qsv/issues/4357)).
- **the repo's `cargo-audit` suppressions were never being read.** `cargo-audit` only loads `.cargo/audit.toml`, but an `audit.toml` also sat at the repo root (renamed from `deny.toml`) where it is silently ignored - so its two quick-xml suppressions never took effect, the advisories kept firing daily, and each was auto-filed twice. The inert root file is deleted and its entries folded into `.cargo/audit.toml`, with a header note so it is not re-created. Also suppressed [RUSTSEC-2026-0235](https://rustsec.org/advisories/RUSTSEC-2026-0235) (out-of-bounds reads in `rkyv` 0.7.x archives containing `Rc`/`Arc`): that crate reaches `Cargo.lock` only through `rust_decimal`'s **optional** `rkyv` feature, which none of its three dependents enable, so `cargo tree -i rkyv@0.7.46 --all-features` is empty and the vulnerable code is never compiled into any qsv binary. The `rkyv` that *is* compiled (0.8.18) is already past the patched 0.8.17 ([#4346](https://github.com/dathere/qsv/issues/4346)).

### Packaging & Publishing (post-tag)

> These landed **after the `22.0.1` tag (`0dbd4c50`)** and ship in the 22.0.1 release assets. The initial publish run hit GitHub's 6h job cap, so targets were re-run individually as the fixes landed - a per-target asset refresh is still in flight, and the earliest-built assets do not yet reflect every item below.

- **`qsvmcp` no longer includes the `get` command, `dc:` cache-prefix inputs, or cloud (`s3://`/`gs://`/`az://`) sources.** Both `get` and `get_cloud` were dropped from the `qsvmcp` feature; removing only `get` would have been a no-op, since `get_cloud = ["get", ...]` re-enables it. The motivation is Windows build time - `get_cloud` turns on `object_store`'s `aws`/`azure`/`gcp` backends, the largest resolved-feature delta between `qsvmcp` and the prebuilt `qsv` (`object_store` is in *both* dependency graphs, so it never appears in a crate-count comparison). HTTP/HTTPS, `dathere://` and `ckan://` sources are unaffected, as is gz/zlib/zst/zip/sz auto-decompression, which comes from `feature_capable` rather than `get`. `qsvdp` and the `distrib_features` build keep full cloud support.
- **The "portable" binaries (`qsvp`, `qsvplite`, `qsvpdp`, `qsvpmcp`) are no longer published, and `publish-portable.yml` has been removed.** Those subvariants existed as a SIGILL fallback "compiled without any CPU features enabled" - but `-C target-cpu=native` was removed from the x86_64 publish targets back in `fd74f8f0c` precisely because it caused SIGILL on GitHub runners, so the regular prebuilts have been equally portable ever since. `target-cpu` now appears in exactly one place, `.cargo/config.toml`'s `[target.aarch64-apple-darwin]`, and the portable workflow built no macOS target - so for all five targets it did build, there was no CPU-feature difference to speak of. Reading the central directory of `qsv-22.0.1-x86_64-unknown-linux-gnu.zip` confirms it: `qsvplite` and `qsvpdp` matched `qsvlite`/`qsvdp` byte-for-byte in both compressed and uncompressed size, `qsvpmcp` matched `qsvmcp`'s size, and only `qsvp` differed from `qsv` - because `qsv` is PGO-optimized and `qsvp` was not, which is an optimization difference rather than a portability one. The duplication added 434MB of uncompressed payload to that archive - about 255MB of it byte-identical copies of binaries already inside it - which cost 145MB once compressed. Measured when the portable binaries were actually stripped from the published 22.0.1 asset: 4 entries totalling 433,942,992 uncompressed bytes came out, and the zip went from 479MB to 335MB.
- **Windows prebuilts are now built with thin LTO and 16 codegen units** (Linux/macOS keep fat LTO + `codegen-units = 1`). `lto = true` + `codegen-units = 1` on a polars-sized binary is the most expensive possible setting and the link phase is largely serial; Windows is the only platform where that cost has repeatedly blown GitHub's 6h job cap. This trades a modest amount of runtime performance on the Windows binaries for a build that completes. Applied via `--config` flags on the non-PGO builds and `pgo-lto: thin` for the PGO path. The Windows publish jobs also add Windows Defender exclusions for the build tree - best-effort, and skipped silently if Defender is policy-managed on the runner.
- **`qsvlite`, `qsvdp` and `qsvmcp` prebuilts are now built with the same cargo profile as `qsv`.** They previously hardcoded `--release` while `qsv` builds as `release-luau` on any target whose feature list includes `luau` (mlua needs `panic="unwind"`, [#3937](https://github.com/dathere/qsv/issues/3937)). Cargo caches per profile *directory*, so the two sets shared nothing: 679 of `qsvmcp`'s 689 crates are also in `qsv`'s dependency graph - including the entire polars stack, resolved to an identical 53 polars features in both - and every one of them was compiled twice per publish job. On Windows this made `qsvmcp` roughly 3× slower than `qsv` despite having *fewer* dependencies (689 vs 758); it was not doing more work, it was redoing `qsv`'s work from a cold cache. **Consequence:** on luau-enabled targets these three binaries now inherit `release-luau`'s `panic = "unwind"` instead of `release`'s `panic = "abort"` - marginally larger binaries, and panics unwind rather than abort the process.
- **Prebuilt binaries now ship the `synthesize` command, and `profile` where polars is enabled.** Both features were in `distrib_features` but had never been added to the publish workflows' hand-rolled per-target feature lists, so no released `qsv` binary has ever contained them - which also made `scripts/pgo-train.sh`'s `profile` and `synthesize` training steps silently no-op on every PGO target (they were 2 of 67 invocations, and the `t` helper tolerates a missing subcommand by design). `profile` requires `polars`, so it is added only to targets that already enable it - `x86_64-unknown-linux-gnu`, and `aarch64-apple-darwin` via the self-hosted macOS ARM64 workflows (`publish.yml`'s entry for that target is commented out, so those workflows build the real Apple Silicon prebuilt). `synthesize` (`fake` + `time`) is added to every published target **except the Windows ones**: `x86_64-pc-windows-msvc`, `x86_64-pc-windows-gnu` and `aarch64-pc-windows-msvc` carry neither `profile` nor `synthesize`, having been left out pending the 6h-cap publish retry and not revisited since. `profile` in particular is deliberately NOT added to `aarch64-unknown-linux-gnu`, `x86_64-unknown-linux-musl`, `s390x` or `powerpc64le` (all four of which *do* get `synthesize`), where pulling in polars would be required - the hosted ARM runner already OOMs on Polars fat-LTO, which is why `qsvdp`/`qsvmcp` are excluded there.

**Full Changelog**: https://github.com/dathere/qsv/compare/21.1.0...22.0.1

**Post-tag packaging**: https://github.com/dathere/qsv/compare/22.0.1...0464746e5
...
**[FAIR Data is AI-Ready Data](https://dathere.com/2025/08/the-case-for-a-fair-tech-stack-for-americas-ai-action-plan/)**. It is the perfect context for AI applications - as its compact, token-efficient and vastly improves an Agent's understanding of your Data. A few hundred kilobytes of FAIR metadata is often all it takes to comprehensively describe giga/terabyte level data.

This major release builds even further on qsv's existing [FAIRification](https://www.go-fair.org/fair-principles/fairification-process/) capabilities, with two new commands and expanded geocoding:

1.  **`profile`** - infer/extract/compute standards-compliant dataset FAIR metadata using configurable MiniJinja templates ([DCAT-US v3](https://resources.data.gov/resources/dcat-us3/), [DCAT-AP v3](https://semiceu.github.io/DCAT-AP/releases/3.0.0/), [Croissant 1.1](https://docs.mlcommons.org/croissant/docs/croissant-spec-1.1.html) and [Geoconnex](https://docs.geoconnex.us/reference/overview) [YAML profiles bundled](https://github.com/dathere/qsv/tree/master/resources/profiles));
2. **`get`** - fetches tabular data from HTTP(S), cloud object stores (AWS S3/Google Cloud/Microsoft Azure) and CKAN portals into a content-addressed local cache - making it even easier to FAIRify remote data and/or to use these remote data to enrich your data corpus while making for much faster, more robust Harvesting by eliminating the need to re-fetch remote data that hasn't changed ;
3. **`geocode` goes online with OpenCage.** Geospatially contextualize and normalize your location data with [OpenCage](https://opencagedata.com/). Unlike other geocoders - OpenCage is built from open data; has the most permissive licensing - allowing displaying data on ANY Map and indefinite caching; and is much cheaper to boot!
...
A new command that profiles a dataset and projects it into open metadata standards — **DCAT-US v3**, **DCAT-AP v3**, **Croissant 1.1**, and **Geoconnex** — via configurable YAML-driven MiniJinja-powered projection engine, with optional SHACL/`mlcroissant`/`pyshacl` validation and embedded descriptive statistics & frequency tables so you can further customize the metadata schema mappings ([#3898](https://github.com/dathere/qsv/pull/3898), [#3901](https://github.com/dathere/qsv/pull/3901), [#3908](https://github.com/dathere/qsv/pull/3908), [#3912](https://github.com/dathere/qsv/pull/3912), [#3916](https://github.com/dathere/qsv/pull/3916), [#3918](https://github.com/dathere/qsv/pull/3918)).
- **`get` - fetch tabular data from anywhere into a local cache.**
A new command (issue [#2263](https://github.com/dathere/qsv/issues/2263)) that retrieves CSV/TSV and other tabular data from HTTP(S) URLs, cloud object stores (`s3://`, `gs://`, `az://`), and CKAN portals (`ckan://`), then stores it in a content-addressed disk cache. Cached entries are addressable via a `dc:<name>` input prefix usable by any other qsv command, carry BLAKE3 + ETag provenance, support TTL/policy controls, and revalidate conditionally (HTTP `If-None-Match` / `304 Not Modified`). Cloud sources are gated behind the opt-in `get_cloud` sub-feature; streaming, ranged/parallel downloads and a `dc:` stats cache landed in Phase 3 ([#3953](https://github.com/dathere/qsv/pull/3953), [#3958](https://github.com/dathere/qsv/pull/3958)).
- **`geocode` with OpenCage support.**
New `geocode` subcommands call the OpenCage geocoding API for forward and reverse geocoding, with a persistent on-disk result cache and `%dyncols:` support (issue [#1295](https://github.com/dathere/qsv/issues/1295), [#3876](https://github.com/dathere/qsv/pull/3876), [#3878](https://github.com/dathere/qsv/pull/3878)).
- **`describegpt` describes meaning, not just types.**
A richer _[Semantic Markdown](https://semanticmd.org/)_ Data Dictionary format for optimized agents & data catalogs; a JSON Schema (draft 2020-12) output format; and LLM-inferred date/datetime content types round out describegpt's semantic-description capabilities ([#3933](https://github.com/dathere/qsv/pull/3933), [#3935](https://github.com/dathere/qsv/pull/3935), [#3871](https://github.com/dathere/qsv/pull/3871), [#3884](https://github.com/dathere/qsv/pull/3884)).
...
two new sampling modes plus a sketch-IO surface that lets users sample sharded inputs and combine the results without re-reading the whole corpus. Both modes are native Rust implementations written from the original algorithm papers. The Apache DataSketches project's [Sampling family](https://datasketches.apache.org/docs/Sampling/SamplingSketches.html) implements the same family of algorithms in C++/Java/Python — qsv does not bind to or depend on that code (the `datasketches` Rust crate doesn't expose Sampling-family sketches), so the on-disk format is qsv-specific and not interoperable with DataSketches serialized sketches.
  - **`--varopt <col>`**
  [variance-bounded weighted reservoir sampling using the A-ExpJ keying scheme of Efraimidis & Spirakis (2006)](https://doi.org/10.1016/j.ipl.2005.11.003). Each record gets a key `u^(1/w)` and the top-`k` keys are retained. Unlike `--weighted` (which is single-pass acceptance-rejection requiring a `max_weight` from the stats cache), `--varopt` is a true reservoir sampler — no stats cache required, single pass, bounded memory, and mergeable across partitions.
  - **`--mergeable-reservoir`**
  uniform reservoir using [Vitter's Algorithm R](https://dl.acm.org/doi/10.1145/3147.3165). Same statistical distribution as the default RESERVOIR method, but the resulting sampler state is mergeable.
...
- `profile`: new bundled `geoconnex` projection profile + `pyshacl` validator wired to the Internet of Water's [Geoconnex SHACL shapes](https://docs.geoconnex.us/reference/data-formats/shacl_shape) (vendored under `resources/geoconnex/shacl/`, embedded in the qsv binary). Phase 1 is dataset-level only — `DatasetShape` / `ProviderShape` / `PublisherShape` / `DistributionShape` coverage; the row-per-feature `LocationOrientedShape` (with mandatory `gsp:asWKT` geometry synthesis from lat/lon columns) is deferred to a follow-up. Gated behind a new `geoconnex` cargo feature — present in `qsv` (via `distrib_features`) and as an opt-in for `qsvdp` (`-F datapusher_plus,geoconnex`); not available in `qsvlite` / `qsvmcp`.
- **🆕 `get`**: new command for fetching tabular data from HTTP(S) URLs, cloud object stores (`s3://`/`gs://`/`az://`) and CKAN portals (`ckan://`) into a content-addressed local disk cache. Cached entries are reusable by any other qsv command via the `dc:<name>` input prefix, carry BLAKE3/ETag provenance plus record-count and TTL metadata, and revalidate conditionally over HTTP (`If-None-Match` → `304 Not Modified`). Subcommands include `cache-set-ttl`, `cache-set-policy` and `cache-list --verify`. Cloud sources are gated behind the opt-in `get_cloud` sub-feature (via `object_store`, no new transitive crates). Available in `qsv`/`qsvmcp`/`qsvdp` (not `qsvlite`). Issue [#2263](https://github.com/dathere/qsv/issues/2263) ([#3953](https://github.com/dathere/qsv/pull/3953), [#3958](https://github.com/dathere/qsv/pull/3958)).
- **🆕 `profile`**: new command for profiling a dataset and projecting it into open metadata standards — DCAT-US v3, DCAT-AP v3 and Croissant — through a YAML-driven projection engine, with optional external validation (`mlcroissant` for Croissant, `pyshacl` for DCAT-AP/Geoconnex SHACL shapes) and embedded descriptive statistics & frequency tables. Accepts local files, URL inputs and stdin. Available in `qsv`, `qsvmcp` and `qsvdp`; not in `qsvlite`. (The bundled `geoconnex` projection profile is the only part gated further — to `qsv`/`qsvdp` via the `geoconnex` feature.) ([#3898](https://github.com/dathere/qsv/pull/3898), [#3901](https://github.com/dathere/qsv/pull/3901), [#3904](https://github.com/dathere/qsv/pull/3904), [#3908](https://github.com/dathere/qsv/pull/3908), [#3910](https://github.com/dathere/qsv/pull/3910), [#3911](https://github.com/dathere/qsv/pull/3911), [#3912](https://github.com/dathere/qsv/pull/3912), [#3918](https://github.com/dathere/qsv/pull/3918)).
- `geocode`: new OpenCage online geocoding subcommands for forward and reverse geocoding via the OpenCage API, including `%dyncols:` support to materialize multiple result fields as new columns. Issue [#1295](https://github.com/dathere/qsv/issues/1295) ([#3876](https://github.com/dathere/qsv/pull/3876), [#3878](https://github.com/dathere/qsv/pull/3878)).
- `describegpt`: new `--format semanticmd` output — a richer Markdown Data Dictionary describing what each column *means* (not just its data type), designed for agents and data catalogs ([#3933](https://github.com/dathere/qsv/pull/3933), [#3935](https://github.com/dathere/qsv/pull/3935)).
- `describegpt`: new JSON Schema (draft 2020-12) output format ([#3871](https://github.com/dathere/qsv/pull/3871)).
- `describegpt`: date/datetime content-type tokens with LLM-inferred chrono format, and inferred date format rendered for `Min`/`Max` in the JSON/TOON/JSONSchema dictionaries ([#3884](https://github.com/dathere/qsv/pull/3884), [#3922](https://github.com/dathere/qsv/pull/3922)).
- `synthesize`: preserve inter-column relationships when generating synthetic data, so correlated columns stay correlated in the output ([#3888](https://github.com/dathere/qsv/pull/3888)).
- `stats`: new `--zero-padded-numeric` flag/column to opt into treating zero-padded numeric codes as a marked numeric type ([#3934](https://github.com/dathere/qsv/pull/3934), [#3938](https://github.com/dathere/qsv/pull/3938)).
- `sniff`: indexed distributed sampling for better type and date inference on large files ([#3926](https://github.com/dathere/qsv/pull/3926)).
- `excel`: expose `has_1904_epoch` in the workbook metadata output ([#3905](https://github.com/dathere/qsv/pull/3905)).
- `template`: new shared data-wrangling MiniJinja filters available across commands that use templates ([#3921](https://github.com/dathere/qsv/pull/3921)).
- `help-md`: render `NOTE`/`WARNING`/`IMPORTANT`/`TIP`/`CAUTION` blocks as GitHub Alerts in generated help markdown ([#3927](https://github.com/dathere/qsv/pull/3927)).

### Changed
- `deps`: migrate `cached` 1.1 → 2.0.1 (`cached_proc_macro` 1.1.0 → 2.0.0; see the upstream [migration guides](https://github.com/jaemk/cached/tree/master/docs/migrations)). Moved `fetch`/`fetchpost`/`geocode` to 2.0's builder-based store construction (`LruCache::builder().max_size(n).build()`) and to the new automatic `Result`/`Option` handling (the `result`/`option` macro attributes were removed and `size` renamed to `max_size`; 2.0 skips caching `Err`/`None` by default). On-disk cache format is unchanged (`DISK_FILE_VERSION` is still `1`), so existing `fetch`/`fetchpost`/`describegpt`/`geocode` caches remain valid. **Behavioral note:** under 2.0's defaults, `geocode` no longer caches *misses* — failed DNS resolutions and "country not found" lookups are retried per-record instead of being cached for the run (more correct, since transient DNS failures are no longer sticky).
- `perf(geocode)`: replaced the single-`RwLock<LruCache>` in-memory caches behind `geocode suggest`/`reverse` (the `search_index` result cache) and `iplookup` (the `cached_dns_lookup` hostname→IP cache) with **sharded** concurrent LRUs (cached 2.0's `#[concurrent_cached]` `ShardedLruCache`). The old caches took an exclusive lock on *every* read hit (LRU recency bump), serializing geocode's rayon-parallel pipeline; sharding spreads that across many independent locks (count auto-derived from `available_parallelism()`). **~2.2x faster** on a 16-core, high-cache-hit workload (5M rows: 3.17s → 1.41s; system time from lock contention dropped 16.5s → 1.1s), byte-identical output, and **no single-threaded regression** (`--jobs 1` is unchanged). Real-world gains scale with cache-hit ratio and core count.
- `build`: set `panic = "abort"` in the release profile, shrinking the all-features `qsv` binary **~18% (138.5 MB → 113.5 MB)**. Removes the stack-unwinding machinery (`__eh_frame`, `__gcc_except_tab`, `__unwind_info` ≈ 14.5 MB) plus the per-callsite landing-pad code (~10 MB of `__text`). Tests are unaffected (Cargo forces `panic = "unwind"` for the test/bench harness) and `human-panic` still reports before aborting. The `release-samply` profiling profile keeps `panic = "unwind"` for full backtraces. **Exception:** Luau-enabled builds (the main `qsv` binary) use the new `release-luau` profile (`panic = "unwind"`) because mlua requires unwinding — see Fixed below ([#3937](https://github.com/dathere/qsv/issues/3937)).
- `build`: dropped the `luau` feature from the `qsvmcp` binary and the `qsvdp` Debian variant — those binaries keep `panic = "abort"` for the size win, and Luau (which needs `panic = "unwind"`) is unnecessary in them. The full `qsv` binary still bundles Luau. [#3937](https://github.com/dathere/qsv/issues/3937)
- `perf(stats)`: resolve the `--infer-dates` "sniff" dates-whitelist **in-process** instead of forking a `qsv sniff` subprocess — eliminates a second 138 MB binary load, redundant re-sampling, and a JSON round-trip to obtain data the sniff code already has as a struct. Output is byte-identical. ~1.4–2.4x faster on cold `--infer-dates` runs, with the biggest wins on small and many-file workloads. [#3924](https://github.com/dathere/qsv/pull/3924)
- `perf(stats)`: reuse the sniff-resolved dates-whitelist on warm stats-cache hits instead of re-sniffing an unchanged file on every run. The cache sidecar now records the original "sniff" value as provenance (`flag_dates_whitelist_raw`) so it can safely reuse the previously-resolved date columns while preserving content-based cache sharing with `schema`/`profile`/`frequency`. Since "sniff" is the default `--dates-whitelist`, warm `--infer-dates` repeat runs are now ~4.4–6.6x faster (~10 ms regardless of file size). [#3924](https://github.com/dathere/qsv/pull/3924)
- `stats`: zero-padded decimal codes (e.g. ICD-9 `007.1`, Dewey Decimal `05.10`, Harmonized System tariff codes) are now inferred as **String** instead of **Float**, preserving their leading/padding zeros — mirroring how zero-padded integers (zip codes like `07306`) have always been kept as text. Previously they parsed as floats and silently lost their padding (`007.1` → `7.1`) when loaded/cast downstream (`schema`, `tojsonl`, etc.). This is the data-integrity default and is **not** gated behind `--zero-padded-numeric`, which remains an opt-in marker column. Ordinary fractions (`0.5`) and pure trailing-zero codes (`7.10`) are unaffected. The only added cost is a leading-zero byte-check after a successful float parse on the type-inference hot path (negligible — a couple of byte comparisons in the common case, zero cost on non-float columns). Follow-up to [#3938](https://github.com/dathere/qsv/pull/3938).
- `profile`: bumped the bundled `croissant` projection profile from Croissant 1.0 → [Croissant 1.1](https://github.com/mlcommons/croissant/blob/main/docs/croissant-spec-1.1.md). `conformsTo` now points at `.../croissant/1.1`; `@context` expanded to the canonical 1.1 prefix table (adds `sc:`, `rai:`, and shortcut terms for `recordSet`, `field`, `fileObject`, `source`, `extract`, `column`, `dataType`, `citeAs`, etc.); Distribution `@type` switched from `sc:FileObject` to `cr:FileObject`; FileObject and RecordSet now carry stable `@id` slugs so per-Field `source` blocks (`{fileObject: {@id}, extract: {column}}`) can cross-reference them. New 1.1 `citeAs` field populated from `pkg.citation`. **Behavioral note:** the file-hash slot switched from the non-canonical `cr:fileFingerprint`+`cr:Checksum` nested shape (BLAKE3) to the canonical direct `sha256` property `mlcroissant` validates against — slower on multi-GB inputs but spec-compliant. [#3916](https://github.com/dathere/qsv/pull/3916)

- `docs(alloc)`: evaluated two more jemalloc levers from [TUNING.md](https://github.com/jemalloc/jemalloc/blob/dev/TUNING.md) — `metadata_thp:auto` and `percpu_arena:percpu` — beyond the `background_thread` + dirty/muzzy-decay tuning shipped in [#3948](https://github.com/dathere/qsv/pull/3948). On a high-cardinality Linux stress workload neither was a consistent win (`metadata_thp` made `frequency` ~4% faster but `stats` ~3% slower at ~+5% peak RSS; `percpu_arena` showed no RSS-safe gain), so qsv does **not** enable them by default. Documented how to opt in via `_RJEM_MALLOC_CONF`, and added a manual `Bench jemalloc metadata_thp` GitHub Actions workflow (A/B/C wall-clock + peak-RSS on a synthetic high-cardinality dataset) to reproduce the measurement.
...
- `perf(frequency,schema)`: faster top-N and enum-list computation via `qsv-stats` 0.53 ([#3950](https://github.com/dathere/qsv/pull/3950)).
- `perf(polars)`: trimmed Polars features and gated AVX-512 to `x86_64`, shrinking the binary by ~4.5 MiB ([#3930](https://github.com/dathere/qsv/pull/3930)).
- `perf(alloc)`: jemalloc tuning (`background_thread` + dirty/muzzy decay) for parallel aggregation commands ([#3948](https://github.com/dathere/qsv/pull/3948)).
- `profile`: generalized the `--validate` / `--strict` / `--no-projection` flags into a unified projection/validation interface. Marked breaking (`feat(profile)!`), but affects only the new `profile` command, which did not exist in 20.1.0 ([#3915](https://github.com/dathere/qsv/pull/3915)).

### Fixed
- `luau`: error conditions in Luau scripts no longer abort the process in release builds. With `panic = "abort"`, a Luau callback error (e.g. `qsv_cumsum` overflow or an invalid `qsv_shellcmd`) — which mlua surfaces by unwinding across the Lua/C boundary — was turned into a hard process abort instead of the expected error message. Luau-enabled builds now use the `release-luau` profile (`panic = "unwind"`), so these errors are reported gracefully. [#3937](https://github.com/dathere/qsv/issues/3937)
- `describegpt`: honor the `QSV_LLM_BASE_URL` env var and an explicit `--base-url` even when its value matches the default; repaired stale describegpt tests ([#3889](https://github.com/dathere/qsv/pull/3889), [#3893](https://github.com/dathere/qsv/pull/3893)).
- `describegpt`: keep JSON Schema examples valid against the inferred property type ([#3885](https://github.com/dathere/qsv/pull/3885)).
- `geocode`: `index-load` now correctly reports that only the prebuilt `cities15000` index is supported ([#3883](https://github.com/dathere/qsv/pull/3883)).
- `moarstats`: hardened bivariate analysis against page-cache and temp-file races in joined-CSV processing, and fixed the clustered `-o` output guard ([#3873](https://github.com/dathere/qsv/pull/3873), [#3881](https://github.com/dathere/qsv/pull/3881), [#3882](https://github.com/dathere/qsv/pull/3882), [#3892](https://github.com/dathere/qsv/pull/3892), [#3894](https://github.com/dathere/qsv/pull/3894)).

**Full Changelog**: https://github.com/dathere/qsv/compare/20.1.0...21.0.0
...
- `describegpt`: add `--infer-content-type` flag — opt-in, has the LLM also infer a semantic "Content Type" for each Data Dictionary field, chosen from a curated vocabulary (e.g. `email`, `city`, `latitude`, `uuid`, `category`, `free_text`, `unknown`). Primitive types are intentionally excluded as they are already covered by the deterministic Type column. When the flag is absent, the Data Dictionary output is unchanged. Designed to later drive synthetic data generation [#3853](https://github.com/dathere/qsv/pull/3853)

### Changed
- `stats` / `frequency`: on OOM during the in-memory load check (`util::mem_file_check`), qsv now auto-enables the Apache DataSketches-backed estimators (t-digest + HyperLogLog for `stats`; Misra-Gries Frequent Items for `frequency`) in addition to the existing auto-index fallback, where flag conflicts allow. The load check runs whenever the command takes the non-parallel path — in NORMAL mode by default (file size vs. total memory − headroom), or in stricter CONSERVATIVE mode under `--memcheck` / `QSV_MEMORY_CHECK` — so the new fallback can trigger in either mode (just much more readily under `--memcheck`). The OOM error is only propagated when neither fallback engages. A `wwarn!` is emitted listing the auto-enabled estimators. The sketch fallback can also fire when an index is already present (e.g., with `--jobs 1` on a pre-indexed file) — a behavior change from the previous "error out" path in that narrow case. Explicit `--quantile-method exact` / `--cardinality-method exact` / `--sketch-method exact` still suppresses the auto-enable for that method.
