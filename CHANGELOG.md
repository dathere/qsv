...
- `perf(alloc)`: bake jemalloc `metadata_thp:auto` as the default `malloc_conf` to reduce TLB misses in qsv's hashmap-heavy commands (`stats`, `frequency`, `schema`, `moarstats`). Complements the existing `background_thread` and dirty/muzzy-decay levers. Effective on Linux (Transparent Huge Pages) for a small metadata-memory cost; a no-op on macOS/Windows. Since `opt.metadata_thp` is read-only at startup, it is not gated by `QSV_NO_ALLOC_TUNING` (which governs the run-time levers) — override it with `_RJEM_MALLOC_CONF=metadata_thp:disabled`. On Linux, `qsv --version` now reports `jemalloc+...+thp` when active.
