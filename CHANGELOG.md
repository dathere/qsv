...
### Changed
- **`viz`: `--no-tour` is replaced by `--tour-steps <n>`** ([#4389](https://github.com/dathere/qsv/issues/4389)). **This is a breaking CLI change** - `--no-tour` is gone, and `--tour-steps 0` is how a Data Schematic now ships without a guided tour. Scripts passing the old flag get a hard docopt error rather than a silently different page, which is the safer of the two failure modes. The tour's panel budget had been a hardcoded constant (raised 6 -> 8 in this same release) doing a job a flag should do, and `--no-tour` was a second flag for one end of that same knob; `<n>` budgets the per-panel **spotlights** only - the introduction, the Data Dictionary chapter, the data viewer chapter and the conclusion are always included, so a default tour is ~22 steps of which 8 are panels. A dictionary's `x-qsv.tour.panel_order` is capped by the same budget. The flag is deliberately typed as an *optional* value with **no docopt default**, which is what lets an explicit `--tour-steps 8` be told apart from an absent flag - the distinction the frequency-distribution guarantee below turns on, and one a docopt `[default: 8]` would have erased.
