...
- **`src/cmd/`** - Each command is a separate module (74 commands total)
...
**Polars Integration**:
- Commands with 🐻‍❄️ use Polars for vectorized query execution
- Currently: `count`, `joinp`, `pivotp`, `sqlp`
...
- Snappy compression (.sz extension) provides fast compression/decompression (2.58 GB/s compress, 0.89 GB/s decompress)
...
- qsv uses latest stable Rust (currently 1.91+)
...
- Polars pinned to specific commit/tag for stability
