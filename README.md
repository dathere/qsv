...
| [viz](docs/help/viz.md)✨<br>🪄📇🏎️👆🤖🌐🌎 | <a name="viz_deeplink"></a>Generate interactive charts & maps from CSV data using [plotly](https://plotly.com). `viz smart` creates a *"[neuro-symbolic](https://en.wikipedia.org/wiki/Neuro-symbolic_AI)"* dashboard, picking appropriate visualizations using the dataset's statistics, frequency distributions, data dictionary & optional LLM metadata inferencing, with automatic geocoding enrichment. Outputs self-contained, interactive HTML or static PNG/SVG/PDF/JPEG/WebP with the `viz_static` feature. ([Gallery](https://dathere.github.io/qsv/gallery.html)) |
