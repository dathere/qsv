...
* `qsvmcp` - optimized for [MCP (Model Context Protocol)](https://modelcontextprotocol.io/) server use with geocode, get, get_cloud, mcp, polars, profile, self_update, synthesize, to, and viz_static features enabled. Shares `src/main.rs` with `qsv`.
...
It has an extensive test suite with ~5,100 tests, including several [property tests](https://medium.com/criteo-engineering/introduction-to-property-based-testing-f5236229d237) which [randomly generate](https://github.com/BurntSushi/quickcheck#quickcheck) parameters for oft-used commands.   
...
## Testing
qsv has 5,126 tests: 4,400 for the qsv binaries (3,553 integration in the [tests](https://github.com/dathere/qsv/tree/master/tests) directory + 847 unit across `src/` modules), plus 726 for the MCP server & Cowork plugin (in [`.claude/skills/tests`](https://github.com/dathere/qsv/tree/master/.claude/skills/tests)).
