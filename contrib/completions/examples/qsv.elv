
use builtin;
use str;

set edit:completion:arg-completer[qsv] = {|@words|
    fn spaces {|n|
        builtin:repeat $n ' ' | str:join ''
    }
    fn cand {|text desc|
        edit:complex-candidate $text &display=$text' '(spaces (- 14 (wcswidth $text)))$desc
    }
    var command = 'qsv'
    for word $words[1..-1] {
        if (str:has-prefix $word '-') {
            break
        }
        set command = $command';'$word
    }
    var completions = [
        &'qsv'= {
            cand --list 'list'
            cand --envlist 'envlist'
            cand --update 'update'
            cand --updatenow 'updatenow'
            cand -V 'V'
            cand --version 'version'
            cand -h 'Print help'
            cand --help 'Print help'
            cand apply 'apply'
            cand behead 'behead'
            cand blake3 'blake3'
            cand cat 'cat'
            cand clean 'clean'
            cand clipboard 'clipboard'
            cand color 'color'
            cand count 'count'
            cand datefmt 'datefmt'
            cand dedup 'dedup'
            cand denull 'denull'
            cand describegpt 'describegpt'
            cand diff 'diff'
            cand edit 'edit'
            cand enum 'enum'
            cand excel 'excel'
            cand exclude 'exclude'
            cand explode 'explode'
            cand extdedup 'extdedup'
            cand extsort 'extsort'
            cand fetch 'fetch'
            cand fetchpost 'fetchpost'
            cand fill 'fill'
            cand fixedwidth 'fixedwidth'
            cand fixlengths 'fixlengths'
            cand flatten 'flatten'
            cand fmt 'fmt'
            cand foreach 'foreach'
            cand frequency 'frequency'
            cand geocode 'geocode'
            cand geoconvert 'geoconvert'
            cand get 'get'
            cand headers 'headers'
            cand implode 'implode'
            cand index 'index'
            cand input 'input'
            cand join 'join'
            cand joinp 'joinp'
            cand json 'json'
            cand jsonl 'jsonl'
            cand lens 'lens'
            cand log 'log'
            cand luau 'luau'
            cand moarstats 'moarstats'
            cand partition 'partition'
            cand pivotp 'pivotp'
            cand pragmastat 'pragmastat'
            cand pro 'pro'
            cand profile 'profile'
            cand prompt 'prompt'
            cand pseudo 'pseudo'
            cand py 'py'
            cand readstat 'readstat'
            cand rename 'rename'
            cand replace 'replace'
            cand reverse 'reverse'
            cand safenames 'safenames'
            cand sample 'sample'
            cand schema 'schema'
            cand scoresql 'scoresql'
            cand search 'search'
            cand searchset 'searchset'
            cand select 'select'
            cand slice 'slice'
            cand snappy 'snappy'
            cand sniff 'sniff'
            cand sort 'sort'
            cand sortcheck 'sortcheck'
            cand split 'split'
            cand sqlp 'sqlp'
            cand stats 'stats'
            cand synthesize 'synthesize'
            cand table 'table'
            cand template 'template'
            cand to 'to'
            cand tojsonl 'tojsonl'
            cand transpose 'transpose'
            cand validate 'validate'
            cand viz 'viz'
            cand help 'Print this message or the help of the given subcommand(s)'
        }
        &'qsv;apply'= {
            cand --addl-props 'addl-props'
            cand -k 'k'
            cand --api-key 'api-key'
            cand -u 'u'
            cand --base-url 'base-url'
            cand -b 'b'
            cand --batch 'batch'
            cand --cache-dir 'cache-dir'
            cand -C 'C'
            cand --comparand 'comparand'
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand -f 'f'
            cand --formatstr 'formatstr'
            cand -j 'j'
            cand --jobs 'jobs'
            cand -t 't'
            cand --max-tokens 'max-tokens'
            cand -m 'm'
            cand --model 'model'
            cand -c 'c'
            cand --new-column 'new-column'
            cand --on-error 'on-error'
            cand -o 'o'
            cand --output 'output'
            cand --prompt 'prompt'
            cand --prompt-file 'prompt-file'
            cand --rate-limit 'rate-limit'
            cand -r 'r'
            cand --rename 'rename'
            cand -R 'R'
            cand --replacement 'replacement'
            cand --timeout 'timeout'
            cand --user-agent 'user-agent'
            cand --fresh 'fresh'
            cand --no-cache 'no-cache'
            cand -n 'n'
            cand --no-headers 'no-headers'
            cand -p 'p'
            cand --progressbar 'progressbar'
            cand --stats 'stats'
            cand -h 'Print help'
            cand --help 'Print help'
            cand calcconv 'calcconv'
            cand dynfmt 'dynfmt'
            cand emptyreplace 'emptyreplace'
            cand operations 'operations'
            cand summarize 'summarize'
            cand help 'Print this message or the help of the given subcommand(s)'
        }
        &'qsv;apply;calcconv'= {
            cand --addl-props 'addl-props'
            cand -k 'k'
            cand --api-key 'api-key'
            cand -u 'u'
            cand --base-url 'base-url'
            cand -b 'b'
            cand --batch 'batch'
            cand --cache-dir 'cache-dir'
            cand -C 'C'
            cand --comparand 'comparand'
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand -f 'f'
            cand --formatstr 'formatstr'
            cand -j 'j'
            cand --jobs 'jobs'
            cand -t 't'
            cand --max-tokens 'max-tokens'
            cand -m 'm'
            cand --model 'model'
            cand -c 'c'
            cand --new-column 'new-column'
            cand --on-error 'on-error'
            cand -o 'o'
            cand --output 'output'
            cand --prompt 'prompt'
            cand --prompt-file 'prompt-file'
            cand --rate-limit 'rate-limit'
            cand -r 'r'
            cand --rename 'rename'
            cand -R 'R'
            cand --replacement 'replacement'
            cand --timeout 'timeout'
            cand --user-agent 'user-agent'
            cand --fresh 'fresh'
            cand --no-cache 'no-cache'
            cand -n 'n'
            cand --no-headers 'no-headers'
            cand -p 'p'
            cand --progressbar 'progressbar'
            cand --stats 'stats'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;apply;dynfmt'= {
            cand --addl-props 'addl-props'
            cand -k 'k'
            cand --api-key 'api-key'
            cand -u 'u'
            cand --base-url 'base-url'
            cand -b 'b'
            cand --batch 'batch'
            cand --cache-dir 'cache-dir'
            cand -C 'C'
            cand --comparand 'comparand'
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand -f 'f'
            cand --formatstr 'formatstr'
            cand -j 'j'
            cand --jobs 'jobs'
            cand -t 't'
            cand --max-tokens 'max-tokens'
            cand -m 'm'
            cand --model 'model'
            cand -c 'c'
            cand --new-column 'new-column'
            cand --on-error 'on-error'
            cand -o 'o'
            cand --output 'output'
            cand --prompt 'prompt'
            cand --prompt-file 'prompt-file'
            cand --rate-limit 'rate-limit'
            cand -r 'r'
            cand --rename 'rename'
            cand -R 'R'
            cand --replacement 'replacement'
            cand --timeout 'timeout'
            cand --user-agent 'user-agent'
            cand --fresh 'fresh'
            cand --no-cache 'no-cache'
            cand -n 'n'
            cand --no-headers 'no-headers'
            cand -p 'p'
            cand --progressbar 'progressbar'
            cand --stats 'stats'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;apply;emptyreplace'= {
            cand --addl-props 'addl-props'
            cand -k 'k'
            cand --api-key 'api-key'
            cand -u 'u'
            cand --base-url 'base-url'
            cand -b 'b'
            cand --batch 'batch'
            cand --cache-dir 'cache-dir'
            cand -C 'C'
            cand --comparand 'comparand'
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand -f 'f'
            cand --formatstr 'formatstr'
            cand -j 'j'
            cand --jobs 'jobs'
            cand -t 't'
            cand --max-tokens 'max-tokens'
            cand -m 'm'
            cand --model 'model'
            cand -c 'c'
            cand --new-column 'new-column'
            cand --on-error 'on-error'
            cand -o 'o'
            cand --output 'output'
            cand --prompt 'prompt'
            cand --prompt-file 'prompt-file'
            cand --rate-limit 'rate-limit'
            cand -r 'r'
            cand --rename 'rename'
            cand -R 'R'
            cand --replacement 'replacement'
            cand --timeout 'timeout'
            cand --user-agent 'user-agent'
            cand --fresh 'fresh'
            cand --no-cache 'no-cache'
            cand -n 'n'
            cand --no-headers 'no-headers'
            cand -p 'p'
            cand --progressbar 'progressbar'
            cand --stats 'stats'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;apply;operations'= {
            cand --addl-props 'addl-props'
            cand -k 'k'
            cand --api-key 'api-key'
            cand -u 'u'
            cand --base-url 'base-url'
            cand -b 'b'
            cand --batch 'batch'
            cand --cache-dir 'cache-dir'
            cand -C 'C'
            cand --comparand 'comparand'
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand -f 'f'
            cand --formatstr 'formatstr'
            cand -j 'j'
            cand --jobs 'jobs'
            cand -t 't'
            cand --max-tokens 'max-tokens'
            cand -m 'm'
            cand --model 'model'
            cand -c 'c'
            cand --new-column 'new-column'
            cand --on-error 'on-error'
            cand -o 'o'
            cand --output 'output'
            cand --prompt 'prompt'
            cand --prompt-file 'prompt-file'
            cand --rate-limit 'rate-limit'
            cand -r 'r'
            cand --rename 'rename'
            cand -R 'R'
            cand --replacement 'replacement'
            cand --timeout 'timeout'
            cand --user-agent 'user-agent'
            cand --fresh 'fresh'
            cand --no-cache 'no-cache'
            cand -n 'n'
            cand --no-headers 'no-headers'
            cand -p 'p'
            cand --progressbar 'progressbar'
            cand --stats 'stats'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;apply;summarize'= {
            cand --addl-props 'addl-props'
            cand -k 'k'
            cand --api-key 'api-key'
            cand -u 'u'
            cand --base-url 'base-url'
            cand -b 'b'
            cand --batch 'batch'
            cand --cache-dir 'cache-dir'
            cand -C 'C'
            cand --comparand 'comparand'
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand -f 'f'
            cand --formatstr 'formatstr'
            cand -j 'j'
            cand --jobs 'jobs'
            cand -t 't'
            cand --max-tokens 'max-tokens'
            cand -m 'm'
            cand --model 'model'
            cand -c 'c'
            cand --new-column 'new-column'
            cand --on-error 'on-error'
            cand -o 'o'
            cand --output 'output'
            cand --prompt 'prompt'
            cand --prompt-file 'prompt-file'
            cand --rate-limit 'rate-limit'
            cand -r 'r'
            cand --rename 'rename'
            cand -R 'R'
            cand --replacement 'replacement'
            cand --timeout 'timeout'
            cand --user-agent 'user-agent'
            cand --fresh 'fresh'
            cand --no-cache 'no-cache'
            cand -n 'n'
            cand --no-headers 'no-headers'
            cand -p 'p'
            cand --progressbar 'progressbar'
            cand --stats 'stats'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;apply;help'= {
            cand calcconv 'calcconv'
            cand dynfmt 'dynfmt'
            cand emptyreplace 'emptyreplace'
            cand operations 'operations'
            cand summarize 'summarize'
            cand help 'Print this message or the help of the given subcommand(s)'
        }
        &'qsv;apply;help;calcconv'= {
        }
        &'qsv;apply;help;dynfmt'= {
        }
        &'qsv;apply;help;emptyreplace'= {
        }
        &'qsv;apply;help;operations'= {
        }
        &'qsv;apply;help;summarize'= {
        }
        &'qsv;apply;help;help'= {
        }
        &'qsv;behead'= {
            cand -o 'o'
            cand --output 'output'
            cand -f 'f'
            cand --flexible 'flexible'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;blake3'= {
            cand --derive-key 'derive-key'
            cand -j 'j'
            cand --jobs 'jobs'
            cand -l 'l'
            cand --length 'length'
            cand -o 'o'
            cand --output 'output'
            cand -c 'c'
            cand --check 'check'
            cand --keyed 'keyed'
            cand --no-mmap 'no-mmap'
            cand --no-names 'no-names'
            cand -q 'q'
            cand --quiet 'quiet'
            cand --raw 'raw'
            cand --tag 'tag'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;cat'= {
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand -g 'g'
            cand --group 'group'
            cand -N 'N'
            cand --group-name 'group-name'
            cand -o 'o'
            cand --output 'output'
            cand --flexible 'flexible'
            cand -n 'n'
            cand --no-headers 'no-headers'
            cand -p 'p'
            cand --pad 'pad'
            cand -h 'Print help'
            cand --help 'Print help'
            cand columns 'columns'
            cand rows 'rows'
            cand rowskey 'rowskey'
            cand help 'Print this message or the help of the given subcommand(s)'
        }
        &'qsv;cat;columns'= {
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand -g 'g'
            cand --group 'group'
            cand -N 'N'
            cand --group-name 'group-name'
            cand -o 'o'
            cand --output 'output'
            cand --flexible 'flexible'
            cand -n 'n'
            cand --no-headers 'no-headers'
            cand -p 'p'
            cand --pad 'pad'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;cat;rows'= {
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand -g 'g'
            cand --group 'group'
            cand -N 'N'
            cand --group-name 'group-name'
            cand -o 'o'
            cand --output 'output'
            cand --flexible 'flexible'
            cand -n 'n'
            cand --no-headers 'no-headers'
            cand -p 'p'
            cand --pad 'pad'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;cat;rowskey'= {
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand -g 'g'
            cand --group 'group'
            cand -N 'N'
            cand --group-name 'group-name'
            cand -o 'o'
            cand --output 'output'
            cand --flexible 'flexible'
            cand -n 'n'
            cand --no-headers 'no-headers'
            cand -p 'p'
            cand --pad 'pad'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;cat;help'= {
            cand columns 'columns'
            cand rows 'rows'
            cand rowskey 'rowskey'
            cand help 'Print this message or the help of the given subcommand(s)'
        }
        &'qsv;cat;help;columns'= {
        }
        &'qsv;cat;help;rows'= {
        }
        &'qsv;cat;help;rowskey'= {
        }
        &'qsv;cat;help;help'= {
        }
        &'qsv;clean'= {
            cand --all 'all'
            cand -n 'n'
            cand --dry-run 'dry-run'
            cand -f 'f'
            cand --force 'force'
            cand --frequency 'frequency'
            cand --index 'index'
            cand --moarstats 'moarstats'
            cand -q 'q'
            cand --quiet 'quiet'
            cand -r 'r'
            cand --recursive 'recursive'
            cand --schema 'schema'
            cand --stale 'stale'
            cand --stats 'stats'
            cand --validate 'validate'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;clipboard'= {
            cand -s 's'
            cand --save 'save'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;color'= {
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand -o 'o'
            cand --output 'output'
            cand -t 't'
            cand --title 'title'
            cand -C 'C'
            cand --color 'color'
            cand --memcheck 'memcheck'
            cand -n 'n'
            cand --row-numbers 'row-numbers'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;count'= {
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand -f 'f'
            cand --flexible 'flexible'
            cand -H 'H'
            cand --human-readable 'human-readable'
            cand --json 'json'
            cand --low-memory 'low-memory'
            cand -n 'n'
            cand --no-headers 'no-headers'
            cand --no-polars 'no-polars'
            cand --width 'width'
            cand --width-no-delims 'width-no-delims'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;datefmt'= {
            cand -b 'b'
            cand --batch 'batch'
            cand --default-tz 'default-tz'
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand --formatstr 'formatstr'
            cand --input-tz 'input-tz'
            cand -j 'j'
            cand --jobs 'jobs'
            cand -c 'c'
            cand --new-column 'new-column'
            cand -o 'o'
            cand --output 'output'
            cand --output-tz 'output-tz'
            cand -r 'r'
            cand --rename 'rename'
            cand -R 'R'
            cand --ts-resolution 'ts-resolution'
            cand --keep-zero-time 'keep-zero-time'
            cand -n 'n'
            cand --no-headers 'no-headers'
            cand --prefer-dmy 'prefer-dmy'
            cand -p 'p'
            cand --progressbar 'progressbar'
            cand --utc 'utc'
            cand --zulu 'zulu'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;dedup'= {
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand -D 'D'
            cand --dupes-output 'dupes-output'
            cand -j 'j'
            cand --jobs 'jobs'
            cand -o 'o'
            cand --output 'output'
            cand -s 's'
            cand --select 'select'
            cand -H 'H'
            cand --human-readable 'human-readable'
            cand -i 'i'
            cand --ignore-case 'ignore-case'
            cand --memcheck 'memcheck'
            cand -n 'n'
            cand --no-headers 'no-headers'
            cand -N 'N'
            cand --numeric 'numeric'
            cand -q 'q'
            cand --quiet 'quiet'
            cand --sorted 'sorted'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;denull'= {
            cand --add-vocab 'add-vocab'
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand --max-distinct 'max-distinct'
            cand -o 'o'
            cand --output 'output'
            cand -s 's'
            cand --select 'select'
            cand --vocab 'vocab'
            cand --all-columns 'all-columns'
            cand --apply 'apply'
            cand --json 'json'
            cand -n 'n'
            cand --no-headers 'no-headers'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;describegpt'= {
            cand --addl-cols-list 'addl-cols-list'
            cand --addl-props 'addl-props'
            cand -k 'k'
            cand --api-key 'api-key'
            cand -u 'u'
            cand --base-url 'base-url'
            cand --cache-dir 'cache-dir'
            cand --ckan-api 'ckan-api'
            cand --ckan-token 'ckan-token'
            cand --context-file 'context-file'
            cand --disk-cache-dir 'disk-cache-dir'
            cand --ds-license 'ds-license'
            cand --ds-source 'ds-source'
            cand --ds-updated 'ds-updated'
            cand --enum-threshold 'enum-threshold'
            cand --export-prompt 'export-prompt'
            cand --format 'format'
            cand --freq-options 'freq-options'
            cand --language 'language'
            cand --markdown-template 'markdown-template'
            cand -t 't'
            cand --max-tokens 'max-tokens'
            cand -m 'm'
            cand --model 'model'
            cand --num-examples 'num-examples'
            cand --num-tags 'num-tags'
            cand --okf-type 'okf-type'
            cand -o 'o'
            cand --output 'output'
            cand -p 'p'
            cand --prompt 'prompt'
            cand --prompt-file 'prompt-file'
            cand --sample-size 'sample-size'
            cand --score-max-retries 'score-max-retries'
            cand --score-threshold 'score-threshold'
            cand --session 'session'
            cand --session-len 'session-len'
            cand --sql-results 'sql-results'
            cand --stats-options 'stats-options'
            cand --tag-vocab 'tag-vocab'
            cand --timeout 'timeout'
            cand --tour-audience 'tour-audience'
            cand --truncate-str 'truncate-str'
            cand --user-agent 'user-agent'
            cand --addl-cols 'addl-cols'
            cand -A 'A'
            cand --all 'all'
            cand --allow-extra-cols 'allow-extra-cols'
            cand --description 'description'
            cand --dictionary 'dictionary'
            cand --fewshot-examples 'fewshot-examples'
            cand --flush-cache 'flush-cache'
            cand --forget 'forget'
            cand --fresh 'fresh'
            cand --infer-content-type 'infer-content-type'
            cand --infer-null-values 'infer-null-values'
            cand --no-cache 'no-cache'
            cand --no-score-sql 'no-score-sql'
            cand --prepare-context 'prepare-context'
            cand --process-response 'process-response'
            cand -q 'q'
            cand --quiet 'quiet'
            cand --redis-cache 'redis-cache'
            cand --strict-dates 'strict-dates'
            cand --tags 'tags'
            cand --two-pass 'two-pass'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;diff'= {
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand --delimiter-left 'delimiter-left'
            cand --delimiter-output 'delimiter-output'
            cand --delimiter-right 'delimiter-right'
            cand -j 'j'
            cand --jobs 'jobs'
            cand -k 'k'
            cand --key 'key'
            cand -o 'o'
            cand --output 'output'
            cand --sort-columns 'sort-columns'
            cand --drop-equal-columns 'drop-equal-columns'
            cand --drop-equal-fields 'drop-equal-fields'
            cand --no-headers-left 'no-headers-left'
            cand --no-headers-output 'no-headers-output'
            cand --no-headers-right 'no-headers-right'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;edit'= {
            cand -o 'o'
            cand --output 'output'
            cand -i 'i'
            cand --in-place 'in-place'
            cand -n 'n'
            cand --no-headers 'no-headers'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;enum'= {
            cand --constant 'constant'
            cand --copy 'copy'
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand --hash 'hash'
            cand --increment 'increment'
            cand -c 'c'
            cand --new-column 'new-column'
            cand -o 'o'
            cand --output 'output'
            cand --start 'start'
            cand -n 'n'
            cand --no-headers 'no-headers'
            cand --uuid4 'uuid4'
            cand --uuid7 'uuid7'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;excel'= {
            cand --cell 'cell'
            cand --date-format 'date-format'
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand --error-format 'error-format'
            cand --header-row 'header-row'
            cand -j 'j'
            cand --jobs 'jobs'
            cand --metadata 'metadata'
            cand -o 'o'
            cand --output 'output'
            cand --range 'range'
            cand -s 's'
            cand --sheet 'sheet'
            cand --table 'table'
            cand --flexible 'flexible'
            cand --keep-zero-time 'keep-zero-time'
            cand -q 'q'
            cand --quiet 'quiet'
            cand --trim 'trim'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;exclude'= {
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand -o 'o'
            cand --output 'output'
            cand -i 'i'
            cand --ignore-case 'ignore-case'
            cand -v 'v'
            cand --invert 'invert'
            cand --memcheck 'memcheck'
            cand -n 'n'
            cand --no-headers 'no-headers'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;explode'= {
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand -o 'o'
            cand --output 'output'
            cand -r 'r'
            cand --rename 'rename'
            cand -n 'n'
            cand --no-headers 'no-headers'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;extdedup'= {
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand -D 'D'
            cand --dupes-output 'dupes-output'
            cand --memory-limit 'memory-limit'
            cand -s 's'
            cand --select 'select'
            cand --temp-dir 'temp-dir'
            cand -H 'H'
            cand --human-readable 'human-readable'
            cand -n 'n'
            cand --no-headers 'no-headers'
            cand --no-output 'no-output'
            cand -q 'q'
            cand --quiet 'quiet'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;extsort'= {
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand -j 'j'
            cand --jobs 'jobs'
            cand --memory-limit 'memory-limit'
            cand -s 's'
            cand --select 'select'
            cand --tmp-dir 'tmp-dir'
            cand -n 'n'
            cand --no-headers 'no-headers'
            cand -R 'R'
            cand --reverse 'reverse'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;fetch'= {
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand --disk-cache-dir 'disk-cache-dir'
            cand -H 'H'
            cand --http-header 'http-header'
            cand --jaq 'jaq'
            cand --jaqfile 'jaqfile'
            cand --max-errors 'max-errors'
            cand --max-retries 'max-retries'
            cand --mem-cache-size 'mem-cache-size'
            cand -c 'c'
            cand --new-column 'new-column'
            cand -o 'o'
            cand --output 'output'
            cand --rate-limit 'rate-limit'
            cand --report 'report'
            cand --timeout 'timeout'
            cand --url-template 'url-template'
            cand --user-agent 'user-agent'
            cand --cache-error 'cache-error'
            cand --cookies 'cookies'
            cand --disk-cache 'disk-cache'
            cand --flush-cache 'flush-cache'
            cand --no-cache 'no-cache'
            cand -n 'n'
            cand --no-headers 'no-headers'
            cand --pretty 'pretty'
            cand -p 'p'
            cand --progressbar 'progressbar'
            cand --redis-cache 'redis-cache'
            cand --store-error 'store-error'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;fetchpost'= {
            cand --content-type 'content-type'
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand --disk-cache-dir 'disk-cache-dir'
            cand -j 'j'
            cand --globals-json 'globals-json'
            cand -H 'H'
            cand --http-header 'http-header'
            cand --jaq 'jaq'
            cand --jaqfile 'jaqfile'
            cand --max-errors 'max-errors'
            cand --max-retries 'max-retries'
            cand --mem-cache-size 'mem-cache-size'
            cand -c 'c'
            cand --new-column 'new-column'
            cand -o 'o'
            cand --output 'output'
            cand -t 't'
            cand --payload-tpl 'payload-tpl'
            cand --rate-limit 'rate-limit'
            cand --report 'report'
            cand --timeout 'timeout'
            cand --user-agent 'user-agent'
            cand --cache-error 'cache-error'
            cand --compress 'compress'
            cand --cookies 'cookies'
            cand --disk-cache 'disk-cache'
            cand --flush-cache 'flush-cache'
            cand --no-cache 'no-cache'
            cand -n 'n'
            cand --no-headers 'no-headers'
            cand --pretty 'pretty'
            cand -p 'p'
            cand --progressbar 'progressbar'
            cand --redis-cache 'redis-cache'
            cand --store-error 'store-error'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;fill'= {
            cand -v 'v'
            cand --default 'default'
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand -g 'g'
            cand --groupby 'groupby'
            cand -o 'o'
            cand --output 'output'
            cand -b 'b'
            cand --backfill 'backfill'
            cand -f 'f'
            cand --first 'first'
            cand -n 'n'
            cand --no-headers 'no-headers'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;fixedwidth'= {
            cand -o 'o'
            cand --output 'output'
            cand --positions 'positions'
            cand --widths 'widths'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;fixlengths'= {
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand --escape 'escape'
            cand -i 'i'
            cand --insert 'insert'
            cand -l 'l'
            cand --length 'length'
            cand -o 'o'
            cand --output 'output'
            cand --quote 'quote'
            cand -q 'q'
            cand --quiet 'quiet'
            cand -r 'r'
            cand --remove-empty 'remove-empty'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;flatten'= {
            cand -c 'c'
            cand --condense 'condense'
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand -f 'f'
            cand --field-separator 'field-separator'
            cand -s 's'
            cand --separator 'separator'
            cand -n 'n'
            cand --no-headers 'no-headers'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;fmt'= {
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand --escape 'escape'
            cand -t 't'
            cand --out-delimiter 'out-delimiter'
            cand -o 'o'
            cand --output 'output'
            cand --quote 'quote'
            cand --ascii 'ascii'
            cand --crlf 'crlf'
            cand --no-final-newline 'no-final-newline'
            cand --quote-always 'quote-always'
            cand --quote-never 'quote-never'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;foreach'= {
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand --dry-run 'dry-run'
            cand -c 'c'
            cand --new-column 'new-column'
            cand -n 'n'
            cand --no-headers 'no-headers'
            cand -p 'p'
            cand --progressbar 'progressbar'
            cand -u 'u'
            cand --unify 'unify'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;frequency'= {
            cand --all-unique-text 'all-unique-text'
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand --high-card-pct 'high-card-pct'
            cand --high-card-threshold 'high-card-threshold'
            cand -j 'j'
            cand --jobs 'jobs'
            cand -l 'l'
            cand --limit 'limit'
            cand --lmt-threshold 'lmt-threshold'
            cand --no-float 'no-float'
            cand --null-text 'null-text'
            cand --other-text 'other-text'
            cand -o 'o'
            cand --output 'output'
            cand --pct-dec-places 'pct-dec-places'
            cand -r 'r'
            cand --rank-strategy 'rank-strategy'
            cand -s 's'
            cand --select 'select'
            cand --sketch-map-size 'sketch-map-size'
            cand --sketch-method 'sketch-method'
            cand --stats-filter 'stats-filter'
            cand -u 'u'
            cand --unq-limit 'unq-limit'
            cand --weight 'weight'
            cand -a 'a'
            cand --asc 'asc'
            cand --force 'force'
            cand --frequency-jsonl 'frequency-jsonl'
            cand -i 'i'
            cand --ignore-case 'ignore-case'
            cand --json 'json'
            cand --memcheck 'memcheck'
            cand -n 'n'
            cand --no-headers 'no-headers'
            cand --no-nulls 'no-nulls'
            cand --no-other 'no-other'
            cand --no-stats 'no-stats'
            cand --no-trim 'no-trim'
            cand --null-sorted 'null-sorted'
            cand --other-sorted 'other-sorted'
            cand --pct-nulls 'pct-nulls'
            cand --pretty-json 'pretty-json'
            cand --toon 'toon'
            cand --vis-whitespace 'vis-whitespace'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;geocode'= {
            cand --admin1 'admin1'
            cand --api-key 'api-key'
            cand -b 'b'
            cand --batch 'batch'
            cand --cache-dir 'cache-dir'
            cand --cache-ttl 'cache-ttl'
            cand --cities-url 'cities-url'
            cand --country 'country'
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand -f 'f'
            cand --formatstr 'formatstr'
            cand --invalid-result 'invalid-result'
            cand -j 'j'
            cand --jobs 'jobs'
            cand -k 'k'
            cand --k_weight 'k_weight'
            cand -l 'l'
            cand --language 'language'
            cand --languages 'languages'
            cand --min-score 'min-score'
            cand -c 'c'
            cand --new-column 'new-column'
            cand --older-than 'older-than'
            cand -o 'o'
            cand --output 'output'
            cand --rate-limit 'rate-limit'
            cand -r 'r'
            cand --rename 'rename'
            cand --timeout 'timeout'
            cand --force 'force'
            cand --no-annotations 'no-annotations'
            cand --no-cache 'no-cache'
            cand -p 'p'
            cand --progressbar 'progressbar'
            cand --reverse 'reverse'
            cand -h 'Print help'
            cand --help 'Print help'
            cand cache-clear 'cache-clear'
            cand cache-info 'cache-info'
            cand cache-prune 'cache-prune'
            cand countryinfo 'countryinfo'
            cand countryinfonow 'countryinfonow'
            cand index-check 'index-check'
            cand index-load 'index-load'
            cand index-reset 'index-reset'
            cand index-update 'index-update'
            cand iplookup 'iplookup'
            cand iplookupnow 'iplookupnow'
            cand opencage 'opencage'
            cand opencagenow 'opencagenow'
            cand reverse 'reverse'
            cand reversenow 'reversenow'
            cand suggest 'suggest'
            cand suggestnow 'suggestnow'
            cand help 'Print this message or the help of the given subcommand(s)'
        }
        &'qsv;geocode;cache-clear'= {
            cand --admin1 'admin1'
            cand --api-key 'api-key'
            cand -b 'b'
            cand --batch 'batch'
            cand --cache-dir 'cache-dir'
            cand --cache-ttl 'cache-ttl'
            cand --cities-url 'cities-url'
            cand --country 'country'
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand -f 'f'
            cand --formatstr 'formatstr'
            cand --invalid-result 'invalid-result'
            cand -j 'j'
            cand --jobs 'jobs'
            cand -k 'k'
            cand --k_weight 'k_weight'
            cand -l 'l'
            cand --language 'language'
            cand --languages 'languages'
            cand --min-score 'min-score'
            cand -c 'c'
            cand --new-column 'new-column'
            cand --older-than 'older-than'
            cand -o 'o'
            cand --output 'output'
            cand --rate-limit 'rate-limit'
            cand -r 'r'
            cand --rename 'rename'
            cand --timeout 'timeout'
            cand --force 'force'
            cand --no-annotations 'no-annotations'
            cand --no-cache 'no-cache'
            cand -p 'p'
            cand --progressbar 'progressbar'
            cand --reverse 'reverse'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;geocode;cache-info'= {
            cand --admin1 'admin1'
            cand --api-key 'api-key'
            cand -b 'b'
            cand --batch 'batch'
            cand --cache-dir 'cache-dir'
            cand --cache-ttl 'cache-ttl'
            cand --cities-url 'cities-url'
            cand --country 'country'
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand -f 'f'
            cand --formatstr 'formatstr'
            cand --invalid-result 'invalid-result'
            cand -j 'j'
            cand --jobs 'jobs'
            cand -k 'k'
            cand --k_weight 'k_weight'
            cand -l 'l'
            cand --language 'language'
            cand --languages 'languages'
            cand --min-score 'min-score'
            cand -c 'c'
            cand --new-column 'new-column'
            cand --older-than 'older-than'
            cand -o 'o'
            cand --output 'output'
            cand --rate-limit 'rate-limit'
            cand -r 'r'
            cand --rename 'rename'
            cand --timeout 'timeout'
            cand --force 'force'
            cand --no-annotations 'no-annotations'
            cand --no-cache 'no-cache'
            cand -p 'p'
            cand --progressbar 'progressbar'
            cand --reverse 'reverse'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;geocode;cache-prune'= {
            cand --admin1 'admin1'
            cand --api-key 'api-key'
            cand -b 'b'
            cand --batch 'batch'
            cand --cache-dir 'cache-dir'
            cand --cache-ttl 'cache-ttl'
            cand --cities-url 'cities-url'
            cand --country 'country'
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand -f 'f'
            cand --formatstr 'formatstr'
            cand --invalid-result 'invalid-result'
            cand -j 'j'
            cand --jobs 'jobs'
            cand -k 'k'
            cand --k_weight 'k_weight'
            cand -l 'l'
            cand --language 'language'
            cand --languages 'languages'
            cand --min-score 'min-score'
            cand -c 'c'
            cand --new-column 'new-column'
            cand --older-than 'older-than'
            cand -o 'o'
            cand --output 'output'
            cand --rate-limit 'rate-limit'
            cand -r 'r'
            cand --rename 'rename'
            cand --timeout 'timeout'
            cand --force 'force'
            cand --no-annotations 'no-annotations'
            cand --no-cache 'no-cache'
            cand -p 'p'
            cand --progressbar 'progressbar'
            cand --reverse 'reverse'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;geocode;countryinfo'= {
            cand --admin1 'admin1'
            cand --api-key 'api-key'
            cand -b 'b'
            cand --batch 'batch'
            cand --cache-dir 'cache-dir'
            cand --cache-ttl 'cache-ttl'
            cand --cities-url 'cities-url'
            cand --country 'country'
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand -f 'f'
            cand --formatstr 'formatstr'
            cand --invalid-result 'invalid-result'
            cand -j 'j'
            cand --jobs 'jobs'
            cand -k 'k'
            cand --k_weight 'k_weight'
            cand -l 'l'
            cand --language 'language'
            cand --languages 'languages'
            cand --min-score 'min-score'
            cand -c 'c'
            cand --new-column 'new-column'
            cand --older-than 'older-than'
            cand -o 'o'
            cand --output 'output'
            cand --rate-limit 'rate-limit'
            cand -r 'r'
            cand --rename 'rename'
            cand --timeout 'timeout'
            cand --force 'force'
            cand --no-annotations 'no-annotations'
            cand --no-cache 'no-cache'
            cand -p 'p'
            cand --progressbar 'progressbar'
            cand --reverse 'reverse'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;geocode;countryinfonow'= {
            cand --admin1 'admin1'
            cand --api-key 'api-key'
            cand -b 'b'
            cand --batch 'batch'
            cand --cache-dir 'cache-dir'
            cand --cache-ttl 'cache-ttl'
            cand --cities-url 'cities-url'
            cand --country 'country'
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand -f 'f'
            cand --formatstr 'formatstr'
            cand --invalid-result 'invalid-result'
            cand -j 'j'
            cand --jobs 'jobs'
            cand -k 'k'
            cand --k_weight 'k_weight'
            cand -l 'l'
            cand --language 'language'
            cand --languages 'languages'
            cand --min-score 'min-score'
            cand -c 'c'
            cand --new-column 'new-column'
            cand --older-than 'older-than'
            cand -o 'o'
            cand --output 'output'
            cand --rate-limit 'rate-limit'
            cand -r 'r'
            cand --rename 'rename'
            cand --timeout 'timeout'
            cand --force 'force'
            cand --no-annotations 'no-annotations'
            cand --no-cache 'no-cache'
            cand -p 'p'
            cand --progressbar 'progressbar'
            cand --reverse 'reverse'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;geocode;index-check'= {
            cand --admin1 'admin1'
            cand --api-key 'api-key'
            cand -b 'b'
            cand --batch 'batch'
            cand --cache-dir 'cache-dir'
            cand --cache-ttl 'cache-ttl'
            cand --cities-url 'cities-url'
            cand --country 'country'
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand -f 'f'
            cand --formatstr 'formatstr'
            cand --invalid-result 'invalid-result'
            cand -j 'j'
            cand --jobs 'jobs'
            cand -k 'k'
            cand --k_weight 'k_weight'
            cand -l 'l'
            cand --language 'language'
            cand --languages 'languages'
            cand --min-score 'min-score'
            cand -c 'c'
            cand --new-column 'new-column'
            cand --older-than 'older-than'
            cand -o 'o'
            cand --output 'output'
            cand --rate-limit 'rate-limit'
            cand -r 'r'
            cand --rename 'rename'
            cand --timeout 'timeout'
            cand --force 'force'
            cand --no-annotations 'no-annotations'
            cand --no-cache 'no-cache'
            cand -p 'p'
            cand --progressbar 'progressbar'
            cand --reverse 'reverse'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;geocode;index-load'= {
            cand --admin1 'admin1'
            cand --api-key 'api-key'
            cand -b 'b'
            cand --batch 'batch'
            cand --cache-dir 'cache-dir'
            cand --cache-ttl 'cache-ttl'
            cand --cities-url 'cities-url'
            cand --country 'country'
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand -f 'f'
            cand --formatstr 'formatstr'
            cand --invalid-result 'invalid-result'
            cand -j 'j'
            cand --jobs 'jobs'
            cand -k 'k'
            cand --k_weight 'k_weight'
            cand -l 'l'
            cand --language 'language'
            cand --languages 'languages'
            cand --min-score 'min-score'
            cand -c 'c'
            cand --new-column 'new-column'
            cand --older-than 'older-than'
            cand -o 'o'
            cand --output 'output'
            cand --rate-limit 'rate-limit'
            cand -r 'r'
            cand --rename 'rename'
            cand --timeout 'timeout'
            cand --force 'force'
            cand --no-annotations 'no-annotations'
            cand --no-cache 'no-cache'
            cand -p 'p'
            cand --progressbar 'progressbar'
            cand --reverse 'reverse'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;geocode;index-reset'= {
            cand --admin1 'admin1'
            cand --api-key 'api-key'
            cand -b 'b'
            cand --batch 'batch'
            cand --cache-dir 'cache-dir'
            cand --cache-ttl 'cache-ttl'
            cand --cities-url 'cities-url'
            cand --country 'country'
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand -f 'f'
            cand --formatstr 'formatstr'
            cand --invalid-result 'invalid-result'
            cand -j 'j'
            cand --jobs 'jobs'
            cand -k 'k'
            cand --k_weight 'k_weight'
            cand -l 'l'
            cand --language 'language'
            cand --languages 'languages'
            cand --min-score 'min-score'
            cand -c 'c'
            cand --new-column 'new-column'
            cand --older-than 'older-than'
            cand -o 'o'
            cand --output 'output'
            cand --rate-limit 'rate-limit'
            cand -r 'r'
            cand --rename 'rename'
            cand --timeout 'timeout'
            cand --force 'force'
            cand --no-annotations 'no-annotations'
            cand --no-cache 'no-cache'
            cand -p 'p'
            cand --progressbar 'progressbar'
            cand --reverse 'reverse'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;geocode;index-update'= {
            cand --admin1 'admin1'
            cand --api-key 'api-key'
            cand -b 'b'
            cand --batch 'batch'
            cand --cache-dir 'cache-dir'
            cand --cache-ttl 'cache-ttl'
            cand --cities-url 'cities-url'
            cand --country 'country'
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand -f 'f'
            cand --formatstr 'formatstr'
            cand --invalid-result 'invalid-result'
            cand -j 'j'
            cand --jobs 'jobs'
            cand -k 'k'
            cand --k_weight 'k_weight'
            cand -l 'l'
            cand --language 'language'
            cand --languages 'languages'
            cand --min-score 'min-score'
            cand -c 'c'
            cand --new-column 'new-column'
            cand --older-than 'older-than'
            cand -o 'o'
            cand --output 'output'
            cand --rate-limit 'rate-limit'
            cand -r 'r'
            cand --rename 'rename'
            cand --timeout 'timeout'
            cand --force 'force'
            cand --no-annotations 'no-annotations'
            cand --no-cache 'no-cache'
            cand -p 'p'
            cand --progressbar 'progressbar'
            cand --reverse 'reverse'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;geocode;iplookup'= {
            cand --admin1 'admin1'
            cand --api-key 'api-key'
            cand -b 'b'
            cand --batch 'batch'
            cand --cache-dir 'cache-dir'
            cand --cache-ttl 'cache-ttl'
            cand --cities-url 'cities-url'
            cand --country 'country'
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand -f 'f'
            cand --formatstr 'formatstr'
            cand --invalid-result 'invalid-result'
            cand -j 'j'
            cand --jobs 'jobs'
            cand -k 'k'
            cand --k_weight 'k_weight'
            cand -l 'l'
            cand --language 'language'
            cand --languages 'languages'
            cand --min-score 'min-score'
            cand -c 'c'
            cand --new-column 'new-column'
            cand --older-than 'older-than'
            cand -o 'o'
            cand --output 'output'
            cand --rate-limit 'rate-limit'
            cand -r 'r'
            cand --rename 'rename'
            cand --timeout 'timeout'
            cand --force 'force'
            cand --no-annotations 'no-annotations'
            cand --no-cache 'no-cache'
            cand -p 'p'
            cand --progressbar 'progressbar'
            cand --reverse 'reverse'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;geocode;iplookupnow'= {
            cand --admin1 'admin1'
            cand --api-key 'api-key'
            cand -b 'b'
            cand --batch 'batch'
            cand --cache-dir 'cache-dir'
            cand --cache-ttl 'cache-ttl'
            cand --cities-url 'cities-url'
            cand --country 'country'
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand -f 'f'
            cand --formatstr 'formatstr'
            cand --invalid-result 'invalid-result'
            cand -j 'j'
            cand --jobs 'jobs'
            cand -k 'k'
            cand --k_weight 'k_weight'
            cand -l 'l'
            cand --language 'language'
            cand --languages 'languages'
            cand --min-score 'min-score'
            cand -c 'c'
            cand --new-column 'new-column'
            cand --older-than 'older-than'
            cand -o 'o'
            cand --output 'output'
            cand --rate-limit 'rate-limit'
            cand -r 'r'
            cand --rename 'rename'
            cand --timeout 'timeout'
            cand --force 'force'
            cand --no-annotations 'no-annotations'
            cand --no-cache 'no-cache'
            cand -p 'p'
            cand --progressbar 'progressbar'
            cand --reverse 'reverse'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;geocode;opencage'= {
            cand --admin1 'admin1'
            cand --api-key 'api-key'
            cand -b 'b'
            cand --batch 'batch'
            cand --cache-dir 'cache-dir'
            cand --cache-ttl 'cache-ttl'
            cand --cities-url 'cities-url'
            cand --country 'country'
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand -f 'f'
            cand --formatstr 'formatstr'
            cand --invalid-result 'invalid-result'
            cand -j 'j'
            cand --jobs 'jobs'
            cand -k 'k'
            cand --k_weight 'k_weight'
            cand -l 'l'
            cand --language 'language'
            cand --languages 'languages'
            cand --min-score 'min-score'
            cand -c 'c'
            cand --new-column 'new-column'
            cand --older-than 'older-than'
            cand -o 'o'
            cand --output 'output'
            cand --rate-limit 'rate-limit'
            cand -r 'r'
            cand --rename 'rename'
            cand --timeout 'timeout'
            cand --force 'force'
            cand --no-annotations 'no-annotations'
            cand --no-cache 'no-cache'
            cand -p 'p'
            cand --progressbar 'progressbar'
            cand --reverse 'reverse'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;geocode;opencagenow'= {
            cand --admin1 'admin1'
            cand --api-key 'api-key'
            cand -b 'b'
            cand --batch 'batch'
            cand --cache-dir 'cache-dir'
            cand --cache-ttl 'cache-ttl'
            cand --cities-url 'cities-url'
            cand --country 'country'
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand -f 'f'
            cand --formatstr 'formatstr'
            cand --invalid-result 'invalid-result'
            cand -j 'j'
            cand --jobs 'jobs'
            cand -k 'k'
            cand --k_weight 'k_weight'
            cand -l 'l'
            cand --language 'language'
            cand --languages 'languages'
            cand --min-score 'min-score'
            cand -c 'c'
            cand --new-column 'new-column'
            cand --older-than 'older-than'
            cand -o 'o'
            cand --output 'output'
            cand --rate-limit 'rate-limit'
            cand -r 'r'
            cand --rename 'rename'
            cand --timeout 'timeout'
            cand --force 'force'
            cand --no-annotations 'no-annotations'
            cand --no-cache 'no-cache'
            cand -p 'p'
            cand --progressbar 'progressbar'
            cand --reverse 'reverse'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;geocode;reverse'= {
            cand --admin1 'admin1'
            cand --api-key 'api-key'
            cand -b 'b'
            cand --batch 'batch'
            cand --cache-dir 'cache-dir'
            cand --cache-ttl 'cache-ttl'
            cand --cities-url 'cities-url'
            cand --country 'country'
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand -f 'f'
            cand --formatstr 'formatstr'
            cand --invalid-result 'invalid-result'
            cand -j 'j'
            cand --jobs 'jobs'
            cand -k 'k'
            cand --k_weight 'k_weight'
            cand -l 'l'
            cand --language 'language'
            cand --languages 'languages'
            cand --min-score 'min-score'
            cand -c 'c'
            cand --new-column 'new-column'
            cand --older-than 'older-than'
            cand -o 'o'
            cand --output 'output'
            cand --rate-limit 'rate-limit'
            cand -r 'r'
            cand --rename 'rename'
            cand --timeout 'timeout'
            cand --force 'force'
            cand --no-annotations 'no-annotations'
            cand --no-cache 'no-cache'
            cand -p 'p'
            cand --progressbar 'progressbar'
            cand --reverse 'reverse'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;geocode;reversenow'= {
            cand --admin1 'admin1'
            cand --api-key 'api-key'
            cand -b 'b'
            cand --batch 'batch'
            cand --cache-dir 'cache-dir'
            cand --cache-ttl 'cache-ttl'
            cand --cities-url 'cities-url'
            cand --country 'country'
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand -f 'f'
            cand --formatstr 'formatstr'
            cand --invalid-result 'invalid-result'
            cand -j 'j'
            cand --jobs 'jobs'
            cand -k 'k'
            cand --k_weight 'k_weight'
            cand -l 'l'
            cand --language 'language'
            cand --languages 'languages'
            cand --min-score 'min-score'
            cand -c 'c'
            cand --new-column 'new-column'
            cand --older-than 'older-than'
            cand -o 'o'
            cand --output 'output'
            cand --rate-limit 'rate-limit'
            cand -r 'r'
            cand --rename 'rename'
            cand --timeout 'timeout'
            cand --force 'force'
            cand --no-annotations 'no-annotations'
            cand --no-cache 'no-cache'
            cand -p 'p'
            cand --progressbar 'progressbar'
            cand --reverse 'reverse'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;geocode;suggest'= {
            cand --admin1 'admin1'
            cand --api-key 'api-key'
            cand -b 'b'
            cand --batch 'batch'
            cand --cache-dir 'cache-dir'
            cand --cache-ttl 'cache-ttl'
            cand --cities-url 'cities-url'
            cand --country 'country'
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand -f 'f'
            cand --formatstr 'formatstr'
            cand --invalid-result 'invalid-result'
            cand -j 'j'
            cand --jobs 'jobs'
            cand -k 'k'
            cand --k_weight 'k_weight'
            cand -l 'l'
            cand --language 'language'
            cand --languages 'languages'
            cand --min-score 'min-score'
            cand -c 'c'
            cand --new-column 'new-column'
            cand --older-than 'older-than'
            cand -o 'o'
            cand --output 'output'
            cand --rate-limit 'rate-limit'
            cand -r 'r'
            cand --rename 'rename'
            cand --timeout 'timeout'
            cand --force 'force'
            cand --no-annotations 'no-annotations'
            cand --no-cache 'no-cache'
            cand -p 'p'
            cand --progressbar 'progressbar'
            cand --reverse 'reverse'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;geocode;suggestnow'= {
            cand --admin1 'admin1'
            cand --api-key 'api-key'
            cand -b 'b'
            cand --batch 'batch'
            cand --cache-dir 'cache-dir'
            cand --cache-ttl 'cache-ttl'
            cand --cities-url 'cities-url'
            cand --country 'country'
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand -f 'f'
            cand --formatstr 'formatstr'
            cand --invalid-result 'invalid-result'
            cand -j 'j'
            cand --jobs 'jobs'
            cand -k 'k'
            cand --k_weight 'k_weight'
            cand -l 'l'
            cand --language 'language'
            cand --languages 'languages'
            cand --min-score 'min-score'
            cand -c 'c'
            cand --new-column 'new-column'
            cand --older-than 'older-than'
            cand -o 'o'
            cand --output 'output'
            cand --rate-limit 'rate-limit'
            cand -r 'r'
            cand --rename 'rename'
            cand --timeout 'timeout'
            cand --force 'force'
            cand --no-annotations 'no-annotations'
            cand --no-cache 'no-cache'
            cand -p 'p'
            cand --progressbar 'progressbar'
            cand --reverse 'reverse'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;geocode;help'= {
            cand cache-clear 'cache-clear'
            cand cache-info 'cache-info'
            cand cache-prune 'cache-prune'
            cand countryinfo 'countryinfo'
            cand countryinfonow 'countryinfonow'
            cand index-check 'index-check'
            cand index-load 'index-load'
            cand index-reset 'index-reset'
            cand index-update 'index-update'
            cand iplookup 'iplookup'
            cand iplookupnow 'iplookupnow'
            cand opencage 'opencage'
            cand opencagenow 'opencagenow'
            cand reverse 'reverse'
            cand reversenow 'reversenow'
            cand suggest 'suggest'
            cand suggestnow 'suggestnow'
            cand help 'Print this message or the help of the given subcommand(s)'
        }
        &'qsv;geocode;help;cache-clear'= {
        }
        &'qsv;geocode;help;cache-info'= {
        }
        &'qsv;geocode;help;cache-prune'= {
        }
        &'qsv;geocode;help;countryinfo'= {
        }
        &'qsv;geocode;help;countryinfonow'= {
        }
        &'qsv;geocode;help;index-check'= {
        }
        &'qsv;geocode;help;index-load'= {
        }
        &'qsv;geocode;help;index-reset'= {
        }
        &'qsv;geocode;help;index-update'= {
        }
        &'qsv;geocode;help;iplookup'= {
        }
        &'qsv;geocode;help;iplookupnow'= {
        }
        &'qsv;geocode;help;opencage'= {
        }
        &'qsv;geocode;help;opencagenow'= {
        }
        &'qsv;geocode;help;reverse'= {
        }
        &'qsv;geocode;help;reversenow'= {
        }
        &'qsv;geocode;help;suggest'= {
        }
        &'qsv;geocode;help;suggestnow'= {
        }
        &'qsv;geocode;help;help'= {
        }
        &'qsv;geoconvert'= {
            cand -g 'g'
            cand --geometry 'geometry'
            cand -y 'y'
            cand --latitude 'latitude'
            cand -x 'x'
            cand --longitude 'longitude'
            cand -l 'l'
            cand --max-length 'max-length'
            cand -o 'o'
            cand --output 'output'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;get'= {
            cand --cache-dir 'cache-dir'
            cand --ckan-api 'ckan-api'
            cand --ckan-token 'ckan-token'
            cand --cloud-opt 'cloud-opt'
            cand --compress 'compress'
            cand --name 'name'
            cand --offset 'offset'
            cand --older-than 'older-than'
            cand -o 'o'
            cand --output 'output'
            cand --refresh 'refresh'
            cand --sample 'sample'
            cand --timeout 'timeout'
            cand --ttl 'ttl'
            cand --force 'force'
            cand --json 'json'
            cand -q 'q'
            cand --quiet 'quiet'
            cand --random 'random'
            cand --verify 'verify'
            cand -h 'Print help'
            cand --help 'Print help'
            cand cache-clear 'cache-clear'
            cand cache-fetch 'cache-fetch'
            cand cache-info 'cache-info'
            cand cache-list 'cache-list'
            cand cache-prune 'cache-prune'
            cand cache-set-policy 'cache-set-policy'
            cand cache-set-ttl 'cache-set-ttl'
            cand help 'Print this message or the help of the given subcommand(s)'
        }
        &'qsv;get;cache-clear'= {
            cand --cache-dir 'cache-dir'
            cand --ckan-api 'ckan-api'
            cand --ckan-token 'ckan-token'
            cand --cloud-opt 'cloud-opt'
            cand --compress 'compress'
            cand --name 'name'
            cand --offset 'offset'
            cand --older-than 'older-than'
            cand -o 'o'
            cand --output 'output'
            cand --refresh 'refresh'
            cand --sample 'sample'
            cand --timeout 'timeout'
            cand --ttl 'ttl'
            cand --force 'force'
            cand --json 'json'
            cand -q 'q'
            cand --quiet 'quiet'
            cand --random 'random'
            cand --verify 'verify'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;get;cache-fetch'= {
            cand --cache-dir 'cache-dir'
            cand --ckan-api 'ckan-api'
            cand --ckan-token 'ckan-token'
            cand --cloud-opt 'cloud-opt'
            cand --compress 'compress'
            cand --name 'name'
            cand --offset 'offset'
            cand --older-than 'older-than'
            cand -o 'o'
            cand --output 'output'
            cand --refresh 'refresh'
            cand --sample 'sample'
            cand --timeout 'timeout'
            cand --ttl 'ttl'
            cand --force 'force'
            cand --json 'json'
            cand -q 'q'
            cand --quiet 'quiet'
            cand --random 'random'
            cand --verify 'verify'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;get;cache-info'= {
            cand --cache-dir 'cache-dir'
            cand --ckan-api 'ckan-api'
            cand --ckan-token 'ckan-token'
            cand --cloud-opt 'cloud-opt'
            cand --compress 'compress'
            cand --name 'name'
            cand --offset 'offset'
            cand --older-than 'older-than'
            cand -o 'o'
            cand --output 'output'
            cand --refresh 'refresh'
            cand --sample 'sample'
            cand --timeout 'timeout'
            cand --ttl 'ttl'
            cand --force 'force'
            cand --json 'json'
            cand -q 'q'
            cand --quiet 'quiet'
            cand --random 'random'
            cand --verify 'verify'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;get;cache-list'= {
            cand --cache-dir 'cache-dir'
            cand --ckan-api 'ckan-api'
            cand --ckan-token 'ckan-token'
            cand --cloud-opt 'cloud-opt'
            cand --compress 'compress'
            cand --name 'name'
            cand --offset 'offset'
            cand --older-than 'older-than'
            cand -o 'o'
            cand --output 'output'
            cand --refresh 'refresh'
            cand --sample 'sample'
            cand --timeout 'timeout'
            cand --ttl 'ttl'
            cand --force 'force'
            cand --json 'json'
            cand -q 'q'
            cand --quiet 'quiet'
            cand --random 'random'
            cand --verify 'verify'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;get;cache-prune'= {
            cand --cache-dir 'cache-dir'
            cand --ckan-api 'ckan-api'
            cand --ckan-token 'ckan-token'
            cand --cloud-opt 'cloud-opt'
            cand --compress 'compress'
            cand --name 'name'
            cand --offset 'offset'
            cand --older-than 'older-than'
            cand -o 'o'
            cand --output 'output'
            cand --refresh 'refresh'
            cand --sample 'sample'
            cand --timeout 'timeout'
            cand --ttl 'ttl'
            cand --force 'force'
            cand --json 'json'
            cand -q 'q'
            cand --quiet 'quiet'
            cand --random 'random'
            cand --verify 'verify'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;get;cache-set-policy'= {
            cand --cache-dir 'cache-dir'
            cand --ckan-api 'ckan-api'
            cand --ckan-token 'ckan-token'
            cand --cloud-opt 'cloud-opt'
            cand --compress 'compress'
            cand --name 'name'
            cand --offset 'offset'
            cand --older-than 'older-than'
            cand -o 'o'
            cand --output 'output'
            cand --refresh 'refresh'
            cand --sample 'sample'
            cand --timeout 'timeout'
            cand --ttl 'ttl'
            cand --force 'force'
            cand --json 'json'
            cand -q 'q'
            cand --quiet 'quiet'
            cand --random 'random'
            cand --verify 'verify'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;get;cache-set-ttl'= {
            cand --cache-dir 'cache-dir'
            cand --ckan-api 'ckan-api'
            cand --ckan-token 'ckan-token'
            cand --cloud-opt 'cloud-opt'
            cand --compress 'compress'
            cand --name 'name'
            cand --offset 'offset'
            cand --older-than 'older-than'
            cand -o 'o'
            cand --output 'output'
            cand --refresh 'refresh'
            cand --sample 'sample'
            cand --timeout 'timeout'
            cand --ttl 'ttl'
            cand --force 'force'
            cand --json 'json'
            cand -q 'q'
            cand --quiet 'quiet'
            cand --random 'random'
            cand --verify 'verify'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;get;help'= {
            cand cache-clear 'cache-clear'
            cand cache-fetch 'cache-fetch'
            cand cache-info 'cache-info'
            cand cache-list 'cache-list'
            cand cache-prune 'cache-prune'
            cand cache-set-policy 'cache-set-policy'
            cand cache-set-ttl 'cache-set-ttl'
            cand help 'Print this message or the help of the given subcommand(s)'
        }
        &'qsv;get;help;cache-clear'= {
        }
        &'qsv;get;help;cache-fetch'= {
        }
        &'qsv;get;help;cache-info'= {
        }
        &'qsv;get;help;cache-list'= {
        }
        &'qsv;get;help;cache-prune'= {
        }
        &'qsv;get;help;cache-set-policy'= {
        }
        &'qsv;get;help;cache-set-ttl'= {
        }
        &'qsv;get;help;help'= {
        }
        &'qsv;headers'= {
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand -J 'J'
            cand --just-count 'just-count'
            cand -j 'j'
            cand --just-names 'just-names'
            cand --trim 'trim'
            cand --union 'union'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;implode'= {
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand -k 'k'
            cand --keys 'keys'
            cand -o 'o'
            cand --output 'output'
            cand -r 'r'
            cand --rename 'rename'
            cand -v 'v'
            cand --value 'value'
            cand -n 'n'
            cand --no-headers 'no-headers'
            cand --skip-empty 'skip-empty'
            cand --sorted 'sorted'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;index'= {
            cand -o 'o'
            cand --output 'output'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;input'= {
            cand --comment 'comment'
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand --encoding-errors 'encoding-errors'
            cand --escape 'escape'
            cand -o 'o'
            cand --output 'output'
            cand --quote 'quote'
            cand --quote-style 'quote-style'
            cand --skip-lastlines 'skip-lastlines'
            cand --skip-lines 'skip-lines'
            cand --auto-skip 'auto-skip'
            cand --no-quoting 'no-quoting'
            cand --trim-fields 'trim-fields'
            cand --trim-headers 'trim-headers'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;join'= {
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand --keys-output 'keys-output'
            cand -o 'o'
            cand --output 'output'
            cand --cross 'cross'
            cand --full 'full'
            cand -i 'i'
            cand --ignore-case 'ignore-case'
            cand -z 'z'
            cand --ignore-leading-zeros 'ignore-leading-zeros'
            cand --left 'left'
            cand --left-anti 'left-anti'
            cand --left-semi 'left-semi'
            cand -n 'n'
            cand --no-headers 'no-headers'
            cand --nulls 'nulls'
            cand --right 'right'
            cand --right-anti 'right-anti'
            cand --right-semi 'right-semi'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;joinp'= {
            cand --cache-schema 'cache-schema'
            cand --date-format 'date-format'
            cand --datetime-format 'datetime-format'
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand --filter-left 'filter-left'
            cand --filter-right 'filter-right'
            cand --float-precision 'float-precision'
            cand --infer-len 'infer-len'
            cand --left_by 'left_by'
            cand --maintain-order 'maintain-order'
            cand --non-equi 'non-equi'
            cand -N 'N'
            cand --norm-unicode 'norm-unicode'
            cand --null-value 'null-value'
            cand -o 'o'
            cand --output 'output'
            cand --right_by 'right_by'
            cand --sql-filter 'sql-filter'
            cand --strategy 'strategy'
            cand --time-format 'time-format'
            cand --tolerance 'tolerance'
            cand --validate 'validate'
            cand -X 'X'
            cand --allow-exact-matches 'allow-exact-matches'
            cand --asof 'asof'
            cand --coalesce 'coalesce'
            cand --cross 'cross'
            cand --decimal-comma 'decimal-comma'
            cand --full 'full'
            cand -i 'i'
            cand --ignore-case 'ignore-case'
            cand --ignore-errors 'ignore-errors'
            cand -z 'z'
            cand --ignore-leading-zeros 'ignore-leading-zeros'
            cand --left 'left'
            cand --left-anti 'left-anti'
            cand --left-semi 'left-semi'
            cand --low-memory 'low-memory'
            cand --no-optimizations 'no-optimizations'
            cand --no-sort 'no-sort'
            cand --nulls 'nulls'
            cand -q 'q'
            cand --quiet 'quiet'
            cand --right 'right'
            cand --right-anti 'right-anti'
            cand --right-semi 'right-semi'
            cand --streaming 'streaming'
            cand --try-parsedates 'try-parsedates'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;json'= {
            cand --jaq 'jaq'
            cand -o 'o'
            cand --output 'output'
            cand -s 's'
            cand --select 'select'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;jsonl'= {
            cand -b 'b'
            cand --batch 'batch'
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand -j 'j'
            cand --jobs 'jobs'
            cand -o 'o'
            cand --output 'output'
            cand --ignore-errors 'ignore-errors'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;lens'= {
            cand --columns 'columns'
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand --echo-column 'echo-column'
            cand --filter 'filter'
            cand --find 'find'
            cand -f 'f'
            cand --freeze-columns 'freeze-columns'
            cand -P 'P'
            cand --prompt 'prompt'
            cand -W 'W'
            cand --wrap-mode 'wrap-mode'
            cand -A 'A'
            cand --auto-reload 'auto-reload'
            cand --debug 'debug'
            cand -i 'i'
            cand --ignore-case 'ignore-case'
            cand -m 'm'
            cand --monochrome 'monochrome'
            cand --no-headers 'no-headers'
            cand -S 'S'
            cand --streaming-stdin 'streaming-stdin'
            cand -t 't'
            cand --tab-separated 'tab-separated'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;log'= {
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;luau'= {
            cand -B 'B'
            cand --begin 'begin'
            cand --cache-dir 'cache-dir'
            cand --ckan-api 'ckan-api'
            cand --ckan-token 'ckan-token'
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand -E 'E'
            cand --end 'end'
            cand --max-errors 'max-errors'
            cand -o 'o'
            cand --output 'output'
            cand --timeout 'timeout'
            cand --colindex 'colindex'
            cand -g 'g'
            cand --no-globals 'no-globals'
            cand -n 'n'
            cand --no-headers 'no-headers'
            cand -p 'p'
            cand --progressbar 'progressbar'
            cand -r 'r'
            cand --remap 'remap'
            cand -h 'Print help'
            cand --help 'Print help'
            cand filter 'filter'
            cand map 'map'
            cand help 'Print this message or the help of the given subcommand(s)'
        }
        &'qsv;luau;filter'= {
            cand -B 'B'
            cand --begin 'begin'
            cand --cache-dir 'cache-dir'
            cand --ckan-api 'ckan-api'
            cand --ckan-token 'ckan-token'
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand -E 'E'
            cand --end 'end'
            cand --max-errors 'max-errors'
            cand -o 'o'
            cand --output 'output'
            cand --timeout 'timeout'
            cand --colindex 'colindex'
            cand -g 'g'
            cand --no-globals 'no-globals'
            cand -n 'n'
            cand --no-headers 'no-headers'
            cand -p 'p'
            cand --progressbar 'progressbar'
            cand -r 'r'
            cand --remap 'remap'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;luau;map'= {
            cand -B 'B'
            cand --begin 'begin'
            cand --cache-dir 'cache-dir'
            cand --ckan-api 'ckan-api'
            cand --ckan-token 'ckan-token'
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand -E 'E'
            cand --end 'end'
            cand --max-errors 'max-errors'
            cand -o 'o'
            cand --output 'output'
            cand --timeout 'timeout'
            cand --colindex 'colindex'
            cand -g 'g'
            cand --no-globals 'no-globals'
            cand -n 'n'
            cand --no-headers 'no-headers'
            cand -p 'p'
            cand --progressbar 'progressbar'
            cand -r 'r'
            cand --remap 'remap'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;luau;help'= {
            cand filter 'filter'
            cand map 'map'
            cand help 'Print this message or the help of the given subcommand(s)'
        }
        &'qsv;luau;help;filter'= {
        }
        &'qsv;luau;help;map'= {
        }
        &'qsv;luau;help;help'= {
        }
        &'qsv;moarstats'= {
            cand --bivariate-batch 'bivariate-batch'
            cand -S 'S'
            cand --bivariate-stats 'bivariate-stats'
            cand -C 'C'
            cand --cardinality-threshold 'cardinality-threshold'
            cand -e 'e'
            cand --epsilon 'epsilon'
            cand -j 'j'
            cand --jobs 'jobs'
            cand -J 'J'
            cand --join-inputs 'join-inputs'
            cand -K 'K'
            cand --join-keys 'join-keys'
            cand -T 'T'
            cand --join-type 'join-type'
            cand -o 'o'
            cand --output 'output'
            cand --pct-thresholds 'pct-thresholds'
            cand --round 'round'
            cand --stats-options 'stats-options'
            cand --xsd-gdate-scan 'xsd-gdate-scan'
            cand --advanced 'advanced'
            cand -B 'B'
            cand --bivariate 'bivariate'
            cand --force 'force'
            cand -p 'p'
            cand --progressbar 'progressbar'
            cand --use-percentiles 'use-percentiles'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;partition'= {
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand --filename 'filename'
            cand --limit 'limit'
            cand -p 'p'
            cand --prefix-length 'prefix-length'
            cand --drop 'drop'
            cand -n 'n'
            cand --no-headers 'no-headers'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;pivotp'= {
            cand -a 'a'
            cand --agg 'agg'
            cand --col-separator 'col-separator'
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand -i 'i'
            cand --index 'index'
            cand --infer-len 'infer-len'
            cand --max-columns 'max-columns'
            cand -o 'o'
            cand --output 'output'
            cand --total-label 'total-label'
            cand -v 'v'
            cand --values 'values'
            cand --decimal-comma 'decimal-comma'
            cand --grand-total 'grand-total'
            cand --ignore-errors 'ignore-errors'
            cand --maintain-order 'maintain-order'
            cand -q 'q'
            cand --quiet 'quiet'
            cand --sort-columns 'sort-columns'
            cand --subtotal 'subtotal'
            cand --try-parsedates 'try-parsedates'
            cand --validate 'validate'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;pragmastat'= {
            cand --compare1 'compare1'
            cand --compare2 'compare2'
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand -j 'j'
            cand --jobs 'jobs'
            cand -m 'm'
            cand --misrate 'misrate'
            cand -o 'o'
            cand --output 'output'
            cand --round 'round'
            cand --seed 'seed'
            cand -s 's'
            cand --select 'select'
            cand --stats-options 'stats-options'
            cand --subsample 'subsample'
            cand --force 'force'
            cand --memcheck 'memcheck'
            cand --no-bounds 'no-bounds'
            cand -n 'n'
            cand --no-headers 'no-headers'
            cand --standalone 'standalone'
            cand -t 't'
            cand --twosample 'twosample'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;pro'= {
            cand -h 'Print help'
            cand --help 'Print help'
            cand lens 'lens'
            cand workflow 'workflow'
            cand help 'Print this message or the help of the given subcommand(s)'
        }
        &'qsv;pro;lens'= {
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;pro;workflow'= {
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;pro;help'= {
            cand lens 'lens'
            cand workflow 'workflow'
            cand help 'Print this message or the help of the given subcommand(s)'
        }
        &'qsv;pro;help;lens'= {
        }
        &'qsv;pro;help;workflow'= {
        }
        &'qsv;pro;help;help'= {
        }
        &'qsv;profile'= {
            cand --dcat-discovery-timeout 'dcat-discovery-timeout'
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand --initial-context 'initial-context'
            cand -j 'j'
            cand --jobs 'jobs'
            cand -o 'o'
            cand --output 'output'
            cand --profile 'profile'
            cand --spec 'spec'
            cand --allow-external-validator 'allow-external-validator'
            cand --catalog 'catalog'
            cand --croissant-frequency 'croissant-frequency'
            cand --dcat-legacy-license 'dcat-legacy-license'
            cand --force 'force'
            cand --memcheck 'memcheck'
            cand --no-ckan 'no-ckan'
            cand --no-dcat-discovery 'no-dcat-discovery'
            cand -n 'n'
            cand --no-headers 'no-headers'
            cand --no-projection 'no-projection'
            cand --strict 'strict'
            cand --validate 'validate'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;prompt'= {
            cand --base-delay-ms 'base-delay-ms'
            cand -F 'F'
            cand --filters 'filters'
            cand -m 'm'
            cand --msg 'msg'
            cand -o 'o'
            cand --output 'output'
            cand --save-fname 'save-fname'
            cand -d 'd'
            cand --workdir 'workdir'
            cand -f 'f'
            cand --fd-output 'fd-output'
            cand -q 'q'
            cand --quiet 'quiet'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;pseudo'= {
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand --formatstr 'formatstr'
            cand --increment 'increment'
            cand -o 'o'
            cand --output 'output'
            cand --start 'start'
            cand -n 'n'
            cand --no-headers 'no-headers'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;py'= {
            cand -b 'b'
            cand --batch 'batch'
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand -f 'f'
            cand --helper 'helper'
            cand -o 'o'
            cand --output 'output'
            cand -n 'n'
            cand --no-headers 'no-headers'
            cand -p 'p'
            cand --progressbar 'progressbar'
            cand -h 'Print help'
            cand --help 'Print help'
            cand filter 'filter'
            cand map 'map'
            cand help 'Print this message or the help of the given subcommand(s)'
        }
        &'qsv;py;filter'= {
            cand -b 'b'
            cand --batch 'batch'
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand -f 'f'
            cand --helper 'helper'
            cand -o 'o'
            cand --output 'output'
            cand -n 'n'
            cand --no-headers 'no-headers'
            cand -p 'p'
            cand --progressbar 'progressbar'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;py;map'= {
            cand -b 'b'
            cand --batch 'batch'
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand -f 'f'
            cand --helper 'helper'
            cand -o 'o'
            cand --output 'output'
            cand -n 'n'
            cand --no-headers 'no-headers'
            cand -p 'p'
            cand --progressbar 'progressbar'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;py;help'= {
            cand filter 'filter'
            cand map 'map'
            cand help 'Print this message or the help of the given subcommand(s)'
        }
        &'qsv;py;help;filter'= {
        }
        &'qsv;py;help;map'= {
        }
        &'qsv;py;help;help'= {
        }
        &'qsv;readstat'= {
            cand -b 'b'
            cand --batch 'batch'
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand -j 'j'
            cand --jobs 'jobs'
            cand --metadata 'metadata'
            cand -o 'o'
            cand --output 'output'
            cand --value-labels 'value-labels'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;rename'= {
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand -o 'o'
            cand --output 'output'
            cand -n 'n'
            cand --no-headers 'no-headers'
            cand --pairwise 'pairwise'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;replace'= {
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand --dfa-size-limit 'dfa-size-limit'
            cand -j 'j'
            cand --jobs 'jobs'
            cand -o 'o'
            cand --output 'output'
            cand -s 's'
            cand --select 'select'
            cand --size-limit 'size-limit'
            cand --exact 'exact'
            cand -i 'i'
            cand --ignore-case 'ignore-case'
            cand --literal 'literal'
            cand -n 'n'
            cand --no-headers 'no-headers'
            cand --not-one 'not-one'
            cand -p 'p'
            cand --progressbar 'progressbar'
            cand -q 'q'
            cand --quiet 'quiet'
            cand -u 'u'
            cand --unicode 'unicode'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;reverse'= {
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand -o 'o'
            cand --output 'output'
            cand --memcheck 'memcheck'
            cand -n 'n'
            cand --no-headers 'no-headers'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;safenames'= {
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand --mode 'mode'
            cand -o 'o'
            cand --output 'output'
            cand --prefix 'prefix'
            cand --reserved 'reserved'
            cand --collapse 'collapse'
            cand --unicode 'unicode'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;sample'= {
            cand --cluster 'cluster'
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand --max-size 'max-size'
            cand -o 'o'
            cand --output 'output'
            cand --rng 'rng'
            cand --seed 'seed'
            cand --sketch-in 'sketch-in'
            cand --sketch-out 'sketch-out'
            cand --stratified 'stratified'
            cand --systematic 'systematic'
            cand --timeout 'timeout'
            cand --timeseries 'timeseries'
            cand --ts-adaptive 'ts-adaptive'
            cand --ts-aggregate 'ts-aggregate'
            cand --ts-input-tz 'ts-input-tz'
            cand --ts-interval 'ts-interval'
            cand --ts-start 'ts-start'
            cand --user-agent 'user-agent'
            cand --varopt 'varopt'
            cand --weighted 'weighted'
            cand --bernoulli 'bernoulli'
            cand --force 'force'
            cand --mergeable-reservoir 'mergeable-reservoir'
            cand -n 'n'
            cand --no-headers 'no-headers'
            cand --ts-prefer-dmy 'ts-prefer-dmy'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;schema'= {
            cand --dates-whitelist 'dates-whitelist'
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand --enum-threshold 'enum-threshold'
            cand -j 'j'
            cand --jobs 'jobs'
            cand -o 'o'
            cand --output 'output'
            cand --pattern-columns 'pattern-columns'
            cand --force 'force'
            cand -i 'i'
            cand --ignore-case 'ignore-case'
            cand --memcheck 'memcheck'
            cand -n 'n'
            cand --no-headers 'no-headers'
            cand --polars 'polars'
            cand --prefer-dmy 'prefer-dmy'
            cand --stdout 'stdout'
            cand --strict-dates 'strict-dates'
            cand --strict-formats 'strict-formats'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;scoresql'= {
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand --infer-len 'infer-len'
            cand -o 'o'
            cand --output 'output'
            cand --duckdb 'duckdb'
            cand --ignore-errors 'ignore-errors'
            cand --json 'json'
            cand -q 'q'
            cand --quiet 'quiet'
            cand --truncate-ragged-lines 'truncate-ragged-lines'
            cand --try-parsedates 'try-parsedates'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;search'= {
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand --dfa-size-limit 'dfa-size-limit'
            cand -f 'f'
            cand --flag 'flag'
            cand -j 'j'
            cand --jobs 'jobs'
            cand -o 'o'
            cand --output 'output'
            cand --preview-match 'preview-match'
            cand -s 's'
            cand --select 'select'
            cand --size-limit 'size-limit'
            cand -c 'c'
            cand --count 'count'
            cand --exact 'exact'
            cand -i 'i'
            cand --ignore-case 'ignore-case'
            cand -v 'v'
            cand --invert-match 'invert-match'
            cand --json 'json'
            cand --literal 'literal'
            cand -n 'n'
            cand --no-headers 'no-headers'
            cand --not-one 'not-one'
            cand -p 'p'
            cand --progressbar 'progressbar'
            cand -Q 'Q'
            cand --quick 'quick'
            cand -q 'q'
            cand --quiet 'quiet'
            cand -u 'u'
            cand --unicode 'unicode'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;searchset'= {
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand --dfa-size-limit 'dfa-size-limit'
            cand -f 'f'
            cand --flag 'flag'
            cand --jobs 'jobs'
            cand -o 'o'
            cand --output 'output'
            cand -s 's'
            cand --select 'select'
            cand --size-limit 'size-limit'
            cand --unmatched-output 'unmatched-output'
            cand -c 'c'
            cand --count 'count'
            cand --exact 'exact'
            cand --flag-matches-only 'flag-matches-only'
            cand -i 'i'
            cand --ignore-case 'ignore-case'
            cand -v 'v'
            cand --invert-match 'invert-match'
            cand -j 'j'
            cand --json 'json'
            cand --literal 'literal'
            cand -n 'n'
            cand --no-headers 'no-headers'
            cand --not-one 'not-one'
            cand -p 'p'
            cand --progressbar 'progressbar'
            cand -Q 'Q'
            cand --quick 'quick'
            cand -q 'q'
            cand --quiet 'quiet'
            cand -u 'u'
            cand --unicode 'unicode'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;select'= {
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand -o 'o'
            cand --output 'output'
            cand --seed 'seed'
            cand -n 'n'
            cand --no-headers 'no-headers'
            cand -R 'R'
            cand --random 'random'
            cand -S 'S'
            cand --sort 'sort'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;slice'= {
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand -e 'e'
            cand --end 'end'
            cand -i 'i'
            cand --index 'index'
            cand -l 'l'
            cand --len 'len'
            cand -o 'o'
            cand --output 'output'
            cand -s 's'
            cand --start 'start'
            cand --invert 'invert'
            cand --json 'json'
            cand -n 'n'
            cand --no-headers 'no-headers'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;snappy'= {
            cand -j 'j'
            cand --jobs 'jobs'
            cand -o 'o'
            cand --output 'output'
            cand --timeout 'timeout'
            cand --user-agent 'user-agent'
            cand -p 'p'
            cand --progressbar 'progressbar'
            cand -q 'q'
            cand --quiet 'quiet'
            cand -h 'Print help'
            cand --help 'Print help'
            cand check 'check'
            cand compress 'compress'
            cand decompress 'decompress'
            cand validate 'validate'
            cand help 'Print this message or the help of the given subcommand(s)'
        }
        &'qsv;snappy;check'= {
            cand -j 'j'
            cand --jobs 'jobs'
            cand -o 'o'
            cand --output 'output'
            cand --timeout 'timeout'
            cand --user-agent 'user-agent'
            cand -p 'p'
            cand --progressbar 'progressbar'
            cand -q 'q'
            cand --quiet 'quiet'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;snappy;compress'= {
            cand -j 'j'
            cand --jobs 'jobs'
            cand -o 'o'
            cand --output 'output'
            cand --timeout 'timeout'
            cand --user-agent 'user-agent'
            cand -p 'p'
            cand --progressbar 'progressbar'
            cand -q 'q'
            cand --quiet 'quiet'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;snappy;decompress'= {
            cand -j 'j'
            cand --jobs 'jobs'
            cand -o 'o'
            cand --output 'output'
            cand --timeout 'timeout'
            cand --user-agent 'user-agent'
            cand -p 'p'
            cand --progressbar 'progressbar'
            cand -q 'q'
            cand --quiet 'quiet'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;snappy;validate'= {
            cand -j 'j'
            cand --jobs 'jobs'
            cand -o 'o'
            cand --output 'output'
            cand --timeout 'timeout'
            cand --user-agent 'user-agent'
            cand -p 'p'
            cand --progressbar 'progressbar'
            cand -q 'q'
            cand --quiet 'quiet'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;snappy;help'= {
            cand check 'check'
            cand compress 'compress'
            cand decompress 'decompress'
            cand validate 'validate'
            cand help 'Print this message or the help of the given subcommand(s)'
        }
        &'qsv;snappy;help;check'= {
        }
        &'qsv;snappy;help;compress'= {
        }
        &'qsv;snappy;help;decompress'= {
        }
        &'qsv;snappy;help;validate'= {
        }
        &'qsv;snappy;help;help'= {
        }
        &'qsv;sniff'= {
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand --quote 'quote'
            cand --sample 'sample'
            cand --save-urlsample 'save-urlsample'
            cand --timeout 'timeout'
            cand --user-agent 'user-agent'
            cand --harvest-mode 'harvest-mode'
            cand --json 'json'
            cand --just-mime 'just-mime'
            cand --no-infer 'no-infer'
            cand --prefer-dmy 'prefer-dmy'
            cand --pretty-json 'pretty-json'
            cand -p 'p'
            cand --progressbar 'progressbar'
            cand -Q 'Q'
            cand --quick 'quick'
            cand --stats-types 'stats-types'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;sort'= {
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand -j 'j'
            cand --jobs 'jobs'
            cand -o 'o'
            cand --output 'output'
            cand --rng 'rng'
            cand --seed 'seed'
            cand -s 's'
            cand --select 'select'
            cand --faster 'faster'
            cand -i 'i'
            cand --ignore-case 'ignore-case'
            cand --memcheck 'memcheck'
            cand --natural 'natural'
            cand -n 'n'
            cand --no-headers 'no-headers'
            cand -N 'N'
            cand --numeric 'numeric'
            cand --random 'random'
            cand -R 'R'
            cand --reverse 'reverse'
            cand -u 'u'
            cand --unique 'unique'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;sortcheck'= {
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand -s 's'
            cand --select 'select'
            cand --all 'all'
            cand -i 'i'
            cand --ignore-case 'ignore-case'
            cand --json 'json'
            cand --natural 'natural'
            cand -n 'n'
            cand --no-headers 'no-headers'
            cand -N 'N'
            cand --numeric 'numeric'
            cand --pretty-json 'pretty-json'
            cand -p 'p'
            cand --progressbar 'progressbar'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;split'= {
            cand -c 'c'
            cand --chunks 'chunks'
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand --filename 'filename'
            cand --filter 'filter'
            cand -j 'j'
            cand --jobs 'jobs'
            cand -k 'k'
            cand --kb-size 'kb-size'
            cand --pad 'pad'
            cand -s 's'
            cand --size 'size'
            cand --filter-cleanup 'filter-cleanup'
            cand --filter-ignore-errors 'filter-ignore-errors'
            cand -n 'n'
            cand --no-headers 'no-headers'
            cand -q 'q'
            cand --quiet 'quiet'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;sqlp'= {
            cand --compress-level 'compress-level'
            cand --compression 'compression'
            cand --date-format 'date-format'
            cand --datetime-format 'datetime-format'
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand --float-precision 'float-precision'
            cand --format 'format'
            cand --infer-len 'infer-len'
            cand -o 'o'
            cand --output 'output'
            cand --rnull-values 'rnull-values'
            cand --time-format 'time-format'
            cand --wnull-value 'wnull-value'
            cand --cache-schema 'cache-schema'
            cand --decimal-comma 'decimal-comma'
            cand --ignore-errors 'ignore-errors'
            cand --low-memory 'low-memory'
            cand --no-optimizations 'no-optimizations'
            cand -q 'q'
            cand --quiet 'quiet'
            cand --statistics 'statistics'
            cand --streaming 'streaming'
            cand --truncate-ragged-lines 'truncate-ragged-lines'
            cand --try-parsedates 'try-parsedates'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;stats'= {
            cand --boolean-patterns 'boolean-patterns'
            cand -c 'c'
            cand --cache-threshold 'cache-threshold'
            cand --cardinality-method 'cardinality-method'
            cand --dates-whitelist 'dates-whitelist'
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand -j 'j'
            cand --jobs 'jobs'
            cand --mode-cardinality-cap 'mode-cardinality-cap'
            cand -o 'o'
            cand --output 'output'
            cand --percentile-list 'percentile-list'
            cand --quantile-method 'quantile-method'
            cand --round 'round'
            cand -s 's'
            cand --select 'select'
            cand --weight 'weight'
            cand --cardinality 'cardinality'
            cand -E 'E'
            cand --everything 'everything'
            cand --force 'force'
            cand --infer-boolean 'infer-boolean'
            cand --infer-dates 'infer-dates'
            cand --jsonl 'jsonl'
            cand --mad 'mad'
            cand --median 'median'
            cand --memcheck 'memcheck'
            cand --mode 'mode'
            cand -n 'n'
            cand --no-headers 'no-headers'
            cand --nulls 'nulls'
            cand --percentiles 'percentiles'
            cand --prefer-dmy 'prefer-dmy'
            cand --pretty-json 'pretty-json'
            cand --quartiles 'quartiles'
            cand --stats-jsonl 'stats-jsonl'
            cand --typesonly 'typesonly'
            cand --vis-whitespace 'vis-whitespace'
            cand --zero-padded-numeric 'zero-padded-numeric'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;synthesize'= {
            cand --correlation-threshold 'correlation-threshold'
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand --dictionary 'dictionary'
            cand --freq-limit 'freq-limit'
            cand -j 'j'
            cand --jobs 'jobs'
            cand --joint-cardinality-cap 'joint-cardinality-cap'
            cand --locale 'locale'
            cand -o 'o'
            cand --output 'output'
            cand -n 'n'
            cand --rows 'rows'
            cand --seed 'seed'
            cand --stats-options 'stats-options'
            cand --consistent-fakes 'consistent-fakes'
            cand --infer-content-type 'infer-content-type'
            cand --no-relationships 'no-relationships'
            cand --strict-relationships 'strict-relationships'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;table'= {
            cand -a 'a'
            cand --align 'align'
            cand -c 'c'
            cand --condense 'condense'
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand -o 'o'
            cand --output 'output'
            cand -p 'p'
            cand --pad 'pad'
            cand -w 'w'
            cand --width 'width'
            cand --memcheck 'memcheck'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;template'= {
            cand -b 'b'
            cand --batch 'batch'
            cand --cache-dir 'cache-dir'
            cand --ckan-api 'ckan-api'
            cand --ckan-token 'ckan-token'
            cand --customfilter-error 'customfilter-error'
            cand --delimiter 'delimiter'
            cand -J 'J'
            cand --globals-json 'globals-json'
            cand -j 'j'
            cand --jobs 'jobs'
            cand --outfilename 'outfilename'
            cand -o 'o'
            cand --output 'output'
            cand --outsubdir-size 'outsubdir-size'
            cand --template 'template'
            cand -t 't'
            cand --template-file 'template-file'
            cand --timeout 'timeout'
            cand -n 'n'
            cand --no-headers 'no-headers'
            cand -p 'p'
            cand --progressbar 'progressbar'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;to'= {
            cand --compress-level 'compress-level'
            cand --compression 'compression'
            cand --delimiter 'delimiter'
            cand --infer-len 'infer-len'
            cand -j 'j'
            cand --jobs 'jobs'
            cand -s 's'
            cand --schema 'schema'
            cand -p 'p'
            cand --separator 'separator'
            cand -c 'c'
            cand --stats-csv 'stats-csv'
            cand -t 't'
            cand --table 'table'
            cand -A 'A'
            cand --all-strings 'all-strings'
            cand -d 'd'
            cand --drop 'drop'
            cand -u 'u'
            cand --dump 'dump'
            cand -e 'e'
            cand --evolve 'evolve'
            cand -i 'i'
            cand --pipe 'pipe'
            cand -k 'k'
            cand --print-package 'print-package'
            cand -q 'q'
            cand --quiet 'quiet'
            cand -a 'a'
            cand --stats 'stats'
            cand --try-parse-dates 'try-parse-dates'
            cand -h 'Print help'
            cand --help 'Print help'
            cand datapackage 'datapackage'
            cand ods 'ods'
            cand parquet 'parquet'
            cand postgres 'postgres'
            cand sqlite 'sqlite'
            cand xlsx 'xlsx'
            cand help 'Print this message or the help of the given subcommand(s)'
        }
        &'qsv;to;datapackage'= {
            cand --compress-level 'compress-level'
            cand --compression 'compression'
            cand --delimiter 'delimiter'
            cand --infer-len 'infer-len'
            cand -j 'j'
            cand --jobs 'jobs'
            cand -s 's'
            cand --schema 'schema'
            cand -p 'p'
            cand --separator 'separator'
            cand -c 'c'
            cand --stats-csv 'stats-csv'
            cand -t 't'
            cand --table 'table'
            cand -A 'A'
            cand --all-strings 'all-strings'
            cand -d 'd'
            cand --drop 'drop'
            cand -u 'u'
            cand --dump 'dump'
            cand -e 'e'
            cand --evolve 'evolve'
            cand -i 'i'
            cand --pipe 'pipe'
            cand -k 'k'
            cand --print-package 'print-package'
            cand -q 'q'
            cand --quiet 'quiet'
            cand -a 'a'
            cand --stats 'stats'
            cand --try-parse-dates 'try-parse-dates'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;to;ods'= {
            cand --compress-level 'compress-level'
            cand --compression 'compression'
            cand --delimiter 'delimiter'
            cand --infer-len 'infer-len'
            cand -j 'j'
            cand --jobs 'jobs'
            cand -s 's'
            cand --schema 'schema'
            cand -p 'p'
            cand --separator 'separator'
            cand -c 'c'
            cand --stats-csv 'stats-csv'
            cand -t 't'
            cand --table 'table'
            cand -A 'A'
            cand --all-strings 'all-strings'
            cand -d 'd'
            cand --drop 'drop'
            cand -u 'u'
            cand --dump 'dump'
            cand -e 'e'
            cand --evolve 'evolve'
            cand -i 'i'
            cand --pipe 'pipe'
            cand -k 'k'
            cand --print-package 'print-package'
            cand -q 'q'
            cand --quiet 'quiet'
            cand -a 'a'
            cand --stats 'stats'
            cand --try-parse-dates 'try-parse-dates'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;to;parquet'= {
            cand --compress-level 'compress-level'
            cand --compression 'compression'
            cand --delimiter 'delimiter'
            cand --infer-len 'infer-len'
            cand -j 'j'
            cand --jobs 'jobs'
            cand -s 's'
            cand --schema 'schema'
            cand -p 'p'
            cand --separator 'separator'
            cand -c 'c'
            cand --stats-csv 'stats-csv'
            cand -t 't'
            cand --table 'table'
            cand -A 'A'
            cand --all-strings 'all-strings'
            cand -d 'd'
            cand --drop 'drop'
            cand -u 'u'
            cand --dump 'dump'
            cand -e 'e'
            cand --evolve 'evolve'
            cand -i 'i'
            cand --pipe 'pipe'
            cand -k 'k'
            cand --print-package 'print-package'
            cand -q 'q'
            cand --quiet 'quiet'
            cand -a 'a'
            cand --stats 'stats'
            cand --try-parse-dates 'try-parse-dates'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;to;postgres'= {
            cand --compress-level 'compress-level'
            cand --compression 'compression'
            cand --delimiter 'delimiter'
            cand --infer-len 'infer-len'
            cand -j 'j'
            cand --jobs 'jobs'
            cand -s 's'
            cand --schema 'schema'
            cand -p 'p'
            cand --separator 'separator'
            cand -c 'c'
            cand --stats-csv 'stats-csv'
            cand -t 't'
            cand --table 'table'
            cand -A 'A'
            cand --all-strings 'all-strings'
            cand -d 'd'
            cand --drop 'drop'
            cand -u 'u'
            cand --dump 'dump'
            cand -e 'e'
            cand --evolve 'evolve'
            cand -i 'i'
            cand --pipe 'pipe'
            cand -k 'k'
            cand --print-package 'print-package'
            cand -q 'q'
            cand --quiet 'quiet'
            cand -a 'a'
            cand --stats 'stats'
            cand --try-parse-dates 'try-parse-dates'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;to;sqlite'= {
            cand --compress-level 'compress-level'
            cand --compression 'compression'
            cand --delimiter 'delimiter'
            cand --infer-len 'infer-len'
            cand -j 'j'
            cand --jobs 'jobs'
            cand -s 's'
            cand --schema 'schema'
            cand -p 'p'
            cand --separator 'separator'
            cand -c 'c'
            cand --stats-csv 'stats-csv'
            cand -t 't'
            cand --table 'table'
            cand -A 'A'
            cand --all-strings 'all-strings'
            cand -d 'd'
            cand --drop 'drop'
            cand -u 'u'
            cand --dump 'dump'
            cand -e 'e'
            cand --evolve 'evolve'
            cand -i 'i'
            cand --pipe 'pipe'
            cand -k 'k'
            cand --print-package 'print-package'
            cand -q 'q'
            cand --quiet 'quiet'
            cand -a 'a'
            cand --stats 'stats'
            cand --try-parse-dates 'try-parse-dates'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;to;xlsx'= {
            cand --compress-level 'compress-level'
            cand --compression 'compression'
            cand --delimiter 'delimiter'
            cand --infer-len 'infer-len'
            cand -j 'j'
            cand --jobs 'jobs'
            cand -s 's'
            cand --schema 'schema'
            cand -p 'p'
            cand --separator 'separator'
            cand -c 'c'
            cand --stats-csv 'stats-csv'
            cand -t 't'
            cand --table 'table'
            cand -A 'A'
            cand --all-strings 'all-strings'
            cand -d 'd'
            cand --drop 'drop'
            cand -u 'u'
            cand --dump 'dump'
            cand -e 'e'
            cand --evolve 'evolve'
            cand -i 'i'
            cand --pipe 'pipe'
            cand -k 'k'
            cand --print-package 'print-package'
            cand -q 'q'
            cand --quiet 'quiet'
            cand -a 'a'
            cand --stats 'stats'
            cand --try-parse-dates 'try-parse-dates'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;to;help'= {
            cand datapackage 'datapackage'
            cand ods 'ods'
            cand parquet 'parquet'
            cand postgres 'postgres'
            cand sqlite 'sqlite'
            cand xlsx 'xlsx'
            cand help 'Print this message or the help of the given subcommand(s)'
        }
        &'qsv;to;help;datapackage'= {
        }
        &'qsv;to;help;ods'= {
        }
        &'qsv;to;help;parquet'= {
        }
        &'qsv;to;help;postgres'= {
        }
        &'qsv;to;help;sqlite'= {
        }
        &'qsv;to;help;xlsx'= {
        }
        &'qsv;to;help;help'= {
        }
        &'qsv;tojsonl'= {
            cand -b 'b'
            cand --batch 'batch'
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand -j 'j'
            cand --jobs 'jobs'
            cand -o 'o'
            cand --output 'output'
            cand --memcheck 'memcheck'
            cand --no-boolean 'no-boolean'
            cand -q 'q'
            cand --quiet 'quiet'
            cand --trim 'trim'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;transpose'= {
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand --long 'long'
            cand -o 'o'
            cand --output 'output'
            cand -s 's'
            cand --select 'select'
            cand --memcheck 'memcheck'
            cand -m 'm'
            cand --multipass 'multipass'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;validate'= {
            cand --backtrack-limit 'backtrack-limit'
            cand -b 'b'
            cand --batch 'batch'
            cand --cache-dir 'cache-dir'
            cand --ckan-api 'ckan-api'
            cand --ckan-token 'ckan-token'
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand --dfa-size-limit 'dfa-size-limit'
            cand --email-min-subdomains 'email-min-subdomains'
            cand --invalid 'invalid'
            cand -j 'j'
            cand --jobs 'jobs'
            cand --size-limit 'size-limit'
            cand --timeout 'timeout'
            cand --valid 'valid'
            cand --valid-output 'valid-output'
            cand --email-display-text 'email-display-text'
            cand --email-domain-literal 'email-domain-literal'
            cand --email-required-tld 'email-required-tld'
            cand --fail-fast 'fail-fast'
            cand --fancy-regex 'fancy-regex'
            cand --json 'json'
            cand --no-format-validation 'no-format-validation'
            cand -n 'n'
            cand --no-headers 'no-headers'
            cand --pretty-json 'pretty-json'
            cand -p 'p'
            cand --progressbar 'progressbar'
            cand -q 'q'
            cand --quiet 'quiet'
            cand --split-ragged 'split-ragged'
            cand --trim 'trim'
            cand -h 'Print help'
            cand --help 'Print help'
            cand schema 'schema'
            cand help 'Print this message or the help of the given subcommand(s)'
        }
        &'qsv;validate;schema'= {
            cand --backtrack-limit 'backtrack-limit'
            cand -b 'b'
            cand --batch 'batch'
            cand --cache-dir 'cache-dir'
            cand --ckan-api 'ckan-api'
            cand --ckan-token 'ckan-token'
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand --dfa-size-limit 'dfa-size-limit'
            cand --email-min-subdomains 'email-min-subdomains'
            cand --invalid 'invalid'
            cand -j 'j'
            cand --jobs 'jobs'
            cand --size-limit 'size-limit'
            cand --timeout 'timeout'
            cand --valid 'valid'
            cand --valid-output 'valid-output'
            cand --email-display-text 'email-display-text'
            cand --email-domain-literal 'email-domain-literal'
            cand --email-required-tld 'email-required-tld'
            cand --fail-fast 'fail-fast'
            cand --fancy-regex 'fancy-regex'
            cand --json 'json'
            cand --no-format-validation 'no-format-validation'
            cand -n 'n'
            cand --no-headers 'no-headers'
            cand --pretty-json 'pretty-json'
            cand -p 'p'
            cand --progressbar 'progressbar'
            cand -q 'q'
            cand --quiet 'quiet'
            cand --split-ragged 'split-ragged'
            cand --trim 'trim'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;validate;help'= {
            cand schema 'schema'
            cand help 'Print this message or the help of the given subcommand(s)'
        }
        &'qsv;validate;help;schema'= {
        }
        &'qsv;validate;help;help'= {
        }
        &'qsv;viz'= {
            cand --agg 'agg'
            cand --annotation 'annotation'
            cand --bins 'bins'
            cand --box-points 'box-points'
            cand --close 'close'
            cand --cluster 'cluster'
            cand --color 'color'
            cand --color-scale 'color-scale'
            cand --cols 'cols'
            cand --dataset-pid 'dataset-pid'
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand --denominator 'denominator'
            cand --denominator-key 'denominator-key'
            cand --denominator-unit 'denominator-unit'
            cand --dictionary 'dictionary'
            cand --dictionary-context 'dictionary-context'
            cand --feature-id-key 'feature-id-key'
            cand --feature-name-key 'feature-name-key'
            cand --geocode-admin1 'geocode-admin1'
            cand --geocode-country 'geocode-country'
            cand --geojson 'geojson'
            cand --grid-cols 'grid-cols'
            cand --heatmap-density 'heatmap-density'
            cand --height 'height'
            cand --hierarchy-style 'hierarchy-style'
            cand --high 'high'
            cand --language 'language'
            cand --lat 'lat'
            cand --limit 'limit'
            cand --location-mode 'location-mode'
            cand --locations 'locations'
            cand --log-scale 'log-scale'
            cand --lon 'lon'
            cand --low 'low'
            cand --max-charts 'max-charts'
            cand --ohlc-open 'ohlc-open'
            cand -o 'o'
            cand --output 'output'
            cand --preview-threshold 'preview-threshold'
            cand --projection 'projection'
            cand --region-state 'region-state'
            cand --scale 'scale'
            cand --series 'series'
            cand --size 'size'
            cand --slider 'slider'
            cand --slider-speed 'slider-speed'
            cand --snap-max-dist 'snap-max-dist'
            cand --source 'source'
            cand --style 'style'
            cand --target 'target'
            cand --text 'text'
            cand --theme 'theme'
            cand --title 'title'
            cand --tour-audience 'tour-audience'
            cand --tour-steps 'tour-steps'
            cand --value 'value'
            cand --violin 'violin'
            cand --width 'width'
            cand -x 'x'
            cand --x 'x'
            cand --x-title 'x-title'
            cand -y 'y'
            cand --y 'y'
            cand --y-range 'y-range'
            cand --y-title 'y-title'
            cand -z 'z'
            cand --z 'z'
            cand --bivariate 'bivariate'
            cand --check-geojson-key 'check-geojson-key'
            cand --density 'density'
            cand --dict-info 'dict-info'
            cand --donut 'donut'
            cand --geocode 'geocode'
            cand --map 'map'
            cand -n 'n'
            cand --no-headers 'no-headers'
            cand --no-nulls 'no-nulls'
            cand --no-other 'no-other'
            cand --no-snap 'no-snap'
            cand --open 'open'
            cand --photos 'photos'
            cand --rangeslider 'rangeslider'
            cand --sankey-value-order 'sankey-value-order'
            cand --slider-cumulative 'slider-cumulative'
            cand --smarter 'smarter'
            cand -h 'Print help'
            cand --help 'Print help'
            cand bar 'bar'
            cand box 'box'
            cand candlestick 'candlestick'
            cand choropleth 'choropleth'
            cand contour 'contour'
            cand funnel 'funnel'
            cand geo 'geo'
            cand heatmap 'heatmap'
            cand histogram 'histogram'
            cand icicle 'icicle'
            cand line 'line'
            cand map 'map'
            cand ohlc 'ohlc'
            cand parcats 'parcats'
            cand pie 'pie'
            cand radar 'radar'
            cand sankey 'sankey'
            cand scatter 'scatter'
            cand scatter3d 'scatter3d'
            cand smart 'smart'
            cand splom 'splom'
            cand sunburst 'sunburst'
            cand treemap 'treemap'
            cand violin 'violin'
            cand help 'Print this message or the help of the given subcommand(s)'
        }
        &'qsv;viz;bar'= {
            cand --agg 'agg'
            cand --annotation 'annotation'
            cand --bins 'bins'
            cand --box-points 'box-points'
            cand --close 'close'
            cand --cluster 'cluster'
            cand --color 'color'
            cand --color-scale 'color-scale'
            cand --cols 'cols'
            cand --dataset-pid 'dataset-pid'
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand --denominator 'denominator'
            cand --denominator-key 'denominator-key'
            cand --denominator-unit 'denominator-unit'
            cand --dictionary 'dictionary'
            cand --dictionary-context 'dictionary-context'
            cand --feature-id-key 'feature-id-key'
            cand --feature-name-key 'feature-name-key'
            cand --geocode-admin1 'geocode-admin1'
            cand --geocode-country 'geocode-country'
            cand --geojson 'geojson'
            cand --grid-cols 'grid-cols'
            cand --heatmap-density 'heatmap-density'
            cand --height 'height'
            cand --hierarchy-style 'hierarchy-style'
            cand --high 'high'
            cand --language 'language'
            cand --lat 'lat'
            cand --limit 'limit'
            cand --location-mode 'location-mode'
            cand --locations 'locations'
            cand --log-scale 'log-scale'
            cand --lon 'lon'
            cand --low 'low'
            cand --max-charts 'max-charts'
            cand --ohlc-open 'ohlc-open'
            cand -o 'o'
            cand --output 'output'
            cand --preview-threshold 'preview-threshold'
            cand --projection 'projection'
            cand --region-state 'region-state'
            cand --scale 'scale'
            cand --series 'series'
            cand --size 'size'
            cand --slider 'slider'
            cand --slider-speed 'slider-speed'
            cand --snap-max-dist 'snap-max-dist'
            cand --source 'source'
            cand --style 'style'
            cand --target 'target'
            cand --text 'text'
            cand --theme 'theme'
            cand --title 'title'
            cand --tour-audience 'tour-audience'
            cand --tour-steps 'tour-steps'
            cand --value 'value'
            cand --violin 'violin'
            cand --width 'width'
            cand -x 'x'
            cand --x 'x'
            cand --x-title 'x-title'
            cand -y 'y'
            cand --y 'y'
            cand --y-range 'y-range'
            cand --y-title 'y-title'
            cand -z 'z'
            cand --z 'z'
            cand --bivariate 'bivariate'
            cand --check-geojson-key 'check-geojson-key'
            cand --density 'density'
            cand --dict-info 'dict-info'
            cand --donut 'donut'
            cand --geocode 'geocode'
            cand --map 'map'
            cand -n 'n'
            cand --no-headers 'no-headers'
            cand --no-nulls 'no-nulls'
            cand --no-other 'no-other'
            cand --no-snap 'no-snap'
            cand --open 'open'
            cand --photos 'photos'
            cand --rangeslider 'rangeslider'
            cand --sankey-value-order 'sankey-value-order'
            cand --slider-cumulative 'slider-cumulative'
            cand --smarter 'smarter'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;viz;box'= {
            cand --agg 'agg'
            cand --annotation 'annotation'
            cand --bins 'bins'
            cand --box-points 'box-points'
            cand --close 'close'
            cand --cluster 'cluster'
            cand --color 'color'
            cand --color-scale 'color-scale'
            cand --cols 'cols'
            cand --dataset-pid 'dataset-pid'
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand --denominator 'denominator'
            cand --denominator-key 'denominator-key'
            cand --denominator-unit 'denominator-unit'
            cand --dictionary 'dictionary'
            cand --dictionary-context 'dictionary-context'
            cand --feature-id-key 'feature-id-key'
            cand --feature-name-key 'feature-name-key'
            cand --geocode-admin1 'geocode-admin1'
            cand --geocode-country 'geocode-country'
            cand --geojson 'geojson'
            cand --grid-cols 'grid-cols'
            cand --heatmap-density 'heatmap-density'
            cand --height 'height'
            cand --hierarchy-style 'hierarchy-style'
            cand --high 'high'
            cand --language 'language'
            cand --lat 'lat'
            cand --limit 'limit'
            cand --location-mode 'location-mode'
            cand --locations 'locations'
            cand --log-scale 'log-scale'
            cand --lon 'lon'
            cand --low 'low'
            cand --max-charts 'max-charts'
            cand --ohlc-open 'ohlc-open'
            cand -o 'o'
            cand --output 'output'
            cand --preview-threshold 'preview-threshold'
            cand --projection 'projection'
            cand --region-state 'region-state'
            cand --scale 'scale'
            cand --series 'series'
            cand --size 'size'
            cand --slider 'slider'
            cand --slider-speed 'slider-speed'
            cand --snap-max-dist 'snap-max-dist'
            cand --source 'source'
            cand --style 'style'
            cand --target 'target'
            cand --text 'text'
            cand --theme 'theme'
            cand --title 'title'
            cand --tour-audience 'tour-audience'
            cand --tour-steps 'tour-steps'
            cand --value 'value'
            cand --violin 'violin'
            cand --width 'width'
            cand -x 'x'
            cand --x 'x'
            cand --x-title 'x-title'
            cand -y 'y'
            cand --y 'y'
            cand --y-range 'y-range'
            cand --y-title 'y-title'
            cand -z 'z'
            cand --z 'z'
            cand --bivariate 'bivariate'
            cand --check-geojson-key 'check-geojson-key'
            cand --density 'density'
            cand --dict-info 'dict-info'
            cand --donut 'donut'
            cand --geocode 'geocode'
            cand --map 'map'
            cand -n 'n'
            cand --no-headers 'no-headers'
            cand --no-nulls 'no-nulls'
            cand --no-other 'no-other'
            cand --no-snap 'no-snap'
            cand --open 'open'
            cand --photos 'photos'
            cand --rangeslider 'rangeslider'
            cand --sankey-value-order 'sankey-value-order'
            cand --slider-cumulative 'slider-cumulative'
            cand --smarter 'smarter'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;viz;candlestick'= {
            cand --agg 'agg'
            cand --annotation 'annotation'
            cand --bins 'bins'
            cand --box-points 'box-points'
            cand --close 'close'
            cand --cluster 'cluster'
            cand --color 'color'
            cand --color-scale 'color-scale'
            cand --cols 'cols'
            cand --dataset-pid 'dataset-pid'
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand --denominator 'denominator'
            cand --denominator-key 'denominator-key'
            cand --denominator-unit 'denominator-unit'
            cand --dictionary 'dictionary'
            cand --dictionary-context 'dictionary-context'
            cand --feature-id-key 'feature-id-key'
            cand --feature-name-key 'feature-name-key'
            cand --geocode-admin1 'geocode-admin1'
            cand --geocode-country 'geocode-country'
            cand --geojson 'geojson'
            cand --grid-cols 'grid-cols'
            cand --heatmap-density 'heatmap-density'
            cand --height 'height'
            cand --hierarchy-style 'hierarchy-style'
            cand --high 'high'
            cand --language 'language'
            cand --lat 'lat'
            cand --limit 'limit'
            cand --location-mode 'location-mode'
            cand --locations 'locations'
            cand --log-scale 'log-scale'
            cand --lon 'lon'
            cand --low 'low'
            cand --max-charts 'max-charts'
            cand --ohlc-open 'ohlc-open'
            cand -o 'o'
            cand --output 'output'
            cand --preview-threshold 'preview-threshold'
            cand --projection 'projection'
            cand --region-state 'region-state'
            cand --scale 'scale'
            cand --series 'series'
            cand --size 'size'
            cand --slider 'slider'
            cand --slider-speed 'slider-speed'
            cand --snap-max-dist 'snap-max-dist'
            cand --source 'source'
            cand --style 'style'
            cand --target 'target'
            cand --text 'text'
            cand --theme 'theme'
            cand --title 'title'
            cand --tour-audience 'tour-audience'
            cand --tour-steps 'tour-steps'
            cand --value 'value'
            cand --violin 'violin'
            cand --width 'width'
            cand -x 'x'
            cand --x 'x'
            cand --x-title 'x-title'
            cand -y 'y'
            cand --y 'y'
            cand --y-range 'y-range'
            cand --y-title 'y-title'
            cand -z 'z'
            cand --z 'z'
            cand --bivariate 'bivariate'
            cand --check-geojson-key 'check-geojson-key'
            cand --density 'density'
            cand --dict-info 'dict-info'
            cand --donut 'donut'
            cand --geocode 'geocode'
            cand --map 'map'
            cand -n 'n'
            cand --no-headers 'no-headers'
            cand --no-nulls 'no-nulls'
            cand --no-other 'no-other'
            cand --no-snap 'no-snap'
            cand --open 'open'
            cand --photos 'photos'
            cand --rangeslider 'rangeslider'
            cand --sankey-value-order 'sankey-value-order'
            cand --slider-cumulative 'slider-cumulative'
            cand --smarter 'smarter'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;viz;choropleth'= {
            cand --agg 'agg'
            cand --annotation 'annotation'
            cand --bins 'bins'
            cand --box-points 'box-points'
            cand --close 'close'
            cand --cluster 'cluster'
            cand --color 'color'
            cand --color-scale 'color-scale'
            cand --cols 'cols'
            cand --dataset-pid 'dataset-pid'
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand --denominator 'denominator'
            cand --denominator-key 'denominator-key'
            cand --denominator-unit 'denominator-unit'
            cand --dictionary 'dictionary'
            cand --dictionary-context 'dictionary-context'
            cand --feature-id-key 'feature-id-key'
            cand --feature-name-key 'feature-name-key'
            cand --geocode-admin1 'geocode-admin1'
            cand --geocode-country 'geocode-country'
            cand --geojson 'geojson'
            cand --grid-cols 'grid-cols'
            cand --heatmap-density 'heatmap-density'
            cand --height 'height'
            cand --hierarchy-style 'hierarchy-style'
            cand --high 'high'
            cand --language 'language'
            cand --lat 'lat'
            cand --limit 'limit'
            cand --location-mode 'location-mode'
            cand --locations 'locations'
            cand --log-scale 'log-scale'
            cand --lon 'lon'
            cand --low 'low'
            cand --max-charts 'max-charts'
            cand --ohlc-open 'ohlc-open'
            cand -o 'o'
            cand --output 'output'
            cand --preview-threshold 'preview-threshold'
            cand --projection 'projection'
            cand --region-state 'region-state'
            cand --scale 'scale'
            cand --series 'series'
            cand --size 'size'
            cand --slider 'slider'
            cand --slider-speed 'slider-speed'
            cand --snap-max-dist 'snap-max-dist'
            cand --source 'source'
            cand --style 'style'
            cand --target 'target'
            cand --text 'text'
            cand --theme 'theme'
            cand --title 'title'
            cand --tour-audience 'tour-audience'
            cand --tour-steps 'tour-steps'
            cand --value 'value'
            cand --violin 'violin'
            cand --width 'width'
            cand -x 'x'
            cand --x 'x'
            cand --x-title 'x-title'
            cand -y 'y'
            cand --y 'y'
            cand --y-range 'y-range'
            cand --y-title 'y-title'
            cand -z 'z'
            cand --z 'z'
            cand --bivariate 'bivariate'
            cand --check-geojson-key 'check-geojson-key'
            cand --density 'density'
            cand --dict-info 'dict-info'
            cand --donut 'donut'
            cand --geocode 'geocode'
            cand --map 'map'
            cand -n 'n'
            cand --no-headers 'no-headers'
            cand --no-nulls 'no-nulls'
            cand --no-other 'no-other'
            cand --no-snap 'no-snap'
            cand --open 'open'
            cand --photos 'photos'
            cand --rangeslider 'rangeslider'
            cand --sankey-value-order 'sankey-value-order'
            cand --slider-cumulative 'slider-cumulative'
            cand --smarter 'smarter'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;viz;contour'= {
            cand --agg 'agg'
            cand --annotation 'annotation'
            cand --bins 'bins'
            cand --box-points 'box-points'
            cand --close 'close'
            cand --cluster 'cluster'
            cand --color 'color'
            cand --color-scale 'color-scale'
            cand --cols 'cols'
            cand --dataset-pid 'dataset-pid'
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand --denominator 'denominator'
            cand --denominator-key 'denominator-key'
            cand --denominator-unit 'denominator-unit'
            cand --dictionary 'dictionary'
            cand --dictionary-context 'dictionary-context'
            cand --feature-id-key 'feature-id-key'
            cand --feature-name-key 'feature-name-key'
            cand --geocode-admin1 'geocode-admin1'
            cand --geocode-country 'geocode-country'
            cand --geojson 'geojson'
            cand --grid-cols 'grid-cols'
            cand --heatmap-density 'heatmap-density'
            cand --height 'height'
            cand --hierarchy-style 'hierarchy-style'
            cand --high 'high'
            cand --language 'language'
            cand --lat 'lat'
            cand --limit 'limit'
            cand --location-mode 'location-mode'
            cand --locations 'locations'
            cand --log-scale 'log-scale'
            cand --lon 'lon'
            cand --low 'low'
            cand --max-charts 'max-charts'
            cand --ohlc-open 'ohlc-open'
            cand -o 'o'
            cand --output 'output'
            cand --preview-threshold 'preview-threshold'
            cand --projection 'projection'
            cand --region-state 'region-state'
            cand --scale 'scale'
            cand --series 'series'
            cand --size 'size'
            cand --slider 'slider'
            cand --slider-speed 'slider-speed'
            cand --snap-max-dist 'snap-max-dist'
            cand --source 'source'
            cand --style 'style'
            cand --target 'target'
            cand --text 'text'
            cand --theme 'theme'
            cand --title 'title'
            cand --tour-audience 'tour-audience'
            cand --tour-steps 'tour-steps'
            cand --value 'value'
            cand --violin 'violin'
            cand --width 'width'
            cand -x 'x'
            cand --x 'x'
            cand --x-title 'x-title'
            cand -y 'y'
            cand --y 'y'
            cand --y-range 'y-range'
            cand --y-title 'y-title'
            cand -z 'z'
            cand --z 'z'
            cand --bivariate 'bivariate'
            cand --check-geojson-key 'check-geojson-key'
            cand --density 'density'
            cand --dict-info 'dict-info'
            cand --donut 'donut'
            cand --geocode 'geocode'
            cand --map 'map'
            cand -n 'n'
            cand --no-headers 'no-headers'
            cand --no-nulls 'no-nulls'
            cand --no-other 'no-other'
            cand --no-snap 'no-snap'
            cand --open 'open'
            cand --photos 'photos'
            cand --rangeslider 'rangeslider'
            cand --sankey-value-order 'sankey-value-order'
            cand --slider-cumulative 'slider-cumulative'
            cand --smarter 'smarter'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;viz;funnel'= {
            cand --agg 'agg'
            cand --annotation 'annotation'
            cand --bins 'bins'
            cand --box-points 'box-points'
            cand --close 'close'
            cand --cluster 'cluster'
            cand --color 'color'
            cand --color-scale 'color-scale'
            cand --cols 'cols'
            cand --dataset-pid 'dataset-pid'
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand --denominator 'denominator'
            cand --denominator-key 'denominator-key'
            cand --denominator-unit 'denominator-unit'
            cand --dictionary 'dictionary'
            cand --dictionary-context 'dictionary-context'
            cand --feature-id-key 'feature-id-key'
            cand --feature-name-key 'feature-name-key'
            cand --geocode-admin1 'geocode-admin1'
            cand --geocode-country 'geocode-country'
            cand --geojson 'geojson'
            cand --grid-cols 'grid-cols'
            cand --heatmap-density 'heatmap-density'
            cand --height 'height'
            cand --hierarchy-style 'hierarchy-style'
            cand --high 'high'
            cand --language 'language'
            cand --lat 'lat'
            cand --limit 'limit'
            cand --location-mode 'location-mode'
            cand --locations 'locations'
            cand --log-scale 'log-scale'
            cand --lon 'lon'
            cand --low 'low'
            cand --max-charts 'max-charts'
            cand --ohlc-open 'ohlc-open'
            cand -o 'o'
            cand --output 'output'
            cand --preview-threshold 'preview-threshold'
            cand --projection 'projection'
            cand --region-state 'region-state'
            cand --scale 'scale'
            cand --series 'series'
            cand --size 'size'
            cand --slider 'slider'
            cand --slider-speed 'slider-speed'
            cand --snap-max-dist 'snap-max-dist'
            cand --source 'source'
            cand --style 'style'
            cand --target 'target'
            cand --text 'text'
            cand --theme 'theme'
            cand --title 'title'
            cand --tour-audience 'tour-audience'
            cand --tour-steps 'tour-steps'
            cand --value 'value'
            cand --violin 'violin'
            cand --width 'width'
            cand -x 'x'
            cand --x 'x'
            cand --x-title 'x-title'
            cand -y 'y'
            cand --y 'y'
            cand --y-range 'y-range'
            cand --y-title 'y-title'
            cand -z 'z'
            cand --z 'z'
            cand --bivariate 'bivariate'
            cand --check-geojson-key 'check-geojson-key'
            cand --density 'density'
            cand --dict-info 'dict-info'
            cand --donut 'donut'
            cand --geocode 'geocode'
            cand --map 'map'
            cand -n 'n'
            cand --no-headers 'no-headers'
            cand --no-nulls 'no-nulls'
            cand --no-other 'no-other'
            cand --no-snap 'no-snap'
            cand --open 'open'
            cand --photos 'photos'
            cand --rangeslider 'rangeslider'
            cand --sankey-value-order 'sankey-value-order'
            cand --slider-cumulative 'slider-cumulative'
            cand --smarter 'smarter'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;viz;geo'= {
            cand --agg 'agg'
            cand --annotation 'annotation'
            cand --bins 'bins'
            cand --box-points 'box-points'
            cand --close 'close'
            cand --cluster 'cluster'
            cand --color 'color'
            cand --color-scale 'color-scale'
            cand --cols 'cols'
            cand --dataset-pid 'dataset-pid'
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand --denominator 'denominator'
            cand --denominator-key 'denominator-key'
            cand --denominator-unit 'denominator-unit'
            cand --dictionary 'dictionary'
            cand --dictionary-context 'dictionary-context'
            cand --feature-id-key 'feature-id-key'
            cand --feature-name-key 'feature-name-key'
            cand --geocode-admin1 'geocode-admin1'
            cand --geocode-country 'geocode-country'
            cand --geojson 'geojson'
            cand --grid-cols 'grid-cols'
            cand --heatmap-density 'heatmap-density'
            cand --height 'height'
            cand --hierarchy-style 'hierarchy-style'
            cand --high 'high'
            cand --language 'language'
            cand --lat 'lat'
            cand --limit 'limit'
            cand --location-mode 'location-mode'
            cand --locations 'locations'
            cand --log-scale 'log-scale'
            cand --lon 'lon'
            cand --low 'low'
            cand --max-charts 'max-charts'
            cand --ohlc-open 'ohlc-open'
            cand -o 'o'
            cand --output 'output'
            cand --preview-threshold 'preview-threshold'
            cand --projection 'projection'
            cand --region-state 'region-state'
            cand --scale 'scale'
            cand --series 'series'
            cand --size 'size'
            cand --slider 'slider'
            cand --slider-speed 'slider-speed'
            cand --snap-max-dist 'snap-max-dist'
            cand --source 'source'
            cand --style 'style'
            cand --target 'target'
            cand --text 'text'
            cand --theme 'theme'
            cand --title 'title'
            cand --tour-audience 'tour-audience'
            cand --tour-steps 'tour-steps'
            cand --value 'value'
            cand --violin 'violin'
            cand --width 'width'
            cand -x 'x'
            cand --x 'x'
            cand --x-title 'x-title'
            cand -y 'y'
            cand --y 'y'
            cand --y-range 'y-range'
            cand --y-title 'y-title'
            cand -z 'z'
            cand --z 'z'
            cand --bivariate 'bivariate'
            cand --check-geojson-key 'check-geojson-key'
            cand --density 'density'
            cand --dict-info 'dict-info'
            cand --donut 'donut'
            cand --geocode 'geocode'
            cand --map 'map'
            cand -n 'n'
            cand --no-headers 'no-headers'
            cand --no-nulls 'no-nulls'
            cand --no-other 'no-other'
            cand --no-snap 'no-snap'
            cand --open 'open'
            cand --photos 'photos'
            cand --rangeslider 'rangeslider'
            cand --sankey-value-order 'sankey-value-order'
            cand --slider-cumulative 'slider-cumulative'
            cand --smarter 'smarter'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;viz;heatmap'= {
            cand --agg 'agg'
            cand --annotation 'annotation'
            cand --bins 'bins'
            cand --box-points 'box-points'
            cand --close 'close'
            cand --cluster 'cluster'
            cand --color 'color'
            cand --color-scale 'color-scale'
            cand --cols 'cols'
            cand --dataset-pid 'dataset-pid'
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand --denominator 'denominator'
            cand --denominator-key 'denominator-key'
            cand --denominator-unit 'denominator-unit'
            cand --dictionary 'dictionary'
            cand --dictionary-context 'dictionary-context'
            cand --feature-id-key 'feature-id-key'
            cand --feature-name-key 'feature-name-key'
            cand --geocode-admin1 'geocode-admin1'
            cand --geocode-country 'geocode-country'
            cand --geojson 'geojson'
            cand --grid-cols 'grid-cols'
            cand --heatmap-density 'heatmap-density'
            cand --height 'height'
            cand --hierarchy-style 'hierarchy-style'
            cand --high 'high'
            cand --language 'language'
            cand --lat 'lat'
            cand --limit 'limit'
            cand --location-mode 'location-mode'
            cand --locations 'locations'
            cand --log-scale 'log-scale'
            cand --lon 'lon'
            cand --low 'low'
            cand --max-charts 'max-charts'
            cand --ohlc-open 'ohlc-open'
            cand -o 'o'
            cand --output 'output'
            cand --preview-threshold 'preview-threshold'
            cand --projection 'projection'
            cand --region-state 'region-state'
            cand --scale 'scale'
            cand --series 'series'
            cand --size 'size'
            cand --slider 'slider'
            cand --slider-speed 'slider-speed'
            cand --snap-max-dist 'snap-max-dist'
            cand --source 'source'
            cand --style 'style'
            cand --target 'target'
            cand --text 'text'
            cand --theme 'theme'
            cand --title 'title'
            cand --tour-audience 'tour-audience'
            cand --tour-steps 'tour-steps'
            cand --value 'value'
            cand --violin 'violin'
            cand --width 'width'
            cand -x 'x'
            cand --x 'x'
            cand --x-title 'x-title'
            cand -y 'y'
            cand --y 'y'
            cand --y-range 'y-range'
            cand --y-title 'y-title'
            cand -z 'z'
            cand --z 'z'
            cand --bivariate 'bivariate'
            cand --check-geojson-key 'check-geojson-key'
            cand --density 'density'
            cand --dict-info 'dict-info'
            cand --donut 'donut'
            cand --geocode 'geocode'
            cand --map 'map'
            cand -n 'n'
            cand --no-headers 'no-headers'
            cand --no-nulls 'no-nulls'
            cand --no-other 'no-other'
            cand --no-snap 'no-snap'
            cand --open 'open'
            cand --photos 'photos'
            cand --rangeslider 'rangeslider'
            cand --sankey-value-order 'sankey-value-order'
            cand --slider-cumulative 'slider-cumulative'
            cand --smarter 'smarter'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;viz;histogram'= {
            cand --agg 'agg'
            cand --annotation 'annotation'
            cand --bins 'bins'
            cand --box-points 'box-points'
            cand --close 'close'
            cand --cluster 'cluster'
            cand --color 'color'
            cand --color-scale 'color-scale'
            cand --cols 'cols'
            cand --dataset-pid 'dataset-pid'
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand --denominator 'denominator'
            cand --denominator-key 'denominator-key'
            cand --denominator-unit 'denominator-unit'
            cand --dictionary 'dictionary'
            cand --dictionary-context 'dictionary-context'
            cand --feature-id-key 'feature-id-key'
            cand --feature-name-key 'feature-name-key'
            cand --geocode-admin1 'geocode-admin1'
            cand --geocode-country 'geocode-country'
            cand --geojson 'geojson'
            cand --grid-cols 'grid-cols'
            cand --heatmap-density 'heatmap-density'
            cand --height 'height'
            cand --hierarchy-style 'hierarchy-style'
            cand --high 'high'
            cand --language 'language'
            cand --lat 'lat'
            cand --limit 'limit'
            cand --location-mode 'location-mode'
            cand --locations 'locations'
            cand --log-scale 'log-scale'
            cand --lon 'lon'
            cand --low 'low'
            cand --max-charts 'max-charts'
            cand --ohlc-open 'ohlc-open'
            cand -o 'o'
            cand --output 'output'
            cand --preview-threshold 'preview-threshold'
            cand --projection 'projection'
            cand --region-state 'region-state'
            cand --scale 'scale'
            cand --series 'series'
            cand --size 'size'
            cand --slider 'slider'
            cand --slider-speed 'slider-speed'
            cand --snap-max-dist 'snap-max-dist'
            cand --source 'source'
            cand --style 'style'
            cand --target 'target'
            cand --text 'text'
            cand --theme 'theme'
            cand --title 'title'
            cand --tour-audience 'tour-audience'
            cand --tour-steps 'tour-steps'
            cand --value 'value'
            cand --violin 'violin'
            cand --width 'width'
            cand -x 'x'
            cand --x 'x'
            cand --x-title 'x-title'
            cand -y 'y'
            cand --y 'y'
            cand --y-range 'y-range'
            cand --y-title 'y-title'
            cand -z 'z'
            cand --z 'z'
            cand --bivariate 'bivariate'
            cand --check-geojson-key 'check-geojson-key'
            cand --density 'density'
            cand --dict-info 'dict-info'
            cand --donut 'donut'
            cand --geocode 'geocode'
            cand --map 'map'
            cand -n 'n'
            cand --no-headers 'no-headers'
            cand --no-nulls 'no-nulls'
            cand --no-other 'no-other'
            cand --no-snap 'no-snap'
            cand --open 'open'
            cand --photos 'photos'
            cand --rangeslider 'rangeslider'
            cand --sankey-value-order 'sankey-value-order'
            cand --slider-cumulative 'slider-cumulative'
            cand --smarter 'smarter'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;viz;icicle'= {
            cand --agg 'agg'
            cand --annotation 'annotation'
            cand --bins 'bins'
            cand --box-points 'box-points'
            cand --close 'close'
            cand --cluster 'cluster'
            cand --color 'color'
            cand --color-scale 'color-scale'
            cand --cols 'cols'
            cand --dataset-pid 'dataset-pid'
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand --denominator 'denominator'
            cand --denominator-key 'denominator-key'
            cand --denominator-unit 'denominator-unit'
            cand --dictionary 'dictionary'
            cand --dictionary-context 'dictionary-context'
            cand --feature-id-key 'feature-id-key'
            cand --feature-name-key 'feature-name-key'
            cand --geocode-admin1 'geocode-admin1'
            cand --geocode-country 'geocode-country'
            cand --geojson 'geojson'
            cand --grid-cols 'grid-cols'
            cand --heatmap-density 'heatmap-density'
            cand --height 'height'
            cand --hierarchy-style 'hierarchy-style'
            cand --high 'high'
            cand --language 'language'
            cand --lat 'lat'
            cand --limit 'limit'
            cand --location-mode 'location-mode'
            cand --locations 'locations'
            cand --log-scale 'log-scale'
            cand --lon 'lon'
            cand --low 'low'
            cand --max-charts 'max-charts'
            cand --ohlc-open 'ohlc-open'
            cand -o 'o'
            cand --output 'output'
            cand --preview-threshold 'preview-threshold'
            cand --projection 'projection'
            cand --region-state 'region-state'
            cand --scale 'scale'
            cand --series 'series'
            cand --size 'size'
            cand --slider 'slider'
            cand --slider-speed 'slider-speed'
            cand --snap-max-dist 'snap-max-dist'
            cand --source 'source'
            cand --style 'style'
            cand --target 'target'
            cand --text 'text'
            cand --theme 'theme'
            cand --title 'title'
            cand --tour-audience 'tour-audience'
            cand --tour-steps 'tour-steps'
            cand --value 'value'
            cand --violin 'violin'
            cand --width 'width'
            cand -x 'x'
            cand --x 'x'
            cand --x-title 'x-title'
            cand -y 'y'
            cand --y 'y'
            cand --y-range 'y-range'
            cand --y-title 'y-title'
            cand -z 'z'
            cand --z 'z'
            cand --bivariate 'bivariate'
            cand --check-geojson-key 'check-geojson-key'
            cand --density 'density'
            cand --dict-info 'dict-info'
            cand --donut 'donut'
            cand --geocode 'geocode'
            cand --map 'map'
            cand -n 'n'
            cand --no-headers 'no-headers'
            cand --no-nulls 'no-nulls'
            cand --no-other 'no-other'
            cand --no-snap 'no-snap'
            cand --open 'open'
            cand --photos 'photos'
            cand --rangeslider 'rangeslider'
            cand --sankey-value-order 'sankey-value-order'
            cand --slider-cumulative 'slider-cumulative'
            cand --smarter 'smarter'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;viz;line'= {
            cand --agg 'agg'
            cand --annotation 'annotation'
            cand --bins 'bins'
            cand --box-points 'box-points'
            cand --close 'close'
            cand --cluster 'cluster'
            cand --color 'color'
            cand --color-scale 'color-scale'
            cand --cols 'cols'
            cand --dataset-pid 'dataset-pid'
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand --denominator 'denominator'
            cand --denominator-key 'denominator-key'
            cand --denominator-unit 'denominator-unit'
            cand --dictionary 'dictionary'
            cand --dictionary-context 'dictionary-context'
            cand --feature-id-key 'feature-id-key'
            cand --feature-name-key 'feature-name-key'
            cand --geocode-admin1 'geocode-admin1'
            cand --geocode-country 'geocode-country'
            cand --geojson 'geojson'
            cand --grid-cols 'grid-cols'
            cand --heatmap-density 'heatmap-density'
            cand --height 'height'
            cand --hierarchy-style 'hierarchy-style'
            cand --high 'high'
            cand --language 'language'
            cand --lat 'lat'
            cand --limit 'limit'
            cand --location-mode 'location-mode'
            cand --locations 'locations'
            cand --log-scale 'log-scale'
            cand --lon 'lon'
            cand --low 'low'
            cand --max-charts 'max-charts'
            cand --ohlc-open 'ohlc-open'
            cand -o 'o'
            cand --output 'output'
            cand --preview-threshold 'preview-threshold'
            cand --projection 'projection'
            cand --region-state 'region-state'
            cand --scale 'scale'
            cand --series 'series'
            cand --size 'size'
            cand --slider 'slider'
            cand --slider-speed 'slider-speed'
            cand --snap-max-dist 'snap-max-dist'
            cand --source 'source'
            cand --style 'style'
            cand --target 'target'
            cand --text 'text'
            cand --theme 'theme'
            cand --title 'title'
            cand --tour-audience 'tour-audience'
            cand --tour-steps 'tour-steps'
            cand --value 'value'
            cand --violin 'violin'
            cand --width 'width'
            cand -x 'x'
            cand --x 'x'
            cand --x-title 'x-title'
            cand -y 'y'
            cand --y 'y'
            cand --y-range 'y-range'
            cand --y-title 'y-title'
            cand -z 'z'
            cand --z 'z'
            cand --bivariate 'bivariate'
            cand --check-geojson-key 'check-geojson-key'
            cand --density 'density'
            cand --dict-info 'dict-info'
            cand --donut 'donut'
            cand --geocode 'geocode'
            cand --map 'map'
            cand -n 'n'
            cand --no-headers 'no-headers'
            cand --no-nulls 'no-nulls'
            cand --no-other 'no-other'
            cand --no-snap 'no-snap'
            cand --open 'open'
            cand --photos 'photos'
            cand --rangeslider 'rangeslider'
            cand --sankey-value-order 'sankey-value-order'
            cand --slider-cumulative 'slider-cumulative'
            cand --smarter 'smarter'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;viz;map'= {
            cand --agg 'agg'
            cand --annotation 'annotation'
            cand --bins 'bins'
            cand --box-points 'box-points'
            cand --close 'close'
            cand --cluster 'cluster'
            cand --color 'color'
            cand --color-scale 'color-scale'
            cand --cols 'cols'
            cand --dataset-pid 'dataset-pid'
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand --denominator 'denominator'
            cand --denominator-key 'denominator-key'
            cand --denominator-unit 'denominator-unit'
            cand --dictionary 'dictionary'
            cand --dictionary-context 'dictionary-context'
            cand --feature-id-key 'feature-id-key'
            cand --feature-name-key 'feature-name-key'
            cand --geocode-admin1 'geocode-admin1'
            cand --geocode-country 'geocode-country'
            cand --geojson 'geojson'
            cand --grid-cols 'grid-cols'
            cand --heatmap-density 'heatmap-density'
            cand --height 'height'
            cand --hierarchy-style 'hierarchy-style'
            cand --high 'high'
            cand --language 'language'
            cand --lat 'lat'
            cand --limit 'limit'
            cand --location-mode 'location-mode'
            cand --locations 'locations'
            cand --log-scale 'log-scale'
            cand --lon 'lon'
            cand --low 'low'
            cand --max-charts 'max-charts'
            cand --ohlc-open 'ohlc-open'
            cand -o 'o'
            cand --output 'output'
            cand --preview-threshold 'preview-threshold'
            cand --projection 'projection'
            cand --region-state 'region-state'
            cand --scale 'scale'
            cand --series 'series'
            cand --size 'size'
            cand --slider 'slider'
            cand --slider-speed 'slider-speed'
            cand --snap-max-dist 'snap-max-dist'
            cand --source 'source'
            cand --style 'style'
            cand --target 'target'
            cand --text 'text'
            cand --theme 'theme'
            cand --title 'title'
            cand --tour-audience 'tour-audience'
            cand --tour-steps 'tour-steps'
            cand --value 'value'
            cand --violin 'violin'
            cand --width 'width'
            cand -x 'x'
            cand --x 'x'
            cand --x-title 'x-title'
            cand -y 'y'
            cand --y 'y'
            cand --y-range 'y-range'
            cand --y-title 'y-title'
            cand -z 'z'
            cand --z 'z'
            cand --bivariate 'bivariate'
            cand --check-geojson-key 'check-geojson-key'
            cand --density 'density'
            cand --dict-info 'dict-info'
            cand --donut 'donut'
            cand --geocode 'geocode'
            cand --map 'map'
            cand -n 'n'
            cand --no-headers 'no-headers'
            cand --no-nulls 'no-nulls'
            cand --no-other 'no-other'
            cand --no-snap 'no-snap'
            cand --open 'open'
            cand --photos 'photos'
            cand --rangeslider 'rangeslider'
            cand --sankey-value-order 'sankey-value-order'
            cand --slider-cumulative 'slider-cumulative'
            cand --smarter 'smarter'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;viz;ohlc'= {
            cand --agg 'agg'
            cand --annotation 'annotation'
            cand --bins 'bins'
            cand --box-points 'box-points'
            cand --close 'close'
            cand --cluster 'cluster'
            cand --color 'color'
            cand --color-scale 'color-scale'
            cand --cols 'cols'
            cand --dataset-pid 'dataset-pid'
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand --denominator 'denominator'
            cand --denominator-key 'denominator-key'
            cand --denominator-unit 'denominator-unit'
            cand --dictionary 'dictionary'
            cand --dictionary-context 'dictionary-context'
            cand --feature-id-key 'feature-id-key'
            cand --feature-name-key 'feature-name-key'
            cand --geocode-admin1 'geocode-admin1'
            cand --geocode-country 'geocode-country'
            cand --geojson 'geojson'
            cand --grid-cols 'grid-cols'
            cand --heatmap-density 'heatmap-density'
            cand --height 'height'
            cand --hierarchy-style 'hierarchy-style'
            cand --high 'high'
            cand --language 'language'
            cand --lat 'lat'
            cand --limit 'limit'
            cand --location-mode 'location-mode'
            cand --locations 'locations'
            cand --log-scale 'log-scale'
            cand --lon 'lon'
            cand --low 'low'
            cand --max-charts 'max-charts'
            cand --ohlc-open 'ohlc-open'
            cand -o 'o'
            cand --output 'output'
            cand --preview-threshold 'preview-threshold'
            cand --projection 'projection'
            cand --region-state 'region-state'
            cand --scale 'scale'
            cand --series 'series'
            cand --size 'size'
            cand --slider 'slider'
            cand --slider-speed 'slider-speed'
            cand --snap-max-dist 'snap-max-dist'
            cand --source 'source'
            cand --style 'style'
            cand --target 'target'
            cand --text 'text'
            cand --theme 'theme'
            cand --title 'title'
            cand --tour-audience 'tour-audience'
            cand --tour-steps 'tour-steps'
            cand --value 'value'
            cand --violin 'violin'
            cand --width 'width'
            cand -x 'x'
            cand --x 'x'
            cand --x-title 'x-title'
            cand -y 'y'
            cand --y 'y'
            cand --y-range 'y-range'
            cand --y-title 'y-title'
            cand -z 'z'
            cand --z 'z'
            cand --bivariate 'bivariate'
            cand --check-geojson-key 'check-geojson-key'
            cand --density 'density'
            cand --dict-info 'dict-info'
            cand --donut 'donut'
            cand --geocode 'geocode'
            cand --map 'map'
            cand -n 'n'
            cand --no-headers 'no-headers'
            cand --no-nulls 'no-nulls'
            cand --no-other 'no-other'
            cand --no-snap 'no-snap'
            cand --open 'open'
            cand --photos 'photos'
            cand --rangeslider 'rangeslider'
            cand --sankey-value-order 'sankey-value-order'
            cand --slider-cumulative 'slider-cumulative'
            cand --smarter 'smarter'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;viz;parcats'= {
            cand --agg 'agg'
            cand --annotation 'annotation'
            cand --bins 'bins'
            cand --box-points 'box-points'
            cand --close 'close'
            cand --cluster 'cluster'
            cand --color 'color'
            cand --color-scale 'color-scale'
            cand --cols 'cols'
            cand --dataset-pid 'dataset-pid'
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand --denominator 'denominator'
            cand --denominator-key 'denominator-key'
            cand --denominator-unit 'denominator-unit'
            cand --dictionary 'dictionary'
            cand --dictionary-context 'dictionary-context'
            cand --feature-id-key 'feature-id-key'
            cand --feature-name-key 'feature-name-key'
            cand --geocode-admin1 'geocode-admin1'
            cand --geocode-country 'geocode-country'
            cand --geojson 'geojson'
            cand --grid-cols 'grid-cols'
            cand --heatmap-density 'heatmap-density'
            cand --height 'height'
            cand --hierarchy-style 'hierarchy-style'
            cand --high 'high'
            cand --language 'language'
            cand --lat 'lat'
            cand --limit 'limit'
            cand --location-mode 'location-mode'
            cand --locations 'locations'
            cand --log-scale 'log-scale'
            cand --lon 'lon'
            cand --low 'low'
            cand --max-charts 'max-charts'
            cand --ohlc-open 'ohlc-open'
            cand -o 'o'
            cand --output 'output'
            cand --preview-threshold 'preview-threshold'
            cand --projection 'projection'
            cand --region-state 'region-state'
            cand --scale 'scale'
            cand --series 'series'
            cand --size 'size'
            cand --slider 'slider'
            cand --slider-speed 'slider-speed'
            cand --snap-max-dist 'snap-max-dist'
            cand --source 'source'
            cand --style 'style'
            cand --target 'target'
            cand --text 'text'
            cand --theme 'theme'
            cand --title 'title'
            cand --tour-audience 'tour-audience'
            cand --tour-steps 'tour-steps'
            cand --value 'value'
            cand --violin 'violin'
            cand --width 'width'
            cand -x 'x'
            cand --x 'x'
            cand --x-title 'x-title'
            cand -y 'y'
            cand --y 'y'
            cand --y-range 'y-range'
            cand --y-title 'y-title'
            cand -z 'z'
            cand --z 'z'
            cand --bivariate 'bivariate'
            cand --check-geojson-key 'check-geojson-key'
            cand --density 'density'
            cand --dict-info 'dict-info'
            cand --donut 'donut'
            cand --geocode 'geocode'
            cand --map 'map'
            cand -n 'n'
            cand --no-headers 'no-headers'
            cand --no-nulls 'no-nulls'
            cand --no-other 'no-other'
            cand --no-snap 'no-snap'
            cand --open 'open'
            cand --photos 'photos'
            cand --rangeslider 'rangeslider'
            cand --sankey-value-order 'sankey-value-order'
            cand --slider-cumulative 'slider-cumulative'
            cand --smarter 'smarter'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;viz;pie'= {
            cand --agg 'agg'
            cand --annotation 'annotation'
            cand --bins 'bins'
            cand --box-points 'box-points'
            cand --close 'close'
            cand --cluster 'cluster'
            cand --color 'color'
            cand --color-scale 'color-scale'
            cand --cols 'cols'
            cand --dataset-pid 'dataset-pid'
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand --denominator 'denominator'
            cand --denominator-key 'denominator-key'
            cand --denominator-unit 'denominator-unit'
            cand --dictionary 'dictionary'
            cand --dictionary-context 'dictionary-context'
            cand --feature-id-key 'feature-id-key'
            cand --feature-name-key 'feature-name-key'
            cand --geocode-admin1 'geocode-admin1'
            cand --geocode-country 'geocode-country'
            cand --geojson 'geojson'
            cand --grid-cols 'grid-cols'
            cand --heatmap-density 'heatmap-density'
            cand --height 'height'
            cand --hierarchy-style 'hierarchy-style'
            cand --high 'high'
            cand --language 'language'
            cand --lat 'lat'
            cand --limit 'limit'
            cand --location-mode 'location-mode'
            cand --locations 'locations'
            cand --log-scale 'log-scale'
            cand --lon 'lon'
            cand --low 'low'
            cand --max-charts 'max-charts'
            cand --ohlc-open 'ohlc-open'
            cand -o 'o'
            cand --output 'output'
            cand --preview-threshold 'preview-threshold'
            cand --projection 'projection'
            cand --region-state 'region-state'
            cand --scale 'scale'
            cand --series 'series'
            cand --size 'size'
            cand --slider 'slider'
            cand --slider-speed 'slider-speed'
            cand --snap-max-dist 'snap-max-dist'
            cand --source 'source'
            cand --style 'style'
            cand --target 'target'
            cand --text 'text'
            cand --theme 'theme'
            cand --title 'title'
            cand --tour-audience 'tour-audience'
            cand --tour-steps 'tour-steps'
            cand --value 'value'
            cand --violin 'violin'
            cand --width 'width'
            cand -x 'x'
            cand --x 'x'
            cand --x-title 'x-title'
            cand -y 'y'
            cand --y 'y'
            cand --y-range 'y-range'
            cand --y-title 'y-title'
            cand -z 'z'
            cand --z 'z'
            cand --bivariate 'bivariate'
            cand --check-geojson-key 'check-geojson-key'
            cand --density 'density'
            cand --dict-info 'dict-info'
            cand --donut 'donut'
            cand --geocode 'geocode'
            cand --map 'map'
            cand -n 'n'
            cand --no-headers 'no-headers'
            cand --no-nulls 'no-nulls'
            cand --no-other 'no-other'
            cand --no-snap 'no-snap'
            cand --open 'open'
            cand --photos 'photos'
            cand --rangeslider 'rangeslider'
            cand --sankey-value-order 'sankey-value-order'
            cand --slider-cumulative 'slider-cumulative'
            cand --smarter 'smarter'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;viz;radar'= {
            cand --agg 'agg'
            cand --annotation 'annotation'
            cand --bins 'bins'
            cand --box-points 'box-points'
            cand --close 'close'
            cand --cluster 'cluster'
            cand --color 'color'
            cand --color-scale 'color-scale'
            cand --cols 'cols'
            cand --dataset-pid 'dataset-pid'
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand --denominator 'denominator'
            cand --denominator-key 'denominator-key'
            cand --denominator-unit 'denominator-unit'
            cand --dictionary 'dictionary'
            cand --dictionary-context 'dictionary-context'
            cand --feature-id-key 'feature-id-key'
            cand --feature-name-key 'feature-name-key'
            cand --geocode-admin1 'geocode-admin1'
            cand --geocode-country 'geocode-country'
            cand --geojson 'geojson'
            cand --grid-cols 'grid-cols'
            cand --heatmap-density 'heatmap-density'
            cand --height 'height'
            cand --hierarchy-style 'hierarchy-style'
            cand --high 'high'
            cand --language 'language'
            cand --lat 'lat'
            cand --limit 'limit'
            cand --location-mode 'location-mode'
            cand --locations 'locations'
            cand --log-scale 'log-scale'
            cand --lon 'lon'
            cand --low 'low'
            cand --max-charts 'max-charts'
            cand --ohlc-open 'ohlc-open'
            cand -o 'o'
            cand --output 'output'
            cand --preview-threshold 'preview-threshold'
            cand --projection 'projection'
            cand --region-state 'region-state'
            cand --scale 'scale'
            cand --series 'series'
            cand --size 'size'
            cand --slider 'slider'
            cand --slider-speed 'slider-speed'
            cand --snap-max-dist 'snap-max-dist'
            cand --source 'source'
            cand --style 'style'
            cand --target 'target'
            cand --text 'text'
            cand --theme 'theme'
            cand --title 'title'
            cand --tour-audience 'tour-audience'
            cand --tour-steps 'tour-steps'
            cand --value 'value'
            cand --violin 'violin'
            cand --width 'width'
            cand -x 'x'
            cand --x 'x'
            cand --x-title 'x-title'
            cand -y 'y'
            cand --y 'y'
            cand --y-range 'y-range'
            cand --y-title 'y-title'
            cand -z 'z'
            cand --z 'z'
            cand --bivariate 'bivariate'
            cand --check-geojson-key 'check-geojson-key'
            cand --density 'density'
            cand --dict-info 'dict-info'
            cand --donut 'donut'
            cand --geocode 'geocode'
            cand --map 'map'
            cand -n 'n'
            cand --no-headers 'no-headers'
            cand --no-nulls 'no-nulls'
            cand --no-other 'no-other'
            cand --no-snap 'no-snap'
            cand --open 'open'
            cand --photos 'photos'
            cand --rangeslider 'rangeslider'
            cand --sankey-value-order 'sankey-value-order'
            cand --slider-cumulative 'slider-cumulative'
            cand --smarter 'smarter'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;viz;sankey'= {
            cand --agg 'agg'
            cand --annotation 'annotation'
            cand --bins 'bins'
            cand --box-points 'box-points'
            cand --close 'close'
            cand --cluster 'cluster'
            cand --color 'color'
            cand --color-scale 'color-scale'
            cand --cols 'cols'
            cand --dataset-pid 'dataset-pid'
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand --denominator 'denominator'
            cand --denominator-key 'denominator-key'
            cand --denominator-unit 'denominator-unit'
            cand --dictionary 'dictionary'
            cand --dictionary-context 'dictionary-context'
            cand --feature-id-key 'feature-id-key'
            cand --feature-name-key 'feature-name-key'
            cand --geocode-admin1 'geocode-admin1'
            cand --geocode-country 'geocode-country'
            cand --geojson 'geojson'
            cand --grid-cols 'grid-cols'
            cand --heatmap-density 'heatmap-density'
            cand --height 'height'
            cand --hierarchy-style 'hierarchy-style'
            cand --high 'high'
            cand --language 'language'
            cand --lat 'lat'
            cand --limit 'limit'
            cand --location-mode 'location-mode'
            cand --locations 'locations'
            cand --log-scale 'log-scale'
            cand --lon 'lon'
            cand --low 'low'
            cand --max-charts 'max-charts'
            cand --ohlc-open 'ohlc-open'
            cand -o 'o'
            cand --output 'output'
            cand --preview-threshold 'preview-threshold'
            cand --projection 'projection'
            cand --region-state 'region-state'
            cand --scale 'scale'
            cand --series 'series'
            cand --size 'size'
            cand --slider 'slider'
            cand --slider-speed 'slider-speed'
            cand --snap-max-dist 'snap-max-dist'
            cand --source 'source'
            cand --style 'style'
            cand --target 'target'
            cand --text 'text'
            cand --theme 'theme'
            cand --title 'title'
            cand --tour-audience 'tour-audience'
            cand --tour-steps 'tour-steps'
            cand --value 'value'
            cand --violin 'violin'
            cand --width 'width'
            cand -x 'x'
            cand --x 'x'
            cand --x-title 'x-title'
            cand -y 'y'
            cand --y 'y'
            cand --y-range 'y-range'
            cand --y-title 'y-title'
            cand -z 'z'
            cand --z 'z'
            cand --bivariate 'bivariate'
            cand --check-geojson-key 'check-geojson-key'
            cand --density 'density'
            cand --dict-info 'dict-info'
            cand --donut 'donut'
            cand --geocode 'geocode'
            cand --map 'map'
            cand -n 'n'
            cand --no-headers 'no-headers'
            cand --no-nulls 'no-nulls'
            cand --no-other 'no-other'
            cand --no-snap 'no-snap'
            cand --open 'open'
            cand --photos 'photos'
            cand --rangeslider 'rangeslider'
            cand --sankey-value-order 'sankey-value-order'
            cand --slider-cumulative 'slider-cumulative'
            cand --smarter 'smarter'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;viz;scatter'= {
            cand --agg 'agg'
            cand --annotation 'annotation'
            cand --bins 'bins'
            cand --box-points 'box-points'
            cand --close 'close'
            cand --cluster 'cluster'
            cand --color 'color'
            cand --color-scale 'color-scale'
            cand --cols 'cols'
            cand --dataset-pid 'dataset-pid'
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand --denominator 'denominator'
            cand --denominator-key 'denominator-key'
            cand --denominator-unit 'denominator-unit'
            cand --dictionary 'dictionary'
            cand --dictionary-context 'dictionary-context'
            cand --feature-id-key 'feature-id-key'
            cand --feature-name-key 'feature-name-key'
            cand --geocode-admin1 'geocode-admin1'
            cand --geocode-country 'geocode-country'
            cand --geojson 'geojson'
            cand --grid-cols 'grid-cols'
            cand --heatmap-density 'heatmap-density'
            cand --height 'height'
            cand --hierarchy-style 'hierarchy-style'
            cand --high 'high'
            cand --language 'language'
            cand --lat 'lat'
            cand --limit 'limit'
            cand --location-mode 'location-mode'
            cand --locations 'locations'
            cand --log-scale 'log-scale'
            cand --lon 'lon'
            cand --low 'low'
            cand --max-charts 'max-charts'
            cand --ohlc-open 'ohlc-open'
            cand -o 'o'
            cand --output 'output'
            cand --preview-threshold 'preview-threshold'
            cand --projection 'projection'
            cand --region-state 'region-state'
            cand --scale 'scale'
            cand --series 'series'
            cand --size 'size'
            cand --slider 'slider'
            cand --slider-speed 'slider-speed'
            cand --snap-max-dist 'snap-max-dist'
            cand --source 'source'
            cand --style 'style'
            cand --target 'target'
            cand --text 'text'
            cand --theme 'theme'
            cand --title 'title'
            cand --tour-audience 'tour-audience'
            cand --tour-steps 'tour-steps'
            cand --value 'value'
            cand --violin 'violin'
            cand --width 'width'
            cand -x 'x'
            cand --x 'x'
            cand --x-title 'x-title'
            cand -y 'y'
            cand --y 'y'
            cand --y-range 'y-range'
            cand --y-title 'y-title'
            cand -z 'z'
            cand --z 'z'
            cand --bivariate 'bivariate'
            cand --check-geojson-key 'check-geojson-key'
            cand --density 'density'
            cand --dict-info 'dict-info'
            cand --donut 'donut'
            cand --geocode 'geocode'
            cand --map 'map'
            cand -n 'n'
            cand --no-headers 'no-headers'
            cand --no-nulls 'no-nulls'
            cand --no-other 'no-other'
            cand --no-snap 'no-snap'
            cand --open 'open'
            cand --photos 'photos'
            cand --rangeslider 'rangeslider'
            cand --sankey-value-order 'sankey-value-order'
            cand --slider-cumulative 'slider-cumulative'
            cand --smarter 'smarter'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;viz;scatter3d'= {
            cand --agg 'agg'
            cand --annotation 'annotation'
            cand --bins 'bins'
            cand --box-points 'box-points'
            cand --close 'close'
            cand --cluster 'cluster'
            cand --color 'color'
            cand --color-scale 'color-scale'
            cand --cols 'cols'
            cand --dataset-pid 'dataset-pid'
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand --denominator 'denominator'
            cand --denominator-key 'denominator-key'
            cand --denominator-unit 'denominator-unit'
            cand --dictionary 'dictionary'
            cand --dictionary-context 'dictionary-context'
            cand --feature-id-key 'feature-id-key'
            cand --feature-name-key 'feature-name-key'
            cand --geocode-admin1 'geocode-admin1'
            cand --geocode-country 'geocode-country'
            cand --geojson 'geojson'
            cand --grid-cols 'grid-cols'
            cand --heatmap-density 'heatmap-density'
            cand --height 'height'
            cand --hierarchy-style 'hierarchy-style'
            cand --high 'high'
            cand --language 'language'
            cand --lat 'lat'
            cand --limit 'limit'
            cand --location-mode 'location-mode'
            cand --locations 'locations'
            cand --log-scale 'log-scale'
            cand --lon 'lon'
            cand --low 'low'
            cand --max-charts 'max-charts'
            cand --ohlc-open 'ohlc-open'
            cand -o 'o'
            cand --output 'output'
            cand --preview-threshold 'preview-threshold'
            cand --projection 'projection'
            cand --region-state 'region-state'
            cand --scale 'scale'
            cand --series 'series'
            cand --size 'size'
            cand --slider 'slider'
            cand --slider-speed 'slider-speed'
            cand --snap-max-dist 'snap-max-dist'
            cand --source 'source'
            cand --style 'style'
            cand --target 'target'
            cand --text 'text'
            cand --theme 'theme'
            cand --title 'title'
            cand --tour-audience 'tour-audience'
            cand --tour-steps 'tour-steps'
            cand --value 'value'
            cand --violin 'violin'
            cand --width 'width'
            cand -x 'x'
            cand --x 'x'
            cand --x-title 'x-title'
            cand -y 'y'
            cand --y 'y'
            cand --y-range 'y-range'
            cand --y-title 'y-title'
            cand -z 'z'
            cand --z 'z'
            cand --bivariate 'bivariate'
            cand --check-geojson-key 'check-geojson-key'
            cand --density 'density'
            cand --dict-info 'dict-info'
            cand --donut 'donut'
            cand --geocode 'geocode'
            cand --map 'map'
            cand -n 'n'
            cand --no-headers 'no-headers'
            cand --no-nulls 'no-nulls'
            cand --no-other 'no-other'
            cand --no-snap 'no-snap'
            cand --open 'open'
            cand --photos 'photos'
            cand --rangeslider 'rangeslider'
            cand --sankey-value-order 'sankey-value-order'
            cand --slider-cumulative 'slider-cumulative'
            cand --smarter 'smarter'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;viz;smart'= {
            cand --agg 'agg'
            cand --annotation 'annotation'
            cand --bins 'bins'
            cand --box-points 'box-points'
            cand --close 'close'
            cand --cluster 'cluster'
            cand --color 'color'
            cand --color-scale 'color-scale'
            cand --cols 'cols'
            cand --dataset-pid 'dataset-pid'
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand --denominator 'denominator'
            cand --denominator-key 'denominator-key'
            cand --denominator-unit 'denominator-unit'
            cand --dictionary 'dictionary'
            cand --dictionary-context 'dictionary-context'
            cand --feature-id-key 'feature-id-key'
            cand --feature-name-key 'feature-name-key'
            cand --geocode-admin1 'geocode-admin1'
            cand --geocode-country 'geocode-country'
            cand --geojson 'geojson'
            cand --grid-cols 'grid-cols'
            cand --heatmap-density 'heatmap-density'
            cand --height 'height'
            cand --hierarchy-style 'hierarchy-style'
            cand --high 'high'
            cand --language 'language'
            cand --lat 'lat'
            cand --limit 'limit'
            cand --location-mode 'location-mode'
            cand --locations 'locations'
            cand --log-scale 'log-scale'
            cand --lon 'lon'
            cand --low 'low'
            cand --max-charts 'max-charts'
            cand --ohlc-open 'ohlc-open'
            cand -o 'o'
            cand --output 'output'
            cand --preview-threshold 'preview-threshold'
            cand --projection 'projection'
            cand --region-state 'region-state'
            cand --scale 'scale'
            cand --series 'series'
            cand --size 'size'
            cand --slider 'slider'
            cand --slider-speed 'slider-speed'
            cand --snap-max-dist 'snap-max-dist'
            cand --source 'source'
            cand --style 'style'
            cand --target 'target'
            cand --text 'text'
            cand --theme 'theme'
            cand --title 'title'
            cand --tour-audience 'tour-audience'
            cand --tour-steps 'tour-steps'
            cand --value 'value'
            cand --violin 'violin'
            cand --width 'width'
            cand -x 'x'
            cand --x 'x'
            cand --x-title 'x-title'
            cand -y 'y'
            cand --y 'y'
            cand --y-range 'y-range'
            cand --y-title 'y-title'
            cand -z 'z'
            cand --z 'z'
            cand --bivariate 'bivariate'
            cand --check-geojson-key 'check-geojson-key'
            cand --density 'density'
            cand --dict-info 'dict-info'
            cand --donut 'donut'
            cand --geocode 'geocode'
            cand --map 'map'
            cand -n 'n'
            cand --no-headers 'no-headers'
            cand --no-nulls 'no-nulls'
            cand --no-other 'no-other'
            cand --no-snap 'no-snap'
            cand --open 'open'
            cand --photos 'photos'
            cand --rangeslider 'rangeslider'
            cand --sankey-value-order 'sankey-value-order'
            cand --slider-cumulative 'slider-cumulative'
            cand --smarter 'smarter'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;viz;splom'= {
            cand --agg 'agg'
            cand --annotation 'annotation'
            cand --bins 'bins'
            cand --box-points 'box-points'
            cand --close 'close'
            cand --cluster 'cluster'
            cand --color 'color'
            cand --color-scale 'color-scale'
            cand --cols 'cols'
            cand --dataset-pid 'dataset-pid'
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand --denominator 'denominator'
            cand --denominator-key 'denominator-key'
            cand --denominator-unit 'denominator-unit'
            cand --dictionary 'dictionary'
            cand --dictionary-context 'dictionary-context'
            cand --feature-id-key 'feature-id-key'
            cand --feature-name-key 'feature-name-key'
            cand --geocode-admin1 'geocode-admin1'
            cand --geocode-country 'geocode-country'
            cand --geojson 'geojson'
            cand --grid-cols 'grid-cols'
            cand --heatmap-density 'heatmap-density'
            cand --height 'height'
            cand --hierarchy-style 'hierarchy-style'
            cand --high 'high'
            cand --language 'language'
            cand --lat 'lat'
            cand --limit 'limit'
            cand --location-mode 'location-mode'
            cand --locations 'locations'
            cand --log-scale 'log-scale'
            cand --lon 'lon'
            cand --low 'low'
            cand --max-charts 'max-charts'
            cand --ohlc-open 'ohlc-open'
            cand -o 'o'
            cand --output 'output'
            cand --preview-threshold 'preview-threshold'
            cand --projection 'projection'
            cand --region-state 'region-state'
            cand --scale 'scale'
            cand --series 'series'
            cand --size 'size'
            cand --slider 'slider'
            cand --slider-speed 'slider-speed'
            cand --snap-max-dist 'snap-max-dist'
            cand --source 'source'
            cand --style 'style'
            cand --target 'target'
            cand --text 'text'
            cand --theme 'theme'
            cand --title 'title'
            cand --tour-audience 'tour-audience'
            cand --tour-steps 'tour-steps'
            cand --value 'value'
            cand --violin 'violin'
            cand --width 'width'
            cand -x 'x'
            cand --x 'x'
            cand --x-title 'x-title'
            cand -y 'y'
            cand --y 'y'
            cand --y-range 'y-range'
            cand --y-title 'y-title'
            cand -z 'z'
            cand --z 'z'
            cand --bivariate 'bivariate'
            cand --check-geojson-key 'check-geojson-key'
            cand --density 'density'
            cand --dict-info 'dict-info'
            cand --donut 'donut'
            cand --geocode 'geocode'
            cand --map 'map'
            cand -n 'n'
            cand --no-headers 'no-headers'
            cand --no-nulls 'no-nulls'
            cand --no-other 'no-other'
            cand --no-snap 'no-snap'
            cand --open 'open'
            cand --photos 'photos'
            cand --rangeslider 'rangeslider'
            cand --sankey-value-order 'sankey-value-order'
            cand --slider-cumulative 'slider-cumulative'
            cand --smarter 'smarter'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;viz;sunburst'= {
            cand --agg 'agg'
            cand --annotation 'annotation'
            cand --bins 'bins'
            cand --box-points 'box-points'
            cand --close 'close'
            cand --cluster 'cluster'
            cand --color 'color'
            cand --color-scale 'color-scale'
            cand --cols 'cols'
            cand --dataset-pid 'dataset-pid'
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand --denominator 'denominator'
            cand --denominator-key 'denominator-key'
            cand --denominator-unit 'denominator-unit'
            cand --dictionary 'dictionary'
            cand --dictionary-context 'dictionary-context'
            cand --feature-id-key 'feature-id-key'
            cand --feature-name-key 'feature-name-key'
            cand --geocode-admin1 'geocode-admin1'
            cand --geocode-country 'geocode-country'
            cand --geojson 'geojson'
            cand --grid-cols 'grid-cols'
            cand --heatmap-density 'heatmap-density'
            cand --height 'height'
            cand --hierarchy-style 'hierarchy-style'
            cand --high 'high'
            cand --language 'language'
            cand --lat 'lat'
            cand --limit 'limit'
            cand --location-mode 'location-mode'
            cand --locations 'locations'
            cand --log-scale 'log-scale'
            cand --lon 'lon'
            cand --low 'low'
            cand --max-charts 'max-charts'
            cand --ohlc-open 'ohlc-open'
            cand -o 'o'
            cand --output 'output'
            cand --preview-threshold 'preview-threshold'
            cand --projection 'projection'
            cand --region-state 'region-state'
            cand --scale 'scale'
            cand --series 'series'
            cand --size 'size'
            cand --slider 'slider'
            cand --slider-speed 'slider-speed'
            cand --snap-max-dist 'snap-max-dist'
            cand --source 'source'
            cand --style 'style'
            cand --target 'target'
            cand --text 'text'
            cand --theme 'theme'
            cand --title 'title'
            cand --tour-audience 'tour-audience'
            cand --tour-steps 'tour-steps'
            cand --value 'value'
            cand --violin 'violin'
            cand --width 'width'
            cand -x 'x'
            cand --x 'x'
            cand --x-title 'x-title'
            cand -y 'y'
            cand --y 'y'
            cand --y-range 'y-range'
            cand --y-title 'y-title'
            cand -z 'z'
            cand --z 'z'
            cand --bivariate 'bivariate'
            cand --check-geojson-key 'check-geojson-key'
            cand --density 'density'
            cand --dict-info 'dict-info'
            cand --donut 'donut'
            cand --geocode 'geocode'
            cand --map 'map'
            cand -n 'n'
            cand --no-headers 'no-headers'
            cand --no-nulls 'no-nulls'
            cand --no-other 'no-other'
            cand --no-snap 'no-snap'
            cand --open 'open'
            cand --photos 'photos'
            cand --rangeslider 'rangeslider'
            cand --sankey-value-order 'sankey-value-order'
            cand --slider-cumulative 'slider-cumulative'
            cand --smarter 'smarter'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;viz;treemap'= {
            cand --agg 'agg'
            cand --annotation 'annotation'
            cand --bins 'bins'
            cand --box-points 'box-points'
            cand --close 'close'
            cand --cluster 'cluster'
            cand --color 'color'
            cand --color-scale 'color-scale'
            cand --cols 'cols'
            cand --dataset-pid 'dataset-pid'
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand --denominator 'denominator'
            cand --denominator-key 'denominator-key'
            cand --denominator-unit 'denominator-unit'
            cand --dictionary 'dictionary'
            cand --dictionary-context 'dictionary-context'
            cand --feature-id-key 'feature-id-key'
            cand --feature-name-key 'feature-name-key'
            cand --geocode-admin1 'geocode-admin1'
            cand --geocode-country 'geocode-country'
            cand --geojson 'geojson'
            cand --grid-cols 'grid-cols'
            cand --heatmap-density 'heatmap-density'
            cand --height 'height'
            cand --hierarchy-style 'hierarchy-style'
            cand --high 'high'
            cand --language 'language'
            cand --lat 'lat'
            cand --limit 'limit'
            cand --location-mode 'location-mode'
            cand --locations 'locations'
            cand --log-scale 'log-scale'
            cand --lon 'lon'
            cand --low 'low'
            cand --max-charts 'max-charts'
            cand --ohlc-open 'ohlc-open'
            cand -o 'o'
            cand --output 'output'
            cand --preview-threshold 'preview-threshold'
            cand --projection 'projection'
            cand --region-state 'region-state'
            cand --scale 'scale'
            cand --series 'series'
            cand --size 'size'
            cand --slider 'slider'
            cand --slider-speed 'slider-speed'
            cand --snap-max-dist 'snap-max-dist'
            cand --source 'source'
            cand --style 'style'
            cand --target 'target'
            cand --text 'text'
            cand --theme 'theme'
            cand --title 'title'
            cand --tour-audience 'tour-audience'
            cand --tour-steps 'tour-steps'
            cand --value 'value'
            cand --violin 'violin'
            cand --width 'width'
            cand -x 'x'
            cand --x 'x'
            cand --x-title 'x-title'
            cand -y 'y'
            cand --y 'y'
            cand --y-range 'y-range'
            cand --y-title 'y-title'
            cand -z 'z'
            cand --z 'z'
            cand --bivariate 'bivariate'
            cand --check-geojson-key 'check-geojson-key'
            cand --density 'density'
            cand --dict-info 'dict-info'
            cand --donut 'donut'
            cand --geocode 'geocode'
            cand --map 'map'
            cand -n 'n'
            cand --no-headers 'no-headers'
            cand --no-nulls 'no-nulls'
            cand --no-other 'no-other'
            cand --no-snap 'no-snap'
            cand --open 'open'
            cand --photos 'photos'
            cand --rangeslider 'rangeslider'
            cand --sankey-value-order 'sankey-value-order'
            cand --slider-cumulative 'slider-cumulative'
            cand --smarter 'smarter'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;viz;violin'= {
            cand --agg 'agg'
            cand --annotation 'annotation'
            cand --bins 'bins'
            cand --box-points 'box-points'
            cand --close 'close'
            cand --cluster 'cluster'
            cand --color 'color'
            cand --color-scale 'color-scale'
            cand --cols 'cols'
            cand --dataset-pid 'dataset-pid'
            cand -d 'd'
            cand --delimiter 'delimiter'
            cand --denominator 'denominator'
            cand --denominator-key 'denominator-key'
            cand --denominator-unit 'denominator-unit'
            cand --dictionary 'dictionary'
            cand --dictionary-context 'dictionary-context'
            cand --feature-id-key 'feature-id-key'
            cand --feature-name-key 'feature-name-key'
            cand --geocode-admin1 'geocode-admin1'
            cand --geocode-country 'geocode-country'
            cand --geojson 'geojson'
            cand --grid-cols 'grid-cols'
            cand --heatmap-density 'heatmap-density'
            cand --height 'height'
            cand --hierarchy-style 'hierarchy-style'
            cand --high 'high'
            cand --language 'language'
            cand --lat 'lat'
            cand --limit 'limit'
            cand --location-mode 'location-mode'
            cand --locations 'locations'
            cand --log-scale 'log-scale'
            cand --lon 'lon'
            cand --low 'low'
            cand --max-charts 'max-charts'
            cand --ohlc-open 'ohlc-open'
            cand -o 'o'
            cand --output 'output'
            cand --preview-threshold 'preview-threshold'
            cand --projection 'projection'
            cand --region-state 'region-state'
            cand --scale 'scale'
            cand --series 'series'
            cand --size 'size'
            cand --slider 'slider'
            cand --slider-speed 'slider-speed'
            cand --snap-max-dist 'snap-max-dist'
            cand --source 'source'
            cand --style 'style'
            cand --target 'target'
            cand --text 'text'
            cand --theme 'theme'
            cand --title 'title'
            cand --tour-audience 'tour-audience'
            cand --tour-steps 'tour-steps'
            cand --value 'value'
            cand --violin 'violin'
            cand --width 'width'
            cand -x 'x'
            cand --x 'x'
            cand --x-title 'x-title'
            cand -y 'y'
            cand --y 'y'
            cand --y-range 'y-range'
            cand --y-title 'y-title'
            cand -z 'z'
            cand --z 'z'
            cand --bivariate 'bivariate'
            cand --check-geojson-key 'check-geojson-key'
            cand --density 'density'
            cand --dict-info 'dict-info'
            cand --donut 'donut'
            cand --geocode 'geocode'
            cand --map 'map'
            cand -n 'n'
            cand --no-headers 'no-headers'
            cand --no-nulls 'no-nulls'
            cand --no-other 'no-other'
            cand --no-snap 'no-snap'
            cand --open 'open'
            cand --photos 'photos'
            cand --rangeslider 'rangeslider'
            cand --sankey-value-order 'sankey-value-order'
            cand --slider-cumulative 'slider-cumulative'
            cand --smarter 'smarter'
            cand -h 'Print help'
            cand --help 'Print help'
        }
        &'qsv;viz;help'= {
            cand bar 'bar'
            cand box 'box'
            cand candlestick 'candlestick'
            cand choropleth 'choropleth'
            cand contour 'contour'
            cand funnel 'funnel'
            cand geo 'geo'
            cand heatmap 'heatmap'
            cand histogram 'histogram'
            cand icicle 'icicle'
            cand line 'line'
            cand map 'map'
            cand ohlc 'ohlc'
            cand parcats 'parcats'
            cand pie 'pie'
            cand radar 'radar'
            cand sankey 'sankey'
            cand scatter 'scatter'
            cand scatter3d 'scatter3d'
            cand smart 'smart'
            cand splom 'splom'
            cand sunburst 'sunburst'
            cand treemap 'treemap'
            cand violin 'violin'
            cand help 'Print this message or the help of the given subcommand(s)'
        }
        &'qsv;viz;help;bar'= {
        }
        &'qsv;viz;help;box'= {
        }
        &'qsv;viz;help;candlestick'= {
        }
        &'qsv;viz;help;choropleth'= {
        }
        &'qsv;viz;help;contour'= {
        }
        &'qsv;viz;help;funnel'= {
        }
        &'qsv;viz;help;geo'= {
        }
        &'qsv;viz;help;heatmap'= {
        }
        &'qsv;viz;help;histogram'= {
        }
        &'qsv;viz;help;icicle'= {
        }
        &'qsv;viz;help;line'= {
        }
        &'qsv;viz;help;map'= {
        }
        &'qsv;viz;help;ohlc'= {
        }
        &'qsv;viz;help;parcats'= {
        }
        &'qsv;viz;help;pie'= {
        }
        &'qsv;viz;help;radar'= {
        }
        &'qsv;viz;help;sankey'= {
        }
        &'qsv;viz;help;scatter'= {
        }
        &'qsv;viz;help;scatter3d'= {
        }
        &'qsv;viz;help;smart'= {
        }
        &'qsv;viz;help;splom'= {
        }
        &'qsv;viz;help;sunburst'= {
        }
        &'qsv;viz;help;treemap'= {
        }
        &'qsv;viz;help;violin'= {
        }
        &'qsv;viz;help;help'= {
        }
        &'qsv;help'= {
            cand apply 'apply'
            cand behead 'behead'
            cand blake3 'blake3'
            cand cat 'cat'
            cand clean 'clean'
            cand clipboard 'clipboard'
            cand color 'color'
            cand count 'count'
            cand datefmt 'datefmt'
            cand dedup 'dedup'
            cand denull 'denull'
            cand describegpt 'describegpt'
            cand diff 'diff'
            cand edit 'edit'
            cand enum 'enum'
            cand excel 'excel'
            cand exclude 'exclude'
            cand explode 'explode'
            cand extdedup 'extdedup'
            cand extsort 'extsort'
            cand fetch 'fetch'
            cand fetchpost 'fetchpost'
            cand fill 'fill'
            cand fixedwidth 'fixedwidth'
            cand fixlengths 'fixlengths'
            cand flatten 'flatten'
            cand fmt 'fmt'
            cand foreach 'foreach'
            cand frequency 'frequency'
            cand geocode 'geocode'
            cand geoconvert 'geoconvert'
            cand get 'get'
            cand headers 'headers'
            cand implode 'implode'
            cand index 'index'
            cand input 'input'
            cand join 'join'
            cand joinp 'joinp'
            cand json 'json'
            cand jsonl 'jsonl'
            cand lens 'lens'
            cand log 'log'
            cand luau 'luau'
            cand moarstats 'moarstats'
            cand partition 'partition'
            cand pivotp 'pivotp'
            cand pragmastat 'pragmastat'
            cand pro 'pro'
            cand profile 'profile'
            cand prompt 'prompt'
            cand pseudo 'pseudo'
            cand py 'py'
            cand readstat 'readstat'
            cand rename 'rename'
            cand replace 'replace'
            cand reverse 'reverse'
            cand safenames 'safenames'
            cand sample 'sample'
            cand schema 'schema'
            cand scoresql 'scoresql'
            cand search 'search'
            cand searchset 'searchset'
            cand select 'select'
            cand slice 'slice'
            cand snappy 'snappy'
            cand sniff 'sniff'
            cand sort 'sort'
            cand sortcheck 'sortcheck'
            cand split 'split'
            cand sqlp 'sqlp'
            cand stats 'stats'
            cand synthesize 'synthesize'
            cand table 'table'
            cand template 'template'
            cand to 'to'
            cand tojsonl 'tojsonl'
            cand transpose 'transpose'
            cand validate 'validate'
            cand viz 'viz'
            cand help 'Print this message or the help of the given subcommand(s)'
        }
        &'qsv;help;apply'= {
            cand calcconv 'calcconv'
            cand dynfmt 'dynfmt'
            cand emptyreplace 'emptyreplace'
            cand operations 'operations'
            cand summarize 'summarize'
        }
        &'qsv;help;apply;calcconv'= {
        }
        &'qsv;help;apply;dynfmt'= {
        }
        &'qsv;help;apply;emptyreplace'= {
        }
        &'qsv;help;apply;operations'= {
        }
        &'qsv;help;apply;summarize'= {
        }
        &'qsv;help;behead'= {
        }
        &'qsv;help;blake3'= {
        }
        &'qsv;help;cat'= {
            cand columns 'columns'
            cand rows 'rows'
            cand rowskey 'rowskey'
        }
        &'qsv;help;cat;columns'= {
        }
        &'qsv;help;cat;rows'= {
        }
        &'qsv;help;cat;rowskey'= {
        }
        &'qsv;help;clean'= {
        }
        &'qsv;help;clipboard'= {
        }
        &'qsv;help;color'= {
        }
        &'qsv;help;count'= {
        }
        &'qsv;help;datefmt'= {
        }
        &'qsv;help;dedup'= {
        }
        &'qsv;help;denull'= {
        }
        &'qsv;help;describegpt'= {
        }
        &'qsv;help;diff'= {
        }
        &'qsv;help;edit'= {
        }
        &'qsv;help;enum'= {
        }
        &'qsv;help;excel'= {
        }
        &'qsv;help;exclude'= {
        }
        &'qsv;help;explode'= {
        }
        &'qsv;help;extdedup'= {
        }
        &'qsv;help;extsort'= {
        }
        &'qsv;help;fetch'= {
        }
        &'qsv;help;fetchpost'= {
        }
        &'qsv;help;fill'= {
        }
        &'qsv;help;fixedwidth'= {
        }
        &'qsv;help;fixlengths'= {
        }
        &'qsv;help;flatten'= {
        }
        &'qsv;help;fmt'= {
        }
        &'qsv;help;foreach'= {
        }
        &'qsv;help;frequency'= {
        }
        &'qsv;help;geocode'= {
            cand cache-clear 'cache-clear'
            cand cache-info 'cache-info'
            cand cache-prune 'cache-prune'
            cand countryinfo 'countryinfo'
            cand countryinfonow 'countryinfonow'
            cand index-check 'index-check'
            cand index-load 'index-load'
            cand index-reset 'index-reset'
            cand index-update 'index-update'
            cand iplookup 'iplookup'
            cand iplookupnow 'iplookupnow'
            cand opencage 'opencage'
            cand opencagenow 'opencagenow'
            cand reverse 'reverse'
            cand reversenow 'reversenow'
            cand suggest 'suggest'
            cand suggestnow 'suggestnow'
        }
        &'qsv;help;geocode;cache-clear'= {
        }
        &'qsv;help;geocode;cache-info'= {
        }
        &'qsv;help;geocode;cache-prune'= {
        }
        &'qsv;help;geocode;countryinfo'= {
        }
        &'qsv;help;geocode;countryinfonow'= {
        }
        &'qsv;help;geocode;index-check'= {
        }
        &'qsv;help;geocode;index-load'= {
        }
        &'qsv;help;geocode;index-reset'= {
        }
        &'qsv;help;geocode;index-update'= {
        }
        &'qsv;help;geocode;iplookup'= {
        }
        &'qsv;help;geocode;iplookupnow'= {
        }
        &'qsv;help;geocode;opencage'= {
        }
        &'qsv;help;geocode;opencagenow'= {
        }
        &'qsv;help;geocode;reverse'= {
        }
        &'qsv;help;geocode;reversenow'= {
        }
        &'qsv;help;geocode;suggest'= {
        }
        &'qsv;help;geocode;suggestnow'= {
        }
        &'qsv;help;geoconvert'= {
        }
        &'qsv;help;get'= {
            cand cache-clear 'cache-clear'
            cand cache-fetch 'cache-fetch'
            cand cache-info 'cache-info'
            cand cache-list 'cache-list'
            cand cache-prune 'cache-prune'
            cand cache-set-policy 'cache-set-policy'
            cand cache-set-ttl 'cache-set-ttl'
        }
        &'qsv;help;get;cache-clear'= {
        }
        &'qsv;help;get;cache-fetch'= {
        }
        &'qsv;help;get;cache-info'= {
        }
        &'qsv;help;get;cache-list'= {
        }
        &'qsv;help;get;cache-prune'= {
        }
        &'qsv;help;get;cache-set-policy'= {
        }
        &'qsv;help;get;cache-set-ttl'= {
        }
        &'qsv;help;headers'= {
        }
        &'qsv;help;implode'= {
        }
        &'qsv;help;index'= {
        }
        &'qsv;help;input'= {
        }
        &'qsv;help;join'= {
        }
        &'qsv;help;joinp'= {
        }
        &'qsv;help;json'= {
        }
        &'qsv;help;jsonl'= {
        }
        &'qsv;help;lens'= {
        }
        &'qsv;help;log'= {
        }
        &'qsv;help;luau'= {
            cand filter 'filter'
            cand map 'map'
        }
        &'qsv;help;luau;filter'= {
        }
        &'qsv;help;luau;map'= {
        }
        &'qsv;help;moarstats'= {
        }
        &'qsv;help;partition'= {
        }
        &'qsv;help;pivotp'= {
        }
        &'qsv;help;pragmastat'= {
        }
        &'qsv;help;pro'= {
            cand lens 'lens'
            cand workflow 'workflow'
        }
        &'qsv;help;pro;lens'= {
        }
        &'qsv;help;pro;workflow'= {
        }
        &'qsv;help;profile'= {
        }
        &'qsv;help;prompt'= {
        }
        &'qsv;help;pseudo'= {
        }
        &'qsv;help;py'= {
            cand filter 'filter'
            cand map 'map'
        }
        &'qsv;help;py;filter'= {
        }
        &'qsv;help;py;map'= {
        }
        &'qsv;help;readstat'= {
        }
        &'qsv;help;rename'= {
        }
        &'qsv;help;replace'= {
        }
        &'qsv;help;reverse'= {
        }
        &'qsv;help;safenames'= {
        }
        &'qsv;help;sample'= {
        }
        &'qsv;help;schema'= {
        }
        &'qsv;help;scoresql'= {
        }
        &'qsv;help;search'= {
        }
        &'qsv;help;searchset'= {
        }
        &'qsv;help;select'= {
        }
        &'qsv;help;slice'= {
        }
        &'qsv;help;snappy'= {
            cand check 'check'
            cand compress 'compress'
            cand decompress 'decompress'
            cand validate 'validate'
        }
        &'qsv;help;snappy;check'= {
        }
        &'qsv;help;snappy;compress'= {
        }
        &'qsv;help;snappy;decompress'= {
        }
        &'qsv;help;snappy;validate'= {
        }
        &'qsv;help;sniff'= {
        }
        &'qsv;help;sort'= {
        }
        &'qsv;help;sortcheck'= {
        }
        &'qsv;help;split'= {
        }
        &'qsv;help;sqlp'= {
        }
        &'qsv;help;stats'= {
        }
        &'qsv;help;synthesize'= {
        }
        &'qsv;help;table'= {
        }
        &'qsv;help;template'= {
        }
        &'qsv;help;to'= {
            cand datapackage 'datapackage'
            cand ods 'ods'
            cand parquet 'parquet'
            cand postgres 'postgres'
            cand sqlite 'sqlite'
            cand xlsx 'xlsx'
        }
        &'qsv;help;to;datapackage'= {
        }
        &'qsv;help;to;ods'= {
        }
        &'qsv;help;to;parquet'= {
        }
        &'qsv;help;to;postgres'= {
        }
        &'qsv;help;to;sqlite'= {
        }
        &'qsv;help;to;xlsx'= {
        }
        &'qsv;help;tojsonl'= {
        }
        &'qsv;help;transpose'= {
        }
        &'qsv;help;validate'= {
            cand schema 'schema'
        }
        &'qsv;help;validate;schema'= {
        }
        &'qsv;help;viz'= {
            cand bar 'bar'
            cand box 'box'
            cand candlestick 'candlestick'
            cand choropleth 'choropleth'
            cand contour 'contour'
            cand funnel 'funnel'
            cand geo 'geo'
            cand heatmap 'heatmap'
            cand histogram 'histogram'
            cand icicle 'icicle'
            cand line 'line'
            cand map 'map'
            cand ohlc 'ohlc'
            cand parcats 'parcats'
            cand pie 'pie'
            cand radar 'radar'
            cand sankey 'sankey'
            cand scatter 'scatter'
            cand scatter3d 'scatter3d'
            cand smart 'smart'
            cand splom 'splom'
            cand sunburst 'sunburst'
            cand treemap 'treemap'
            cand violin 'violin'
        }
        &'qsv;help;viz;bar'= {
        }
        &'qsv;help;viz;box'= {
        }
        &'qsv;help;viz;candlestick'= {
        }
        &'qsv;help;viz;choropleth'= {
        }
        &'qsv;help;viz;contour'= {
        }
        &'qsv;help;viz;funnel'= {
        }
        &'qsv;help;viz;geo'= {
        }
        &'qsv;help;viz;heatmap'= {
        }
        &'qsv;help;viz;histogram'= {
        }
        &'qsv;help;viz;icicle'= {
        }
        &'qsv;help;viz;line'= {
        }
        &'qsv;help;viz;map'= {
        }
        &'qsv;help;viz;ohlc'= {
        }
        &'qsv;help;viz;parcats'= {
        }
        &'qsv;help;viz;pie'= {
        }
        &'qsv;help;viz;radar'= {
        }
        &'qsv;help;viz;sankey'= {
        }
        &'qsv;help;viz;scatter'= {
        }
        &'qsv;help;viz;scatter3d'= {
        }
        &'qsv;help;viz;smart'= {
        }
        &'qsv;help;viz;splom'= {
        }
        &'qsv;help;viz;sunburst'= {
        }
        &'qsv;help;viz;treemap'= {
        }
        &'qsv;help;viz;violin'= {
        }
        &'qsv;help;help'= {
        }
    ]
    $completions[$command]
}
