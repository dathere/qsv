...
| `QSV_DUCKDB_PATH` | The fully qualified path to the DuckDB binary for SQL query execution in the `describegpt` and `scoresql` commands. When set, DuckDB will be used instead of the default Polars SQL engine. Note that in `describegpt`, all loaded DuckDB extensions will be sent as additional context to the LLM to let it know what functions (even UDFs!) it can use in the SQL queries it generates. |
