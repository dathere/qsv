...
| `QSV_MAPBOX_TOKEN` | Deprecated and ignored: the `viz map` command now uses token-free MapLibre basemaps, so no access token is needed. Still accepted (via this variable or the `--mapbox-token` option) for backward compatibility only. Requires the `viz` feature. |
