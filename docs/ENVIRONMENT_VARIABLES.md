...
| `QSV_THP` | if set (Linux, jemalloc builds only), opts into jemalloc Transparent Huge Pages (`thp:always,metadata_thp:always`). Because `thp`/`metadata_thp` are read-only jemalloc `opt.*` knobs that can only be read from the config env var before the allocator initializes, qsv injects them into `_RJEM_MALLOC_CONF` and **re-execs itself once** at startup (a `QSV_THP_APPLIED` sentinel prevents a re-exec loop). 2 MB huge pages can cut TLB misses on aggregation-heavy commands over large inputs, but huge-page allocation overhead can erase that win on small inputs — so it is strictly opt-in and off by default. On any Linux jemalloc build, setting it performs the config injection + one-time re-exec; the **performance effect** then depends on the kernel — huge pages are only granted when `/sys/kernel/mm/transparent_hugepage/enabled` is `always` or `madvise` (with `never`, jemalloc still runs but is handed no huge pages). Compiles to a true no-op on non-Linux or non-jemalloc builds. Skipped when `QSV_NO_ALLOC_TUNING` is also set. |
