...
| `QSV_UPDATE_CHECK_TTL_SECS` | Time-to-live in seconds for the local update check cache (`.update_check` file in `QSV_CACHE_DIR`, defaulting to `~/.qsv-cache`). Passive update checks (`qsv help`, random no-command check) use this cache to avoid hitting the GitHub API on every invocation. Set to `0` to always check. (default: 86400 — 24 hours) |
