...
* `viz_static` - extends `viz` with static image export (PNG/SVG/PDF/JPEG/WebP) via [plotly_static](https://crates.io/crates/plotly_static). Requires a headless browser (Chrome or Firefox) and a webdriver at runtime (webdriver is auto-downloaded).
...
* `qsvmcp` - enable to build `qsvmcp` binary variant - optimized for [MCP](https://modelcontextprotocol.io/) server use with geocode, get, get_cloud, mcp, polars, profile, self_update, synthesize, to, and viz_static features. Shares `src/main.rs` with `qsv`. (mutually exclusive with `lite` and `datapusher_plus`)
