...
* `color` - enable `color` command. Enables colorized tabular output with support for multiple formats via Polars.
...
* `magika` - enable [Magika](https://github.com/nicholasgasior/magika-rs)-powered, AI-based file type detection in the `sniff` command for enhanced MIME type identification. Falls back to the `file-format` crate in `qsvlite`/`qsvdp` variants. Not available on MUSL builds.
* `mcp` - enable MCP (Model Context Protocol) skill JSON generation for AI agent integration.
