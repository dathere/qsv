...
* `viz` - enable `viz` command. Generates interactive [Plotly](https://plotly.com/javascript/) charts (bar, line, scatter, histogram, box, pie, heatmap, candlestick, ohlc, sankey, radar, and a `smart` auto-dashboard) as self-contained, browser-openable HTML (set `QSV_VIZ_CDN` to load plotly.js from a CDN instead, for much smaller, online-only pages).
