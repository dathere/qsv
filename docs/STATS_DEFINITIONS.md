...
| `zero_padded_numeric` | Variable | `true` when leading/padding zeros would be lost if the column were cast to a number — either a `String` column of zero-padded integers (zip codes, barcodes, padded IDs) or a `Float` column of zero-padded decimal codes (e.g. `007.1`, `05.10`); empty otherwise. Opt-in via `--zero-padded-numeric` or `--everything`. | While scanning, a column stays "qualified" only if every non-null value is numeric-shaped (an all-ASCII-digit integer OR a parseable float); the first non-numeric value disqualifies it. A float whose integer part is a leading `0` followed by another digit (`007.1`, `05.10`) additionally marks the column as float-padded. At output, `true` is emitted when the column qualified, saw at least one value, AND either its final `type` is `String` (which implies a leading-zero integer — an all-digit column with no leading zero infers as `Integer`) OR its final `type` is `Float` and at least one value was float-padded. |
