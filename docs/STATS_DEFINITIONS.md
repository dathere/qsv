...
| `robust_cv` | Variable | Robust Coefficient of Variation. Non-negative, outlier-resistant alternative to CV using MAD and the magnitude of the median instead of stddev and mean. | `MAD / |median|`. Requires: `mad`, `median` (or `q2_median`). Returns `None` if median is zero. See: [Robust measures of scale](https://en.wikipedia.org/wiki/Robust_measures_of_scale) |
