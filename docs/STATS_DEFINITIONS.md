...
| 48 | `zero_padded_numeric` | `--zero-padded-numeric` (`true` when leading/padding zeros would be lost if the column were cast to a number — either the inferred `type` is `String` with every non-null value numeric (an all-digit integer or zero-padded decimal code) and at least one carrying a leading zero (zip codes, barcodes, padded IDs), OR the inferred `type` is `Float` with every non-null value numeric and at least one a zero-padded decimal code such as `007.1`/`05.10` (ICD-9, Dewey Decimal, Harmonized System codes); empty otherwise) |
...
| `zero_padded_numeric` | Variable | `true` when leading/padding zeros would be lost if the column were cast to a number — either a `String` column of zero-padded integers/codes (zip codes, barcodes, padded IDs) or a `Float` column of zero-padded decimal codes (e.g. `007.1`, `05.10`); empty otherwise. Opt-in via `--zero-padded-numeric` or `--everything`. | While scanning, a column stays "qualified" only if every non-null value is numeric-shaped; the first non-numeric value disqualifies it. A value counts as numeric-shaped if it is (a) an all-ASCII-digit integer; (b) a zero-padded decimal code — a leading `0` in the integer part followed by another digit (`007.1`, `05.10`), which also marks the column as float-padded; or (c) a plain float freshly parsed as `Float`. Note (c) reuses the type already inferred by `from_sample()`: a plain float arriving *after* the column has widened to `String` short-circuits to `String` and so disqualifies the column, matching the all-digit intent of the `String` path. At output, `true` is emitted when the column qualified, saw at least one value, AND either its final `type` is `String` (which implies a leading-zero integer or zero-padded code — a plain all-digit column with no leading zero infers as `Integer`) OR its final `type` is `Float` and at least one value was float-padded. |

**Zero-Padded Numeric Detection:**
- qsv deliberately keeps integers with leading zeros (e.g. `07306`) as `String` rather than `Integer`, to avoid silently dropping the leading zeros. Zero-padded decimal *codes* (e.g. `007.1`) still parse as `Float`, so the type alone can't reveal the padding — they are detected from the value's string form instead.
...
- Float detection uses a leading-zero rule that mirrors the integer one: a `0` in the integer part immediately followed by another digit (`007.1`, `05.10`, `0601.10`). This deliberately **excludes** ordinary fractions like `0.5`/`0.25` (a single `0` before the decimal point). Pure trailing-zero codes (`7.10` → `7.1`) are **not** flagged — they're indistinguishable from rounded measurements without the original string. Multi-dot codes like `0601.10.00` are neither all-digit nor parseable floats, so they disqualify the column (they count as non-numeric values) and it is never flagged — even though qsv does keep such columns as `String`.
