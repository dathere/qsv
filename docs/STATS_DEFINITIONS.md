...
| `--cardinality-method approx` | `exact` | [HyperLogLog](https://en.wikipedia.org/wiki/HyperLogLog) (Apache DataSketches port, lg_k=12) | ~5KB per column | ~1.5% relative standard error | Replaces exact `cardinality`/`uniqueness_ratio`. Reproducible across *parallel* `--jobs` values (the HLL union used at merge time is associative and order-invariant, so chunk completion order does not affect the final estimate), but `--jobs 1` never reaches that union and so can report a different estimate than any parallel run — see [Recipe: stats on Very Large Files](#recipe-stats-on-very-large-files). `--infer-boolean` forces `exact` (boolean inference needs `cardinality == 2` exactness); a one-time warning is emitted. The `--mode-cardinality-cap` `>=<n>` sentinel is **never** emitted under `approx` — only mode/antimode columns remain gated by the cap. |
...
The mean and central moments used below are computed from the collected values (with a refinement pass, so a constant column has exactly zero central moments), **not** taken from the stats cache (whose `mean`/`stddev` are rounded, by default to 4 decimal places, which badly distorts moment statistics on small-scale data). Date/DateTime values are processed as days since the epoch.
...
| `l_cv` | Variable | L-CV (coefficient of L-variation). Robust, bounded analogue of the coefficient of variation. | `λ2 / λ1`, from sample L-moments computed via unbiased probability-weighted moments on the sorted values (Hosking, 1990). Matches `scipy.stats.lmoment`. Only computed when `λ1 > 0` (0 for a constant column); not computed for Date/DateTime columns. PWMs are accumulated on mean-centered values, so λ2..λ4 are unaffected by a large offset. Requires n >= 4. See: [L-moment](https://en.wikipedia.org/wiki/L-moment) |
...
| `l_kurtosis` | Variable | L-kurtosis (τ4). Robust analogue of kurtosis. About 0.1226 for a normal distribution. The population value lies in [-1/4, 1]; the sample estimate can fall below -1/4 for small n (e.g. `0,0,6,6` gives -1.5). | `λ4 / λ2`. Requires n >= 4. |
...
**All-unique fields:** a field whose every value is unique (cardinality ≥ row count, typical of continuous measurements and of IDs) keeps its pairs with other numeric/date fields. Correlation, covariance and regression are reported, while the frequency-based statistics (`mutual_information`, `normalized_mutual_information`, Theil's U and `cramers_v`) are left empty, because mutual information against an all-unique field just saturates at log(n). A pair is dropped only if nothing could be computed for it: an all-unique field paired with a non-numeric field, or a run whose `--bivariate-stats` requests only frequency statistics. Identifier columns are all-unique too, so read their correlations with care. A sequence ID correlates with a creation date simply because IDs are issued in order (check `uniqueness_ratio` / `xsd_type`). If no pair survives, the sidecar is still written with a header only, so it is never a stale file left over from an earlier run.
...
| `r_squared` | Pairwise | Coefficient of determination of the OLS fit. | `pearson_correlation²`, computed as `(cxy / (sqrt(m2_x) * sqrt(m2_y)))²` so it cannot overflow. Empty when field2 is constant over the valid pairs (slope 0 and intercept are still reported). |
...
The `pragmastat` command computes robust, median-of-pairwise statistics using the [Pragmastat library](https://pragmastat.dev/) (v14.0.1). Designed for messy, heavy-tailed, or outlier-prone data where mean/stddev can mislead.
...
See: [Pragmastat manual (PDF)](https://github.com/AndreyAkinshin/pragmastat/releases/download/v14.0.1/pragmastat-v14.0.1.pdf), [pragmastat.dev](https://pragmastat.dev/)
...
3. **HLL for cardinality.** `cardinality` and `uniqueness_ratio` come from a ~5 KB HyperLogLog per column. ~1.5% RSE. The HLL union is associative and order-invariant, so the estimate is stable across *parallel* `--jobs` values — `--jobs 2`, `4` and `8` agree exactly. **Caveat:** `--jobs 1` is NOT in that set. Only the chunked parallel path merges per-chunk sketches through the HLL union, and a single-threaded run never reaches it, so it **can** report a different estimate than a parallel run. Whether it actually does depends on the column: while the distinct-value count is low enough that the sketch is still exact, both paths agree (a 5-distinct-value column reads 5 either way), and they can diverge once the sketch starts estimating. Measured on a 39,627-row indexed fixture, one column read 39,215 at `--jobs 1` and 38,524 at `--jobs 2/4/8` (exact: 39,627). Both are within tolerance and each is deterministic; they are simply not the same number. Pin `--jobs` for run-to-run comparability.
