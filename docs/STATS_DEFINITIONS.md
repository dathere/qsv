...
| `trimean` | Variable | Tukey's Trimean. Robust estimator of central tendency combining median with the midhinge. More robust than mean, more efficient than median alone. | `(Q1 + 2*median + Q3) / 4`. Requires: `q1`, `median` (or `q2_median`), `q3`. See: [Trimean](https://en.wikipedia.org/wiki/Trimean) |
| `midhinge` | Variable | Midhinge. Midpoint of the middle 50% of data. A robust central tendency measure that complements the mean and median. | `(Q1 + Q3) / 2`. Requires: `q1`, `q3`. See: [Midhinge](https://en.wikipedia.org/wiki/Midhinge) |
| `robust_cv` | Variable | Robust Coefficient of Variation. Outlier-resistant alternative to CV using MAD and median instead of stddev and mean. | `MAD / median`. Requires: `mad`, `median` (or `q2_median`). Returns `None` if median is zero. See: [Robust measures of scale](https://en.wikipedia.org/wiki/Robust_measures_of_scale) |
...
| `jarque_bera` | Variable | Jarque-Bera test statistic. Standard test for normality using skewness and kurtosis. Higher values indicate greater departure from normality. | Computed as `(n/6) * (S² + K²/4)` where S is skewness and K is excess kurtosis. Requires: `skewness` (from base stats), `kurtosis` (from `--advanced` flag), and sample size n (from `n_positive + n_negative + n_zero`). See: [Jarque-Bera test](https://en.wikipedia.org/wiki/Jarque%E2%80%93Bera_test) |
...
| `theil_index` | Variable | Theil Index (Generalized Entropy GE(1)). Measures inequality/concentration in the distribution. Unlike Gini, it is decomposable into within-group and between-group components. Only computed for positive values. | Computed as `(1/n) * Σ((x_i / mean) * ln(x_i / mean))` for positive values. Uses precalculated mean from baseline stats. Requires: `mean`. See: [Theil Index](https://en.wikipedia.org/wiki/Theil_index) |
| `mean_ad` | Variable | Mean Absolute Deviation from mean. Average absolute distance of values from the arithmetic mean. Less robust than MAD (which uses median) but more statistically efficient. | Computed as `(1/n) * Σ|x_i - mean|`. Uses precalculated mean from baseline stats. Requires: `mean`. |
...
| `simpsons_diversity_index` | Variable | Simpson's Diversity Index. Probability that two randomly chosen values are different. More intuitive than entropy for many users. Ranges from 0 (all identical) to 1 (all unique). | Computed as `1 - Σ(p_i²)` where p_i are value proportions from frequency data. Computed alongside Shannon Entropy. Works for **all field types**. For all-unique fields, returns `1 - 1/n`. See: [Simpson's Diversity Index](https://en.wikipedia.org/wiki/Diversity_index#Simpson_index) |
