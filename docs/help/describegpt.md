...
| &nbsp;`‑‑markdown‑template`&nbsp; | string | TOML file with Mini Jinja templates for Markdown output, with per-inference-type templates: dictionary_md_template, description_md_template, tags_md_template and custom_prompt_md_template. If no file is provided, built-in defaults are used (matching legacy output). See <https://github.com/dathere/qsv/blob/master/resources/describegpt_md_defaults.toml> |  |
