...
| &nbsp;`‑‑memory‑limit`&nbsp; | integer | The maximum amount of memory used to buffer rows before spilling them to the on-disk hash table. If less than 50, this is a percentage of total memory. If more than 50, this is the memory in MB to allocate, capped at 90 percent of total memory. Defaults to 100 MB. The on-disk hash table is far more compact than this buffer, so raising the limit trades a lot of memory for very little. |  |
