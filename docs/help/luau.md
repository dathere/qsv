...
executing a Luau 0.723 script for every row (SEQUENTIAL MODE) or for
...
| &nbsp;`‑‑max‑errors`&nbsp; | integer | The maximum number of errors to tolerate before aborting. Set to zero to disable error limit. | `10` |
| &nbsp;`‑‑timeout`&nbsp; | integer | Timeout for downloading lookup_tables using the qsv_register_lookup() helper function. | `60` |
