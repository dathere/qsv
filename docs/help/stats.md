...
| &nbsp;`‑‑mode‑cardinality‑cap`&nbsp; | string | Bound mode-tracking memory on high-cardinality columns. When > 0, if a column accumulates more than <n> mode samples (~ row count for unique-per-row "ID" columns), qsv drops the mode tracker for that column and emits sentinel values instead of exact modes/cardinality:<ul><li>mode columns: "*HIGH_CARDINALITY"</li><li>cardinality column: ">=<n>" (the ">=" prefix DOES break downstream parsers expecting a plain integer; cap is opt-in only).</li></ul> Useful on wide tables with many ID/UUID/timestamp columns where tracking exact cardinality is wasted work. | `0` |
