...
The eighteen **smart dashboards** are embedded as `<iframe>`s of their genuine
...
| `regions_growth.csv` | **844** monthly observations for **6 synthetic regions** over **24 months**: `region`, `month_date`, `gdp_index`, `wellbeing_index`, `population_m` | `smart` (**Gapminder-style animated bubble chart** — one bubble per region tracing a curved path through the measure space, Play/Pause + scrub slider) |
