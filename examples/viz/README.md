...
| `district_requests_dict.schema.json` | 4 | `district_requests.csv` — the only one carrying `x-qsv.denominator`, which names the column holding each region's population so a region map can chart a **rate** beside its raw count |
