...
def main():
    ...
    # iframe styling for the embedded smart-dashboard pages (idempotent; head is reused verbatim).
    if ".dash{" not in head:
        head = head.replace(
            "</style>",
            " figure.full iframe.dash{width:100%;border:0;height:1200px;display:block;"
            "border-radius:6px}\n</style>", 1)
    ...
    for idx, fig in enumerate(FIGURES):
        ...
        if iframe_name:
            ...
            fig_divs.append(
                ...
                f'<iframe src="{iframe_name}" class="dash" loading="lazy" title="{title}" '
                f'onload="this.style.height=(this.contentWindow.document.body.scrollHeight+24)+\'px\'">'
                f'</iframe></figure>'
            )
            continue
