...
# Attribution for the third-party software this page loads. The gallery references plotly.js from
# the version-pinned CDN (with SRI) rather than embedding it, and renders map figures on MapLibre
# basemaps. Kept in step with `third_party_comment`/`third_party_footer` in src/cmd/viz.rs, which
# emit the equivalent markup for `qsv viz`'s own output.
...
FIGURES = [
    ("smart Data Schematic (--smarter, geospatial)",
     ...
     "offshore Pacific seismicity, so under the auto-derived snap cap (18&nbsp;km here, derived from "
     "the boundary file's median region size and the coordinates' precision) near-coast quakes snap "
     "to their nearest prefecture (44 of them) while far-offshore quakes are dropped (269 of 417 — "
     "the panel title reports both) and the on-land/near-coast prefectures are colored; raise "
     "<code>--snap-max-dist</code> to snap distant quakes to the "
    ...
    ("smart Data Schematic (denominator-aware region rate)",
     ...
     "— here by <code>x-qsv.denominator: {&quot;column&quot;: &quot;population&quot;, "
     "&quot;level&quot;: &quot;geo.county&quot;}</code> on the region column in the curated "
     "dictionary. The <b>level</b> declares which geography those values describe, and is checked "
     "against the region column's own concept: a denominator describing a <i>different</i> "
     "geography from the one the map is keyed by is refused rather than charted. The case that "
     "motivates it is a <i>coarser</i> one — state populations against county regions — which is "
     "constant within each region and positive, so it passes every data-derived check while "
     "charting a confidently wrong rate. A hint that declares no level still charts, and says "
     "so in its subtitle. <code>--denominator-key properties.POP2020</code> "
    ...
    # The same lesson as above, on geography a reader recognizes. Deliberately NOT in
    # SMART_IFRAME/PREGENERATED: it is deterministic and renders inline like its sibling, and
    # joining PREGENERATED would make llm_dictionary_sidecars() compute (and, under
    # QSV_VIZ_REGEN_LLM=2, DELETE) a sidecar path near this figure's curated dictionary.
    ("smart Data Schematic (denominator-aware region rate, real counties)",
     "The same count-vs-rate lesson as the figure above, on <b>real geography with a real "
     "denominator</b>. <b>210 counties</b> across Ohio, Pennsylvania and West Virginia "
     "(88 + 67 + 55), keyed by 5-digit county FIPS. The <b>count</b> panel is led by "
     "<b>Philadelphia</b>, <b>Allegheny</b> and <b>Franklin</b> — the most populous counties in "
     "the set — because a raw-count choropleth of an event log is substantially a population "
     "map. The <b>rate</b> panel beside it divides by each county's population and the ranking "
     "inverts completely: the top becomes the <b>rural</b> tail (Forest&nbsp;PA, Wirt&nbsp;WV, "
     "Braxton&nbsp;WV) at 2.5&ndash;2.6 incidents per 1,000 residents on a hundredth of the "
     "volume — the two top-tens do not overlap at all. <b>The denominator is not in the file.</b> "
     "<code>--denominator census@2024</code> fetches ACS 5-year total population "
     "(<code>B01003</code>) from the Census Data API for the 210 matched GEOIDs and stamps the "
     "release beneath the map — <i>Denominator: ACS 2020&ndash;2024 5-yr (B01003)</i> — because a "
     ...
     "one county (Mercer and Wayne to all three), so a bare name would silently sum them in the "
     "frequency bar. The <b>incidents are SYNTHETIC</b>, generated by "
     "<code>gen_tristate_county_incidents.py</code> with a per-capita rate deliberately "
     "anti-correlated with population; the counties, boundaries and denominator are all real.",
     True, ["smart", "tristate_county_incidents.csv", "--dict-info",
            "--dictionary", "tristate_county_incidents_dict.schema.json",
    ...
    ("smart Data Schematic (real per-capita county rate, WPRDC 2-1-1)",
     ...
     "residents</b> \u2014 0.05 per 1,000, a thousandth of its neighbours. Nobody believes McKean "
...
SCREENSHOTS = [
    ...
    },
    {
        ...
        "desc": (
            ...
            "<b>188 neighborhood</b> polygons (no geocoding — every one of them draws requests, 110 "
            "near-boundary points snap to their nearest neighborhood under the auto-derived "
            "0.36&nbsp;km cap, and 10 of the 7,464 located points fall too far to snap), and because "
            "the matched regions "
...
def main():
    ...
    # Drop any prior third-party notice before re-adding the current one. The head is reused
    # verbatim from the previous gallery.html, which already carries this comment, so without
