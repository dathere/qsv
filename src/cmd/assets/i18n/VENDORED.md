...
|---|---|---|
| `datatables/*.json` | DataTables combo `dt-3.0.4/b-4.0.3/cc-2.0.2/date-2.0.0/sb-2.0.1` | `DATATABLES_CDN_COMBO`, `src/cmd/viz.rs` |
...
The same held again on **2026-09-12**, when the pin moved from `dt-3.0.3/b-4.0.2/cc-2.0.1/sb-2.0.0`
to `dt-3.0.4/b-4.0.3/cc-2.0.2/sb-2.0.1`: all seven files were sha256-compared against
`https://cdn.datatables.net/plug-ins/3.0.4/i18n/<code>.json` and every one was **byte-identical**
once more. The `zh` SearchBuilder date-condition inversion is *still* unfixed at 3.0.4 (`after` is
rendered 早于, "earlier than"), so the splice-time override and the
`zh_date_condition_labels_are_corrected` canary both stay. The `columnControl` coverage gaps below
are unchanged by the ColumnControl 2.0.2 bump, and SearchBuilder moving off 2.0.0 for the first
time changed none of the vendored strings.
...
the old pin **or the current one** will match them. The 2026-09-12 paragraph is the newest; the
...
* Source: <https://cdn.datatables.net/plug-ins/3.0.4/i18n/es-ES.json>
...
* Supplies every DataTables-authored string in the data-viewer drawer: pagination, search,
  ...
  `cc-2.0.2`, and this file covers it).
...
|---|---|---|
| `datatables/fr.json` | 10,838 | `.../3.0.4/i18n/fr-FR.json` |
| `datatables/de.json` | 10,523 | `.../3.0.4/i18n/de-DE.json` |
| `datatables/it.json` | 10,443 | `.../3.0.4/i18n/it-IT.json` |
| `datatables/pt-BR.json` | 8,659 | `.../3.0.4/i18n/pt-BR.json` |
...
* `pt-BR.json` omits the whole `columnControl` group, so the per-column search widgets
  (ColumnControl 2.0.2) stay English in a Brazilian Portuguese drawer.

### ja, zh-CN — retrieved 2026-07-30

| Local file | Bytes | Fetched from |
|---|---|---|
| `datatables/ja.json` | 8,845 | `.../3.0.4/i18n/ja.json` |
| `datatables/zh-CN.json` | 7,844 | `.../3.0.4/i18n/**zh**.json` |
