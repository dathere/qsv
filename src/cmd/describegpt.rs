...
fn get_completion(
    ...
    // if flag_prompt is set, ensure each line of the Attribution begins with
    // '--' so its treated as a SQL comment
    let att_prefix = if args.flag_prompt.is_some() {
        "-- "
    } else {
        ""
    };
    ...
    let completion = completion.replace(
        ...
        &format!(
            r#"{att_prefix}Generated by {qsv_variant} v{qsv_version} describegpt
