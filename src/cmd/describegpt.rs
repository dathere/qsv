    ...
    let tag_vocab = if prompt_type == PromptType::Tags && args.flag_tag_vocab.is_some() {
        ...
        format!(
            "Use the following Tag Vocabulary:\n\n{tag_vocab_content}\n\n
             The tag and its corresponding description are separated by a colon.\nOnly use tags \
             from the Tag Vocabulary, using the description to guide the tag inference."
        )
    } else {
...
fn get_completion(
    ...
    // Replace "{GENERATED_BY_SIGNATURE}" with Attribution metadata
    let completion = completion.replace(
        "{GENERATED_BY_SIGNATURE}",
        &format!(
            "Attribution: Generated by {qsv_variant} v{qsv_version} describegpt with \
             {prompt_file_kind} v{prompt_file_ver} using {model} running on {llm_url} at {ts}",
            ...
            prompt_file_kind = if let Some(prompt_file) = args.flag_prompt_file.as_ref() {
                format!("custom prompt file (file: {prompt_file})")
            } else {
                "default prompt file".to_string()
            },
