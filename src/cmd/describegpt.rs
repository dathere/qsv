...
fn get_completion(
    ...
    // with prompt file version, model name & datetime the completion was generated
    let completion = completion.replace(
        "{GENERATED_BY_SIGNATURE}",
        &format!(
            "Generated by qsv's describegpt command with prompt file v{ver} using {model} on {ts}",
            ver = prompt_file.version,
            model = model,
