...
///
/// `pub(crate)` so `viz --dict-info` can de-duplicate the same attribution block out of a
/// dictionary's dataset description (it's also carried separately in `x-qsv.generated_by`,
/// which the dictionary page renders as its footer).
pub(crate) fn strip_attribution_block(s: &str) -> String {
    let cut = s.find("Generated by").map_or(s, |idx| &s[..idx]);
    let mut out = cut.replace("{GENERATED_BY_SIGNATURE}", "");
    out = out.trim_end().to_string();
    // Cutting at "Generated by" can leave trailing attribution scaffolding the LLM (or the
    // footer template) emitted just above the signature: a bare "Attribution" heading in any
    // markdown form ("*Attribution:" markdown footer, "Attribution:" toon footer,
    // "**Attribution**"/"## Attribution" the LLM sometimes writes) and/or a horizontal rule.
    // Strip them iteratively so none dangle before the following section (e.g. the OKF
    // `# Schema` heading).
    loop {
        let trimmed = out.trim_end();
        let (rest, last) = match trimmed.rfind('\n') {
...
"#;
    ...
    fn semanticmd_strip_attribution_block() {
        ...
        let s = "Some description.\n\n{GENERATED_BY_SIGNATURE}";
        assert_eq!(strip_attribution_block(s), "Some description.");
        // Bold "**Attribution**" heading the LLM sometimes writes above the signature,
        // preceded by a `---` rule — both must be stripped so nothing dangles (OKF body).
