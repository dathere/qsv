...
-r, --rank-strategy <arg>   The strategy to use when there are count-tied values in the frequency table.
   ...
   --all-unique-text <arg>  The text to use for the "<ALL_UNIQUE>" category.
    ...
    --vis-whitespace        Visualize whitespace characters in the output. See
                            https://github.com/dathere/qsv/wiki/Aggregation-and-Statistics#whitespace-markers
...
impl Args {
    ...
    fn write_frequency_jsonl(
        ...
        // Built in a sibling temp created at its final mode and renamed into place, so a
        // failed write leaves the PREVIOUS cache intact (still stale, but still healable -
        // a truncated one has no parseable metadata line and misses forever) and a 0600 CSV
        // never yields a 0644 cache of its values. The rules, and why mode-bit intersection
        // alone is not enough, are on `util::DerivedFile`.
        let mut cache_file = util::DerivedFile::create(path, &cache_path, "freqcache")?;
        io::Write::write_all(&mut cache_file, jsonl.as_bytes())?;
        cache_file.install()?;
