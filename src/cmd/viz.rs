...
///
/// Unlike a blanket cut at the first `"Generated by"`, this anchors on a *line* that begins the
/// describegpt signature (a line starting with `Generated by ` that also names `describegpt`),
/// so a description that merely mentions the phrase mid-sentence is left intact.
fn strip_dataset_attribution(desc: &str) -> String {
    let bytes = desc.as_bytes();
    let mut search = 0;
    while let Some(rel) = desc[search..].find("Generated by ") {
        let pos = search + rel;
        let at_line_start = pos == 0 || bytes[pos - 1] == b'\n';
        if at_line_start {
            let line_end = desc[pos..].find('\n').map_or(desc.len(), |e| pos + e);
            if desc[pos..line_end].contains("describegpt") {
                // Drop the footer and any trailing separator/blank lines before it.
                return desc[..pos]
                    .trim_end()
                    .trim_end_matches(['-', '*', '_'])
                    .trim_end()
