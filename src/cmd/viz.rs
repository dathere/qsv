...
fn parse_dictionary_semantics(json_text: &str) -> Option<DictData> {
    ...
    if let Some(props) = v.get("properties").and_then(serde_json::Value::as_object) {
        ...
        // it from the description so it isn't shown twice. `strip_attribution_block` cuts at the
        // established "Generated by" anchor.
        let dataset_description = top_str(v.get("description"))
            .map(|d| crate::cmd::describegpt::strip_attribution_block(&d))
            .filter(|s| !s.is_empty());
...
/// common in describegpt descriptions). Returns the trimmed text after the marker, with any
/// trailing closing `#`/space run stripped. `None` when the line isn't a heading.
fn heading_content(line: &str) -> Option<&str> {
    let t = line.trim_start();
    let hashes = t.bytes().take_while(|&b| b == b'#').count();
    if (1..=6).contains(&hashes) && t.as_bytes().get(hashes) == Some(&b' ') {
        Some(t[hashes + 1..].trim().trim_end_matches(['#', ' ']))
    } else {
        None
    }
}
