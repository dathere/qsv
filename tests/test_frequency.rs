...
/// Decode a JSONL cache file and return the entry lines (skipping the metadata line).
...
/// Tamper with a JSONL cache file by replacing the first occurrence of `old_count`
...
fn tamper_cache(path: &std::path::Path, old_count: u64, new_count: u64) {
    ...
    let mut found = false;
    // lines[0] is metadata; lines[1..] are entries
    'outer: for line in lines.iter_mut().skip(1) {
        let mut entry: Value =
            serde_json::from_str(line).expect("tamper_cache: failed to parse entry line");
        for freq in entry["frequencies"]
            .as_array_mut()
            .expect("tamper_cache: 'frequencies' key missing or not an array in entry")
        {
            if freq["count"].as_u64() == Some(old_count) {
                freq["count"] = Value::from(new_count);
                found = true;
                *line =
                    serde_json::to_string(&entry).expect("tamper_cache: failed to re-encode entry");
                break 'outer;
            }
        }
    }
...
fn param_prop_frequency(name: &str, rows: CsvData, idx: bool) -> bool {
    ...
    // Skip if any value (header or data) contains Unicode whitespace (e.g. EN QUAD \u{2000},
    // PUNCTUATION SPACE \u{2008}, IDEOGRAPHIC SPACE \u{3000}, etc.).
    // `.trim()` strips all Unicode whitespace while `.trim_ascii()` only strips ASCII
    // whitespace. When they differ, the field contains Unicode-only whitespace chars.
...
fn frequency_jsonl_cache_all_unique() {
    ...
    assert_eq!(fields[1]["field"], "value");
    let freqs = fields[1]["frequencies"].as_array().unwrap();
    assert_eq!(freqs.len(), 1);
    assert_eq!(freqs[0]["value"], "<ALL_UNIQUE>");
}
...
fn frequency_jsonl_cache_partial_hit() {
    ...
    // Create frequency cache with threshold 3: "code" (4 unique) → HIGH_CARDINALITY,
    ...
    assert_eq!(
        got_fresh, got_partial,
        "Partial cache output should match fresh computation"
    );
}
...
fn frequency_jsonl_cache_no_nulls_incompatible() {
    ...
    assert_ne!(
        got_with_nulls, got_no_nulls,
        "With-nulls and no-nulls output should differ"
    );

    // Verify no-nulls output has fewer rows (no null entry for "color")
    assert!(
        got_no_nulls.len() < got_with_nulls.len(),
        "no-nulls output ({}) should have fewer rows than with-nulls ({})",
        got_no_nulls.len(),
        got_with_nulls.len()
    );
}
...
fn frequency_jsonl_cache_metadata_written() {
    // Verify that the .freq.csv.data.jsonl cache contains metadata + entries
    ...
    // Cache file should be JSONL: metadata on line 1, entries on subsequent lines
    let cache_content = std::fs::read_to_string(&cache_path).unwrap();
    let lines: Vec<&str> = cache_content.lines().collect();
    // 2 columns → metadata + 2 entries = 3 lines
    assert!(
        lines.len() >= 3,
        "JSONL cache should have at least 3 lines (metadata + 2 entries), got {}",
        lines.len()
    );
    // Line 1 is metadata with qsv_version
    let metadata: Value = serde_json::from_str(lines[0]).unwrap();
    assert!(
        metadata.get("qsv_version").is_some(),
        "First line should be metadata with qsv_version"
    );
    // Line 2 is an entry with a "field" key
    let entry: Value = serde_json::from_str(lines[1]).unwrap();
    assert!(
        entry.get("field").is_some(),
        "Second line should be an entry with a 'field' key"
    );
}
...
fn frequency_jsonl_cache_embedded_numbers() {
    ...
    let cached: Vec<(&str, u64)> = district_freqs
        .iter()
        .map(|f| (f["value"].as_str().unwrap(), f["count"].as_u64().unwrap()))
        .collect();
    assert!(
...
/// Size parameter for quickcheck property tests. Controls the maximum size of
...
fn prop_preamble(rows: &CsvData) -> Option<Vec<Vec<String>>> {
    ...
    // skip if any field contains BOM or control characters that cause encoding edge cases.
    // Checks C0 controls (< 0x20 except tab/newline/CR) and C1 controls (U+0080–U+009F).
    for row in &vecs {
        for field in row {
...
fn prop_frequency_count_sum() {
    ...
    qcheck_sized(p as fn(CsvData) -> bool, PROP_TEST_SIZE);
}
...
fn param_prop_frequency_count_sum(name: &str, rows: CsvData) -> bool {
    ...
    for (field, field_rows) in &grouped {
        ...
        assert_eq!(
            sum, data_row_count,
            "Count sum mismatch for field '{field}': got {sum}, expected {data_row_count}"
        );
    }
    true
}
...
fn param_prop_frequency_pct_sum(name: &str, rows: CsvData) -> bool {
    let _ = match prop_preamble(&rows) {
        Some(v) => v,
        None => return true,
    };
    ...
    for (field, field_rows) in &grouped {
        // With --pct-nulls, all rows (including NULL) should have percentages.
        // Parse all percentage fields directly — an empty percentage would indicate
        // a bug in the frequency command when --pct-nulls is set.
...
fn param_prop_frequency_limit_correctness(name: &str, rows: CsvData) -> bool {
    ...
    for (field, field_rows) in &grouped {
        ...
        assert!(
            explicit_count <= 2,
            "Field '{field}' has {explicit_count} explicit values with --limit 2"
        );
        ...
        for r in field_rows.iter().filter(|r| r.value == "Other") {
            if !r.rank.is_empty() {
                assert_eq!(
                    r.rank, "0",
                    "Other row in field '{field}' should have rank 0, got '{}'",
                    r.rank
                );
            }
        }
    ...
    for (field, field_rows) in &grouped {
        ...
        for (i, rank) in ranks.iter().enumerate() {
            assert_eq!(
                *rank,
                i + 1,
                "Dense rank gap in field '{field}': expected {}, got {rank}",
                i + 1
            );
        }
    }
...
fn param_prop_frequency_desc_order(name: &str, rows: CsvData) -> bool {
    ...
    for (field, field_rows) in &grouped {
        // Filter out synthetic values which may appear at the end.
        // NOTE: if new synthetic values are added to the frequency command,
        // they must be added to this filter as well.
        ...
        for window in counts.windows(2) {
            assert!(
                window[0] >= window[1],
                "Counts not in descending order for field '{field}': {} < {}",
                window[0],
                window[1]
            );
        }
