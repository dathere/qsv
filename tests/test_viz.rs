...
//
// The values below differ WITHIN each region (so `SkipReason::ConstantColumn` does not fire on the
// column) while every region's median is the same 5.
...
fn viz_smart_draws_median_region_map_when_the_medians_differ() {
    // same shape, but each region's median is distinct -- the map is informative and must survive
    let html = median_region_html(
